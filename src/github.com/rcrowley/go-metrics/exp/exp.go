...
func ExpHandler(r vendor.Registry) http.Handler {
	e := exp{sync.Mutex{}, r}
...
func (exp *exp) publishCounter(name string, metric vendor.Counter) {
	v := exp.getInt(name)
	v.Set(metric.Count())
}

func (exp *exp) publishGauge(name string, metric vendor.Gauge) {
	v := exp.getInt(name)
	v.Set(metric.Value())
}
func (exp *exp) publishGaugeFloat64(name string, metric vendor.GaugeFloat64) {
	exp.getFloat(name).Set(metric.Value())
}

func (exp *exp) publishHistogram(name string, metric vendor.Histogram) {
	h := metric.Snapshot()
...
func (exp *exp) publishMeter(name string, metric vendor.Meter) {
	m := metric.Snapshot()
...
func (exp *exp) publishTimer(name string, metric vendor.Timer) {
	t := metric.Snapshot()
...
func (exp *exp) syncToExpvar() {
	exp.registry.Each(func(name string, i interface{}) {
		switch i.(type) {
		case vendor.Counter:
			exp.publishCounter(name, i.(vendor.Counter))
		case vendor.Gauge:
			exp.publishGauge(name, i.(vendor.Gauge))
		case vendor.GaugeFloat64:
			exp.publishGaugeFloat64(name, i.(vendor.GaugeFloat64))
		case vendor.Histogram:
			exp.publishHistogram(name, i.(vendor.Histogram))
		case vendor.Meter:
			exp.publishMeter(name, i.(vendor.Meter))
		case vendor.Timer:
			exp.publishTimer(name, i.(vendor.Timer))
		default:
