...
func GetOrRegisterMeter(name string, r vendor.Registry) Meter {
	if nil == r {
...
// Be sure to unregister the meter from the registry once it is of no use to
// allow for garbage collection.
func NewRegisteredMeter(name string, r vendor.Registry) Meter {
	c := NewMeter()
	if nil == r {
		r = vendor.DefaultRegistry
	}
