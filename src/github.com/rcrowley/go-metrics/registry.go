...
func (r *StandardRegistry) RunHealthchecks() {
	...
	for _, i := range r.metrics {
		if h, ok := i.(vendor.Healthcheck); ok {
			h.Check()
		}
	}
}
...
func (r *StandardRegistry) register(name string, i interface{}) error {
	...
	switch i.(type) {
	case vendor.Counter, vendor.Gauge, vendor.GaugeFloat64, vendor.Healthcheck, vendor.Histogram, vendor.Meter, vendor.Timer:
		r.metrics[name] = i
	}
	return nil
}
