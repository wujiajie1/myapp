...
func setupBenchmarkProtoCodecInputs(payloadBaseSize uint32) []vendor.Message {
	...
	protoStructs := make([]vendor.Message, 0)
...
func benchmarkProtoCodec(codec *vendor.codec, protoStructs []vendor.Message, pb *testing.PB, b *testing.B) {
	counter := 0
	for pb.Next() {
		counter++
...
func fastMarshalAndUnmarshal(codec encoding.Codec, protoStruct vendor.Message, b *testing.B) {
