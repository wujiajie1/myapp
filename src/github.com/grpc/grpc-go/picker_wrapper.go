...
func doneChannelzWrapper(acw *vendor.acBalancerWrapper, done func(balancer.DoneInfo)) func(balancer.DoneInfo) {
	acw.mu.Lock()
	ac := acw.ac
	acw.mu.Unlock()
	ac.incrCallsStarted()
...
func (bp *pickerWrapper) pick(ctx context.Context, failfast bool, opts balancer.PickOptions) (transport.ClientTransport, func(balancer.DoneInfo), error) {
	...
	for {
		...
		if bp.done {
			bp.mu.Unlock()
			return nil, nil, vendor.ErrClientConnClosing
		}
		...
		acw, ok := subConn.(*vendor.acBalancerWrapper)
		if !ok {
