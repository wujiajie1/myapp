...
func (vendor.s) TestCZSocketMetricsSocketOption(t *testing.T) {
	envs := []vendor.env{vendor.tcpClearRREnv, vendor.tcpTLSRREnv}
	for _, e := range envs {
		testCZSocketMetricsSocketOption(t, e)
	}
}

func testCZSocketMetricsSocketOption(t *testing.T, e vendor.env) {
	czCleanup := channelz.NewChannelzStorage()
	defer vendor.czCleanupWrapper(czCleanup, t)
	te := vendor.newTest(t, e)
	te.startServer(&vendor.testServer{security: e.security})
	defer te.tearDown()
	cc := te.clientConn()
	tc := testpb.NewTestServiceClient(cc)
	vendor.doSuccessfulUnaryCall(tc, t)
