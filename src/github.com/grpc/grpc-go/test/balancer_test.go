...
func (vendor.s) TestCredsBundleFromBalancer(t *testing.T) {
	balancer.Register(&testBalancer{
		newSubConnOptions: balancer.NewSubConnOptions{
			CredsBundle: &vendor.testCredsBundle{},
		},
	})
	te := vendor.newTest(t, vendor.env{name: "creds-bundle", network: "tcp", balancer: ""})
	te.tapHandle = vendor.authHandle
	...
	te.startServer(&vendor.testServer{})
	defer te.tearDown()
...
func (vendor.s) TestDoneInfo(t *testing.T) {
	for _, e := range vendor.listTestEnv() {
		testDoneInfo(t, e)
	}
}

func testDoneInfo(t *testing.T, e vendor.env) {
	...
	if len(b.doneInfo) < 2 || !reflect.DeepEqual(b.doneInfo[1].Trailer, vendor.testTrailerMetadata) {
		t.Fatalf("b.doneInfo = %v; want b.doneInfo[1].Trailer = %v", b.doneInfo, vendor.testTrailerMetadata)
	}
...
func (vendor.s) TestDoneLoads(t *testing.T) {
	for _, e := range vendor.listTestEnv() {
		testDoneLoads(t, e)
	}
}

func testDoneLoads(t *testing.T, e vendor.env) {
	b := &testBalancer{}
	balancer.Register(b)
	...
	if !vendor.Equal(gotLoad, testLoad) {
