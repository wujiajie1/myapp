...
func newRawConnWrapperFromConn(cc io.ReadWriteCloser) *rawConnWrapper {
	rcw := &rawConnWrapper{
		cc:     cc,
		frc:    make(chan vendor.Frame, 1),
		frErrc: make(chan error, 1),
	}
	rcw.hpackEnc = vendor.NewEncoder(&rcw.headerBuf)
	rcw.fr = vendor.NewFramer(cc, cc)
	rcw.fr.ReadMetaHeaders = vendor.NewDecoder(4096 /*initialHeaderTableSize*/, nil)
...
func (rcw *rawConnWrapper) readFrame() (vendor.Frame, error) {
	go func() {
...
func (rcw *rawConnWrapper) writePreface() error {
	n, err := rcw.cc.Write([]byte(vendor.ClientPreface))
	if err != nil {
		return fmt.Errorf("error writing client preface: %v", err)
	}
	if n != len(vendor.ClientPreface) {
		return fmt.Errorf("writing client preface, wrote %d bytes; want %d", n, len(vendor.ClientPreface))
	}
	return nil
}
...
func (rcw *rawConnWrapper) wantSettings() (*vendor.SettingsFrame, error) {
	f, err := rcw.readFrame()
	if err != nil {
...
func (rcw *rawConnWrapper) wantAnyFrame() (vendor.Frame, error) {
...
func (rcw *rawConnWrapper) encodeHeaderField(k, v string) error {
	err := rcw.hpackEnc.WriteField(vendor.HeaderField{Name: k, Value: v})
	if err != nil {
...
func (rcw *rawConnWrapper) writeHeaders(p vendor.HeadersFrameParam) error {
...
func (rcw *rawConnWrapper) writeRSTStream(streamID uint32, code vendor.ErrCode) error {
...
func (rcw *rawConnWrapper) writeGoAway(maxStreamID uint32, code vendor.ErrCode, debugData []byte) error {
...
func (rcw *rawConnWrapper) writeRawFrame(t vendor.FrameType, flags vendor.Flags, streamID uint32, payload []byte) error {
