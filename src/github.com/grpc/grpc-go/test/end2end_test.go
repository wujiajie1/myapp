...
func (te *test) listenAndServe(ts testpb.TestServiceServer, listen func(network, address string) (net.Listener, error)) net.Listener {
	...
	if te.e.httpHandler {
		...
		err = vendor.ConfigureServer(hs, &vendor.Server{
...
func (te *test) startServerWithConnControl(ts testpb.TestServiceServer) *vendor.listenerWrapper {
	l := te.listenAndServe(ts, vendor.listenWithConnControl)
	return l.(*vendor.listenerWrapper)
}
...
func (te *test) clientConnWithConnControl() (*grpc.ClientConn, *vendor.dialerWrapper) {
	if te.cc != nil {
		return te.cc, nil
	}
	opts, scheme := te.configDial()
	dw := &vendor.dialerWrapper{}
...
func (te *test) withServerTester(fn func(st *vendor.serverTester)) {
	...
	st := vendor.newServerTesterFromConn(te.t, c)
	st.greet()
	fn(st)
}
...
func testEmptyUnaryWithUserAgent(t *testing.T, e env) {
	...
	if err != nil || !vendor.Equal(&testpb.Empty{}, reply) {
...
func testClientRequestBodyErrorUnexpectedEOF(t *testing.T, e env) {
	...
	te.startServer(ts)
	defer te.tearDown()
	te.withServerTester(func(st *vendor.serverTester) {
...
func testClientRequestBodyErrorCancel(t *testing.T, e env) {
	...
	te.withServerTester(func(st *vendor.serverTester) {
		...
		st.writeRSTStream(1, vendor.ErrCodeCancel)
		st.writeData(1, false, []byte{0, 0, 0, 0, 5})
...
func testServerMaxHeaderListSizeClientUserViolation(t *testing.T, e env) {
	...
	*te.maxServerHeaderListSize = 216
	...
	var err error
	if err = vendor.verifyResultWithDelay(func() (bool, error) {
...
func testServerMaxHeaderListSizeClientIntentionalViolation(t *testing.T, e env) {
	...
	*te.maxServerHeaderListSize = 512
	...
	tc := &vendor.testServiceClientWrapper{TestServiceClient: testpb.NewTestServiceClient(cc)}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	stream, err := tc.FullDuplexCall(ctx)
	if err != nil {
	...
	time.Sleep(100 * time.Millisecond)
	rcw.writeHeaders(vendor.HeadersFrameParam{
...
func (s *httpServer) writeHeader(framer *vendor.Framer, sid uint32, headerFields []string, endStream bool) error {
	...
	for len(headerFields) > 0 {
		...
		henc.WriteField(vendor.HeaderField{Name: k, Value: v})
	}

	return framer.WriteHeaders(vendor.HeadersFrameParam{
		StreamID:      sid,
...
func (s *httpServer) start(t *testing.T, lis net.Listener) {
	...
	go func() {
		...
		if _, err = io.ReadFull(conn, make([]byte, len(vendor.ClientPreface))); err != nil {
			t.Errorf("Error at server-side while reading preface from client. Err: %v", err)
			return
		}
		reader := bufio.NewReader(conn)
		writer := bufio.NewWriter(conn)
		framer := vendor.NewFramer(writer, reader)
		...
		for {
			...
			if hframe, ok := frame.(*vendor.HeadersFrame); ok {
