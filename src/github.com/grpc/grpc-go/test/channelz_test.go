...
func (te *vendor.test) startServers(ts testpb.TestServiceServer, num int) {
	for i := 0; i < num; i++ {
		te.startServer(ts)
...
func (vendor.s) TestCZServerRegistrationAndDeletion(t *testing.T) {
	testcases := []struct {
		total  int
		start  int64
		max    int64
	...
	for _, c := range testcases {
		...
		e := vendor.tcpClearRREnv
		te := vendor.newTest(t, e)
		te.startServers(&vendor.testServer{security: e.security}, c.total)
...
func (vendor.s) TestCZGetServer(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
	te.startServer(&vendor.testServer{security: e.security})
	defer te.tearDown()
...
func (vendor.s) TestCZTopChannelRegistrationAndDeletion(t *testing.T) {
	testcases := []struct {
		total  int
		start  int64
		max    int64
...
func (vendor.s) TestCZTopChannelRegistrationAndDeletionWhenDialFail(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
...
func (vendor.s) TestCZNestedChannelRegistrationAndDeletion(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
...
func (vendor.s) TestCZClientSubChannelSocketRegistrationAndDeletion(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	...
	te.startServers(&vendor.testServer{security: e.security}, num)
	r, cleanup := manual.GenerateAndRegisterManualResolver()
	defer cleanup()
...
func (vendor.s) TestCZServerSocketRegistrationAndDeletion(t *testing.T) {
	testcases := []struct {
		total  int
		start  int64
		max    int64
...
func (vendor.s) TestCZServerListenSocketDeletion(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	s := grpc.NewServer()
...
func (vendor.s) TestCZRecusivelyDeletionOfEntry(t *testing.T) {
	//           +--+TopChan+---+
	//           |              |
	//           v              v
	//    +-+SubChan1+--+   SubChan2
...
func (vendor.s) TestCZChannelMetrics(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	num := 3 // number of backends
	te := vendor.newTest(t, e)
	te.maxClientSendMsgSize = vendor.newInt(8)
	var svrAddrs []resolver.Address
	te.startServers(&vendor.testServer{security: e.security}, num)
	r, cleanup := manual.GenerateAndRegisterManualResolver()
	defer cleanup()
	...
	largePayload, err := vendor.newPayload(testpb.PayloadType_COMPRESSABLE, largeSize)
	if err != nil {
		t.Fatal(err)
	}
...
func (vendor.s) TestCZServerMetrics(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
	te.maxServerReceiveMsgSize = vendor.newInt(8)
	te.startServer(&vendor.testServer{security: e.security})
	defer te.tearDown()
	cc := te.clientConn()
	tc := testpb.NewTestServiceClient(cc)
...
func doStreamingInputCallWithLargePayload(tc testpb.TestServiceClient, t *testing.T) {
	...
	payload, err := vendor.newPayload(testpb.PayloadType_COMPRESSABLE, 10000)
	if err != nil {
		t.Fatal(err)
	}
...
func doClientSideInitiatedFailedStream(tc testpb.TestServiceClient, t *testing.T) {
	...
	const smallSize = 1
	smallPayload, err := vendor.newPayload(testpb.PayloadType_COMPRESSABLE, smallSize)
	if err != nil {
		t.Fatal(err)
	}
...
func doServerSideInitiatedFailedStreamWithRSTStream(tc testpb.TestServiceClient, t *testing.T, l *vendor.listenerWrapper) {
	stream, err := tc.FullDuplexCall(context.Background())
	if err != nil {
		t.Fatalf("TestService/FullDuplexCall(_) = _, %v, want <nil>", err)
	}
	...
	if rcw != nil {
		rcw.writeRSTStream(tc.(*testServiceClientWrapper).getCurrentStreamID(), vendor.ErrCodeCancel)
	}
	if _, err := stream.Recv(); err == nil {
...
// this func is to be used to test client side counting of failed streams.
func doServerSideInitiatedFailedStreamWithGoAway(tc testpb.TestServiceClient, t *testing.T, l *vendor.listenerWrapper) {
	...
	rcw := l.getLastConn()
	if rcw != nil {
		rcw.writeGoAway(tc.(*testServiceClientWrapper).getCurrentStreamID()-2, vendor.ErrCodeCancel, []byte{})
	}
	if _, err := s.Recv(); err == nil {
...
// this func is to be used to test client side counting of failed streams.
func doServerSideInitiatedFailedStreamWithClientBreakFlowControl(tc testpb.TestServiceClient, t *testing.T, dw *vendor.dialerWrapper) {
	stream, err := tc.FullDuplexCall(context.Background())
	if err != nil {
		t.Fatalf("TestService/FullDuplexCall(_) = _, %v, want <nil>", err)
	}
	...
	dw.getRawConnWrapper().writeRawFrame(vendor.FrameData, 0, tc.(*testServiceClientWrapper).getCurrentStreamID(), payload)
...
func (vendor.s) TestCZClientSocketMetricsStreamsAndMessagesCount(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
	te.maxServerReceiveMsgSize = vendor.newInt(20)
	te.maxClientReceiveMsgSize = vendor.newInt(20)
	rcw := te.startServerWithConnControl(&vendor.testServer{security: e.security})
	defer te.tearDown()
	cc := te.clientConn()
...
func (vendor.s) TestCZClientAndServerSocketMetricsStreamsCountFlowControlRSTStream(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
...
func (vendor.s) TestCZClientAndServerSocketMetricsFlowControl(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
...
func (vendor.s) TestCZClientSocketMetricsKeepAlive(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
...
func (vendor.s) TestCZServerSocketMetricsStreamsAndMessagesCount(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
...
func (vendor.s) TestCZServerSocketMetricsKeepAlive(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
...
func (vendor.s) TestCZSocketGetSecurityValueTLS(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpTLSRREnv
	te := vendor.newTest(t, e)
	te.startServer(&vendor.testServer{security: e.security})
	defer te.tearDown()
	te.clientConn()
...
func (vendor.s) TestCZChannelTraceCreationDeletion(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	// avoid calling API to set balancer type, which will void service config's change of balancer.
...
func (vendor.s) TestCZSubChannelTraceCreationDeletion(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
...
func (vendor.s) TestCZChannelAddressResolutionChange(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	e.balancer = ""
...
func (vendor.s) TestCZSubChannelPickedNewAddress(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	e.balancer = ""
	te := vendor.newTest(t, e)
	te.startServers(&vendor.testServer{security: e.security}, 3)
	r, cleanup := manual.GenerateAndRegisterManualResolver()
	defer cleanup()
	var svrAddrs []resolver.Address
	for _, a := range te.srvAddrs {
...
func (vendor.s) TestCZSubChannelConnectivityState(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
...
func (vendor.s) TestCZChannelConnectivityState(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
...
func (vendor.s) TestCZTraceOverwriteChannelDeletion(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	// avoid calling API to set balancer type, which will void service config's change of balancer.
...
func (vendor.s) TestCZTraceOverwriteSubChannelDeletion(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
...
func (vendor.s) TestCZTraceTopChannelDeletionTraceClear(t *testing.T) {
	czCleanup := channelz.NewChannelzStorage()
	defer czCleanupWrapper(czCleanup, t)
	e := vendor.tcpClearRREnv
	te := vendor.newTest(t, e)
