...
func TestConn(t *testing.T) {
	p1, p2 := vendor.newPipe(10), vendor.newPipe(10)
	c1, c2 := &vendor.conn{p1, p2}, &vendor.conn{p2, p1}
...
func TestCloseWhileDialing(t *testing.T) {
	...
	l.Close()
	<-done
	if c != nil || err != vendor.errClosed {
		t.Fatalf("c, err = %v, %v; want nil, %v", c, err, vendor.errClosed)
	}
}
