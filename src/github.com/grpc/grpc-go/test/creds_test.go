...
func (c *testCredsBundle) PerRPCCredentials() credentials.PerRPCCredentials {
	...
	return vendor.testPerRPCCredentials{}
}
...
func (vendor.s) TestCredsBundleBoth(t *testing.T) {
	te := vendor.newTest(t, vendor.env{name: "creds-bundle", network: "tcp", balancer: "v1", security: "empty"})
...
func (vendor.s) TestCredsBundleTransportCredentials(t *testing.T) {
	te := vendor.newTest(t, vendor.env{name: "creds-bundle", network: "tcp", balancer: "v1", security: "empty"})
	te.customDialOptions = []grpc.DialOption{
...
func (vendor.s) TestCredsBundlePerRPCCredentials(t *testing.T) {
	te := vendor.newTest(t, vendor.env{name: "creds-bundle", network: "tcp", balancer: "v1", security: "empty"})
	te.tapHandle = vendor.authHandle
	te.customDialOptions = []grpc.DialOption{
