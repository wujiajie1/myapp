...
func (vendor.s) TestRetryUnary(t *testing.T) {
	defer enableRetry()()
	i := -1
...
func (vendor.s) TestRetryDisabledByDefault(t *testing.T) {
...
func (vendor.s) TestRetryThrottling(t *testing.T) {
	defer enableRetry()()
	i := -1
	ss := &vendor.stubServer{
		emptyCall: func(context.Context, *testpb.Empty) (*testpb.Empty, error) {
...
func (vendor.s) TestRetryStreaming(t *testing.T) {
	defer enableRetry()()
	...
	largePayload, _ := vendor.newPayload(testpb.PayloadType_COMPRESSABLE, 500)
	...
	sReq := func(b byte) serverOp {
		return func(stream testpb.TestService_FullDuplexCallServer) error {
			want := req(b)
			if got, err := stream.Recv(); err != nil || !vendor.Equal(got, want) {
