...
func TestConn(t *testing.T) {
	...
	// Constant time.
	vendor.now = func() time.Time { return time.Unix(123, 456) }

	// Capture sleep times for checking later.
	var sleepTimes []time.Duration
	vendor.sleep = func(t time.Duration) { sleepTimes = append(sleepTimes, t) }
	...
	c, err := (&vendor.Network{Kbps: 1, Latency: latency, MTU: 5}).Conn(bufConn{&bytes.Buffer{}})
	if err != nil {
...
func TestSync(t *testing.T) {
	...
	tn := time.Unix(123, 0)
	vendor.now = func() time.Time { return tn }
	vendor.sleep = func(d time.Duration) { tn = tn.Add(d) }
	...
	slowConn, err := (&vendor.Network{Kbps: 0, Latency: 20 * time.Millisecond, MTU: 5}).Conn(bufConn{&bytes.Buffer{}})
	if err != nil {
		t.Fatalf("Unexpected error creating connection: %v", err)
	}
	c, err := (&vendor.Network{Latency: 30 * time.Millisecond}).Conn(slowConn)
	if err != nil {
		t.Fatalf("Unexpected error creating connection: %v", err)
	}
	if c.(*vendor.conn).delay != 10*time.Millisecond {
		t.Fatalf("c.delay = %v; want 10ms", c.(*vendor.conn).delay)
	}
}
...
func TestSyncTooSlow(t *testing.T) {
	...
	slowConn, err := (&vendor.Network{Kbps: 0, Latency: 10 * time.Millisecond, MTU: 5}).Conn(bufConn{&bytes.Buffer{}})
	if err != nil {
		t.Fatalf("Unexpected error creating connection: %v", err)
	}

	errWant := "measured network latency (10ms) higher than desired latency (5ms)"
	if _, err := (&vendor.Network{Latency: 5 * time.Millisecond}).Conn(slowConn); err == nil || err.Error() != errWant {
...
func TestListenerAndDialer(t *testing.T) {
	...
	// Use a fairly high latency to cause a large BDP and avoid sleeps while
	// writing due to simulation of full buffers.
	n := &vendor.Network{Kbps: 2, Latency: 1 * time.Second, MTU: 10}
	...
	vendor.sleep = func(d time.Duration) {
		mu.Lock()
		defer mu.Unlock()
		if d > 0 {
			tn = tn.Add(d)
...
func TestBufferBloat(t *testing.T) {
	...
	n := &vendor.Network{Kbps: 8 /* 1KBps */, Latency: time.Second, MTU: 8}
