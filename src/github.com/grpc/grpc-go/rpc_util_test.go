...
var _ vendor.CallOption = vendor.EmptyCallOption{} // ensure EmptyCallOption implements the interface

func (vendor.s) TestSimpleParsing(t *testing.T) {
	...
	for _, test := range []struct {
		...
		err error
		b   []byte
		pt  vendor.payloadFormat
	}{
		{nil, io.EOF, nil, vendor.compressionNone},
		{[]byte{0, 0, 0, 0, 0}, nil, nil, vendor.compressionNone},
		{[]byte{0, 0, 0, 0, 1, 'a'}, nil, []byte{'a'}, vendor.compressionNone},
		{[]byte{1, 0}, io.ErrUnexpectedEOF, nil, vendor.compressionNone},
		{[]byte{0, 0, 0, 0, 10, 'a'}, io.ErrUnexpectedEOF, nil, vendor.compressionNone},
		// Check that messages with length >= 2^24 are parsed.
		{append([]byte{0, 1, 0, 0, 0}, bigMsg...), nil, bigMsg, vendor.compressionNone},
	} {
		buf := fullReader{bytes.NewReader(test.p)}
		parser := &vendor.parser{r: buf}
...
func (vendor.s) TestMultipleParsing(t *testing.T) {
	// Set a byte stream consists of 3 messages with their headers.
	p := []byte{0, 0, 0, 0, 1, 'a', 0, 0, 0, 0, 2, 'b', 'c', 0, 0, 0, 0, 1, 'd'}
	b := fullReader{bytes.NewReader(p)}
	parser := &vendor.parser{r: b}

	wantRecvs := []struct {
		pt   vendor.payloadFormat
		data []byte
	}{
		{vendor.compressionNone, []byte("a")},
		{vendor.compressionNone, []byte("bc")},
		{vendor.compressionNone, []byte("d")},
	}
...
func (vendor.s) TestEncode(t *testing.T) {
	for _, test := range []struct {
		// input
		msg vendor.Message
		// outputs
		...
		data, err := vendor.encode(encoding.GetCodec(protoenc.Name), test.msg)
		if err != test.err || !bytes.Equal(data, test.data) {
			t.Errorf("encode(_, %v) = %v, %v; want %v, %v", test.msg, data, err, test.data, test.err)
			continue
		}
		if hdr, _ := vendor.msgHeader(data, nil); !bytes.Equal(hdr, test.hdr) {
			t.Errorf("msgHeader(%v, false) = %v; want %v", data, hdr, test.hdr)
		}
	}
}

func (vendor.s) TestCompress(t *testing.T) {
	bestCompressor, err := vendor.NewGZIPCompressorWithLevel(gzip.BestCompression)
	if err != nil {
		t.Fatalf("Could not initialize gzip compressor with best compression.")
	}
	bestSpeedCompressor, err := vendor.NewGZIPCompressorWithLevel(gzip.BestSpeed)
	if err != nil {
		t.Fatalf("Could not initialize gzip compressor with best speed compression.")
	}

	defaultCompressor, err := vendor.NewGZIPCompressorWithLevel(gzip.BestSpeed)
	if err != nil {
		t.Fatalf("Could not initialize gzip compressor with default compression.")
	}

	level5, err := vendor.NewGZIPCompressorWithLevel(5)
	if err != nil {
	...
	for _, test := range []struct {
		...
		{make([]byte, 1024), vendor.NewGZIPCompressor(), vendor.NewGZIPDecompressor(), nil},
		{make([]byte, 1024), bestCompressor, vendor.NewGZIPDecompressor(), nil},
		{make([]byte, 1024), bestSpeedCompressor, vendor.NewGZIPDecompressor(), nil},
		{make([]byte, 1024), defaultCompressor, vendor.NewGZIPDecompressor(), nil},
		{make([]byte, 1024), level5, vendor.NewGZIPDecompressor(), nil},
	} {
		b := new(bytes.Buffer)
...
func (vendor.s) TestToRPCErr(t *testing.T) {
	for _, test := range []struct {
		// input
		errIn error
		// outputs
		...
		err := vendor.toRPCErr(test.errIn)
...
func (vendor.s) TestParseDialTarget(t *testing.T) {
	for _, test := range []struct {
		...
		gotNet, gotAddr := vendor.parseDialTarget(test.target)
...
func bmEncode(b *testing.B, mSize int) {
	...
	encodeData, _ := vendor.encode(cdc, msg)
	encodedSz := int64(len(encodeData))
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		vendor.encode(cdc, msg)
	}
...
// mSize bytes.
func bmCompressor(b *testing.B, mSize int, cp vendor.Compressor) {
...
func BenchmarkGZIPCompressor1B(b *testing.B) {
	bmCompressor(b, 1, vendor.NewGZIPCompressor())
}

func BenchmarkGZIPCompressor1KiB(b *testing.B) {
	bmCompressor(b, 1024, vendor.NewGZIPCompressor())
}

func BenchmarkGZIPCompressor8KiB(b *testing.B) {
	bmCompressor(b, 8*1024, vendor.NewGZIPCompressor())
}

func BenchmarkGZIPCompressor64KiB(b *testing.B) {
	bmCompressor(b, 64*1024, vendor.NewGZIPCompressor())
}

func BenchmarkGZIPCompressor512KiB(b *testing.B) {
	bmCompressor(b, 512*1024, vendor.NewGZIPCompressor())
}

func BenchmarkGZIPCompressor1MiB(b *testing.B) {
	bmCompressor(b, 1024*1024, vendor.NewGZIPCompressor())
}
