...
func (lb *vendor.lbBalancer) processServerList(l *lbpb.ServerList) {
	if grpclog.V(2) {
	...
	for i, s := range l.Servers {
		...
		md := metadata.Pairs(vendor.lbTokeyKey, s.LoadBalanceToken)
...
func (lb *vendor.lbBalancer) refreshSubConns(backendAddrs []resolver.Address, fallback bool, pickFirst bool) {
	defer func() {
...
func (lb *vendor.lbBalancer) readServerList(s *vendor.balanceLoadClientStream) error {
	for {
		reply, err := s.Recv()
		if err != nil {
			if err == io.EOF {
				return vendor.errServerTerminatedConnection
			}
...
func (lb *vendor.lbBalancer) sendLoadReport(s *vendor.balanceLoadClientStream, interval time.Duration) {
	...
	for {
		...
		t := time.Now()
		stats.Timestamp = &vendor.Timestamp{
			Seconds: t.Unix(),
...
func (lb *vendor.lbBalancer) callRemoteBalancer() (backoff bool, _ error) {
	lbClient := &vendor.loadBalancerClient{cc: lb.ccRemoteLB}
	...
	go func() {
		if d := vendor.convertDuration(initResp.ClientStatsReportInterval); d > 0 {
...
func (lb *vendor.lbBalancer) watchRemoteBalancer() {
	var retryCount int
	for {
		...
		default:
			if err != nil {
				if err == vendor.errServerTerminatedConnection {
					grpclog.Info(err)
				} else {
					grpclog.Warning(err)
				}
...
func (lb *vendor.lbBalancer) dialRemoteLB(remoteLBName string) {
