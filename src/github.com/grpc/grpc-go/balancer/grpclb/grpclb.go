...
func convertDuration(d *vendor.Duration) time.Duration {
	if d == nil {
		return 0
	}
...
func (b *lbBuilder) Build(cc balancer.ClientConn, opt balancer.BuildOptions) balancer.Balancer {
	...
	r := &vendor.lbManualResolver{scheme: scheme, ccb: cc}

	lb := &lbBalancer{
		cc:              vendor.newLBCacheClientConn(cc),
		...
		picker:         &vendor.errPicker{err: balancer.ErrNoSubConnAvailable},
		clientStats:    vendor.newRPCStats(),
...
type lbBalancer struct {
	...
	manualResolver *vendor.lbManualResolver
...
func (lb *lbBalancer) regeneratePicker(resetDrop bool) {
	if lb.state == connectivity.TransientFailure {
		lb.picker = &vendor.errPicker{err: balancer.ErrTransientFailure}
		return
	}

	if lb.state == connectivity.Connecting {
		lb.picker = &vendor.errPicker{err: balancer.ErrNoSubConnAvailable}
		return
	}
	...
	if lb.inFallback {
		lb.picker = vendor.newRRPicker(readySCs)
		return
	}
	if resetDrop {
		lb.picker = vendor.newLBPicker(lb.fullServerList, readySCs, lb.clientStats)
		return
	}
	prevLBPicker, ok := lb.picker.(*vendor.lbPicker)
	if !ok {
		lb.picker = vendor.newLBPicker(lb.fullServerList, readySCs, lb.clientStats)
		return
	}
...
func (lb *lbBalancer) handleServiceConfig(sc string) {
	...
	newUsePickFirst := vendor.childIsPickFirst(sc)
