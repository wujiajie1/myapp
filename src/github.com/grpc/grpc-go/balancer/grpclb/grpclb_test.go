...
func (s *vendor.rpcStats) merge(cs *lbpb.ClientStats) {
...
//
// It's a test-only method. rpcStats is defined in grpclb_picker.
func (s *vendor.rpcStats) equal(o *vendor.rpcStats) bool {
	...
	resp := &lbpb.LoadBalanceResponse{
		LoadBalanceResponseType: &lbpb.LoadBalanceResponse_InitialResponse{
			InitialResponse: &lbpb.InitialLoadBalanceResponse{
				ClientStatsReportInterval: &vendor.Duration{
...
func newLoadBalancer(numberOfBackends int) (tss *testServers, cleanup func(), err error) {
	...
	for i := 0; i < numberOfBackends; i++ {
		...
		beListeners = append(beListeners, vendor.newRestartableListener(beLis))
	}
	...
	lbLis = vendor.newRestartableListener(lbLis)
...
func TestFallback(t *testing.T) {
	balancer.Register(vendor.newLBBuilderWithFallbackTimeout(100 * time.Millisecond))
	defer balancer.Register(vendor.newLBBuilder())
	...
	tss.beListeners[0].(*vendor.restartableListener).stopPreviousConns()
	tss.lbListener.(*vendor.restartableListener).stopPreviousConns()
	time.Sleep(time.Second)
	...
	tss.beListeners[0].(*vendor.restartableListener).restart()
	tss.lbListener.(*vendor.restartableListener).restart()
	tss.ls.sls <- sl
...
func checkStats(stats, expected *vendor.rpcStats) error {
...
func runAndGetStats(t *testing.T, drop bool, runRPCs func(*grpc.ClientConn)) *vendor.rpcStats {
	defer leakcheck.Check(t)
...
func TestGRPCLBStatsUnarySuccess(t *testing.T) {
	...
	if err := checkStats(stats, &vendor.rpcStats{
