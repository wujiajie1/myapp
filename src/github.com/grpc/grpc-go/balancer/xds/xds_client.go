...
type client struct {
	...
	enableCDS    bool
	newADS       func(ctx context.Context, resp vendor.Message) error
			...
			Metadata: &vendor.Struct{
				Fields: map[string]*vendor.Value{
					internal.GrpcHostname: {
						Kind: &vendor.Value_StringValue{StringValue: c.serviceName},
					},
				},
			},
		},
	...
	edsReq := &discoverypb.DiscoveryRequest{
		Node: &basepb.Node{
			Metadata: &vendor.Struct{
				Fields: map[string]*vendor.Value{
					endpointRequired: {
						Kind: &vendor.Value_BoolValue{BoolValue: c.enableCDS},
					},
				},
			},
		},
...
func (c *client) adsCallAttempt() (firstRespReceived bool) {
	...
	for {
		...
		var adsResp vendor.DynamicAny
		if err := vendor.UnmarshalAny(resources[0], &adsResp); err != nil {
...
func newXDSClient(balancerName string, enableCDS bool, opts balancer.BuildOptions, loadStore lrs.Store, newADS func(context.Context, vendor.Message) error, loseContact func(ctx context.Context), exitCleanup func()) *client {
	c := &client{
