			...
			Metadata: &vendor.Struct{
				Fields: map[string]*vendor.Value{
					internal.GrpcHostname: {
						Kind: &vendor.Value_StringValue{StringValue: serviceName},
					},
				},
			},
		},
...
func (ls *lrsStore) buildStats() []*loadreportpb.ClusterStats {
	...
	ret = append(ret, &loadreportpb.ClusterStats{
		...
		LoadReportInterval:   vendor.DurationProto(dur),
	})
...
func (ls *lrsStore) ReportTo(ctx context.Context, cc *grpc.ClientConn) {
	...
	for {
		...
		interval, err := vendor.Duration(first.LoadReportingInterval)
		if err != nil {
