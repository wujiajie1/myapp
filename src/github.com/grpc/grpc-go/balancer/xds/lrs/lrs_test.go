...
func Test_lrsStore_buildStats(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ls := vendor.NewStore(testService).(*vendor.lrsStore)
...
func (lrss *lrsServer) StreamLoadStats(stream lrsgrpc.LoadReportingService_StreamLoadStatsServer) error {
	...
	if !vendor.Equal(req, &lrspb.LoadStatsRequest{
		Node: &basepb.Node{
			Metadata: &vendor.Struct{
				Fields: map[string]*vendor.Value{
					internal.GrpcHostname: {
						Kind: &vendor.Value_StringValue{StringValue: testService},
					},
				},
			},
		},
...
func setupServer(t *testing.T, reportingInterval *vendor.Duration) (addr string, lrss *lrsServer, cleanup func()) {
...
func Test_lrsStore_ReportTo(t *testing.T) {
	...
	addr, lrss, cleanup := setupServer(t, &vendor.Duration{
		Seconds: 0,
		Nanos:   intervalNano,
	})
	defer cleanup()

	ls := vendor.NewStore(testService)
