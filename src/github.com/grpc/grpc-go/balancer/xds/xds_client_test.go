...
var (
	...
	testCDSReq      = &discoverypb.DiscoveryRequest{
		Node: &basepb.Node{
			Metadata: &vendor.Struct{
				Fields: map[string]*vendor.Value{
					internal.GrpcHostname: {
						Kind: &vendor.Value_StringValue{StringValue: testServiceName},
					},
				},
			},
		},
	...
	testEDSReq = &discoverypb.DiscoveryRequest{
		Node: &basepb.Node{
			Metadata: &vendor.Struct{
				Fields: map[string]*vendor.Value{
					endpointRequired: {
						Kind: &vendor.Value_BoolValue{BoolValue: true},
					},
				},
			},
		},
	...
	testEDSReqWithoutEndpoints = &discoverypb.DiscoveryRequest{
		Node: &basepb.Node{
			Metadata: &vendor.Struct{
				Fields: map[string]*vendor.Value{
					endpointRequired: {
						Kind: &vendor.Value_BoolValue{BoolValue: false},
					},
				},
			},
		},
	...
	marshaledCluster, _ = vendor.Marshal(testCluster)
	testCDSResp         = &discoverypb.DiscoveryResponse{
		Resources: []*vendor.Any{
			{
				TypeUrl: cdsType,
		...
		Endpoints: []*endpointpb.LocalityLbEndpoints{
			{
				...
				LbEndpoints: []*endpointpb.LbEndpoint{
					{
						...
						Metadata: &basepb.Metadata{
							FilterMetadata: map[string]*vendor.Struct{
								"xx.lb": {
									Fields: map[string]*vendor.Value{
										"endpoint_name": {
											Kind: &vendor.Value_StringValue{
				...
				LoadBalancingWeight: &vendor.UInt32Value{
					Value: 1,
				},
				Priority: 0,
			},
	...
	marshaledClusterLoadAssignment, _ = vendor.Marshal(testClusterLoadAssignment)
	...
	marshaledClusterLoadAssignmentWithoutEndpoints, _ = vendor.Marshal(testClusterLoadAssignmentWithoutEndpoints)
...
type testConfig struct {
	...
	expectedADSResponses []vendor.Message
	adsErr               error
	svrErr               error
}
...
func setupServer(t *testing.T) (addr string, td *testTrafficDirector, lrss *lrsServer, cleanup func()) {
	...
	lrss = &lrsServer{
		...
		reportingInterval: &vendor.Duration{
	...
	for _, test := range []*testConfig{
		{
			...
			expectedADSResponses: []vendor.Message{testCluster, testClusterLoadAssignment},
		},
		{
			doCDS:                false,
			...
			expectedADSResponses: []vendor.Message{testClusterLoadAssignmentWithoutEndpoints},
		},
	} {
...
func testXdsClientResponseHandling(t *testing.T, test *testConfig) {
	...
	defer cleanup()
	adsChan := make(chan vendor.Message, 10)
	newADS := func(ctx context.Context, i vendor.Message) error {
		adsChan <- i
		return nil
	}
	...
	for _, expectedReq := range test.expectedRequests {
		...
		if !vendor.Equal(req.req, expectedReq) {
	...
	for i, resp := range test.responsesToSend {
		...
		ads := <-adsChan
		if !vendor.Equal(ads, test.expectedADSResponses[i]) {
