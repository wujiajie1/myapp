...
func replaceNetFunc(ch chan struct{}) func() {
	oldResolver := vendor.defaultResolver
	vendor.defaultResolver = &testResolver{ch: ch}

	return func() {
		vendor.defaultResolver = oldResolver
	}
}

func replaceDNSResRate(d time.Duration) func() {
	oldMinDNSResRate := vendor.minDNSResRate
	vendor.minDNSResRate = d

	return func() {
		vendor.minDNSResRate = oldMinDNSResRate
	}
}
...
var scLookupTbl = map[string]bool{
	vendor.txtPrefix + "foo.bar.com":          true,
	vendor.txtPrefix + "srv.ipv4.single.fake": true,
	vendor.txtPrefix + "srv.ipv4.multi.fake":  true,
	vendor.txtPrefix + "no.attribute":         true,
}
...
func generateSCF(name string) []string {
	...
	return div(append([]byte(vendor.txtAttribute), b...))
}
...
func TestResolveFunc(t *testing.T) {
	...
	tests := []struct {
		...
		{"golang.org:http", nil},
		{"[2001:db8::1]:http", nil},
		{"[2001:db8::1]:", vendor.errEndsWithColon},
		{":", vendor.errEndsWithColon},
...
func TestCustomAuthority(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"4.3.2.1:" + vendor.defaultDNSSvrPort,
			"4.3.2.1:" + vendor.defaultDNSSvrPort,
			false,
		},
		{
		...
		{
			"dnsserver.com",
			"dnsserver.com:" + vendor.defaultDNSSvrPort,
			false,
		},
		{
			":123",
	...
	oldCustomAuthorityDialler := vendor.customAuthorityDialler
	defer func() {
		vendor.customAuthorityDialler = oldCustomAuthorityDialler
	}()

	for _, a := range tests {
		errChan := make(chan error, 1)
		vendor.customAuthorityDialler = func(authority string) func(ctx context.Context, network, address string) (net.Conn, error) {
