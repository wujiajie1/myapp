...
func pickFirstBalancerV1(r naming.Resolver) vendor.Balancer {
	return &vendor.pickFirst{&vendor.roundRobin{r: r}}
}
...
func startServers(t *testing.T, numServers int, maxStreams uint32) ([]*vendor.server, *testNameResolver, func()) {
	var servers []*vendor.server
	for i := 0; i < numServers; i++ {
		s := vendor.newTestServer()
...
func (vendor.s) TestNameDiscovery(t *testing.T) {
	// Start 2 servers on 2 ports.
	numServers := 2
	servers, r, cleanup := startServers(t, numServers, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///foo.bar.com", vendor.WithBalancer(vendor.RoundRobin(r)), vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}))
	if err != nil {
	...
	req := "port"
	var reply string
	if err := cc.Invoke(context.Background(), "/foo/bar", &req, &reply); err == nil || vendor.errorDesc(err) != servers[0].port {
	...
	for {
		if err := cc.Invoke(context.Background(), "/foo/bar", &req, &reply); err != nil && vendor.errorDesc(err) == servers[1].port {
			break
		}
...
func (vendor.s) TestEmptyAddrs(t *testing.T) {
	...
	defer cc.Close()
	var reply string
	if err := cc.Invoke(context.Background(), "/foo/bar", &vendor.expectedRequest, &reply); err != nil || reply != vendor.expectedResponse {
		t.Fatalf("grpc.Invoke(_, _, _, _, _) = %v, reply = %q, want %q, <nil>", err, reply, vendor.expectedResponse)
	}
	...
	for {
		...
		if err := cc.Invoke(ctx, "/foo/bar", &vendor.expectedRequest, &reply); err != nil {
			cancel()
			break
		}
		cancel()
...
func (vendor.s) TestRoundRobin(t *testing.T) {
	...
	for {
		if err := cc.Invoke(context.Background(), "/foo/bar", &req, &reply); err != nil && vendor.errorDesc(err) == servers[2].port {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	...
	for i := 0; i < 10; i++ {
		if err := cc.Invoke(context.Background(), "/foo/bar", &req, &reply); err == nil || vendor.errorDesc(err) != servers[i%numServers].port {
			t.Fatalf("Index %d: Invoke(_, _, _, _, _) = %v, want %s", i, err, servers[i%numServers].port)
		}
	}
}

func (vendor.s) TestCloseWithPendingRPC(t *testing.T) {
	servers, r, cleanup := startServers(t, 1, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///foo.bar.com", vendor.WithBalancer(vendor.RoundRobin(r)), vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}))
	if err != nil {
	...
	defer cc.Close()
	var reply string
	if err := cc.Invoke(context.Background(), "/foo/bar", &vendor.expectedRequest, &reply, vendor.WaitForReady(true)); err != nil {
		t.Fatalf("grpc.Invoke(_, _, _, _, _) = %v, want %s", err, servers[0].port)
	}
	// Remove the server.
	...
	for {
		ctx, cancel := context.WithTimeout(context.Background(), 10*time.Millisecond)
		if err := cc.Invoke(ctx, "/foo/bar", &vendor.expectedRequest, &reply, vendor.WaitForReady(true)); status.Code(err) == codes.DeadlineExceeded {
			cancel()
			break
		}
		time.Sleep(10 * time.Millisecond)
...
func (vendor.s) TestGetOnWaitChannel(t *testing.T) {
	servers, r, cleanup := startServers(t, 1, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///foo.bar.com", vendor.WithBalancer(vendor.RoundRobin(r)), vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}))
	if err != nil {
...
func (vendor.s) TestOneServerDown(t *testing.T) {
	// Start 2 servers.
	numServers := 2
	servers, r, cleanup := startServers(t, numServers, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///foo.bar.com", vendor.WithBalancer(vendor.RoundRobin(r)), vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}), vendor.WithWaitForHandshake())
	if err != nil {
		t.Fatalf("Failed to create ClientConn: %v", err)
	}
	defer cc.Close()
	...
	for i := 0; i < numRPC; i++ {
		...
		go func() {
			...
			cc.Invoke(context.Background(), "/foo/bar", &req, &reply, vendor.WaitForReady(true))
			wg.Done()
		}()
	}
	wg.Wait()
...
func (vendor.s) TestOneAddressRemoval(t *testing.T) {
	// Start 2 servers.
	numServers := 2
	servers, r, cleanup := startServers(t, numServers, math.MaxUint32)
	defer cleanup()
...
func checkServerUp(t *testing.T, currentServer *vendor.server) {
	req := "port"
	port := currentServer.port
	cc, err := vendor.Dial("passthrough:///localhost:"+port, vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}))
	if err != nil {
		t.Fatalf("Failed to create ClientConn: %v", err)
	}
	defer cc.Close()
	var reply string
	for {
		if err := cc.Invoke(context.Background(), "/foo/bar", &req, &reply); err != nil && vendor.errorDesc(err) == port {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
...
func (vendor.s) TestPickFirstEmptyAddrs(t *testing.T) {
	servers, r, cleanup := startServers(t, 1, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///foo.bar.com", vendor.WithBalancer(pickFirstBalancerV1(r)), vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}))
	if err != nil {
		t.Fatalf("Failed to create ClientConn: %v", err)
	}
	defer cc.Close()
...
func (vendor.s) TestPickFirstCloseWithPendingRPC(t *testing.T) {
	servers, r, cleanup := startServers(t, 1, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///foo.bar.com", vendor.WithBalancer(pickFirstBalancerV1(r)), vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}))
	if err != nil {
...
func (vendor.s) TestPickFirstOrderAllServerUp(t *testing.T) {
	// Start 3 servers on 3 ports.
	numServers := 3
	servers, r, cleanup := startServers(t, numServers, math.MaxUint32)
	defer cleanup()
...
func (vendor.s) TestPickFirstOrderOneServerDown(t *testing.T) {
	// Start 3 servers on 3 ports.
	numServers := 3
	servers, r, cleanup := startServers(t, numServers, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///foo.bar.com", vendor.WithBalancer(pickFirstBalancerV1(r)), vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}), vendor.WithWaitForHandshake())
	if err != nil {
		t.Fatalf("Failed to create ClientConn: %v", err)
	}
	defer cc.Close()
	...
	servers[0] = vendor.newTestServer()
...
func (vendor.s) TestPickFirstOneAddressRemoval(t *testing.T) {
	// Start 2 servers.
	numServers := 2
	servers, r, cleanup := startServers(t, numServers, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///localhost:"+servers[0].port, vendor.WithBalancer(pickFirstBalancerV1(r)), vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}))
	if err != nil {
		t.Fatalf("Failed to create ClientConn: %v", err)
	}
	defer cc.Close()
