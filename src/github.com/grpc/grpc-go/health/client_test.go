...
func TestClientHealthCheckBackoff(t *testing.T) {
	...
	oldBackoffFunc := vendor.backoffFunc
	vendor.backoffFunc = func(ctx context.Context, retries int) bool {
		got = append(got, time.Duration(retries+1)*time.Second)
		return true
	}
	defer func() { vendor.backoffFunc = oldBackoffFunc }()

	vendor.clientHealthCheck(context.Background(), newStream, func(_ bool) {}, "test")
