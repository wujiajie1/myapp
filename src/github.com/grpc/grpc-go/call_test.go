...
func (h *testStreamHandler) handleStream(t *testing.T, s *transport.Stream) {
	...
	for {
		...
		if pf != vendor.compressionNone {
			t.Errorf("Received the mistaken message format %d, want %d", pf, vendor.compressionNone)
			return
		}
		var v string
		codec := testCodec{}
	...
	data, err := vendor.encode(testCodec{}, &expectedResponse)
	if err != nil {
		t.Errorf("Failed to encode the response: %v", err)
		return
	}
	hdr, payload := vendor.msgHeader(data, nil)
...
func setUp(t *testing.T, port int, maxStreams uint32) (*server, *vendor.ClientConn) {
	return setUpWithOptions(t, port, maxStreams)
}

func setUpWithOptions(t *testing.T, port int, maxStreams uint32, dopts ...vendor.DialOption) (*server, *vendor.ClientConn) {
	server := newTestServer()
	go server.start(t, port, maxStreams)
	server.wait(t, 2*time.Second)
	addr := "localhost:" + server.port
	dopts = append(dopts, vendor.WithBlock(), vendor.WithInsecure(), vendor.WithCodec(testCodec{}))
	cc, err := vendor.Dial(addr, dopts...)
	if err != nil {
...
func (vendor.s) TestUnaryClientInterceptor(t *testing.T) {
	parentKey := ctxKey("parentKey")

	interceptor := func(ctx context.Context, method string, req, reply interface{}, cc *vendor.ClientConn, invoker vendor.UnaryInvoker, opts ...vendor.CallOption) error {
	...
	server, cc := setUpWithOptions(t, 0, math.MaxUint32, vendor.WithUnaryInterceptor(interceptor))
	defer func() {
		cc.Close()
		server.stop()
	}()
...
func (vendor.s) TestChainUnaryClientInterceptor(t *testing.T) {
	var (
		parentKey    = ctxKey("parentKey")
	...
	firstInt := func(ctx context.Context, method string, req, reply interface{}, cc *vendor.ClientConn, invoker vendor.UnaryInvoker, opts ...vendor.CallOption) error {
		if ctx.Value(parentKey) == nil {
	...
	secondInt := func(ctx context.Context, method string, req, reply interface{}, cc *vendor.ClientConn, invoker vendor.UnaryInvoker, opts ...vendor.CallOption) error {
		if ctx.Value(parentKey) == nil {
	...
	lastInt := func(ctx context.Context, method string, req, reply interface{}, cc *vendor.ClientConn, invoker vendor.UnaryInvoker, opts ...vendor.CallOption) error {
		if ctx.Value(parentKey) == nil {
	...
	server, cc := setUpWithOptions(t, 0, math.MaxUint32, vendor.WithChainUnaryInterceptor(firstInt, secondInt, lastInt))
	defer func() {
		cc.Close()
		server.stop()
	}()
...
func (vendor.s) TestChainOnBaseUnaryClientInterceptor(t *testing.T) {
	var (
		parentKey  = ctxKey("parentKey")
		baseIntKey = ctxKey("baseIntKey")
	)

	baseInt := func(ctx context.Context, method string, req, reply interface{}, cc *vendor.ClientConn, invoker vendor.UnaryInvoker, opts ...vendor.CallOption) error {
		if ctx.Value(parentKey) == nil {
	...
	chainInt := func(ctx context.Context, method string, req, reply interface{}, cc *vendor.ClientConn, invoker vendor.UnaryInvoker, opts ...vendor.CallOption) error {
		if ctx.Value(parentKey) == nil {
	...
	server, cc := setUpWithOptions(t, 0, math.MaxUint32, vendor.WithUnaryInterceptor(baseInt), vendor.WithChainUnaryInterceptor(chainInt))
	defer func() {
		cc.Close()
		server.stop()
	}()
...
func (vendor.s) TestChainStreamClientInterceptor(t *testing.T) {
	var (
		parentKey    = ctxKey("parentKey")
		firstIntKey  = ctxKey("firstIntKey")
		secondIntKey = ctxKey("secondIntKey")
	...
	firstInt := func(ctx context.Context, desc *vendor.StreamDesc, cc *vendor.ClientConn, method string, streamer vendor.Streamer, opts ...vendor.CallOption) (vendor.ClientStream, error) {
		if ctx.Value(parentKey) == nil {
			t.Fatalf("first interceptor should have %v in context", parentKey)
		}
		if ctx.Value(firstIntKey) != nil {
	...
	secondInt := func(ctx context.Context, desc *vendor.StreamDesc, cc *vendor.ClientConn, method string, streamer vendor.Streamer, opts ...vendor.CallOption) (vendor.ClientStream, error) {
		if ctx.Value(parentKey) == nil {
			t.Fatalf("second interceptor should have %v in context", parentKey)
		}
		if ctx.Value(firstIntKey) == nil {
	...
	lastInt := func(ctx context.Context, desc *vendor.StreamDesc, cc *vendor.ClientConn, method string, streamer vendor.Streamer, opts ...vendor.CallOption) (vendor.ClientStream, error) {
		if ctx.Value(parentKey) == nil {
			t.Fatalf("last interceptor should have %v in context", parentKey)
		}
		if ctx.Value(firstIntKey) == nil {
	...
	server, cc := setUpWithOptions(t, 0, math.MaxUint32, vendor.WithChainStreamInterceptor(firstInt, secondInt, lastInt))
	defer func() {
		cc.Close()
		server.stop()
	}()
	...
	ctx := context.Background()
	parentCtx := context.WithValue(ctx, ctxKey("parentKey"), 0)
	_, err := cc.NewStream(parentCtx, &vendor.StreamDesc{}, "/foo/bar")
	if err != nil {
		t.Fatalf("grpc.NewStream(_, _, _) = %v, want <nil>", err)
	}
}

func (vendor.s) TestInvoke(t *testing.T) {
...
func (vendor.s) TestInvokeLargeErr(t *testing.T) {
	server, cc := setUp(t, 0, math.MaxUint32)
	var reply string
	req := "hello"
	...
	if status.Code(err) != codes.Internal || len(vendor.errorDesc(err)) != sizeLargeErr {
...
func (vendor.s) TestInvokeErrorSpecialChars(t *testing.T) {
	server, cc := setUp(t, 0, math.MaxUint32)
	var reply string
	req := "weird error"
	err := cc.Invoke(context.Background(), "/foo/bar", &req, &reply)
	...
	if got, want := vendor.errorDesc(err), weirdError; got != want {
...
func (vendor.s) TestInvokeCancel(t *testing.T) {
	server, cc := setUp(t, 0, math.MaxUint32)
	var reply string
	req := "canceled"
	for i := 0; i < 100; i++ {
...
func (vendor.s) TestInvokeCancelClosedNonFailFast(t *testing.T) {
	server, cc := setUp(t, 0, math.MaxUint32)
	var reply string
	cc.Close()
	req := "hello"
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if err := cc.Invoke(ctx, "/foo/bar", &req, &reply, vendor.WaitForReady(true)); err == nil {
