...
func (vendor.s) TestStateTransitions_SingleAddress(t *testing.T) {
...
func testStateTransitionSingleAddress(t *testing.T, want []connectivity.State, server func(net.Listener) net.Conn) {
	...
	client, err := vendor.DialContext(ctx,
		...
		vendor.WithInsecure(),
		vendor.WithBalancerName(stateRecordingBalancerName),
		vendor.WithDialer(pl.Dialer()),
		vendor.withBackoff(noBackoff{}),
		vendor.withMinConnectDeadline(func() time.Duration { return time.Millisecond * 100 }))
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func (vendor.s) TestStateTransitions_ReadyToTransientFailure(t *testing.T) {
	want := []connectivity.State{
		connectivity.Connecting,
		connectivity.Ready,
		connectivity.TransientFailure,
	...
	client, err := vendor.DialContext(ctx, lis.Addr().String(), vendor.WithWaitForHandshake(), vendor.WithInsecure(), vendor.WithBalancerName(stateRecordingBalancerName))
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func (vendor.s) TestStateTransitions_TriesAllAddrsBeforeTransientFailure(t *testing.T) {
	want := []connectivity.State{
		connectivity.Connecting,
		connectivity.Ready,
	}
	...
	client, err := vendor.DialContext(ctx, "this-gets-overwritten", vendor.WithInsecure(), vendor.WithWaitForHandshake(), vendor.WithBalancerName(stateRecordingBalancerName), vendor.withResolverBuilder(rb))
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func (vendor.s) TestStateTransitions_MultipleAddrsEntersReady(t *testing.T) {
	want := []connectivity.State{
		connectivity.Connecting,
		connectivity.Ready,
		connectivity.TransientFailure,
...
func (b *stateRecordingBalancerBuilder) Build(cc balancer.ClientConn, opts balancer.BuildOptions) balancer.Balancer {
	...
	return &stateRecordingBalancer{
		...
		Balancer: balancer.Get(vendor.PickFirstBalancerName).Build(cc, opts),
	}
}
