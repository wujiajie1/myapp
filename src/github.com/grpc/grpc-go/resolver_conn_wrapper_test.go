...
func (vendor.s) TestParseTarget(t *testing.T) {
...
func (vendor.s) TestParseTargetString(t *testing.T) {
	for _, test := range []struct {
		...
		got := vendor.parseTarget(test.targetStr)
		if got != test.want {
...
// the dialer.
func (vendor.s) TestDialParseTargetUnknownScheme(t *testing.T) {
	for _, test := range []struct {
		targetStr string
		want      string
	}{
		...
		cc, err := vendor.Dial(test.targetStr, vendor.WithInsecure(), vendor.WithDialer(func(addr string, _ time.Duration) (net.Conn, error) {
			select {
			case dialStrCh <- addr:
			default:
			}
