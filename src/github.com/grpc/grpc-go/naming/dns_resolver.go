...
func NewDNSResolverWithFreq(freq time.Duration) (vendor.Resolver, error) {
...
func NewDNSResolver() (vendor.Resolver, error) {
...
func (r *dnsResolver) Resolve(target string) (vendor.Watcher, error) {
	...
	if net.ParseIP(host) != nil {
		ipWatcher := &ipWatcher{
			updateChan: make(chan *vendor.Update, 1),
		}
		host, _ = formatIP(host)
		ipWatcher.updateChan <- &vendor.Update{Op: vendor.Add, Addr: host + ":" + port}
...
type dnsWatcher struct {
	...
	curAddrs map[string]*vendor.Update
	ctx      context.Context
...
func (i *ipWatcher) Next() ([]*vendor.Update, error) {
	u, ok := <-i.updateChan
	if !ok {
		return nil, errWatcherClose
	}
	return []*vendor.Update{u}, nil
}
...
func (w *dnsWatcher) compileUpdate(newAddrs map[string]*vendor.Update) []*vendor.Update {
	var res []*vendor.Update
...
func (w *dnsWatcher) lookupSRV() map[string]*vendor.Update {
	newAddrs := make(map[string]*vendor.Update)
	...
	for _, s := range srvs {
		...
		for _, a := range lbAddrs {
			...
			newAddrs[addr] = &vendor.Update{Addr: addr,
...
func (w *dnsWatcher) lookupHost() map[string]*vendor.Update {
	newAddrs := make(map[string]*vendor.Update)
...
func (w *dnsWatcher) lookup() []*vendor.Update {
...
func (w *dnsWatcher) Next() ([]*vendor.Update, error) {
	for {
		select {
		case <-w.ctx.Done():
			return nil, errWatcherClose
