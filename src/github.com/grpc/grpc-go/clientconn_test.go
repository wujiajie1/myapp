...
func assertState(wantState connectivity.State, cc *vendor.ClientConn) (connectivity.State, bool) {
...
func (vendor.s) TestDialWithTimeout(t *testing.T) {
	...
	go func() {
		...
		framer := vendor.NewFramer(conn, conn)
		if err := framer.WriteSettings(vendor.Setting{}); err != nil {
	...
	client, err := vendor.Dial(r.Scheme()+":///test.server", vendor.WithInsecure(), vendor.WithTimeout(5*time.Second))
	close(dialDone)
	if err != nil {
...
func (vendor.s) TestDialWithMultipleBackendsNotSendingServerPreface(t *testing.T) {
	...
	client, err := vendor.Dial(r.Scheme()+":///test.server", vendor.WithInsecure())
	if err != nil {
		t.Fatalf("Dial failed. Err: %v", err)
	}
	defer client.Close()
...
func (vendor.s) TestDialWaitsForServerSettings(t *testing.T) {
	...
	for _, setting := range allReqHSSettings {
		...
		defer cancel()
		client, err := vendor.DialContext(ctx, lis.Addr().String(), vendor.WithInsecure(), vendor.WithWaitForHandshake(), vendor.WithBlock())
		close(dialDone)
		if err != nil {
...
func (vendor.s) TestDialWaitsForServerSettingsViaEnv(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	client, err := vendor.DialContext(ctx, lis.Addr().String(), vendor.WithInsecure(), vendor.WithBlock())
	close(dialDone)
	if err != nil {
		t.Fatalf("Error while dialing. Err: %v", err)
	}
...
func (vendor.s) TestDialWaitsForServerSettingsAndFails(t *testing.T) {
	// Restore current setting after test.
	old := envconfig.RequireHandshake
	defer func() { envconfig.RequireHandshake = old }()
	...
	for _, setting := range allReqHSSettings {
		...
		defer cancel()
		client, err := vendor.DialContext(ctx,
			lis.Addr().String(),
			vendor.WithInsecure(),
			vendor.WithWaitForHandshake(),
			vendor.WithBlock(),
			vendor.withBackoff(vendor.noBackoff{}),
			vendor.withMinConnectDeadline(func() time.Duration { return time.Second / 4 }))
		lis.Close()
		if err == nil {
			client.Close()
...
func (vendor.s) TestDialWaitsForServerSettingsViaEnvAndFails(t *testing.T) {
	// Set default behavior and restore current setting after test.
	old := envconfig.RequireHandshake
	envconfig.RequireHandshake = envconfig.RequireHandshakeOn
	defer func() { envconfig.RequireHandshake = old }()
...
func (vendor.s) TestDialDoesNotWaitForServerSettings(t *testing.T) {
	// Restore current setting after test.
	old := envconfig.RequireHandshake
	defer func() { envconfig.RequireHandshake = old }()
...
func (vendor.s) TestCloseConnectionWhenServerPrefaceNotReceived(t *testing.T) {
	// Restore current setting after test.
	old := envconfig.RequireHandshake
	defer func() { envconfig.RequireHandshake = old }()
	envconfig.RequireHandshake = envconfig.RequireHandshakeOn
	...
	go func() { // Launch the server.
		...
		close(accepted)
		framer := vendor.NewFramer(conn2, conn2)
		if err = framer.WriteSettings(vendor.Setting{}); err != nil {
			t.Errorf("Error while writing settings. Err: %v", err)
			return
		}
	...
	client, err := vendor.Dial(lis.Addr().String(), vendor.WithInsecure(), vendor.withMinConnectDeadline(func() time.Duration { return time.Millisecond * 500 }))
	if err != nil {
		t.Fatalf("Error while dialing. Err: %v", err)
	}
...
func (vendor.s) TestBackoffWhenNoServerPrefaceReceived(t *testing.T) {
	lis, err := net.Listen("tcp", "localhost:0")
	if err != nil {
		t.Fatalf("Error while listening. Err: %v", err)
	}
	...
	client, err := vendor.Dial(lis.Addr().String(), vendor.WithInsecure())
	if err != nil {
		t.Fatalf("Error while dialing. Err: %v", err)
	}
	defer client.Close()
...
func (vendor.s) TestConnectivityStates(t *testing.T) {
	servers, resolver, cleanup := vendor.startServers(t, 2, math.MaxUint32)
	defer cleanup()
	cc, err := vendor.Dial("passthrough:///foo.bar.com", vendor.WithBalancer(vendor.RoundRobin(resolver)), vendor.WithInsecure())
	if err != nil {
...
func (vendor.s) TestWithTimeout(t *testing.T) {
	conn, err := vendor.Dial("passthrough:///Non-Existent.Server:80", vendor.WithTimeout(time.Millisecond), vendor.WithBlock(), vendor.WithInsecure())
	if err == nil {
		conn.Close()
	}
	if err != context.DeadlineExceeded {
...
func (vendor.s) TestWithTransportCredentialsTLS(t *testing.T) {
	...
	conn, err := vendor.DialContext(ctx, "passthrough:///Non-Existent.Server:80", vendor.WithTransportCredentials(creds), vendor.WithBlock())
	if err == nil {
		conn.Close()
	}
	if err != context.DeadlineExceeded {
...
func (vendor.s) TestDefaultAuthority(t *testing.T) {
	target := "Non-Existent.Server:8080"
	conn, err := vendor.Dial(target, vendor.WithInsecure())
	if err != nil {
...
func (vendor.s) TestTLSServerNameOverwrite(t *testing.T) {
	overwriteServerName := "over.write.server.name"
	creds, err := credentials.NewClientTLSFromFile(testdata.Path("ca.pem"), overwriteServerName)
	if err != nil {
		t.Fatalf("Failed to create credentials %v", err)
	}
	conn, err := vendor.Dial("passthrough:///Non-Existent.Server:80", vendor.WithTransportCredentials(creds))
	if err != nil {
		t.Fatalf("Dial(_, _) = _, %v, want _, <nil>", err)
	}
	defer conn.Close()
...
func (vendor.s) TestWithAuthority(t *testing.T) {
	overwriteServerName := "over.write.server.name"
	conn, err := vendor.Dial("passthrough:///Non-Existent.Server:80", vendor.WithInsecure(), vendor.WithAuthority(overwriteServerName))
	if err != nil {
		t.Fatalf("Dial(_, _) = _, %v, want _, <nil>", err)
	}
	defer conn.Close()
...
func (vendor.s) TestWithAuthorityAndTLS(t *testing.T) {
	overwriteServerName := "over.write.server.name"
	creds, err := credentials.NewClientTLSFromFile(testdata.Path("ca.pem"), overwriteServerName)
	if err != nil {
		t.Fatalf("Failed to create credentials %v", err)
	}
	conn, err := vendor.Dial("passthrough:///Non-Existent.Server:80", vendor.WithTransportCredentials(creds), vendor.WithAuthority("no.effect.authority"))
	if err != nil {
		t.Fatalf("Dial(_, _) = _, %v, want _, <nil>", err)
	}
	defer conn.Close()
...
func (vendor.s) TestDial_OneBackoffPerRetryGroup(t *testing.T) {
	var attempts uint32
	...
	client, err := vendor.DialContext(ctx, "this-gets-overwritten",
		vendor.WithInsecure(),
		vendor.WithBalancerName(vendor.stateRecordingBalancerName),
		vendor.withResolverBuilder(rb),
		vendor.withMinConnectDeadline(getMinConnectTimeout))
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func (vendor.s) TestDialContextCancel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := vendor.DialContext(ctx, "Non-Existent.Server:80", vendor.WithBlock(), vendor.WithInsecure()); err != context.Canceled {
...
func (vendor.s) TestDialContextFailFast(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	_, err := vendor.DialContext(ctx, "Non-Existent.Server:80", vendor.WithBlock(), vendor.WithInsecure(), vendor.WithDialer(dialer), vendor.FailOnNonTempDialError(true))
...
type blockingBalancer struct {
	ch chan []vendor.Address
}

func newBlockingBalancer() vendor.Balancer {
	return &blockingBalancer{ch: make(chan []vendor.Address)}
}
func (b *blockingBalancer) Start(target string, config vendor.BalancerConfig) error {
	return nil
}
func (b *blockingBalancer) Up(addr vendor.Address) func(error) {
	return nil
}
func (b *blockingBalancer) Get(ctx context.Context, opts vendor.BalancerGetOptions) (addr vendor.Address, put func(), err error) {
	return vendor.Address{}, nil, nil
}
func (b *blockingBalancer) Notify() <-chan []vendor.Address {
	return b.ch
}
...
func (vendor.s) TestDialWithBlockingBalancer(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	dialDone := make(chan struct{})
	go func() {
		vendor.DialContext(ctx, "Non-Existent.Server:80", vendor.WithBlock(), vendor.WithInsecure(), vendor.WithBalancer(newBlockingBalancer()))
		close(dialDone)
	}()
	cancel()
	<-dialDone
...
func (vendor.s) TestCredentialsMisuse(t *testing.T) {
	tlsCreds, err := credentials.NewClientTLSFromFile(testdata.Path("ca.pem"), "x.test.youtube.com")
	if err != nil {
		t.Fatalf("Failed to create authenticator %v", err)
	}
	// Two conflicting credential configurations
	if _, err := vendor.Dial("passthrough:///Non-Existent.Server:80", vendor.WithTransportCredentials(tlsCreds), vendor.WithBlock(), vendor.WithInsecure()); err != vendor.errCredentialsConflict {
		t.Fatalf("Dial(_, _) = _, %v, want _, %v", err, vendor.errCredentialsConflict)
	}
	// security info on insecure connection
	if _, err := vendor.Dial("passthrough:///Non-Existent.Server:80", vendor.WithPerRPCCredentials(securePerRPCCredentials{}), vendor.WithBlock(), vendor.WithInsecure()); err != vendor.errTransportCredentialsMissing {
		t.Fatalf("Dial(_, _) = _, %v, want _, %v", err, vendor.errTransportCredentialsMissing)
	}
}

func (vendor.s) TestWithBackoffConfigDefault(t *testing.T) {
	testBackoffConfigSet(t, &vendor.DefaultBackoffConfig)
}

func (vendor.s) TestWithBackoffConfig(t *testing.T) {
	b := vendor.BackoffConfig{MaxDelay: vendor.DefaultBackoffConfig.MaxDelay / 2}
	expected := b
	testBackoffConfigSet(t, &expected, vendor.WithBackoffConfig(b))
}

func (vendor.s) TestWithBackoffMaxDelay(t *testing.T) {
	md := vendor.DefaultBackoffConfig.MaxDelay / 2
	expected := vendor.BackoffConfig{MaxDelay: md}
	testBackoffConfigSet(t, &expected, vendor.WithBackoffMaxDelay(md))
}

func testBackoffConfigSet(t *testing.T, expected *vendor.BackoffConfig, opts ...vendor.DialOption) {
	opts = append(opts, vendor.WithInsecure())
	conn, err := vendor.Dial("passthrough:///foo:80", opts...)
	if err != nil {
...
func newEmptyBalancer() vendor.Balancer {
	return &emptyBalancer{ch: make(chan []vendor.Address, 1)}
}
func (b *emptyBalancer) Start(_ string, _ vendor.BalancerConfig) error {
	b.ch <- nil
	return nil
}
func (b *emptyBalancer) Up(_ vendor.Address) func(error) {
	return nil
}
func (b *emptyBalancer) Get(_ context.Context, _ vendor.BalancerGetOptions) (vendor.Address, func(), error) {
	return vendor.Address{}, nil, nil
}
func (b *emptyBalancer) Notify() <-chan []vendor.Address {
	return b.ch
}
...
func (vendor.s) TestNonblockingDialWithEmptyBalancer(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	dialDone := make(chan error)
	go func() {
		dialDone <- func() error {
			conn, err := vendor.DialContext(ctx, "Non-Existent.Server:80", vendor.WithInsecure(), vendor.WithBalancer(newEmptyBalancer()))
			if err != nil {
				return err
			}
			return conn.Close()
...
func (vendor.s) TestResolverServiceConfigBeforeAddressNotPanic(t *testing.T) {
	r, rcleanup := manual.GenerateAndRegisterManualResolver()
	defer rcleanup()

	cc, err := vendor.Dial(r.Scheme()+":///test.server", vendor.WithInsecure())
	if err != nil {
	...
	r.UpdateState(resolver.State{vendor.ServiceConfig: `{"loadBalancingPolicy": "round_robin"}`}) // This should not panic.

	time.Sleep(time.Second) // Sleep to make sure the service config is handled by ClientConn.
}

func (vendor.s) TestResolverServiceConfigWhileClosingNotPanic(t *testing.T) {
	for i := 0; i < 10; i++ { // Run this multiple times to make sure it doesn't panic.
		...
		go cc.Close()
		go r.UpdateState(resolver.State{vendor.ServiceConfig: `{"loadBalancingPolicy": "round_robin"}`}) // This should not panic.
	}
}

func (vendor.s) TestResolverEmptyUpdateNotPanic(t *testing.T) {
	r, rcleanup := manual.GenerateAndRegisterManualResolver()
	defer rcleanup()
...
func (vendor.s) TestClientUpdatesParamsAfterGoAway(t *testing.T) {
	lis, err := net.Listen("tcp", "localhost:0")
	if err != nil {
	...
	go func() {
		...
		if err := f.WriteSettings(vendor.Setting{}); err != nil {
			t.Errorf("error writing settings: %v", err)
			return
		}
		<-connected
		if err := f.WriteGoAway(0, vendor.ErrCodeEnhanceYourCalm, []byte("too_many_pings")); err != nil {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	cc, err := vendor.DialContext(ctx, addr, vendor.WithBlock(), vendor.WithInsecure(), vendor.WithKeepaliveParams(keepalive.ClientParameters{
		Time:                10 * time.Second,
...
func (vendor.s) TestDisableServiceConfigOption(t *testing.T) {
	r, cleanup := manual.GenerateAndRegisterManualResolver()
	defer cleanup()
	addr := r.Scheme() + ":///non.existent"
	cc, err := vendor.Dial(addr, vendor.WithInsecure(), vendor.WithDisableServiceConfig())
	if err != nil {
		t.Fatalf("Dial(%s, _) = _, %v, want _, <nil>", addr, err)
	}
	defer cc.Close()
	r.UpdateState(resolver.State{vendor.ServiceConfig: `{
    "methodConfig": [
        {
            "name": [
                {
...
func (vendor.s) TestGetClientConnTarget(t *testing.T) {
	addr := "nonexist:///non.existent"
	cc, err := vendor.Dial(addr, vendor.WithInsecure())
	if err != nil {
		t.Fatalf("Dial(%s, _) = _, %v, want _, <nil>", addr, err)
	}
	defer cc.Close()
...
func (vendor.s) TestResetConnectBackoff(t *testing.T) {
	...
	cc, err := vendor.Dial("any", vendor.WithInsecure(), vendor.WithDialer(dialer), vendor.withBackoff(backoffForever{}))
	if err != nil {
...
func (vendor.s) TestBackoffCancel(t *testing.T) {
	dialStrCh := make(chan string)
	cc, err := vendor.Dial("any", vendor.WithInsecure(), vendor.WithDialer(func(t string, _ time.Duration) (net.Conn, error) {
		dialStrCh <- t
...
func (vendor.s) TestUpdateAddresses_RetryFromFirstAddr(t *testing.T) {
	lis1, err := net.Listen("tcp", "localhost:0")
	if err != nil {
		t.Fatalf("Error while listening. Err: %v", err)
	}
	...
	go func() {
		...
		go vendor.keepReading(conn1)

		framer := vendor.NewFramer(conn1, conn1)
		if err := framer.WriteSettings(vendor.Setting{}); err != nil {
		...
		stateNotifications := vendor.testBalancerBuilder.nextStateNotifier()
	...
	client, err := vendor.Dial("this-gets-overwritten",
		vendor.WithInsecure(),
		vendor.WithWaitForHandshake(),
		vendor.withResolverBuilder(rb),
		vendor.withBackoff(vendor.noBackoff{}),
		vendor.WithBalancerName(vendor.stateRecordingBalancerName),
		vendor.withMinConnectDeadline(func() time.Duration { return time.Hour }))
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func (vendor.s) TestDefaultServiceConfig(t *testing.T) {
	r, cleanup := manual.GenerateAndRegisterManualResolver()
	defer cleanup()
	addr := r.Scheme() + ":///non.existent"
	js := `{
...
func verifyWaitForReadyEqualsTrue(cc *vendor.ClientConn) bool {
	var i int
	for i = 0; i < 10; i++ {
...
func testInvalidDefaultServiceConfig(t *testing.T) {
	_, err := vendor.Dial("fake.com", vendor.WithInsecure(), vendor.WithDefaultServiceConfig(""))
	if !strings.Contains(err.Error(), vendor.invalidDefaultServiceConfigErrPrefix) {
		t.Fatalf("Dial got err: %v, want err contains: %v", err, vendor.invalidDefaultServiceConfigErrPrefix)
	}
}

func testDefaultServiceConfigWhenResolverServiceConfigDisabled(t *testing.T, r resolver.Resolver, addr string, js string) {
	cc, err := vendor.Dial(addr, vendor.WithInsecure(), vendor.WithDisableServiceConfig(), vendor.WithDefaultServiceConfig(js))
	if err != nil {
		t.Fatalf("Dial(%s, _) = _, %v, want _, <nil>", addr, err)
	}
	defer cc.Close()
...
func testDefaultServiceConfigWhenResolverDoesNotReturnServiceConfig(t *testing.T, r resolver.Resolver, addr string, js string) {
	cc, err := vendor.Dial(addr, vendor.WithInsecure(), vendor.WithDefaultServiceConfig(js))
	if err != nil {
		t.Fatalf("Dial(%s, _) = _, %v, want _, <nil>", addr, err)
	}
	defer cc.Close()
...
func testDefaultServiceConfigWhenResolverReturnInvalidServiceConfig(t *testing.T, r resolver.Resolver, addr string, js string) {
	...
	r.(*manual.Resolver).UpdateState(resolver.State{
		Addresses:            []resolver.Address{{Addr: addr}},
		vendor.ServiceConfig: "{something wrong,}",
	})
	if !verifyWaitForReadyEqualsTrue(cc) {
		t.Fatal("default service config failed to be applied after 1s")
	}
