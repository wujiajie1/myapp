...
func TestInfoServerName(t *testing.T) {
	...
	alts := vendor.NewServerCreds(vendor.DefaultServerOptions())
...
func TestOverrideServerName(t *testing.T) {
	...
	// This is not testing any handshaker functionality, so it's fine to only
	// use NewServerCreds and not NewClientCreds.
	c := vendor.NewServerCreds(vendor.DefaultServerOptions())
...
func TestCloneClient(t *testing.T) {
	wantServerName := "server.name"
	opt := vendor.DefaultClientOptions()
	opt.TargetServiceAccounts = []string{"not", "empty"}
	c := vendor.NewClientCreds(opt)
	c.OverrideServerName(wantServerName)
	cc := c.Clone()
...
func TestCompareRPCVersions(t *testing.T) {
	for _, tc := range []struct {
		...
		if got, want := vendor.compareRPCVersions(tc.v1, tc.v2), tc.output; got != want {
...
func TestCheckRPCVersions(t *testing.T) {
	for _, tc := range []struct {
		...
		output, maxCommonVersion := vendor.checkRPCVersions(tc.local, tc.peer)
		if got, want := output, tc.output; got != want {
			t.Errorf("%v: checkRPCVersions(%v, %v)=(%v, _), want (%v, _)", tc.desc, tc.local, tc.peer, got, want)
		}
		if got, want := maxCommonVersion, tc.maxCommonVersion; !vendor.Equal(got, want) {
