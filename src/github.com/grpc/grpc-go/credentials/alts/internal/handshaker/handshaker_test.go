...
var (
	testRecordProtocol = vendor.rekeyRecordProtocolName
	testKey            = []byte{
		// 44 arbitrary bytes.
...
func TestClientHandshake(t *testing.T) {
	for _, testCase := range []struct {
		...
		{100 * time.Millisecond, 10 * vendor.maxPendingHandshakes},
	} {
		...
		for i := 0; i < testCase.numberOfHandshakes; i++ {
			...
			chs := &vendor.altsHandshaker{
				stream: stream,
				conn:   tc,
				clientOpts: &vendor.ClientHandshakerOptions{
		...
		for i := 0; i < testCase.numberOfHandshakes; i++ {
			if err := <-errc; err != nil && err != vendor.errDropped {
				t.Errorf("ClientHandshake() = _, %v, want _, <nil> or %v", err, vendor.errDropped)
			}
		}

		// Ensure that there are no concurrent calls more than the limit.
		if stat.MaxConcurrentCalls > vendor.maxPendingHandshakes {
			t.Errorf("Observed %d concurrent handshakes; want <= %d", stat.MaxConcurrentCalls, vendor.maxPendingHandshakes)
		}
	}
}
...
func TestServerHandshake(t *testing.T) {
	for _, testCase := range []struct {
		...
		for i := 0; i < testCase.numberOfHandshakes; i++ {
			...
			out := new(bytes.Buffer)
			tc := testutil.NewTestConn(in, out)
			shs := &vendor.altsHandshaker{
				stream:     stream,
				conn:       tc,
				serverOpts: vendor.DefaultServerHandshakerOptions(),
		...
		for i := 0; i < testCase.numberOfHandshakes; i++ {
			if err := <-errc; err != nil && err != vendor.errDropped {
				t.Errorf("ServerHandshake() = _, %v, want _, <nil> or %v", err, vendor.errDropped)
			}
		}
