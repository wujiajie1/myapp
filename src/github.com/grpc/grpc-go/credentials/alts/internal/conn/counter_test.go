...
func TestCounterSides(t *testing.T) {
	for _, side := range []core.Side{core.ClientSide, core.ServerSide} {
		outCounter := vendor.NewOutCounter(side, testOverflowLen)
		inCounter := vendor.NewInCounter(side, testOverflowLen)
		for i := 0; i < 1024; i++ {
			value, _ := outCounter.Value()
			if g, w := vendor.CounterSide(value), side; g != w {
...
func TestCounterInc(t *testing.T) {
	for _, test := range []struct {
		...
		c := vendor.CounterFromValue(test.counter, vendor.overflowLenAES128GCM)
		c.Inc()
		value, _ := c.Value()
...
func TestRolloverCounter(t *testing.T) {
	for _, test := range []struct {
		...
		c := vendor.CounterFromValue(test.value, vendor.overflowLenAES128GCM)
		...
		c.Inc()
		_, err = c.Value()
		if err != vendor.errInvalidCounter {
			t.Errorf("%v: second Inc() + Value() unexpectedly succeeded: want %v", test.desc, vendor.errInvalidCounter)
		}
		...
		_, err = c.Value()
		if err != vendor.errInvalidCounter {
			t.Errorf("%v: Third Inc() + Value() unexpectedly succeeded: want %v", test.desc, vendor.errInvalidCounter)
		}
	}
}
