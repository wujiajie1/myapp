...
func getRekeyCryptoPair(key []byte, counter []byte, t *testing.T) (vendor.ALTSRecordCrypto, vendor.ALTSRecordCrypto) {
	client, err := vendor.NewAES128GCMRekey(core.ClientSide, key)
	if err != nil {
		t.Fatalf("NewAES128GCMRekey(ClientSide, key) = %v", err)
	}
	server, err := vendor.NewAES128GCMRekey(core.ServerSide, key)
	if err != nil {
	...
	if counter != nil {
		if vendor.CounterSide(counter) == core.ClientSide {
			client.(*vendor.aes128gcmRekey).outCounter = vendor.CounterFromValue(counter, vendor.overflowLenAES128GCMRekey)
			server.(*vendor.aes128gcmRekey).inCounter = vendor.CounterFromValue(counter, vendor.overflowLenAES128GCMRekey)
		} else {
			server.(*vendor.aes128gcmRekey).outCounter = vendor.CounterFromValue(counter, vendor.overflowLenAES128GCMRekey)
			client.(*vendor.aes128gcmRekey).inCounter = vendor.CounterFromValue(counter, vendor.overflowLenAES128GCMRekey)
		}
	}
	return client, server
}

func testRekeyEncryptRoundtrip(client vendor.ALTSRecordCrypto, server vendor.ALTSRecordCrypto, t *testing.T) {
	// Encrypt.
