...
type aes128gcmRekey struct {
	...
	inCounter  vendor.Counter
	outCounter vendor.Counter
	inAEAD     cipher.AEAD
	outAEAD    cipher.AEAD
}
...
func NewAES128GCMRekey(side core.Side, key []byte) (vendor.ALTSRecordCrypto, error) {
	inCounter := vendor.NewInCounter(side, overflowLenAES128GCMRekey)
	outCounter := vendor.NewOutCounter(side, overflowLenAES128GCMRekey)
	inAEAD, err := vendor.newRekeyAEAD(key)
	if err != nil {
		return nil, err
	}
	outAEAD, err := vendor.newRekeyAEAD(key)
	if err != nil {
		return nil, err
	}
...
func (s *aes128gcmRekey) Encrypt(dst, plaintext []byte) ([]byte, error) {
	...
	dlen := len(dst)
	dst, out := vendor.SliceForAppend(dst, len(plaintext)+vendor.GcmTagSize)
