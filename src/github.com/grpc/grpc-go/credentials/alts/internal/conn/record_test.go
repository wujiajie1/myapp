...
var (
	...
	altsRecordFuncs = map[string]vendor.ALTSRecordFunc{
		// ALTS handshaker protocols.
		"ALTSRP_GCM_AES128": func(s core.Side, keyData []byte) (vendor.ALTSRecordCrypto, error) {
			return vendor.NewAES128GCM(s, keyData)
		},
	}
)
...
func init() {
	for protocol, f := range altsRecordFuncs {
		if err := vendor.RegisterProtocol(protocol, f); err != nil {
			panic(err)
		}
	}
}
...
func newTestALTSRecordConn(in, out *bytes.Buffer, side core.Side, np string) *vendor.conn {
	key := []byte{
		// 16 arbitrary bytes.
	...
	c, err := vendor.NewConn(&tc, side, np, key, nil)
	if err != nil {
		panic(fmt.Sprintf("Unexpected error creating test ALTS record connection: %v", err))
	}
	return c.(*vendor.conn)
}

func newConnPair(np string) (client, server *vendor.conn) {
...
func testLargeMsg(t *testing.T, np string) {
	...
	msgLen := vendor.altsRecordDefaultLength - vendor.msgTypeFieldSize - clientConn.crypto.EncryptionOverhead() + 1
...
func testIncorrectMsgType(t *testing.T, np string) {
	...
	// message type.
	framedMsg := make([]byte, vendor.MsgLenFieldSize+vendor.msgTypeFieldSize)
	binary.LittleEndian.PutUint32(framedMsg[:vendor.MsgLenFieldSize], vendor.msgTypeFieldSize)
	wrongMsgType := uint32(0x22)
	binary.LittleEndian.PutUint32(framedMsg[vendor.MsgLenFieldSize:], wrongMsgType)
...
func testFrameTooLarge(t *testing.T, np string) {
	...
	payloadLen := vendor.altsRecordLengthLimit - vendor.msgTypeFieldSize - clientConn.crypto.EncryptionOverhead() + 1
	payload := make([]byte, payloadLen)
	c, err := clientConn.crypto.Encrypt(nil, payload)
	if err != nil {
		t.Fatalf(fmt.Sprintf("Error encrypting message: %v", err))
	}
	msgLen := vendor.msgTypeFieldSize + len(c)
	framedMsg := make([]byte, vendor.MsgLenFieldSize+msgLen)
	binary.LittleEndian.PutUint32(framedMsg[:vendor.MsgLenFieldSize], uint32(vendor.msgTypeFieldSize+len(c)))
	msg := framedMsg[vendor.MsgLenFieldSize:]
	binary.LittleEndian.PutUint32(msg[:vendor.msgTypeFieldSize], vendor.altsRecordMsgType)
	copy(msg[vendor.msgTypeFieldSize:], c)
	...
	if n, err := serverConn.Read(b); n != 0 || err == nil {
		t.Fatalf("Read() = <nil>, want %v", fmt.Errorf("received the frame length %d larger than the limit %d", vendor.altsRecordLengthLimit+1, vendor.altsRecordLengthLimit))
	}
}
...
func testWriteLargeData(t *testing.T, np string) {
	...
	// payloadLengthLimtit.
	msgSize := vendor.altsWriteBufferMaxSize + (100 * 1024)
