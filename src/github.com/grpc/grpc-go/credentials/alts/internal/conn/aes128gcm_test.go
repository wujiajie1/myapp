...
func getGCMCryptoPair(key []byte, counter []byte, t *testing.T) (vendor.ALTSRecordCrypto, vendor.ALTSRecordCrypto) {
	client, err := vendor.NewAES128GCM(core.ClientSide, key)
	if err != nil {
		t.Fatalf("NewAES128GCM(ClientSide, key) = %v", err)
	}
	server, err := vendor.NewAES128GCM(core.ServerSide, key)
	if err != nil {
		t.Fatalf("NewAES128GCM(ServerSide, key) = %v", err)
	}
	// set counter if provided.
	if counter != nil {
		if vendor.CounterSide(counter) == core.ClientSide {
			client.(*vendor.aes128gcm).outCounter = vendor.CounterFromValue(counter, vendor.overflowLenAES128GCM)
			server.(*vendor.aes128gcm).inCounter = vendor.CounterFromValue(counter, vendor.overflowLenAES128GCM)
		} else {
			server.(*vendor.aes128gcm).outCounter = vendor.CounterFromValue(counter, vendor.overflowLenAES128GCM)
			client.(*vendor.aes128gcm).inCounter = vendor.CounterFromValue(counter, vendor.overflowLenAES128GCM)
		}
	}
	return client, server
}

func testGCMEncryptionDecryption(sender vendor.ALTSRecordCrypto, receiver vendor.ALTSRecordCrypto, test *cryptoTestVector, withCounter bool, t *testing.T) {
...
func TestAES128GCMEncrypt(t *testing.T) {
	for _, test := range []cryptoTestVector{
		{
			key:         vendor.dehex("11754cd72aec309bf52f7687212e8957"),
			counter:     vendor.dehex("3c819d9a9bed087615030b65"),
			plaintext:   nil,
			ciphertext:  nil,
			tag:         vendor.dehex("250327c674aaf477aef2675748cf6971"),
			allocateDst: false,
		},
		{
			key:         vendor.dehex("ca47248ac0b6f8372a97ac43508308ed"),
			counter:     vendor.dehex("ffd2b598feabc9019262d2be"),
			plaintext:   nil,
			ciphertext:  nil,
			tag:         vendor.dehex("60d20404af527d248d893ae495707d1a"),
			allocateDst: false,
		},
		{
			key:         vendor.dehex("7fddb57453c241d03efbed3ac44e371c"),
			counter:     vendor.dehex("ee283a3fc75575e33efd4887"),
			plaintext:   vendor.dehex("d5de42b461646c255c87bd2962d3b9a2"),
			ciphertext:  vendor.dehex("2ccda4a5415cb91e135c2a0f78c9b2fd"),
			tag:         vendor.dehex("b36d1df9b9d5e596f83e8b7f52971cb3"),
			allocateDst: false,
		},
		{
			key:         vendor.dehex("ab72c77b97cb5fe9a382d9fe81ffdbed"),
			counter:     vendor.dehex("54cc7dc2c37ec006bcc6d1da"),
			plaintext:   vendor.dehex("007c5e5b3e59df24a7c355584fc1518d"),
			ciphertext:  vendor.dehex("0e1bde206a07a9c2c1b65300f8c64997"),
			tag:         vendor.dehex("2b4401346697138c7a4891ee59867d0c"),
			allocateDst: false,
		},
		{
			key:         vendor.dehex("11754cd72aec309bf52f7687212e8957"),
		...
		if vendor.CounterSide(test.counter) == core.ClientSide {
...
func testGCMEncryptRoundtrip(client vendor.ALTSRecordCrypto, server vendor.ALTSRecordCrypto, t *testing.T) {
	// Encrypt.
