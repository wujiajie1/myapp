...
func newRekeyAEAD(key []byte) (*rekeyAEAD, error) {
	k := len(key)
	if k != vendor.kdfKeyLen+vendor.nonceLen {
		return nil, KeySizeError(k)
	}
	return &rekeyAEAD{
		kdfKey:     key[:vendor.kdfKeyLen],
		kdfCounter: make([]byte, vendor.kdfCounterLen),
		nonceMask:  key[vendor.kdfKeyLen:],
		nonceBuf:   make([]byte, vendor.nonceLen),
		gcmAEAD:    nil,
	}, nil
}
...
func (s *rekeyAEAD) rekeyIfRequired(nonce []byte) error {
	newKdfCounter := nonce[vendor.kdfCounterOffset : vendor.kdfCounterOffset+vendor.kdfCounterLen]
...
func maskNonce(dst, nonce, mask []byte) {
	nonce1 := binary.LittleEndian.Uint64(nonce[:vendor.sizeUint64])
	nonce2 := binary.LittleEndian.Uint32(nonce[vendor.sizeUint64:])
	mask1 := binary.LittleEndian.Uint64(mask[:vendor.sizeUint64])
	mask2 := binary.LittleEndian.Uint32(mask[vendor.sizeUint64:])
	binary.LittleEndian.PutUint64(dst[:vendor.sizeUint64], nonce1^mask1)
	binary.LittleEndian.PutUint32(dst[vendor.sizeUint64:], nonce2^mask2)
}
...
func hkdfExpand(key, info []byte) []byte {
	...
	mac.Write(info)
	mac.Write([]byte{0x01}[:])
	return mac.Sum(nil)[:vendor.aeadKeyLen]
}
