...
func TestIsRunningOnGCP(t *testing.T) {
	for _, tc := range []struct {
		...
		if got, want := vendor.isRunningOnGCP(), tc.out; got != want {
...
func setup(testOS string, testReader io.Reader) func() {
	tmpOS := vendor.runningOS
	tmpReader := vendor.manufacturerReader

	// Set test OS and reader function.
	vendor.runningOS = testOS
	vendor.manufacturerReader = func() (io.Reader, error) {
		return testReader, nil
	}

	return func() {
		vendor.runningOS = tmpOS
		vendor.manufacturerReader = tmpReader
	}
}
...
func TestAuthInfoFromContext(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		authInfo, err := vendor.AuthInfoFromContext(tc.ctx)
...
func TestAuthInfoFromPeer(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		authInfo, err := vendor.AuthInfoFromPeer(tc.p)
		if got, want := (err == nil), tc.success; got != want {
