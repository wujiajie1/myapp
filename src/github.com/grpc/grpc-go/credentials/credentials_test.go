...
type serverHandshake func(net.Conn) (vendor.AuthInfo, error)

func TestClientHandshakeReturnsAuthInfo(t *testing.T) {
	done := make(chan vendor.AuthInfo, 1)
...
func compare(a1, a2 vendor.AuthInfo) bool {
	if a1.AuthType() != a2.AuthType() {
		return false
	}
	switch a1.AuthType() {
	case "tls":
		state1 := a1.(vendor.TLSInfo).State
		state2 := a2.(vendor.TLSInfo).State
...
func launchServer(t *testing.T, hs serverHandshake, done chan vendor.AuthInfo) net.Listener {
...
func serverHandle(t *testing.T, hs serverHandshake, done chan vendor.AuthInfo, lis net.Listener) {
...
func clientHandle(t *testing.T, hs func(net.Conn, string) (vendor.AuthInfo, error), lisAddr string) vendor.AuthInfo {
...
func gRPCServerHandshake(conn net.Conn) (vendor.AuthInfo, error) {
	serverTLS, err := vendor.NewServerTLSFromFile(testdata.Path("server1.pem"), testdata.Path("server1.key"))
	if err != nil {
		return nil, err
	}
...
func gRPCClientHandshake(conn net.Conn, lisAddr string) (vendor.AuthInfo, error) {
	clientTLS := vendor.NewTLS(&tls.Config{InsecureSkipVerify: true})
...
func tlsServerHandshake(conn net.Conn) (vendor.AuthInfo, error) {
	...
	return vendor.TLSInfo{State: serverConn.ConnectionState()}, nil
}

func tlsClientHandshake(conn net.Conn, _ string) (vendor.AuthInfo, error) {
	clientTLSConfig := &tls.Config{InsecureSkipVerify: true}
	clientConn := tls.Client(conn, clientTLSConfig)
	if err := clientConn.Handshake(); err != nil {
		return nil, err
	}
	return vendor.TLSInfo{State: clientConn.ConnectionState()}, nil
}
...
func TestAppendH2ToNextProtos(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := vendor.appendH2ToNextProtos(tt.ps); !reflect.DeepEqual(got, tt.want) {
