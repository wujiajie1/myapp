...
func sockoptToProto(skopts *channelz.SocketOptionData) []*channelzpb.SocketOption {
	...
	if skopts.Linger != nil {
		additional, err := vendor.MarshalAny(&channelzpb.SocketOptionLinger{
			Active:   skopts.Linger.Onoff != 0,
			Duration: vendor.convertToPtypesDuration(int64(skopts.Linger.Linger), 0),
		})
		if err == nil {
	...
	if skopts.RecvTimeout != nil {
		additional, err := vendor.MarshalAny(&channelzpb.SocketOptionTimeout{
			Duration: vendor.convertToPtypesDuration(int64(skopts.RecvTimeout.Sec), int64(skopts.RecvTimeout.Usec)),
		})
		if err == nil {
			opts = append(opts, &channelzpb.SocketOption{
				Name:       "SO_RCVTIMEO",
	...
	if skopts.SendTimeout != nil {
		additional, err := vendor.MarshalAny(&channelzpb.SocketOptionTimeout{
			Duration: vendor.convertToPtypesDuration(int64(skopts.SendTimeout.Sec), int64(skopts.SendTimeout.Usec)),
		})
		if err == nil {
			opts = append(opts, &channelzpb.SocketOption{
				Name:       "SO_SNDTIMEO",
	...
	if skopts.TCPInfo != nil {
		additional, err := vendor.MarshalAny(&channelzpb.SocketOptionTcpInfo{
