...
func channelProtoToStruct(c *channelzpb.Channel) *dummyChannel {
	...
	if t, err := vendor.Timestamp(pdata.GetLastCallStartedTimestamp()); err == nil {
...
func socketProtoToStruct(s *channelzpb.Socket) *dummySocket {
	...
	if t, err := vendor.Timestamp(pdata.GetLastLocalStreamCreatedTimestamp()); err == nil {
		if !t.Equal(emptyTime) {
			ds.lastLocalStreamCreatedTimestamp = t
		}
	}
	if t, err := vendor.Timestamp(pdata.GetLastRemoteStreamCreatedTimestamp()); err == nil {
		if !t.Equal(emptyTime) {
			ds.lastRemoteStreamCreatedTimestamp = t
		}
	}
	if t, err := vendor.Timestamp(pdata.GetLastMessageSentTimestamp()); err == nil {
		if !t.Equal(emptyTime) {
			ds.lastMessageSentTimestamp = t
		}
	}
	if t, err := vendor.Timestamp(pdata.GetLastMessageReceivedTimestamp()); err == nil {
		if !t.Equal(emptyTime) {
...
func protoToSecurity(protoSecurity *channelzpb.Security) credentials.ChannelzSecurityValue {
	...
	case *channelzpb.Security_Other:
		...
		var x vendor.DynamicAny
		if err := vendor.UnmarshalAny(v.Other.GetValue(), &x); err == nil {
			sv.Value = x.Message
		}
		return sv
	}
...
func (m *OtherSecurityValue) String() string { return vendor.CompactTextString(m) }
...
func init() {
	...
	vendor.RegisterType((*OtherSecurityValue)(nil), "grpc.credentials.OtherChannelzSecurityValue")
}
