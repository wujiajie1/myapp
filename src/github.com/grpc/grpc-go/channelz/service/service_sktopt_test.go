...
func init() {
	...
	vendor.protoToSocketOpt = protoToSocketOption
}

func convertToDuration(d *vendor.Duration) (sec int64, usec int64) {
	if d != nil {
		if dur, err := vendor.Duration(d); err == nil {
...
func protoToSocketOption(skopts []*channelzpb.SocketOption) *channelz.SocketOptionData {
	...
	for _, opt := range skopts {
		...
		case "SO_LINGER":
			...
			err := vendor.UnmarshalAny(opt.GetAdditional(), protoLinger)
			if err == nil {
				skdata.Linger = protoToLinger(protoLinger)
			}
		case "SO_RCVTIMEO":
			protoTimeout := &channelzpb.SocketOptionTimeout{}
			err := vendor.UnmarshalAny(opt.GetAdditional(), protoTimeout)
			if err == nil {
		...
		case "TCP_INFO":
			...
			err := vendor.UnmarshalAny(opt.GetAdditional(), tcpi)
			if err == nil {
...
func TestGetSocketOptions(t *testing.T) {
	...
	defer vendor.cleanupWrapper(czCleanup, t)
	ss := []*vendor.dummySocket{
		{
	...
	svr := vendor.newCZServer()
	ids := make([]int64, len(ss))
	svrID := channelz.RegisterServer(&vendor.dummyServer{}, "")
	...
	for i, s := range ss {
		...
		if !reflect.DeepEqual(metrics.GetRef(), &channelzpb.SocketRef{SocketId: ids[i], Name: strconv.Itoa(i)}) || !reflect.DeepEqual(vendor.socketProtoToStruct(metrics), s) {
			t.Fatalf("resp.GetSocket() want: metrics.GetRef() = %#v and %#v, got: metrics.GetRef() = %#v and %#v", &channelzpb.SocketRef{SocketId: ids[i], Name: strconv.Itoa(i)}, s, metrics.GetRef(), vendor.socketProtoToStruct(metrics))
		}
	}
}
