...
func convertToPtypesDuration(sec int64, usec int64) *vendor.Duration {
	return vendor.DurationProto(time.Duration(sec*1e9 + usec*1e3))
}
...
func channelTraceToProto(ct *channelz.ChannelTrace) *channelzpb.ChannelTrace {
	...
	if ts, err := vendor.TimestampProto(ct.CreationTime); err == nil {
	...
	for _, e := range ct.Events {
		...
		if ts, err := vendor.TimestampProto(e.Timestamp); err == nil {
			cte.Timestamp = ts
		}
		if e.RefID != 0 {
			switch e.RefType {
...
func channelMetricToProto(cm *channelz.ChannelMetric) *channelzpb.Channel {
	...
	if ts, err := vendor.TimestampProto(cm.ChannelData.LastCallStartedTimestamp); err == nil {
...
func securityToProto(se credentials.ChannelzSecurityValue) *channelzpb.Security {
	...
	case *credentials.OtherChannelzSecurityValue:
		...
		if anyval, err := vendor.MarshalAny(v.Value); err == nil {
...
func socketMetricToProto(sm *channelz.SocketMetric) *channelzpb.Socket {
	...
	if ts, err := vendor.TimestampProto(sm.SocketData.LastLocalStreamCreatedTimestamp); err == nil {
		s.Data.LastLocalStreamCreatedTimestamp = ts
	}
	if ts, err := vendor.TimestampProto(sm.SocketData.LastRemoteStreamCreatedTimestamp); err == nil {
		s.Data.LastRemoteStreamCreatedTimestamp = ts
	}
	if ts, err := vendor.TimestampProto(sm.SocketData.LastMessageSentTimestamp); err == nil {
		s.Data.LastMessageSentTimestamp = ts
	}
	if ts, err := vendor.TimestampProto(sm.SocketData.LastMessageReceivedTimestamp); err == nil {
		s.Data.LastMessageReceivedTimestamp = ts
	}
	s.Data.LocalFlowControlWindow = &vendor.Int64Value{Value: sm.SocketData.LocalFlowControlWindow}
	s.Data.RemoteFlowControlWindow = &vendor.Int64Value{Value: sm.SocketData.RemoteFlowControlWindow}

	if sm.SocketData.SocketOptions != nil {
		s.Data.Option = vendor.sockoptToProto(sm.SocketData.SocketOptions)
	}
...
func serverMetricToProto(sm *channelz.ServerMetric) *channelzpb.Server {
	...
	if ts, err := vendor.TimestampProto(sm.ServerData.LastCallStartedTimestamp); err == nil {
