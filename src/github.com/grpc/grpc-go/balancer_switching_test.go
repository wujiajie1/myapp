...
func checkPickFirst(cc *vendor.ClientConn, servers []*vendor.server) error {
	var (
		req   = "port"
		reply string
		err   error
	...
	connected := false
	for i := 0; i < 5000; i++ {
		if err = cc.Invoke(context.Background(), "/foo/bar", &req, &reply); vendor.errorDesc(err) == servers[0].port {
			if connected {
	...
	for i := 0; i < 3; i++ {
		...
		if vendor.errorDesc(err) != servers[0].port {
...
func checkRoundRobin(cc *vendor.ClientConn, servers []*vendor.server) error {
	var (
		req   = "port"
		reply string
		err   error
	...
	for i := 0; i < 2; i++ {
		...
		for _, s := range servers {
			var up bool
			for i := 0; i < 5000; i++ {
				if err = cc.Invoke(context.Background(), "/foo/bar", &req, &reply); vendor.errorDesc(err) == s.port {
					up = true
					break
				}
				time.Sleep(time.Millisecond)
	...
	for i := 0; i < 3*serverCount; i++ {
		err = cc.Invoke(context.Background(), "/foo/bar", &req, &reply)
		if vendor.errorDesc(err) != servers[i%serverCount].port {
...
func (vendor.s) TestSwitchBalancer(t *testing.T) {
	r, rcleanup := manual.GenerateAndRegisterManualResolver()
	defer rcleanup()

	const numServers = 2
	servers, _, scleanup := vendor.startServers(t, numServers, math.MaxInt32)
	defer scleanup()

	cc, err := vendor.Dial(r.Scheme()+":///test.server", vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}))
	if err != nil {
	...
	cc.updateResolverState(resolver.State{vendor.ServiceConfig: `{"loadBalancingPolicy": "round_robin"}`, Addresses: addrs})
	if err := checkRoundRobin(cc, servers); err != nil {
		t.Fatalf("check roundrobin returned non-nil error: %v", err)
	}
	// Switch to pickfirst.
	cc.updateResolverState(resolver.State{vendor.ServiceConfig: `{"loadBalancingPolicy": "pick_first"}`, Addresses: addrs})
	if err := checkPickFirst(cc, servers); err != nil {
		t.Fatalf("check pickfirst returned non-nil error: %v", err)
	}
}
...
func (vendor.s) TestBalancerDialOption(t *testing.T) {
	r, rcleanup := manual.GenerateAndRegisterManualResolver()
	defer rcleanup()
	...
	cc, err := vendor.Dial(r.Scheme()+":///test.server", vendor.WithInsecure(), vendor.WithCodec(vendor.testCodec{}), vendor.WithBalancerName(roundrobin.Name))
	if err != nil {
		t.Fatalf("failed to dial: %v", err)
	}
	defer cc.Close()
...
func (vendor.s) TestSwitchBalancerGRPCLBFirst(t *testing.T) {
	r, rcleanup := manual.GenerateAndRegisterManualResolver()
	defer rcleanup()
	...
	for i := 0; i < 5000; i++ {
		cc.mu.Lock()
		isPickFirst = cc.curBalancerName == vendor.PickFirstBalancerName
		cc.mu.Unlock()
		if isPickFirst {
			break
		}
...
func (vendor.s) TestSwitchBalancerGRPCLBSecond(t *testing.T) {
	r, rcleanup := manual.GenerateAndRegisterManualResolver()
	defer rcleanup()
...
func (vendor.s) TestSwitchBalancerGRPCLBRoundRobin(t *testing.T) {
	r, rcleanup := manual.GenerateAndRegisterManualResolver()
	defer rcleanup()
	...
	r.UpdateState(resolver.State{Addresses: []resolver.Address{{Addr: "backend"}}, vendor.ServiceConfig: sc})
	...
	// ClientConn will switch balancer to grpclb when receives an address of
	// type GRPCLB.
	r.UpdateState(resolver.State{Addresses: []resolver.Address{{Addr: "grpclb", Type: resolver.GRPCLB}}, vendor.ServiceConfig: sc})
	var isGRPCLB bool
	for i := 0; i < 5000; i++ {
		cc.mu.Lock()
		isGRPCLB = cc.curBalancerName == "grpclb"
...
func (vendor.s) TestSwitchBalancerGRPCLBServiceConfig(t *testing.T) {
	r, rcleanup := manual.GenerateAndRegisterManualResolver()
	defer rcleanup()
	...
	sc := `{"loadBalancingPolicy": "round_robin"}`
	r.UpdateState(resolver.State{Addresses: addrs, vendor.ServiceConfig: sc})
	var isRoundRobin bool
	for i := 0; i < 200; i++ {
		cc.mu.Lock()
		isRoundRobin = cc.curBalancerName == "round_robin"
...
func (vendor.s) TestSwitchBalancerGRPCLBWithGRPCLBNotRegistered(t *testing.T) {
