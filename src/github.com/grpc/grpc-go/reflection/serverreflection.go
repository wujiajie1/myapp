...
type serverReflectionServer struct {
	...
	serviceNames []string
	symbols      map[string]*vendor.FileDescriptorProto // map of fully-qualified names to files
}
...
func (s *serverReflectionServer) getSymbols() (svcNames []string, symbolIndex map[string]*vendor.FileDescriptorProto) {
	s.initSymbols.Do(func() {
		serviceInfo := s.s.GetServiceInfo()

		s.symbols = map[string]*vendor.FileDescriptorProto{}
...
func (s *serverReflectionServer) processFile(fd *vendor.FileDescriptorProto, processed map[string]struct{}) {
	filename := fd.GetName()
	...
	for _, dep := range fd.Dependency {
		fdenc := vendor.FileDescriptor(dep)
...
func (s *serverReflectionServer) processMessage(fd *vendor.FileDescriptorProto, prefix string, msg *vendor.DescriptorProto) {
...
func (s *serverReflectionServer) processEnum(fd *vendor.FileDescriptorProto, prefix string, en *vendor.EnumDescriptorProto) {
...
func (s *serverReflectionServer) processField(fd *vendor.FileDescriptorProto, prefix string, fld *vendor.FieldDescriptorProto) {
...
func (s *serverReflectionServer) fileDescForType(st reflect.Type) (*vendor.FileDescriptorProto, error) {
...
func decodeFileDesc(enc []byte) (*vendor.FileDescriptorProto, error) {
	raw, err := decompress(enc)
	if err != nil {
		return nil, fmt.Errorf("failed to decompress enc: %v", err)
	}

	fd := new(vendor.FileDescriptorProto)
	if err := vendor.Unmarshal(raw, fd); err != nil {
...
func fileDescContainingExtension(st reflect.Type, ext int32) (*vendor.FileDescriptorProto, error) {
	m, ok := reflect.Zero(reflect.PtrTo(st)).Interface().(vendor.Message)
	if !ok {
		return nil, fmt.Errorf("failed to create message from type: %v", st)
	}

	var extDesc *vendor.ExtensionDesc
	for id, desc := range vendor.RegisteredExtensions(m) {
		if id == ext {
			extDesc = desc
			break
		}
	...
	return decodeFileDesc(vendor.FileDescriptor(extDesc.Filename))
}
...
func (s *serverReflectionServer) allExtensionNumbersForType(st reflect.Type) ([]int32, error) {
	...
	exts := vendor.RegisteredExtensions(m)
...
func (s *serverReflectionServer) fileDescEncodingByFilename(name string) ([]byte, error) {
	enc := vendor.FileDescriptor(name)
	if enc == nil {
...
func parseMetadata(meta interface{}) ([]byte, bool) {
	...
	if fileNameForMeta, ok := meta.(string); ok {
		return vendor.FileDescriptor(fileNameForMeta), true
	}
