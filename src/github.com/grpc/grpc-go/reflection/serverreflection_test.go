...
var (
	s = &vendor.serverReflectionServer{}
	// fileDescriptor of each test proto file.
	fdTest       *vendor.FileDescriptorProto
	fdTestv3     *vendor.FileDescriptorProto
	fdProto2     *vendor.FileDescriptorProto
	fdProto2Ext  *vendor.FileDescriptorProto
	fdProto2Ext2 *vendor.FileDescriptorProto
...
func loadFileDesc(filename string) (*vendor.FileDescriptorProto, []byte) {
	enc := vendor.FileDescriptor(filename)
	if enc == nil {
		panic(fmt.Sprintf("failed to find fd for file: %v", filename))
	}
	fd, err := vendor.decodeFileDesc(enc)
	if err != nil {
		panic(fmt.Sprintf("failed to decode enc: %v", err))
	}
	b, err := vendor.Marshal(fd)
	if err != nil {
...
func TestTypeForName(t *testing.T) {
	for _, test := range []struct {
		...
		r, err := vendor.typeForName(test.name)
...
func TestFileDescContainingExtension(t *testing.T) {
	for _, test := range []struct {
		...
		fd, err := vendor.fileDescContainingExtension(test.st, test.extNum)
		if err != nil || !vendor.Equal(fd, test.want) {
