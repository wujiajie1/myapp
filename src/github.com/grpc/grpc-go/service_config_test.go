...
func (vendor.s) TestParseLoadBalancer(t *testing.T) {
	testcases := []struct {
		scjs    string
		wantSC  *vendor.ServiceConfig
		wantErr bool
	}{
		{
			`{
    ...
    "methodConfig": [
        {
			...
			&vendor.ServiceConfig{
				...
				Methods: map[string]vendor.MethodConfig{
					"/foo/Bar": {
	...
	for _, c := range testcases {
		sc, err := vendor.parseServiceConfig(c.scjs)
...
func (vendor.s) TestParseWaitForReady(t *testing.T) {
	testcases := []struct {
		scjs    string
		wantSC  *vendor.ServiceConfig
		wantErr bool
...
func (vendor.s) TestPraseTimeOut(t *testing.T) {
	testcases := []struct {
		scjs    string
		wantSC  *vendor.ServiceConfig
		wantErr bool
...
func (vendor.s) TestPraseMsgSize(t *testing.T) {
	testcases := []struct {
		scjs    string
		wantSC  *vendor.ServiceConfig
		wantErr bool
    ...
    "methodConfig": [
        {
			...
			&vendor.ServiceConfig{
				Methods: map[string]vendor.MethodConfig{
					"/foo/Bar": {
						MaxReqSize:  vendor.newInt(1024),
						MaxRespSize: vendor.newInt(2048),
					},
				},
			},
			false,
...
func (vendor.s) TestParseDuration(t *testing.T) {
	testCases := []struct {
		s    *string
		want *time.Duration
		err  bool
	...
	for _, tc := range testCases {
		got, err := vendor.parseDuration(tc.s)
		if tc.err != (err != nil) ||
			(got == nil) != (tc.want == nil) ||
			(got != nil && *got != *tc.want) {
			wantErr := "<nil>"
...
func scCompareWithRawJSONSkipped(s1, s2 *vendor.ServiceConfig) bool {
	if s1 == nil && s2 == nil {
		return true
	}
	if (s1 == nil) != (s2 == nil) {
