...
func newCCBalancerWrapper(cc *vendor.ClientConn, b balancer.Builder, bopts balancer.BuildOptions) *ccBalancerWrapper {
...
func (ccb *ccBalancerWrapper) watcher() {
	for {
		...
		case <-ccb.done:
			...
			ccb.mu.Unlock()
			for acbw := range scs {
				ccb.cc.removeAddrConn(acbw.getAddrConn(), vendor.errConnDrain)
			}
			return
		default:
		}
...
func (ccb *ccBalancerWrapper) updateResolverState(s resolver.State) {
	if ccb.cc.curBalancerName != vendor.grpclbName {
...
func (acbw *acBalancerWrapper) UpdateAddresses(addrs []resolver.Address) {
	...
	if len(addrs) <= 0 {
		acbw.ac.tearDown(vendor.errConnDrain)
		return
	}
...
func (acbw *acBalancerWrapper) getAddrConn() *vendor.addrConn {
	acbw.mu.Lock()
	defer acbw.mu.Unlock()
	return acbw.ac
}
