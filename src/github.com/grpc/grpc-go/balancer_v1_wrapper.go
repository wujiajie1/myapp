...
type balancerWrapperBuilder struct {
	b vendor.Balancer // The v1 balancer.
}

func (bwb *balancerWrapperBuilder) Build(cc balancer.ClientConn, opts balancer.BuildOptions) balancer.Balancer {
	bwb.b.Start(opts.Target.Endpoint, vendor.BalancerConfig{
		DialCreds: opts.DialCreds,
		Dialer:    opts.Dialer,
	})
	_, pickfirst := bwb.b.(*vendor.pickFirst)
	bw := &balancerWrapper{
		balancer:   bwb.b,
		pickfirst:  pickfirst,
		cc:         cc,
...
type scState struct {
	addr vendor.Address // The v1 address type.
	s    connectivity.State
	down func(error)
}

type balancerWrapper struct {
	balancer  vendor.Balancer // The v1 balancer.
	pickfirst bool
...
func (bw *balancerWrapper) lbWatcher() {
	...
	if notifyCh == nil {
		...
		if err != nil {
			...
			bw.connSt[sc] = &scState{
				addr: vendor.Address{Addr: bw.targetAddr},
				s:    connectivity.Idle,
			}
			bw.mu.Unlock()
			sc.Connect()
	...
	for addrs := range notifyCh {
		...
		if bw.pickfirst {
			...
			resAddrs := make(map[resolver.Address]vendor.Address)
			for _, a := range addrs {
...
func (bw *balancerWrapper) HandleSubConnStateChange(sc balancer.SubConn, s connectivity.State) {
	...
	if oldS != connectivity.Ready && s == connectivity.Ready {
		...
		if scSt.down != nil {
			scSt.down(vendor.errConnClosing)
		}
	}
...
func (bw *balancerWrapper) Pick(ctx context.Context, opts balancer.PickOptions) (sc balancer.SubConn, done func(balancer.DoneInfo), err error) {
	...
	if ss, ok := vendor.rpcInfoFromContext(ctx); ok {
		failfast = ss.failfast
	}
	a, p, err := bw.balancer.Get(ctx, vendor.BalancerGetOptions{BlockingWait: !failfast})
	if err != nil {
		return nil, nil, err
	}
	if p != nil {
