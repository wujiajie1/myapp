...
func overwrite(hpfe func(req *http.Request) (*url.URL, error)) func() {
	backHPFE := vendor.httpProxyFromEnvironment
	vendor.httpProxyFromEnvironment = hpfe
	return func() {
		vendor.httpProxyFromEnvironment = backHPFE
	}
}
...
func testHTTPConnect(t *testing.T, proxyURLModify func(*url.URL) *url.URL, proxyReqCheck func(*http.Request) error) {
	...
	dialer := vendor.newProxyDialer(func(ctx context.Context, addr string) (net.Conn, error) {
...
func (vendor.s) TestHTTPConnect(t *testing.T) {
	testHTTPConnect(t,
		...
		func(req *http.Request) error {
			...
			if req.UserAgent() != vendor.grpcUA {
				return fmt.Errorf("unexpect user agent %q, want %q", req.UserAgent(), vendor.grpcUA)
			}
			return nil
		},
	)
...
func (vendor.s) TestHTTPConnectBasicAuth(t *testing.T) {
	const (
		user     = "notAUser"
	...
	testHTTPConnect(t,
		...
		func(req *http.Request) error {
			...
			if got := req.Header.Get(vendor.proxyAuthHeaderKey); got != wantProxyAuthStr {
...
func (vendor.s) TestMapAddressEnv(t *testing.T) {
	// Overwrite the function in the test and restore them in defer.
	hpfe := func(req *http.Request) (*url.URL, error) {
	...
	got, err := vendor.mapAddress(context.Background(), envTestAddr)
	if err != nil {
		t.Error(err)
	}
