...
type server struct {
	...
	mu         sync.Mutex
	conns      map[vendor.ServerTransport]bool
	h          *testStreamHandler
	ready      chan struct{}
}
...
var (
	...
	expectedRequestLarge       = make([]byte, vendor.initialWindowSize*2)
	expectedResponseLarge      = make([]byte, vendor.initialWindowSize*2)
...
func (h *testStreamHandler) handleStreamAndNotify(s *vendor.Stream) {
	if h.notify == nil {
		return
	}
	go func() {
...
func (h *testStreamHandler) handleStream(t *testing.T, s *vendor.Stream) {
	req := expectedRequest
	...
	h.t.Write(s, nil, resp, &vendor.Options{})
	// send the trailer to end the stream.
	h.t.WriteStatus(s, status.New(codes.OK, ""))
}

func (h *testStreamHandler) handleStreamPingPong(t *testing.T, s *vendor.Stream) {
	header := make([]byte, 5)
	for {
		...
		copy(buf[5:], msg)
		h.t.Write(s, nil, buf, &vendor.Options{})
	}
}

func (h *testStreamHandler) handleStreamMisbehave(t *testing.T, s *vendor.Stream) {
	conn, ok := s.st.(*vendor.http2Server)
	if !ok {
	...
	var sent int
	p := make([]byte, vendor.http2MaxFrameLen)
	for sent < vendor.initialWindowSize {
		n := vendor.initialWindowSize - sent
		// The last message may be smaller than http2MaxFrameLen
		if n <= vendor.http2MaxFrameLen {
		...
		conn.controlBuf.put(&vendor.dataFrame{
			streamID:    s.id,
			h:           nil,
			d:           p,
			onEachWrite: func() {},
...
func (h *testStreamHandler) handleStreamEncodingRequiredStatus(t *testing.T, s *vendor.Stream) {
	// raw newline is not accepted by http2 framer so it must be encoded.
	h.t.WriteStatus(s, encodingTestStatus)
}

func (h *testStreamHandler) handleStreamInvalidHeaderField(t *testing.T, s *vendor.Stream) {
	headerFields := []vendor.HeaderField{}
	headerFields = append(headerFields, vendor.HeaderField{Name: "content-type", Value: expectedInvalidHeaderField})
	h.t.controlBuf.put(&vendor.headerFrame{
		streamID:  s.id,
		hf:        headerFields,
		endStream: false,
	})
...
func (h *testStreamHandler) handleStreamDelayRead(t *testing.T, s *vendor.Stream) {
	req := expectedRequest
	resp := expectedResponse
	if s.Method() == "foo.Large" {
		req = expectedRequestLarge
	...
	go func() {
		for {
			...
			if getTotal() == vendor.defaultWindowSize {
	...
	// until that happens.
	if err := h.t.Write(s, nil, resp, &vendor.Options{}); err != nil {
...
func (s *server) start(t *testing.T, port int, serverConfig *vendor.ServerConfig, ht hType) {
	var err error
	if port == 0 {
	...
	s.port = p
	s.conns = make(map[vendor.ServerTransport]bool)
	s.startedErr <- nil
	for {
		...
		transport, err := vendor.NewServerTransport("http2", conn, serverConfig)
		if err != nil {
			return
		}
		s.mu.Lock()
		...
		h := &testStreamHandler{t: transport.(*vendor.http2Server)}
		s.h = h
		s.mu.Unlock()
		switch ht {
		case notifyCall:
		...
		case suspended:
			go transport.HandleStreams(func(*vendor.Stream) {}, // Do nothing to handle the stream.
				func(ctx context.Context, method string) context.Context {
					return ctx
				})
		case misbehaved:
			go transport.HandleStreams(func(s *vendor.Stream) {
...
func setUpServerOnly(t *testing.T, port int, serverConfig *vendor.ServerConfig, ht hType) *server {
...
func setUp(t *testing.T, port int, maxStreams uint32, ht hType) (*server, *vendor.http2Client, func()) {
	return setUpWithOptions(t, port, &vendor.ServerConfig{MaxStreams: maxStreams}, ht, vendor.ConnectOptions{})
}

func setUpWithOptions(t *testing.T, port int, serverConfig *vendor.ServerConfig, ht hType, copts vendor.ConnectOptions) (*server, *vendor.http2Client, func()) {
	server := setUpServerOnly(t, port, serverConfig, ht)
	addr := "localhost:" + server.port
	target := vendor.TargetInfo{
		Addr: addr,
	}
	connectCtx, cancel := context.WithDeadline(context.Background(), time.Now().Add(2*time.Second))
	ct, connErr := vendor.NewClientTransport(connectCtx, context.Background(), target, copts, func() {}, func(vendor.GoAwayReason) {}, func() {})
	if connErr != nil {
		cancel() // Do not cancel in success path.
		t.Fatalf("failed to create transport: %v", connErr)
	}
	return server, ct.(*vendor.http2Client), cancel
}

func setUpWithNoPingServer(t *testing.T, copts vendor.ConnectOptions, done chan net.Conn) (*vendor.http2Client, func()) {
	...
	connectCtx, cancel := context.WithDeadline(context.Background(), time.Now().Add(2*time.Second))
	tr, err := vendor.NewClientTransport(connectCtx, context.Background(), vendor.TargetInfo{Addr: lis.Addr().String()}, copts, func() {}, func(vendor.GoAwayReason) {}, func() {})
	if err != nil {
		cancel() // Do not cancel in success path.
		// Server clean-up.
		lis.Close()
	...
	return tr.(*vendor.http2Client), cancel
}
...
func TestInflightStreamClosing(t *testing.T) {
	serverConfig := &vendor.ServerConfig{}
	server, client, cancel := setUpWithOptions(t, 0, serverConfig, suspended, vendor.ConnectOptions{})
	defer cancel()
	defer server.stop()
	defer client.Close()

	stream, err := client.NewStream(context.Background(), &vendor.CallHdr{})
	if err != nil {
	...
	go func() {
		defer close(donec)
		if _, err := stream.Read(make([]byte, vendor.defaultWindowSize)); err != serr {
...
func TestMaxConnectionIdle(t *testing.T) {
	...
	client.closeStream(stream, io.EOF, true, vendor.ErrCodeCancel, nil, nil, false)
...
func TestMaxConnectionIdleNegative(t *testing.T) {
	...
	defer server.stop()
	defer client.Close()
	_, err := client.NewStream(context.Background(), &vendor.CallHdr{})
	if err != nil {
		t.Fatalf("Client failed to create RPC request: %v", err)
	}
	timeout := time.NewTimer(time.Second * 4)
...
func TestKeepaliveServer(t *testing.T) {
	...
	server, c, cancel := setUpWithOptions(t, 0, serverConfig, suspended, vendor.ConnectOptions{})
	defer cancel()
	defer server.stop()
	defer c.Close()
	...
	if n, err := client.Write(vendor.clientPreface); err != nil || n != len(vendor.clientPreface) {
		t.Fatalf("Error writing client preface; n=%v, err=%v", n, err)
	}
	framer := vendor.newFramer(client, defaultWriteBufSize, defaultReadBufSize, 0)
	if err := framer.fr.WriteSettings(vendor.Setting{}); err != nil {
...
func TestKeepaliveServerNegative(t *testing.T) {
	...
	if client.state != vendor.reachable {
...
func TestKeepaliveClientClosesIdleTransport(t *testing.T) {
	...
	tr, cancel := setUpWithNoPingServer(t, vendor.ConnectOptions{KeepaliveParams: keepalive.ClientParameters{
...
func TestKeepaliveClientClosesWithActiveStreams(t *testing.T) {
	...
	defer conn.Close()
	// Create a stream.
	_, err := tr.NewStream(context.Background(), &vendor.CallHdr{})
	if err != nil {
...
func TestKeepaliveClientStaysHealthyWithResponsiveServer(t *testing.T) {
	s, tr, cancel := setUpWithOptions(t, 0, &vendor.ServerConfig{MaxStreams: math.MaxUint32}, normal, vendor.ConnectOptions{KeepaliveParams: keepalive.ClientParameters{
		Time:                2 * time.Second, // Keepalive time = 2 sec.
		Timeout:             1 * time.Second, // Keepalive timeout = 1 sec.
		PermitWithoutStream: true,            // Run keepalive even with no RPCs.
	}})
...
func TestKeepaliveServerEnforcementWithAbusiveClientNoRPC(t *testing.T) {
	...
	clientOptions := vendor.ConnectOptions{
...
func TestKeepaliveServerEnforcementWithAbusiveClientWithRPC(t *testing.T) {
	...
	if _, err := client.NewStream(context.Background(), &vendor.CallHdr{}); err != nil {
...
func performOneRPC(ct vendor.ClientTransport) {
	callHdr := &vendor.CallHdr{
		Host:   "localhost",
		Method: "foo.Small",
	}
...
func TestClientMix(t *testing.T) {
	...
	go func(ct vendor.ClientTransport) {
		<-ct.Error()
		ct.Close()
	}(ct)
	for i := 0; i < 1000; i++ {
...
func TestLargeMessage(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			...
			if err := ct.Write(s, []byte{}, expectedRequestLarge, &vendor.Options{Last: true}); err != nil && err != io.EOF {
...
func TestLargeMessageWithDelayRead(t *testing.T) {
	...
	sc := &vendor.ServerConfig{
		InitialWindowSize:     vendor.defaultWindowSize,
		InitialConnWindowSize: vendor.defaultWindowSize,
	}
	co := vendor.ConnectOptions{
		InitialWindowSize:     vendor.defaultWindowSize,
		InitialConnWindowSize: vendor.defaultWindowSize,
	}
	...
	// flow control and the other side won't send a window update
	// until that happens.
	if err := ct.Write(s, []byte{}, expectedRequestLarge, &vendor.Options{}); err != nil {
	...
	if err := ct.Write(s, []byte{}, expectedRequestLarge, &vendor.Options{Last: true}); err != nil {
		t.Fatalf("Write(_, _, _) = %v, want <nil>", err)
	}
	if _, err = s.Read(p); err != io.EOF {
...
func TestGracefulClose(t *testing.T) {
	...
	ctx, cancel := context.WithDeadline(context.Background(), time.Now().Add(time.Second*10))
	defer cancel()
	s, err := ct.NewStream(ctx, &vendor.CallHdr{})
	if err != nil {
	...
	if err := ct.Write(s, outgoingHeader, msg, &vendor.Options{}); err != nil {
	...
	for i := 0; i < 200; i++ {
		...
		go func() {
			defer wg.Done()
			str, err := ct.NewStream(context.Background(), &vendor.CallHdr{})
			if err == vendor.ErrConnClosing {
				return
			} else if err != nil {
				t.Errorf("_.NewStream(_, _) = _, %v, want _, %v", err, vendor.ErrConnClosing)
				return
			}
			ct.Write(str, nil, nil, &vendor.Options{Last: true})
			if _, err := str.Read(make([]byte, 8)); err != vendor.errStreamDrain && err != vendor.ErrConnClosing {
				t.Errorf("_.Read(_) = _, %v, want _, %v or %v", err, vendor.errStreamDrain, vendor.ErrConnClosing)
			}
		}()
	}
	ct.Write(s, nil, nil, &vendor.Options{Last: true})
...
func TestLargeMessageSuspension(t *testing.T) {
	...
	go func() {
		<-ctx.Done()
		ct.CloseStream(s, vendor.ContextErr(ctx.Err()))
	}()
	// Write should not be done successfully due to flow control.
	msg := make([]byte, vendor.initialWindowSize*8)
	ct.Write(s, nil, msg, &vendor.Options{})
	err = ct.Write(s, nil, msg, &vendor.Options{Last: true})
	if err != vendor.errStreamDone {
...
func TestMaxStreams(t *testing.T) {
	...
	server, ct, cancel := setUpWithOptions(t, 0, serverConfig, suspended, vendor.ConnectOptions{})
	defer cancel()
	defer ct.Close()
	defer server.stop()
	callHdr := &vendor.CallHdr{
...
func TestServerContextCanceledOnClosedConnection(t *testing.T) {
	...
	ct.controlBuf.put(&vendor.dataFrame{
		streamID:    s.id,
		endStream:   false,
		h:           nil,
		d:           make([]byte, vendor.http2MaxFrameLen),
		onEachWrite: func() {},
	})
	// Loop until the server side stream is created.
	var ss *vendor.Stream
	for {
...
func TestClientConnDecoupledFromApplicationRead(t *testing.T) {
	connectOptions := vendor.ConnectOptions{
		InitialWindowSize:     vendor.defaultWindowSize,
		InitialConnWindowSize: vendor.defaultWindowSize,
	}
	server, client, cancel := setUpWithOptions(t, 0, &vendor.ServerConfig{}, notifyCall, connectOptions)
	defer cancel()
	defer server.stop()
	defer client.Close()
	...
	var st *vendor.http2Server
	server.mu.Lock()
	for k := range server.conns {
		st = k.(*vendor.http2Server)
	}
	notifyChan := make(chan struct{})
	server.h.notify = notifyChan
	server.mu.Unlock()
	cstream1, err := client.NewStream(context.Background(), &vendor.CallHdr{})
	if err != nil {
		t.Fatalf("Client failed to create first stream. Err: %v", err)
	}

	<-notifyChan
	var sstream1 *vendor.Stream
	...
	if err := st.Write(sstream1, []byte{}, make([]byte, vendor.defaultWindowSize), &vendor.Options{}); err != nil {
	...
	cstream2, err := client.NewStream(context.Background(), &vendor.CallHdr{})
	if err != nil {
		t.Fatalf("Client failed to create second stream. Err: %v", err)
	}
	<-notifyChan
	var sstream2 *vendor.Stream
	st.mu.Lock()
	for _, v := range st.activeStreams {
		if v.id == cstream2.id {
			sstream2 = v
	...
	if err := st.Write(sstream2, []byte{}, make([]byte, vendor.defaultWindowSize), &vendor.Options{}); err != nil {
		t.Fatalf("Server failed to write data. Err: %v", err)
	}

	// Client should be able to read data on second stream.
	if _, err := cstream2.Read(make([]byte, vendor.defaultWindowSize)); err != nil {
		t.Fatalf("_.Read(_) = _, %v, want _, <nil>", err)
	}

	// Client should be able to read data on first stream.
	if _, err := cstream1.Read(make([]byte, vendor.defaultWindowSize)); err != nil {
		t.Fatalf("_.Read(_) = _, %v, want _, <nil>", err)
	}
}
...
func TestServerConnDecoupledFromApplicationRead(t *testing.T) {
	...
	if err := client.Write(cstream1, nil, make([]byte, vendor.defaultWindowSize), &vendor.Options{Last: true}); err != nil {
	...
	if err := client.Write(cstream2, nil, make([]byte, vendor.defaultWindowSize), &vendor.Options{}); err != nil {
		t.Fatalf("Client failed to write data. Err: %v", err)
	}
	...
	if _, err := sstream1.Read(make([]byte, vendor.defaultWindowSize)); err != nil {
...
func TestServerWithMisbehavedClient(t *testing.T) {
	server := setUpServerOnly(t, 0, &vendor.ServerConfig{}, suspended)
	defer server.stop()
	...
	if n, err := mconn.Write(vendor.clientPreface); err != nil || n != len(vendor.clientPreface) {
		t.Fatalf("mconn.Write(clientPreface) = %d, %v, want %d, <nil>", n, err, len(vendor.clientPreface))
	}
	// success chan indicates that reader received a RSTStream from server.
	success := make(chan struct{})
	var mu sync.Mutex
	framer := vendor.NewFramer(mconn, mconn)
	...
	go func() { // Launch a reader for this misbehaving client.
		for {
			...
			case *vendor.RSTStreamFrame:
				if frame.Header().StreamID != 1 || vendor.ErrCode(frame.ErrCode) != vendor.ErrCodeFlowControl {
					t.Errorf("RST stream received with streamID: %d and code: %v, want streamID: 1 and code: http2.ErrCodeFlowControl", frame.Header().StreamID, vendor.ErrCode(frame.ErrCode))
				}
				close(success)
				return
			default:
	...
	henc := vendor.NewEncoder(&buf)
	// TODO(mmukhi): Remove unnecessary fields.
	if err := henc.WriteField(vendor.HeaderField{Name: ":method", Value: "POST"}); err != nil {
		t.Fatalf("Error while encoding header: %v", err)
	}
	if err := henc.WriteField(vendor.HeaderField{Name: ":path", Value: "foo"}); err != nil {
		t.Fatalf("Error while encoding header: %v", err)
	}
	if err := henc.WriteField(vendor.HeaderField{Name: ":authority", Value: "localhost"}); err != nil {
		t.Fatalf("Error while encoding header: %v", err)
	}
	if err := henc.WriteField(vendor.HeaderField{Name: "content-type", Value: "application/grpc"}); err != nil {
		t.Fatalf("Error while encoding header: %v", err)
	}
	mu.Lock()
	if err := framer.WriteHeaders(vendor.HeadersFrameParam{StreamID: 1, BlockFragment: buf.Bytes(), EndHeaders: true}); err != nil {
		mu.Unlock()
	...
	dbuf := make([]byte, vendor.http2MaxFrameLen)
	for {
		select {
		case <-timer.C:
...
func TestClientWithMisbehavedServer(t *testing.T) {
	...
	go func() { // Launch the misbehaving server.
		...
		defer sconn.Close()
		if _, err := io.ReadFull(sconn, make([]byte, len(vendor.clientPreface))); err != nil {
			t.Errorf("Error while reading clieng preface: %v", err)
			return
		}
		sfr := vendor.NewFramer(sconn, sconn)
	...
	connectCtx, cancel := context.WithDeadline(context.Background(), time.Now().Add(2*time.Second))
	defer cancel()
	ct, err := vendor.NewClientTransport(connectCtx, context.Background(), vendor.TargetInfo{Addr: lis.Addr().String()}, vendor.ConnectOptions{}, func() {}, func(vendor.GoAwayReason) {}, func() {})
	if err != nil {
...
func TestEncodingRequiredStatus(t *testing.T) {
	...
	opts := vendor.Options{Last: true}
	if err := ct.Write(s, nil, expectedRequest, &opts); err != nil && err != vendor.errStreamDone {
		t.Fatalf("Failed to write the request: %v", err)
	}
	p := make([]byte, vendor.http2MaxFrameLen)
	if _, err := s.trReader.(*vendor.transportReader).Read(p); err != io.EOF {
...
func TestInvalidHeaderField(t *testing.T) {
	...
	p := make([]byte, vendor.http2MaxFrameLen)
	_, err = s.trReader.(*vendor.transportReader).Read(p)
...
func TestIsReservedHeader(t *testing.T) {
	...
	for _, tt := range tests {
		got := vendor.isReservedHeader(tt.h)
		if got != tt.want {
...
func TestContextErr(t *testing.T) {
	for _, test := range []struct {
		...
		err := vendor.ContextErr(test.errIn)
...
func TestAccountCheckWindowSizeWithSmallWindow(t *testing.T) {
	wc := windowSizeConfig{
		serverStream: vendor.defaultWindowSize,
		// Note this is smaller than initialConnWindowSize which is the current default.
		serverConn:   vendor.defaultWindowSize,
		clientStream: vendor.defaultWindowSize,
		clientConn:   vendor.defaultWindowSize,
	}
	testFlowControlAccountCheck(t, 1024*1024, wc)
}
...
func testFlowControlAccountCheck(t *testing.T, msgSize int, wc windowSizeConfig) {
	...
	server.mu.Unlock()
	const numStreams = 10
	clientStreams := make([]*vendor.Stream, numStreams)
	for i := 0; i < numStreams; i++ {
		var err error
		clientStreams[i], err = client.NewStream(context.Background(), &vendor.CallHdr{})
		if err != nil {
	...
	wg.Wait()
	serverStreams := map[uint32]*vendor.Stream{}
	loopyClientStreams := map[uint32]*vendor.outStream{}
	loopyServerStreams := map[uint32]*vendor.outStream{}
	...
	// Close all streams
	for _, stream := range clientStreams {
		client.Write(stream, nil, nil, &vendor.Options{Last: true})
...
func TestReadGivesSameErrorAfterAnyErrorOccurs(t *testing.T) {
	testRecvBuffer := vendor.newRecvBuffer()
	s := &vendor.Stream{
	...
	s.write(vendor.recvMsg{data: testData, err: testErr})
	...
	s.write(vendor.recvMsg{data: testData, err: nil})
	s.write(vendor.recvMsg{data: testData, err: errors.New("different error from first")})
...
func TestHeaderTblSize(t *testing.T) {
	...
	vendor.updateHeaderTblSize = func(e *vendor.Encoder, v uint32) {
	...
	defer ct.Close()
	defer server.stop()
	_, err := ct.NewStream(context.Background(), &vendor.CallHdr{})
	if err != nil {
		t.Fatalf("failed to open stream: %v", err)
	}

	var svrTransport vendor.ServerTransport
	var i int
	for i = 0; i < 1000; i++ {
		server.mu.Lock()
		if len(server.conns) != 0 {
	...
	svrTransport.(*vendor.http2Server).controlBuf.put(&vendor.outgoingSettings{
		ss: []vendor.Setting{
			{
				ID:  vendor.SettingHeaderTableSize,
				Val: uint32(100),
			},
		},
	})
	...
	ct.controlBuf.put(&vendor.outgoingSettings{
		ss: []vendor.Setting{
			{
				ID:  vendor.SettingHeaderTableSize,
				Val: uint32(200),
