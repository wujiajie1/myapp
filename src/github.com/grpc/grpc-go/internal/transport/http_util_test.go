...
func TestTimeoutDecode(t *testing.T) {
	for _, test := range []struct {
		...
		d, err := vendor.decodeTimeout(test.s)
...
func TestContentSubtype(t *testing.T) {
	...
	for _, tt := range tests {
		got, gotValid := vendor.contentSubtype(tt.contentType)
...
func TestEncodeGrpcMessage(t *testing.T) {
	for _, tt := range []struct {
		...
		actual := vendor.encodeGrpcMessage(tt.input)
	...
	for i := ' '; i <= '~' && i != '%'; i++ {
		output := vendor.encodeGrpcMessage(string(i))
		if output != string(i) {
...
func TestDecodeGrpcMessage(t *testing.T) {
	for _, tt := range []struct {
		...
		actual := vendor.decodeGrpcMessage(tt.input)
		if tt.expected != actual {
	...
	for i := ' '; i <= '~' && i != '%'; i++ {
		output := vendor.decodeGrpcMessage(string(i))
		if output != string(i) {
	...
	for i := rune(0); i == '%' || (i >= rune(0) && i < ' ') || (i > '~' && i <= rune(127)); i++ {
		output := vendor.decodeGrpcMessage(fmt.Sprintf("%%%02X", i))
		if output != string(i) {
...
func TestDecodeEncodeGrpcMessage(t *testing.T) {
	...
	for _, tC := range testCases {
		got := vendor.decodeGrpcMessage(vendor.encodeGrpcMessage(tC.orig))
		if got != tC.want {
...
func TestEncodeMetadataHeader(t *testing.T) {
	for _, test := range []struct {
		...
		v := vendor.encodeMetadataHeader(test.kin, test.vin)
...
func TestDecodeMetadataHeader(t *testing.T) {
	for _, test := range []struct {
		...
		v, err := vendor.decodeMetadataHeader(test.kin, test.vin)
