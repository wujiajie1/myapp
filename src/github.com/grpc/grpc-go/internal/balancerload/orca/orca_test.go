...
var (
	...
	testBytes, _ = vendor.Marshal(testMessage)
)
...
func TestToMetadata(t *testing.T) {
	tests := []struct {
		...
		r:    testMessage,
		want: metadata.MD{
			strings.ToLower(vendor.mdKey): []string{string(testBytes)},
		},
	}}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := vendor.ToMetadata(tt.r); !reflect.DeepEqual(got, tt.want) {
...
func TestFromMetadata(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := vendor.FromMetadata(tt.md); !vendor.Equal(got, tt.want) {
