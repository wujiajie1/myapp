...
func TestLog(t *testing.T) {
	vendor.idGen.reset()
	ml := vendor.newMethodLogger(10, 10)
	// Set sink to testing buffer.
	buf := bytes.NewBuffer(nil)
	ml.sink = vendor.newWriterSink(buf)
	...
	testProtoBytes, _ := vendor.Marshal(testProtoMsg)

	testCases := []struct {
		config vendor.LogEntryConfig
		want   *pb.GrpcLogEntry
	}{
		{
			config: &vendor.ClientHeader{
				OnClientSide: false,
			...
			want: &pb.GrpcLogEntry{
				...
				vendor.Logger:        pb.GrpcLogEntry_LOGGER_SERVER,
				Payload: &pb.GrpcLogEntry_ClientHeader{
					vendor.ClientHeader: &pb.ClientHeader{
		...
		},
		{
			...
			want: &pb.GrpcLogEntry{
				...
				vendor.Logger:        pb.GrpcLogEntry_LOGGER_CLIENT,
				Payload: &pb.GrpcLogEntry_ServerHeader{
					vendor.ServerHeader: &pb.ServerHeader{
						Metadata: &pb.Metadata{
							Entry: []*pb.MetadataEntry{
								{Key: "a", Value: []byte{'b'}},
								{Key: "a", Value: []byte{'b', 'b'}},
...
func TestTruncateMetadataNotTruncated(t *testing.T) {
	testCases := []struct {
		ml   *vendor.MethodLogger
		mpPb *pb.Metadata
	}{
		{
			ml: vendor.newMethodLogger(vendor.maxUInt, vendor.maxUInt),
			mpPb: &pb.Metadata{
				Entry: []*pb.MetadataEntry{
					{Key: "", Value: []byte{1}},
				},
		...
		{
			ml: vendor.newMethodLogger(2, vendor.maxUInt),
			mpPb: &pb.Metadata{
				Entry: []*pb.MetadataEntry{
					{Key: "", Value: []byte{1}},
				},
		...
		{
			ml: vendor.newMethodLogger(1, vendor.maxUInt),
			mpPb: &pb.Metadata{
				Entry: []*pb.MetadataEntry{
					{Key: "", Value: nil},
				},
...
func TestTruncateMessageNotTruncated(t *testing.T) {
	testCases := []struct {
		...
		{
			ml: vendor.newMethodLogger(vendor.maxUInt, 3),
			msgPb: &pb.Message{
				Data: []byte{1, 1},
			},
		},
		{
			ml: vendor.newMethodLogger(vendor.maxUInt, 2),
			msgPb: &pb.Message{
				Data: []byte{1, 1},
			},
		},
