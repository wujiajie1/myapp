...
func TestNewLoggerFromConfigString(t *testing.T) {
	...
	l := vendor.NewLoggerFromConfigString(c).(*vendor.logger)
	...
	if ml, ok := l.services[s1]; ok {
		if ml.hdr != vendor.maxUInt || ml.msg != 0 {
	...
	if ml, ok := l.methods[fullM1]; ok {
		if ml.hdr != 0 || ml.msg != vendor.maxUInt {
	...
	if ml, ok := l.methods[fullM2]; ok {
		if ml.hdr != vendor.maxUInt || ml.msg != vendor.maxUInt {
...
func TestNewLoggerFromConfigStringInvalid(t *testing.T) {
	...
	for _, tc := range testCases {
		l := vendor.NewLoggerFromConfigString(tc)
		if l != nil {
...
func TestParseMethodConfigAndSuffix(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		s, m, suffix, err := vendor.parseMethodConfigAndSuffix(tc.in)
		if err != nil {
...
func TestParseMethodConfigAndSuffixInvalid(t *testing.T) {
	...
	for _, tc := range testCases {
		s, m, suffix, err := vendor.parseMethodConfigAndSuffix(tc)
		if err == nil {
...
func TestParseHeaderMessageLengthConfig(t *testing.T) {
	testCases := []struct {
		...
		{
			in:  "",
			hdr: vendor.maxUInt, msg: vendor.maxUInt,
		},
		{
			in:  "{h}",
			hdr: vendor.maxUInt, msg: 0,
		},
		{
			in:  "{h:314}",
			hdr: 314, msg: 0,
		...
		{
			in:  "{h:314;m}",
			hdr: 314, msg: vendor.maxUInt,
		},
		{
			in:  "{h;m:213}",
			hdr: vendor.maxUInt, msg: 213,
		},
		{
			in:  "{h:314;m:213}",
			hdr: 314, msg: 213,
	...
	for _, tc := range testCases {
		...
		hdr, msg, err := vendor.parseHeaderMessageLengthConfig(tc.in)
		if err != nil {
			t.Errorf("returned error %v, want nil", err)
			continue
		}
...
func TestParseHeaderMessageLengthConfigInvalid(t *testing.T) {
	...
	for _, tc := range testCases {
		_, _, err := vendor.parseHeaderMessageLengthConfig(tc)
		if err == nil {
