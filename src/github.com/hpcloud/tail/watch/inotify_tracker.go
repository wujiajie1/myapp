...
type InotifyTracker struct {
	mux       sync.Mutex
	watcher   *vendor.Watcher
	chans     map[string]chan vendor.Event
...
var (
	...
	goRun = func() {
		shared = &InotifyTracker{
			mux:       sync.Mutex{},
			chans:     make(map[string]chan vendor.Event),
...
// filename.
func Events(fname string) <-chan vendor.Event {
	shared.mux.Lock()
...
func (shared *InotifyTracker) addWatch(winfo *watchInfo) error {
	...
	if shared.chans[winfo.fname] == nil {
		shared.chans[winfo.fname] = make(chan vendor.Event)
	}
...
func (shared *InotifyTracker) sendEvent(event vendor.Event) {
...
func (shared *InotifyTracker) run() {
	...
	if err != nil {
		vendor.Fatal("failed to create Watcher")
	}
