...
func args2config() (vendor.Config, int64) {
	config := vendor.Config{Follow: true}
	n := int64(0)
	maxlinesize := int(0)
...
func main() {
	...
	if n != 0 {
		config.Location = &vendor.SeekInfo{-n, os.SEEK_END}
	}
...
func tailFile(filename string, config vendor.Config, done chan bool) {
	defer func() { done <- true }()
	t, err := vendor.TailFile(filename, config)
	if err != nil {
		fmt.Println(err)
		return
	}
