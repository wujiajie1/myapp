...
func TestMain(m *testing.M) {
	...
	vendor.POLL_DURATION = 5 * time.Millisecond
	os.Exit(m.Run())
}

func TestMustExist(t *testing.T) {
	tail, err := vendor.TailFile("/no/such/file", vendor.Config{Follow: true, MustExist: true})
	if err == nil {
		t.Error("MustExist:true is violated")
		tail.Stop()
	}
	tail, err = vendor.TailFile("/no/such/file", vendor.Config{Follow: true, MustExist: false})
	if err != nil {
		t.Error("MustExist:false is violated")
	}
	tail.Stop()
	_, err = vendor.TailFile("README.md", vendor.Config{Follow: true, MustExist: true})
	if err != nil {
...
func TestWaitsForFileToExist(t *testing.T) {
	...
	tail := tailTest.StartTail("test.txt", vendor.Config{})
...
func TestWaitsForFileToExistRelativePath(t *testing.T) {
	...
	tail, err := vendor.TailFile("test.txt", vendor.Config{})
	if err != nil {
		tailTest.Fatal(err)
	}
...
func TestStop(t *testing.T) {
	tail, err := vendor.TailFile("_no_such_file", vendor.Config{Follow: true, MustExist: false})
	if err != nil {
		t.Error("MustExist:false is violated")
	}
	if tail.Stop() != nil {
...
func TestStopAtEOF(t *testing.T) {
	...
	tailTest.CreateFile("test.txt", "hello\nthere\nworld\n")
	tail := tailTest.StartTail("test.txt", vendor.Config{Follow: true, Location: nil})
...
func TestOver4096ByteLineWithSetMaxLineSize(t *testing.T) {
	...
	testString := strings.Repeat("a", 4097)
	tailTest.CreateFile("test.txt", "test\n"+testString+"\nhello\nworld\n")
	tail := tailTest.StartTail("test.txt", vendor.Config{Follow: true, Location: nil, MaxLineSize: 4097})
	go tailTest.VerifyTailOutput(tail, []string{"test", testString, "hello", "world"}, false)
...
func TestLocationFullDontFollow(t *testing.T) {
	...
	tailTest.CreateFile("test.txt", "hello\nworld\n")
	tail := tailTest.StartTail("test.txt", vendor.Config{Follow: false, Location: nil})
	go tailTest.VerifyTailOutput(tail, []string{"hello", "world"}, false)
...
func TestLocationEnd(t *testing.T) {
	...
	tailTest.CreateFile("test.txt", "hello\nworld\n")
	tail := tailTest.StartTail("test.txt", vendor.Config{Follow: true, Location: &vendor.SeekInfo{0, os.SEEK_END}})
...
func TestLocationMiddle(t *testing.T) {
	...
	tailTest.CreateFile("test.txt", "hello\nworld\n")
	tail := tailTest.StartTail("test.txt", vendor.Config{Follow: true, Location: &vendor.SeekInfo{-6, os.SEEK_END}})
...
func TestRateLimiting(t *testing.T) {
	...
	config := vendor.Config{
		Follow:      true,
		RateLimiter: vendor.NewLeakyBucket(2, time.Second)}
...
func TestTell(t *testing.T) {
	...
	config := vendor.Config{
		Follow:   false,
		Location: &vendor.SeekInfo{0, os.SEEK_SET}}
	tail := tailTest.StartTail("test.txt", config)
	// read noe line
	<-tail.Lines
	...
	config = vendor.Config{
		Follow:   false,
		Location: &vendor.SeekInfo{offset, os.SEEK_SET}}
	tail = tailTest.StartTail("test.txt", config)
...
func maxLineSize(t *testing.T, follow bool, fileContent string, expected []string) {
	...
	tail := tailTest.StartTail("test.txt", vendor.Config{Follow: follow, Location: nil, MaxLineSize: 3})
...
func reOpen(t *testing.T, poll bool) {
	...
	tail := tailTest.StartTail(
		"test.txt",
		vendor.Config{Follow: true, ReOpen: true, Poll: poll})
...
func TestInotify_WaitForCreateThenMove(t *testing.T) {
	...
	tail := tailTest.StartTail(
		"test.txt",
		vendor.Config{Follow: true, ReOpen: true, Poll: false})
...
func reSeek(t *testing.T, poll bool) {
	...
	tail := tailTest.StartTail(
		"test.txt",
		vendor.Config{Follow: true, ReOpen: false, Poll: poll})
...
func (t TailTest) StartTail(name string, config vendor.Config) *vendor.Tail {
	tail, err := vendor.TailFile(t.path+"/"+name, config)
	if err != nil {
		t.Fatal(err)
	}
	return tail
...
func (t TailTest) VerifyTailOutput(tail *vendor.Tail, lines []string, expectEOF bool) {
	defer close(t.done)
...
func (t TailTest) ReadLines(tail *vendor.Tail, lines []string) {
...
func (t TailTest) Cleanup(tail *vendor.Tail, stop bool) {
	<-t.done
	if stop {
		tail.Stop()
	}
