...
func (w *Watcher) readEvents() {
	...
	for {
		...
		for len(kevents) > 0 {
			...
			if path.isDir && !(event.Op&vendor.Remove == vendor.Remove) {
				...
				if _, err := os.Lstat(event.Name); os.IsNotExist(err) {
					...
					event.Op |= vendor.Remove
				}
			}

			if event.Op&vendor.Rename == vendor.Rename || event.Op&vendor.Remove == vendor.Remove {
				w.Remove(event.Name)
				w.mu.Lock()
			...
			if path.isDir && event.Op&vendor.Write == vendor.Write && !(event.Op&vendor.Remove == vendor.Remove) {
			...
			if event.Op&vendor.Remove == vendor.Remove {
...
func newEvent(name string, mask uint32) vendor.Event {
	e := vendor.Event{Name: name}
...
func newCreateEvent(name string) vendor.Event {
	return vendor.Event{Name: name, Op: vendor.Create}
}
