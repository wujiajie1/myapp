...
func TestOffsetCommitRequestV0(t *testing.T) {
	request := new(vendor.OffsetCommitRequest)
	request.Version = 0
	request.ConsumerGroup = "foobar"
	vendor.testRequest(t, "no blocks v0", request, offsetCommitRequestNoBlocksV0)

	request.AddBlock("topic", 0x5221, 0xDEADBEEF, 0, "metadata")
	vendor.testRequest(t, "one block v0", request, offsetCommitRequestOneBlockV0)
}
...
func TestOffsetCommitRequestV1(t *testing.T) {
	...
	request.Version = 1
	vendor.testRequest(t, "no blocks v1", request, offsetCommitRequestNoBlocksV1)

	request.AddBlock("topic", 0x5221, 0xDEADBEEF, vendor.ReceiveTime, "metadata")
	vendor.testRequest(t, "one block v1", request, offsetCommitRequestOneBlockV1)
}
...
func TestOffsetCommitRequestV2ToV4(t *testing.T) {
	for version := 2; version <= 4; version++ {
		...
		vendor.testRequest(t, fmt.Sprintf("no blocks v%d", version), request, offsetCommitRequestNoBlocksV2)

		request.AddBlock("topic", 0x5221, 0xDEADBEEF, 0, "metadata")
		vendor.testRequest(t, fmt.Sprintf("one block v%d", version), request, offsetCommitRequestOneBlockV2)
	}
}
