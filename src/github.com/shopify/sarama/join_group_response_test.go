...
func TestJoinGroupResponseV0(t *testing.T) {
	var response *vendor.JoinGroupResponse

	response = new(vendor.JoinGroupResponse)
	vendor.testVersionDecodable(t, "no error", response, joinGroupResponseV0_NoError, 0)
	...
	response = new(vendor.JoinGroupResponse)
	vendor.testVersionDecodable(t, "with error", response, joinGroupResponseV0_WithError, 0)
	if response.Err != vendor.ErrInconsistentGroupProtocol {
	...
	response = new(vendor.JoinGroupResponse)
	vendor.testVersionDecodable(t, "with error", response, joinGroupResponseV0_Leader, 0)
	if response.Err != vendor.ErrNoError {
...
func TestJoinGroupResponseV1(t *testing.T) {
	response := new(vendor.JoinGroupResponse)
	vendor.testVersionDecodable(t, "no error", response, joinGroupResponseV1, 1)
	if response.Err != vendor.ErrNoError {
		t.Error("Decoding Err failed: no error expected but found", response.Err)
	}
	if response.GenerationId != 66051 {
...
func TestJoinGroupResponseV2(t *testing.T) {
	response := new(vendor.JoinGroupResponse)
	vendor.testVersionDecodable(t, "no error", response, joinGroupResponseV2, 2)
