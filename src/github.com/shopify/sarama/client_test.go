...
func TestSimpleClient(t *testing.T) {
	...
	client, err := vendor.NewClient([]string{seedBroker.Addr()}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCachedPartitions(t *testing.T) {
	...
	metadataResponse.AddTopicPartition("my_topic", 0, 2, replicas, isr, []int32{}, vendor.ErrNoError)
	metadataResponse.AddTopicPartition("my_topic", 1, 2, replicas, isr, []int32{}, vendor.ErrLeaderNotAvailable)
	seedBroker.Returns(metadataResponse)

	config := vendor.NewConfig()
	config.Metadata.Retry.Max = 0
	c, err := vendor.NewClient([]string{seedBroker.Addr()}, config)
	if err != nil {
		t.Fatal(err)
	}
	client := c.(*vendor.client)

	// Verify they aren't cached the same
	allP := client.cachedPartitionsResults["my_topic"][vendor.allPartitions]
	writeP := client.cachedPartitionsResults["my_topic"][vendor.writablePartitions]
	...
	tmp[vendor.allPartitions] = []int32{1, 2, 3, 4}
	client.cachedPartitionsResults["my_topic"] = tmp
	if 4 != len(client.cachedPartitions("my_topic", vendor.allPartitions)) {
...
func TestClientDoesntCachePartitionsForTopicsWithErrors(t *testing.T) {
	...
	metadataResponse.AddTopicPartition("my_topic", 1, replicas[0], replicas, replicas, []int32{}, vendor.ErrNoError)
	metadataResponse.AddTopicPartition("my_topic", 2, replicas[0], replicas, replicas, []int32{}, vendor.ErrNoError)
	seedBroker.Returns(metadataResponse)

	config := vendor.NewConfig()
	config.Metadata.Retry.Max = 0
	client, err := vendor.NewClient([]string{seedBroker.Addr()}, config)
	if err != nil {
		t.Fatal(err)
	}
...
func TestClientMetadata(t *testing.T) {
	seedBroker := vendor.NewMockBroker(t, 1)
	leader := vendor.NewMockBroker(t, 5)
	...
	metadataResponse.AddTopicPartition("my_topic", 0, leader.BrokerID(), replicas, isr, []int32{}, vendor.ErrNoError)
	metadataResponse.AddTopicPartition("my_topic", 1, leader.BrokerID(), replicas, isr, []int32{}, vendor.ErrLeaderNotAvailable)
	seedBroker.Returns(metadataResponse)
...
func TestClientMetadataWithOfflineReplicas(t *testing.T) {
	...
	metadataResponse := new(vendor.MetadataResponse)
	metadataResponse.AddBroker(leader.Addr(), leader.BrokerID())
	metadataResponse.AddTopicPartition("my_topic", 0, leader.BrokerID(), replicas, isr, offlineReplicas, vendor.ErrNoError)
	metadataResponse.AddTopicPartition("my_topic", 1, leader.BrokerID(), replicas, isr, []int32{}, vendor.ErrNoError)
...
func TestClientGetOffset(t *testing.T) {
	...
	metadata := new(vendor.MetadataResponse)
	metadata.AddTopicPartition("foo", 0, leader.BrokerID(), nil, nil, nil, vendor.ErrNoError)
	...
	offsetResponse := new(vendor.OffsetResponse)
	offsetResponse.AddTopicPartition("foo", 0, 123)
	leader.Returns(offsetResponse)

	offset, err := client.GetOffset("foo", 0, vendor.OffsetNewest)
	if err != nil {
		t.Error(err)
	}
	if offset != 123 {
...
func TestClientReceivingUnknownTopicWithBackoffFunc(t *testing.T) {
	...
	metadataResponse1 := new(vendor.MetadataResponse)
	...
	metadataUnknownTopic := new(vendor.MetadataResponse)
	metadataUnknownTopic.AddTopic("new_topic", vendor.ErrUnknownTopicOrPartition)
	seedBroker.Returns(metadataUnknownTopic)
	seedBroker.Returns(metadataUnknownTopic)

	if err := client.RefreshMetadata("new_topic"); err != vendor.ErrUnknownTopicOrPartition {
...
func TestClientReceivingUnknownTopic(t *testing.T) {
	...
	if _, err = client.Leader("new_topic", 1); err != vendor.ErrUnknownTopicOrPartition {
...
func TestClientReceivingPartialMetadata(t *testing.T) {
	...
	metadataPartial := new(vendor.MetadataResponse)
	metadataPartial.AddTopic("new_topic", vendor.ErrLeaderNotAvailable)
	metadataPartial.AddTopicPartition("new_topic", 0, leader.BrokerID(), replicas, replicas, []int32{}, vendor.ErrNoError)
	metadataPartial.AddTopicPartition("new_topic", 1, -1, replicas, []int32{}, []int32{}, vendor.ErrLeaderNotAvailable)
	...
	if err != vendor.ErrLeaderNotAvailable {
...
func TestClientRefreshBehaviour(t *testing.T) {
	...
	metadataResponse2 := new(vendor.MetadataResponse)
	metadataResponse2.AddTopicPartition("my_topic", 0xb, leader.BrokerID(), nil, nil, nil, vendor.ErrNoError)
...
func TestClientResurrectDeadSeeds(t *testing.T) {
	initialSeed := vendor.NewMockBroker(t, 0)
	emptyMetadata := new(vendor.MetadataResponse)
	initialSeed.Returns(emptyMetadata)

	conf := vendor.NewConfig()
	conf.Metadata.Retry.Backoff = 0
	conf.Metadata.RefreshFrequency = 0
	c, err := vendor.NewClient([]string{initialSeed.Addr()}, conf)
	if err != nil {
		t.Fatal(err)
	}
	initialSeed.Close()
	...
	seed1 := vendor.NewMockBroker(t, 1)
	seed2 := vendor.NewMockBroker(t, 2)
	seed3 := vendor.NewMockBroker(t, 3)
	addr1 := seed1.Addr()
	addr2 := seed2.Addr()
	addr3 := seed3.Addr()
	...
	client.seedBrokers = []*vendor.Broker{vendor.NewBroker(addr1), vendor.NewBroker(addr2), vendor.NewBroker(addr3)}
	client.deadSeeds = []*vendor.Broker{}
	...
	seed1 = vendor.NewMockBrokerAddr(t, 1, addr1)
	seed2 = vendor.NewMockBrokerAddr(t, 2, addr2)
...
func TestClientController(t *testing.T) {
	...
	controllerBroker := vendor.NewMockBroker(t, 2)
	...
	cfg.Version = vendor.V0_10_0_0
	client1, err := vendor.NewClient([]string{seedBroker.Addr()}, cfg)
	if err != nil {
		t.Fatal(err)
	}
	...
	cfg.Version = vendor.V0_9_0_1
	client2, err := vendor.NewClient([]string{seedBroker.Addr()}, cfg)
	if err != nil {
		t.Fatal(err)
	}
	defer safeClose(t, client2)
	if _, err = client2.Controller(); err != vendor.ErrUnsupportedVersion {
		t.Errorf("Expected Contoller() to return %s, found %s", vendor.ErrUnsupportedVersion, err)
	}
}
...
func TestClientMetadataTimeout(t *testing.T) {
	for _, timeout := range []time.Duration{
		...
		t.Run(fmt.Sprintf("timeout=%v", timeout), func(t *testing.T) {
			...
			// Overwrite the seed brokers with a fixed ordering to make this test deterministic
			safeClose(t, client.seedBrokers[0])
			client.seedBrokers = []*vendor.Broker{vendor.NewBroker(seed1.Addr()), vendor.NewBroker(seed2.Addr())}
			client.deadSeeds = []*vendor.Broker{}
...
func TestClientCoordinatorWithConsumerOffsetsTopic(t *testing.T) {
	seedBroker := vendor.NewMockBroker(t, 1)
	staleCoordinator := vendor.NewMockBroker(t, 2)
	freshCoordinator := vendor.NewMockBroker(t, 3)
	...
	metadataResponse1.AddTopicPartition("__consumer_offsets", 0, replicas[0], replicas, replicas, []int32{}, vendor.ErrNoError)
	seedBroker.Returns(metadataResponse1)
	...
	coordinatorResponse1 := new(vendor.ConsumerMetadataResponse)
	coordinatorResponse1.Err = vendor.ErrConsumerCoordinatorNotAvailable
	seedBroker.Returns(coordinatorResponse1)

	coordinatorResponse2 := new(vendor.ConsumerMetadataResponse)
	...
	coordinatorResponse3 := new(vendor.ConsumerMetadataResponse)
...
func TestClientCoordinatorWithoutConsumerOffsetsTopic(t *testing.T) {
	...
	metadataResponse2 := new(vendor.MetadataResponse)
	metadataResponse2.AddTopic("__consumer_offsets", vendor.ErrUnknownTopicOrPartition)
	seedBroker.Returns(metadataResponse2)

	replicas := []int32{coordinator.BrokerID()}
	metadataResponse3 := new(vendor.MetadataResponse)
	metadataResponse3.AddTopicPartition("__consumer_offsets", 0, replicas[0], replicas, replicas, []int32{}, vendor.ErrNoError)
...
func TestClientAutorefreshShutdownRace(t *testing.T) {
	...
	client, err := vendor.NewClient([]string{seedBroker.Addr()}, conf)
	if err != nil {
		t.Fatal(err)
	}
	...
	leader := vendor.NewMockBroker(t, 2)
	metadataResponse.AddBroker(leader.Addr(), leader.BrokerID())
	metadataResponse.AddTopicPartition("foo", 0, leader.BrokerID(), []int32{2}, []int32{2}, []int32{}, vendor.ErrNoError)
	seedBroker.Returns(metadataResponse)
