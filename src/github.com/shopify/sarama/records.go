...
type Records struct {
	recordsType int
	MsgSet      *vendor.MessageSet
	RecordBatch *vendor.RecordBatch
}

func newLegacyRecords(msgSet *vendor.MessageSet) Records {
	return Records{recordsType: legacyRecords, MsgSet: msgSet}
}

func newDefaultRecords(batch *vendor.RecordBatch) Records {
...
func (r *Records) encode(pe vendor.packetEncoder) error {
...
func (r *Records) setTypeFromMagic(pd vendor.packetDecoder) error {
...
func (r *Records) decode(pd vendor.packetDecoder) error {
	if r.recordsType == unknownRecords {
	...
	switch r.recordsType {
	case legacyRecords:
		r.MsgSet = &vendor.MessageSet{}
		return r.MsgSet.decode(pd)
	case defaultRecords:
		r.RecordBatch = &vendor.RecordBatch{}
...
func magicValue(pd vendor.packetDecoder) (int8, error) {
...
func (r *Records) getControlRecord() (vendor.ControlRecord, error) {
	if r.RecordBatch == nil || len(r.RecordBatch.Records) <= 0 {
		return vendor.ControlRecord{}, fmt.Errorf("cannot get control record, record batch is empty")
	}

	firstRecord := r.RecordBatch.Records[0]
	controlRecord := vendor.ControlRecord{}
	err := controlRecord.decode(&vendor.realDecoder{raw: firstRecord.Key}, &vendor.realDecoder{raw: firstRecord.Value})
	if err != nil {
		return vendor.ControlRecord{}, err
	}
