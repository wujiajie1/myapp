...
func (b *offsetCommitRequestBlock) encode(pe vendor.packetEncoder, version int16) error {
	pe.putInt64(b.offset)
	if version == 1 {
		pe.putInt64(b.timestamp)
	} else if b.timestamp != 0 {
		vendor.Logger.Println("Non-zero timestamp specified for OffsetCommitRequest not v1, it will be ignored")
	}

	return pe.putString(b.metadata)
}

func (b *offsetCommitRequestBlock) decode(pd vendor.packetDecoder, version int16) (err error) {
...
func (r *OffsetCommitRequest) encode(pe vendor.packetEncoder) error {
	if r.Version < 0 || r.Version > 4 {
		return vendor.PacketEncodingError{"invalid or unsupported OffsetCommitRequest version field"}
	}
	...
	if r.Version >= 1 {
		...
		if r.ConsumerGroupGeneration != 0 {
			vendor.Logger.Println("Non-zero ConsumerGroupGeneration specified for OffsetCommitRequest v0, it will be ignored")
		}
		if r.ConsumerID != "" {
			vendor.Logger.Println("Non-empty ConsumerID specified for OffsetCommitRequest v0, it will be ignored")
		}
	}
	...
	if r.Version >= 2 {
		...
		vendor.Logger.Println("Non-zero RetentionTime specified for OffsetCommitRequest version <2, it will be ignored")
	}
...
func (r *OffsetCommitRequest) decode(pd vendor.packetDecoder, version int16) (err error) {
	r.Version = version
...
func (r *OffsetCommitRequest) requiredVersion() vendor.KafkaVersion {
	switch r.Version {
	case 1:
