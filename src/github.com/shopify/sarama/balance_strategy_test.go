...
func TestBalanceStrategyRange(t *testing.T) {
	tests := []struct {
		...
		expected vendor.BalanceStrategyPlan
	}{
		{
	...
	strategy := vendor.BalanceStrategyRange
	if strategy.Name() != "range" {
		t.Errorf("Unexpected stategy name\nexpected: range\nactual: %v", strategy.Name())
	}

	for _, test := range tests {
		members := make(map[string]vendor.ConsumerGroupMemberMetadata)
		for memberID, topics := range test.members {
			members[memberID] = vendor.ConsumerGroupMemberMetadata{Topics: topics}
		}
...
func TestBalanceStrategyRoundRobin(t *testing.T) {
	...
	strategy := vendor.BalanceStrategyRoundRobin
