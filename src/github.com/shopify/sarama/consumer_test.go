...
var testMsg = vendor.StringEncoder("Foo")

// If a particular offset is provided then messages are consumed starting from
// that offset.
func TestConsumerOffsetManual(t *testing.T) {
	// Given
	broker0 := vendor.NewMockBroker(t, 0)

	mockFetchResponse := vendor.NewMockFetchResponse(t, 1)
	for i := 0; i < 10; i++ {
		mockFetchResponse.SetMessage("my_topic", 0, int64(i+1234), testMsg)
	}

	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		"MetadataRequest": vendor.NewMockMetadataResponse(t).
			SetBroker(broker0.Addr(), broker0.BrokerID()).
			SetLeader("my_topic", 0, broker0.BrokerID()),
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 0).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 2345),
		"FetchRequest": mockFetchResponse,
	})

	// When
	master, err := vendor.NewConsumer([]string{broker0.Addr()}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConsumerOffsetNewest(t *testing.T) {
	...
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		"MetadataRequest": vendor.NewMockMetadataResponse(t).
			...
			SetLeader("my_topic", 0, broker0.BrokerID()),
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 10).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 7),
		"FetchRequest": vendor.NewMockFetchResponse(t, 1).
	...
	// When
	consumer, err := master.ConsumePartition("my_topic", 0, vendor.OffsetNewest)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConsumerRecreate(t *testing.T) {
	...
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		...
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 0).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 1000),
		"FetchRequest": vendor.NewMockFetchResponse(t, 1).
			SetMessage("my_topic", 0, 10, testMsg),
	})

	c, err := vendor.NewConsumer([]string{broker0.Addr()}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConsumerDuplicate(t *testing.T) {
	...
	c, err := vendor.NewConsumer([]string{broker0.Addr()}, config)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Then
	if pc2 != nil || err != vendor.ConfigurationError("That topic/partition is already being consumed") {
		t.Fatal("A partition cannot be consumed twice at the same time")
	}

	vendor.safeClose(t, pc1)
	vendor.safeClose(t, c)
	broker0.Close()
}

func runConsumerLeaderRefreshErrorTestWithConfig(t *testing.T, config *vendor.Config) {
	// Given
	broker0 := vendor.NewMockBroker(t, 100)

	// Stage 1: my_topic/0 served by broker0
	vendor.Logger.Printf("    STAGE 1")
	...
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		"MetadataRequest": vendor.NewMockMetadataResponse(t).
			...
			SetLeader("my_topic", 0, broker0.BrokerID()),
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 123).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 1000),
		"FetchRequest": vendor.NewMockFetchResponse(t, 1).
	...
	pc, err := c.ConsumePartition("my_topic", 0, vendor.OffsetOldest)
	if err != nil {
		t.Fatal(err)
	}
	...
	vendor.Logger.Printf("    STAGE 2")

	fetchResponse2 := &vendor.FetchResponse{}
	fetchResponse2.AddError("my_topic", 0, vendor.ErrNotLeaderForPartition)

	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		"FetchRequest": vendor.NewMockWrapper(fetchResponse2),
	})

	if consErr := <-pc.Errors(); consErr.Err != vendor.ErrOutOfBrokers {
	...
	vendor.Logger.Printf("    STAGE 3")

	broker1 := vendor.NewMockBroker(t, 101)

	broker1.SetHandlerByMap(map[string]vendor.MockResponse{
		"FetchRequest": vendor.NewMockFetchResponse(t, 1).
...
func TestConsumerInvalidTopic(t *testing.T) {
	...
	// Then
	if pc != nil || err != vendor.ErrUnknownTopicOrPartition {
...
func TestConsumerShutsDownOutOfRange(t *testing.T) {
	// Given
	broker0 := vendor.NewMockBroker(t, 0)
	fetchResponse := new(vendor.FetchResponse)
	fetchResponse.AddError("my_topic", 0, vendor.ErrOffsetOutOfRange)
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		"MetadataRequest": vendor.NewMockMetadataResponse(t).
			SetBroker(broker0.Addr(), broker0.BrokerID()).
			SetLeader("my_topic", 0, broker0.BrokerID()),
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 1234).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 7),
		"FetchRequest": vendor.NewMockWrapper(fetchResponse),
	})
...
func TestConsumerExtraOffsets(t *testing.T) {
	// Given
	legacyFetchResponse := &vendor.FetchResponse{}
	...
	newFetchResponse := &vendor.FetchResponse{Version: 4}
	...
	for _, fetchResponse1 := range []*vendor.FetchResponse{legacyFetchResponse, newFetchResponse} {
		var offsetResponseVersion int16
		cfg := vendor.NewConfig()
		cfg.Consumer.Return.Errors = true
		if fetchResponse1.Version >= 4 {
			cfg.Version = vendor.V0_11_0_0
		...
		fetchResponse2.AddError("my_topic", 0, vendor.ErrNoError)
		broker0.SetHandlerByMap(map[string]vendor.MockResponse{
			"MetadataRequest": vendor.NewMockMetadataResponse(t).
				SetBroker(broker0.Addr(), broker0.BrokerID()).
				SetLeader("my_topic", 0, broker0.BrokerID()),
			"OffsetRequest": vendor.NewMockOffsetResponse(t).
				...
				SetOffset("my_topic", 0, vendor.OffsetNewest, 1234).
				SetOffset("my_topic", 0, vendor.OffsetOldest, 0),
			"FetchRequest": vendor.NewMockSequence(fetchResponse1, fetchResponse2),
		})

		master, err := vendor.NewConsumer([]string{broker0.Addr()}, cfg)
		if err != nil {
			t.Fatal(err)
		}
...
func TestConsumerReceivingFetchResponseWithTooOldRecords(t *testing.T) {
	// Given
	fetchResponse1 := &vendor.FetchResponse{Version: 4}
	fetchResponse1.AddRecord("my_topic", 0, nil, testMsg, 1)

	fetchResponse2 := &vendor.FetchResponse{Version: 4}
	fetchResponse2.AddRecord("my_topic", 0, nil, testMsg, 1000000)

	cfg := vendor.NewConfig()
	cfg.Consumer.Return.Errors = true
	cfg.Version = vendor.V1_1_0_0
...
func TestConsumerNonSequentialOffsets(t *testing.T) {
	...
	for _, fetchResponse1 := range []*vendor.FetchResponse{legacyFetchResponse, newFetchResponse} {
		...
		broker0 := vendor.NewMockBroker(t, 0)
		fetchResponse2 := &vendor.FetchResponse{Version: fetchResponse1.Version}
		fetchResponse2.AddError("my_topic", 0, vendor.ErrNoError)
		broker0.SetHandlerByMap(map[string]vendor.MockResponse{
			"MetadataRequest": vendor.NewMockMetadataResponse(t).
				SetBroker(broker0.Addr(), broker0.BrokerID()).
...
func TestConsumerRebalancingMultiplePartitions(t *testing.T) {
	// initial setup
	seedBroker := vendor.NewMockBroker(t, 10)
	leader0 := vendor.NewMockBroker(t, 0)
	leader1 := vendor.NewMockBroker(t, 1)

	seedBroker.SetHandlerByMap(map[string]vendor.MockResponse{
		"MetadataRequest": vendor.NewMockMetadataResponse(t).
	...
	mockOffsetResponse1 := vendor.NewMockOffsetResponse(t).
		SetOffset("my_topic", 0, vendor.OffsetOldest, 0).
		SetOffset("my_topic", 0, vendor.OffsetNewest, 1000).
		SetOffset("my_topic", 1, vendor.OffsetOldest, 0).
		SetOffset("my_topic", 1, vendor.OffsetNewest, 1000)
	leader0.SetHandlerByMap(map[string]vendor.MockResponse{
		"OffsetRequest": mockOffsetResponse1,
		"FetchRequest":  vendor.NewMockFetchResponse(t, 1),
	})
	leader1.SetHandlerByMap(map[string]vendor.MockResponse{
		"OffsetRequest": mockOffsetResponse1,
		"FetchRequest":  vendor.NewMockFetchResponse(t, 1),
	})
	...
	master, err := vendor.NewConsumer([]string{seedBroker.Addr()}, config)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := int32(0); i < 2; i++ {
		...
		go func(c vendor.PartitionConsumer) {
		...
		wg.Add(1)
		go func(partition int32, c vendor.PartitionConsumer) {
			for i := 0; i < 10; i++ {
	...
	fetchResponse := new(vendor.FetchResponse)
	fetchResponse.AddError("my_topic", 0, vendor.ErrNotLeaderForPartition)
	leader0.SetHandlerByMap(map[string]vendor.MockResponse{
		"FetchRequest": vendor.NewMockWrapper(fetchResponse),
	})
	...
	mockFetchResponse2 := vendor.NewMockFetchResponse(t, 2)
	for i := 4; i < 7; i++ {
	...
	mockFetchResponse3 := vendor.NewMockFetchResponse(t, 3).
		SetMessage("my_topic", 0, int64(7), testMsg).
		SetMessage("my_topic", 0, int64(8), testMsg).
		SetMessage("my_topic", 0, int64(9), testMsg)
	fetchResponse4 := new(vendor.FetchResponse)
	fetchResponse4.AddError("my_topic", 1, vendor.ErrNotLeaderForPartition)
	leader1.SetHandlerByMap(map[string]vendor.MockResponse{
		"FetchRequest": vendor.NewMockSequence(mockFetchResponse3, fetchResponse4),
	})

	// leader0 provides two messages on partition 1
	mockFetchResponse4 := vendor.NewMockFetchResponse(t, 2)
	for i := 8; i < 10; i++ {
...
func TestConsumerInterleavedClose(t *testing.T) {
	...
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		...
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 1000).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 1100).
			SetOffset("my_topic", 1, vendor.OffsetOldest, 2000).
			SetOffset("my_topic", 1, vendor.OffsetNewest, 2100),
		"FetchRequest": vendor.NewMockFetchResponse(t, 1).
	...
	config := vendor.NewConfig()
	config.ChannelBufferSize = 0
	master, err := vendor.NewConsumer([]string{broker0.Addr()}, config)
	if err != nil {
		t.Fatal(err)
	}
	...
	vendor.safeClose(t, c1)
	vendor.safeClose(t, c0)
	vendor.safeClose(t, master)
	broker0.Close()
}
...
func TestConsumerBounceWithReferenceOpen(t *testing.T) {
	...
	broker1 := vendor.NewMockBroker(t, 1)

	mockMetadataResponse := vendor.NewMockMetadataResponse(t).
		SetBroker(broker0.Addr(), broker0.BrokerID()).
		SetBroker(broker1.Addr(), broker1.BrokerID()).
		SetLeader("my_topic", 0, broker0.BrokerID()).
		SetLeader("my_topic", 1, broker1.BrokerID())

	mockOffsetResponse := vendor.NewMockOffsetResponse(t).
		SetOffset("my_topic", 0, vendor.OffsetOldest, 1000).
		SetOffset("my_topic", 0, vendor.OffsetNewest, 1100).
		SetOffset("my_topic", 1, vendor.OffsetOldest, 2000).
		SetOffset("my_topic", 1, vendor.OffsetNewest, 2100)
	...
	master, err := vendor.NewConsumer([]string{broker1.Addr()}, config)
	if err != nil {
		t.Fatal(err)
	}
	...
	broker0 = vendor.NewMockBrokerAddr(t, 0, broker0Addr)
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		"FetchRequest": mockFetchResponse,
	})
...
func TestConsumerOffsetOutOfRange(t *testing.T) {
	// Given
	broker0 := vendor.NewMockBroker(t, 2)
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		"MetadataRequest": vendor.NewMockMetadataResponse(t).
			SetBroker(broker0.Addr(), broker0.BrokerID()).
			SetLeader("my_topic", 0, broker0.BrokerID()),
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 1234).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 2345),
	})
	...
	// When/Then
	if _, err := master.ConsumePartition("my_topic", 0, 0); err != vendor.ErrOffsetOutOfRange {
		t.Fatal("Should return ErrOffsetOutOfRange, got:", err)
	}
	if _, err := master.ConsumePartition("my_topic", 0, 3456); err != vendor.ErrOffsetOutOfRange {
		t.Fatal("Should return ErrOffsetOutOfRange, got:", err)
	}
	if _, err := master.ConsumePartition("my_topic", 0, -3); err != vendor.ErrOffsetOutOfRange {
		t.Fatal("Should return ErrOffsetOutOfRange, got:", err)
	}
...
func TestConsumerExpiryTicker(t *testing.T) {
	// Given
	broker0 := vendor.NewMockBroker(t, 0)
	fetchResponse1 := &vendor.FetchResponse{}
	for i := 1; i <= 8; i++ {
	...
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		...
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 1234).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 1),
		"FetchRequest": vendor.NewMockSequence(fetchResponse1),
	})
...
func TestConsumerTimestamps(t *testing.T) {
	...
	type testMessage struct {
		key       vendor.Encoder
		value     vendor.Encoder
		offset    int64
		timestamp time.Time
	}
	for _, d := range []struct {
		kversion          vendor.KafkaVersion
		logAppendTime     bool
		messages          []testMessage
		expectedTimestamp []time.Time
	}{
		{vendor.MinVersion, false, []testMessage{
			{nil, testMsg, 1, now},
			{nil, testMsg, 2, now},
		}, []time.Time{{}, {}}},
		{vendor.V0_9_0_0, false, []testMessage{
			{nil, testMsg, 1, now},
			{nil, testMsg, 2, now},
		}, []time.Time{{}, {}}},
		{vendor.V0_10_0_0, false, []testMessage{
			{nil, testMsg, 1, now},
			{nil, testMsg, 2, now},
		}, []time.Time{{}, {}}},
		{vendor.V0_10_2_1, false, []testMessage{
			{nil, testMsg, 1, now.Add(time.Second)},
			{nil, testMsg, 2, now.Add(2 * time.Second)},
		}, []time.Time{now.Add(time.Second), now.Add(2 * time.Second)}},
		{vendor.V0_10_2_1, true, []testMessage{
			{nil, testMsg, 1, now.Add(time.Second)},
			{nil, testMsg, 2, now.Add(2 * time.Second)},
		}, []time.Time{now, now}},
		{vendor.V0_11_0_0, false, []testMessage{
			{nil, testMsg, 1, now.Add(time.Second)},
			{nil, testMsg, 2, now.Add(2 * time.Second)},
		}, []time.Time{now.Add(time.Second), now.Add(2 * time.Second)}},
		{vendor.V0_11_0_0, true, []testMessage{
			{nil, testMsg, 1, now.Add(time.Second)},
			{nil, testMsg, 2, now.Add(2 * time.Second)},
		}, []time.Time{now, now}},
	} {
		var fr *vendor.FetchResponse
		var offsetResponseVersion int16
		cfg := vendor.NewConfig()
		cfg.Version = d.kversion
		switch {
		case d.kversion.IsAtLeast(vendor.V0_11_0_0):
			offsetResponseVersion = 1
			fr = &vendor.FetchResponse{Version: 4, LogAppendTime: d.logAppendTime, Timestamp: now}
		...
		case d.kversion.IsAtLeast(vendor.V0_10_1_0):
			offsetResponseVersion = 1
			fr = &vendor.FetchResponse{Version: 3, LogAppendTime: d.logAppendTime, Timestamp: now}
			for _, m := range d.messages {
		...
		default:
			var version int16
			switch {
			case d.kversion.IsAtLeast(vendor.V0_10_0_0):
				version = 2
			case d.kversion.IsAtLeast(vendor.V0_9_0_0):
				version = 1
			}
			fr = &vendor.FetchResponse{Version: version}
			for _, m := range d.messages {
		...
		broker0.SetHandlerByMap(map[string]vendor.MockResponse{
			...
			"OffsetRequest": vendor.NewMockOffsetResponse(t).
				...
				SetOffset("my_topic", 0, vendor.OffsetNewest, 1234).
				SetOffset("my_topic", 0, vendor.OffsetOldest, 0),
			"FetchRequest": vendor.NewMockSequence(fr),
		})
...
func TestExcludeUncommitted(t *testing.T) {
	...
	fetchResponse := &vendor.FetchResponse{
		Version: 4,
		Blocks: map[string]map[int32]*vendor.FetchResponseBlock{"my_topic": {0: {
			AbortedTransactions: []*vendor.AbortedTransaction{{ProducerID: 7, FirstOffset: 1235}},
		}}},
	}
	...
	fetchResponse.AddControlRecord("my_topic", 0, 1237, 7, vendor.ControlRecordAbort) // abort control record
	...
	broker0.SetHandlerByMap(map[string]vendor.MockResponse{
		...
		"OffsetRequest": vendor.NewMockOffsetResponse(t).
			SetVersion(1).
			SetOffset("my_topic", 0, vendor.OffsetOldest, 0).
			SetOffset("my_topic", 0, vendor.OffsetNewest, 1237),
		"FetchRequest": vendor.NewMockWrapper(fetchResponse),
	})
	...
	cfg.Consumer.Return.Errors = true
	cfg.Version = vendor.V0_11_0_0
	cfg.Consumer.IsolationLevel = vendor.ReadCommitted
...
func assertMessageOffset(t *testing.T, msg *vendor.ConsumerMessage, expectedOffset int64) {
...
func ExampleConsumer() {
	consumer, err := vendor.NewConsumer([]string{"localhost:9092"}, nil)
	if err != nil {
		panic(err)
	}
	...
	partitionConsumer, err := consumer.ConsumePartition("my_topic", 0, vendor.OffsetNewest)
	if err != nil {
		panic(err)
	}
