...
func (p *GroupProtocol) decode(pd vendor.packetDecoder) (err error) {
...
func (p *GroupProtocol) encode(pe vendor.packetEncoder) (err error) {
...
func (r *JoinGroupRequest) encode(pe vendor.packetEncoder) error {
	...
	if len(r.GroupProtocols) > 0 {
		if len(r.OrderedGroupProtocols) > 0 {
			return vendor.PacketDecodingError{"cannot specify both GroupProtocols and OrderedGroupProtocols on JoinGroupRequest"}
		}
...
func (r *JoinGroupRequest) decode(pd vendor.packetDecoder, version int16) (err error) {
	r.Version = version
...
func (r *JoinGroupRequest) requiredVersion() vendor.KafkaVersion {
	switch r.Version {
	case 2:
...
func (r *JoinGroupRequest) AddGroupProtocolMetadata(name string, metadata *vendor.ConsumerGroupMemberMetadata) error {
	bin, err := vendor.encode(metadata, nil)
	if err != nil {
		return err
	}
