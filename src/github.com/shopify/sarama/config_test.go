...
func TestInvalidClientIDConfigValidates(t *testing.T) {
	...
	if err := config.Validate(); string(err.(vendor.ConfigurationError)) != "ClientID is invalid" {
...
func (t *DummyTokenProvider) Token() (*vendor.AccessToken, error) {
	return &vendor.AccessToken{Token: "access-token-string"}, nil
}

func TestNetConfigValidates(t *testing.T) {
	tests := []struct {
		name string
		cfg  func(*vendor.Config) // resorting to using a function as a param because of internal composite structs
		err  string
	}{
		{
			"OpenRequests",
			func(cfg *vendor.Config) {
		...
		{"SASL.Mechanism.OAUTHBEARER - Missing token provider",
			func(cfg *vendor.Config) {
				cfg.Net.SASL.Enable = true
				cfg.Net.SASL.Mechanism = vendor.SASLTypeOAuth
		...
		{"SASL.Mechanism SCRAM-SHA-256 - Missing SCRAM client",
			func(cfg *vendor.Config) {
				cfg.Net.SASL.Enable = true
				cfg.Net.SASL.Mechanism = vendor.SASLTypeSCRAMSHA256
		...
		{"SASL.Mechanism SCRAM-SHA-512 - Missing SCRAM client",
			func(cfg *vendor.Config) {
				cfg.Net.SASL.Enable = true
				cfg.Net.SASL.Mechanism = vendor.SASLTypeSCRAMSHA512
				cfg.Net.SASL.SCRAMClientGeneratorFunc = nil
				cfg.Net.SASL.User = "user"
				cfg.Net.SASL.Password = "stong_password"
			},
	...
	for i, test := range tests {
		c := vendor.NewConfig()
		test.cfg(c)
		if err := c.Validate(); string(err.(vendor.ConfigurationError)) != test.err {
	...
	tests := []struct {
		...
		{"Idempotent with Net.MaxOpenRequests",
			func(cfg *vendor.Config) {
				cfg.Version = vendor.V0_11_0_0
				cfg.Producer.Idempotent = true
				cfg.Producer.RequiredAcks = vendor.WaitForAll
			},
...
func TestConsumerConfigValidates(t *testing.T) {
	tests := []struct {
		...
		{"Incorrect isolation level",
			func(cfg *vendor.Config) {
				cfg.Version = vendor.V0_11_0_0
				cfg.Consumer.IsolationLevel = vendor.IsolationLevel(42)
			},
...
func TestLZ4ConfigValidation(t *testing.T) {
	config := vendor.NewConfig()
	config.Producer.Compression = vendor.CompressionLZ4
	if err := config.Validate(); string(err.(vendor.ConfigurationError)) != "lz4 compression requires Version >= V0_10_0_0" {
		t.Error("Expected invalid lz4/kafka version error, got ", err)
	}
	config.Version = vendor.V0_10_0_0
	if err := config.Validate(); err != nil {
...
func ExampleConfig_metrics() {
	...
	appMetricRegistry := vendor.NewRegistry()
	appGauge := vendor.GetOrRegisterGauge("m1", appMetricRegistry)
	appGauge.Update(1)

	config := vendor.NewConfig()
	// Use a prefix registry instead of the default local one
	config.MetricRegistry = vendor.NewPrefixedChildRegistry(appMetricRegistry, "sarama.")

	// Simulate a metric created by sarama without starting a broker
	saramaGauge := vendor.GetOrRegisterGauge("m2", config.MetricRegistry)
	saramaGauge.Update(2)

	vendor.WriteOnce(appMetricRegistry, os.Stdout)
	// Output:
	// gauge m1
	//   value:               1
	// gauge sarama.m2
