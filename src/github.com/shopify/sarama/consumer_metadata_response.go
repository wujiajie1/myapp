...
func (r *ConsumerMetadataResponse) decode(pd vendor.packetDecoder, version int16) (err error) {
	tmp := new(vendor.FindCoordinatorResponse)
...
func (r *ConsumerMetadataResponse) encode(pe vendor.packetEncoder) error {
	if r.Coordinator == nil {
		r.Coordinator = new(vendor.Broker)
		r.Coordinator.id = r.CoordinatorID
		r.Coordinator.addr = net.JoinHostPort(r.CoordinatorHost, strconv.Itoa(int(r.CoordinatorPort)))
	}

	tmp := &vendor.FindCoordinatorResponse{
		Version:     0,
		Err:         r.Err,
...
func (r *ConsumerMetadataResponse) requiredVersion() vendor.KafkaVersion {
