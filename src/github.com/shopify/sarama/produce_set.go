...
type partitionSet struct {
	msgs          []*vendor.ProducerMessage
	recordsToSend vendor.Records
	bufferBytes   int
}

type produceSet struct {
	parent *vendor.asyncProducer
...
func newProduceSet(parent *vendor.asyncProducer) *produceSet {
	return &produceSet{
...
func (ps *produceSet) add(msg *vendor.ProducerMessage) error {
	var err error
	var key, val []byte
	...
	if set == nil {
		if ps.parent.conf.Version.IsAtLeast(vendor.V0_11_0_0) {
			...
			set = &partitionSet{recordsToSend: vendor.newDefaultRecords(batch)}
			size = vendor.recordBatchOverhead
		} else {
			set = &partitionSet{recordsToSend: vendor.newLegacyRecords(new(vendor.MessageSet))}
		}
	...
	if ps.parent.conf.Version.IsAtLeast(vendor.V0_11_0_0) {
		...
		if len(msg.Headers) > 0 {
			rec.Headers = make([]*vendor.RecordHeader, len(msg.Headers))
		...
		msgToSend := &vendor.Message{Codec: vendor.CompressionNone, Key: key, Value: val}
		if ps.parent.conf.Version.IsAtLeast(vendor.V0_10_0_0) {
			msgToSend.Timestamp = timestamp
			msgToSend.Version = 1
		}
		set.recordsToSend.MsgSet.addMessage(msgToSend)
		size = vendor.producerMessageOverhead + len(key) + len(val)
	}
...
func (ps *produceSet) buildRequest() *vendor.ProduceRequest {
	req := &vendor.ProduceRequest{
	...
	for topic, partitionSets := range ps.msgs {
		for partition, set := range partitionSets {
			...
			if ps.parent.conf.Producer.Compression == vendor.CompressionNone {
				...
				payload, err := vendor.encode(set.recordsToSend.MsgSet, ps.parent.conf.MetricRegistry)
				if err != nil {
					vendor.Logger.Println(err) // if this happens, it's basically our fault.
					panic(err)
				}
				compMsg := &vendor.Message{
					Codec:            ps.parent.conf.Producer.Compression,
					CompressionLevel: ps.parent.conf.Producer.CompressionLevel,
					Key:              nil,
					Value:            payload,
...
func (ps *produceSet) dropPartition(topic string, partition int32) []*vendor.ProducerMessage {
...
func (ps *produceSet) wouldOverflow(msg *vendor.ProducerMessage) bool {
	version := 1
	if ps.parent.conf.Version.IsAtLeast(vendor.V0_11_0_0) {
		version = 2
	}
	...
	case ps.bufferBytes+msg.byteSize(version) >= int(vendor.MaxRequestSize-(10*1024)):
		return true
