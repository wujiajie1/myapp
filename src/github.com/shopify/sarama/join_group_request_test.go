...
func TestJoinGroupRequest(t *testing.T) {
	request := new(vendor.JoinGroupRequest)
	request.GroupId = "TestGroup"
	request.SessionTimeout = 100
	request.ProtocolType = "consumer"
	vendor.testRequest(t, "V0: no protocols", request, joinGroupRequestV0_NoProtocols)
}
...
func TestJoinGroupRequestV0_OneProtocol(t *testing.T) {
	...
	packet := vendor.testRequestEncode(t, "V0: one protocol", request, joinGroupRequestV0_OneProtocol)
	request.GroupProtocols = make(map[string][]byte)
	request.GroupProtocols["one"] = []byte{0x01, 0x02, 0x03}
	vendor.testRequestDecode(t, "V0: one protocol", request, packet)
}
...
func TestJoinGroupRequestV1(t *testing.T) {
	...
	request.ProtocolType = "consumer"
	request.AddGroupProtocol("one", []byte{0x01, 0x02, 0x03})
	packet := vendor.testRequestEncode(t, "V1", request, joinGroupRequestV1)
	request.GroupProtocols = make(map[string][]byte)
	request.GroupProtocols["one"] = []byte{0x01, 0x02, 0x03}
	vendor.testRequestDecode(t, "V1", request, packet)
}
