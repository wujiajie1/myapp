...
func TestMetadataRequestV0(t *testing.T) {
	request := new(vendor.MetadataRequest)
	vendor.testRequest(t, "no topics", request, metadataRequestNoTopicsV0)

	request.Topics = []string{"topic1"}
	vendor.testRequest(t, "one topic", request, metadataRequestOneTopicV0)

	request.Topics = []string{"foo", "bar", "baz"}
	vendor.testRequest(t, "three topics", request, metadataRequestThreeTopicsV0)
}

func TestMetadataRequestV1(t *testing.T) {
	request := new(vendor.MetadataRequest)
	request.Version = 1
	vendor.testRequest(t, "no topics", request, metadataRequestNoTopicsV1)

	request.Topics = []string{"topic1"}
	vendor.testRequest(t, "one topic", request, metadataRequestOneTopicV1)

	request.Topics = []string{"foo", "bar", "baz"}
	vendor.testRequest(t, "three topics", request, metadataRequestThreeTopicsV1)
}

func TestMetadataRequestV2(t *testing.T) {
	request := new(vendor.MetadataRequest)
	request.Version = 2
	vendor.testRequest(t, "no topics", request, metadataRequestNoTopicsV2)

	request.Topics = []string{"topic1"}
	vendor.testRequest(t, "one topic", request, metadataRequestOneTopicV2)

	request.Topics = []string{"foo", "bar", "baz"}
	vendor.testRequest(t, "three topics", request, metadataRequestThreeTopicsV2)
}

func TestMetadataRequestV3(t *testing.T) {
	request := new(vendor.MetadataRequest)
	request.Version = 3
	vendor.testRequest(t, "no topics", request, metadataRequestNoTopicsV3)

	request.Topics = []string{"topic1"}
	vendor.testRequest(t, "one topic", request, metadataRequestOneTopicV3)
}

func TestMetadataRequestV4(t *testing.T) {
	request := new(vendor.MetadataRequest)
	request.Version = 4
	vendor.testRequest(t, "no topics", request, metadataRequestNoTopicsV4)

	request.Topics = []string{"topic1"}

	request.AllowAutoTopicCreation = true
	vendor.testRequest(t, "one topic", request, metadataRequestAutoCreateV4)

	request.AllowAutoTopicCreation = false
	vendor.testRequest(t, "one topic", request, metadataRequestNoAutoCreateV4)
}
...
func TestMetadataRequestV5(t *testing.T) {
	...
	request.AllowAutoTopicCreation = true
	vendor.testRequest(t, "one topic", request, metadataRequestAutoCreateV5)

	request.AllowAutoTopicCreation = false
	vendor.testRequest(t, "one topic", request, metadataRequestNoAutoCreateV5)
}
