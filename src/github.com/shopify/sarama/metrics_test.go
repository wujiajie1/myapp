...
func TestGetOrRegisterHistogram(t *testing.T) {
	metricRegistry := vendor.NewRegistry()
	histogram := vendor.getOrRegisterHistogram("name", metricRegistry)
	...
	sameHistogram := vendor.getOrRegisterHistogram("name", metricRegistry)
...
func TestGetMetricNameForBroker(t *testing.T) {
	metricName := vendor.getMetricNameForBroker("name", &vendor.Broker{id: 1})
...
func (m *metricValidators) registerForBroker(broker *vendor.Broker, validator *metricValidator) {
	m.register(&metricValidator{vendor.getMetricNameForBroker(validator.name, broker), validator.validator})
}
...
func (m *metricValidators) registerForGlobalAndTopic(topic string, validator *metricValidator) {
	...
	m.register(&metricValidator{vendor.getMetricNameForTopic(validator.name, topic), validator.validator})
}

func (m *metricValidators) registerForAllBrokers(broker *vendor.Broker, validator *metricValidator) {
	m.register(validator)
	m.registerForBroker(broker, validator)
}

func (m metricValidators) run(t *testing.T, r vendor.Registry) {
...
func meterValidator(name string, extraValidator func(*testing.T, vendor.Meter)) *metricValidator {
	return &metricValidator{
		name: name,
		validator: func(t *testing.T, metric interface{}) {
			if meter, ok := metric.(vendor.Meter); !ok {
...
func countMeterValidator(name string, expectedCount int) *metricValidator {
	return meterValidator(name, func(t *testing.T, meter vendor.Meter) {
		count := meter.Count()
...
func histogramValidator(name string, extraValidator func(*testing.T, vendor.Histogram)) *metricValidator {
	return &metricValidator{
		name: name,
		validator: func(t *testing.T, metric interface{}) {
			if histogram, ok := metric.(vendor.Histogram); !ok {
...
func countHistogramValidator(name string, expectedCount int) *metricValidator {
	return histogramValidator(name, func(t *testing.T, histogram vendor.Histogram) {
