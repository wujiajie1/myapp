...
func TestProduceResponseDecode(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "no blocks", &response, produceResponseNoBlocksV0, 0)
	...
	for v, produceResponseManyBlocks := range produceResponseManyBlocksVersions {
		...
		vendor.testVersionDecodable(t, "many blocks", &response, produceResponseManyBlocks, int16(v))
		...
		if block == nil {
			...
			if block.Err != vendor.ErrInvalidMessage {
...
func TestProduceResponseEncode(t *testing.T) {
	response := vendor.ProduceResponse{}
	response.Blocks = make(map[string]map[int32]*vendor.ProduceResponseBlock)
	vendor.testEncodable(t, "empty", &response, produceResponseNoBlocksV0)

	response.Blocks["foo"] = make(map[int32]*vendor.ProduceResponseBlock)
	response.Blocks["foo"][1] = &vendor.ProduceResponseBlock{
		Err:       vendor.ErrInvalidMessage,
		Offset:    255,
	...
	for v, produceResponseManyBlocks := range produceResponseManyBlocksVersions {
		...
		vendor.testEncodable(t, fmt.Sprintf("many blocks version %d", v), &response, produceResponseManyBlocks)
	}
}
...
func TestProduceResponseEncodeInvalidTimestamp(t *testing.T) {
	...
	response.Version = 2
	response.Blocks = make(map[string]map[int32]*vendor.ProduceResponseBlock)
	response.Blocks["t"] = make(map[int32]*vendor.ProduceResponseBlock)
	response.Blocks["t"][0] = &vendor.ProduceResponseBlock{
	...
	response.ThrottleTime = 100 * time.Millisecond
	_, err := vendor.encode(&response, nil)
	if err == nil {
		t.Error("Expecting error, got nil")
	}
	if _, ok := err.(vendor.PacketEncodingError); !ok {
