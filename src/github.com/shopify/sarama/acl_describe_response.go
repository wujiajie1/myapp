...
func (d *DescribeAclsResponse) encode(pe vendor.packetEncoder) error {
...
func (d *DescribeAclsResponse) decode(pd vendor.packetDecoder, version int16) (err error) {
	...
	d.ResourceAcls = make([]*vendor.ResourceAcls, n)

	for i := 0; i < n; i++ {
		d.ResourceAcls[i] = new(vendor.ResourceAcls)
...
func (d *DescribeAclsResponse) requiredVersion() vendor.KafkaVersion {
	switch d.Version {
	case 1:
