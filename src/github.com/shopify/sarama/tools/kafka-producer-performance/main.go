...
func parseCompression(scheme string) vendor.CompressionCodec {
	switch scheme {
	case "none":
		return vendor.CompressionNone
	case "gzip":
		return vendor.CompressionGZIP
	case "snappy":
		return vendor.CompressionSnappy
	case "lz4":
		return vendor.CompressionLZ4
	default:
...
func parsePartitioner(scheme string, partition int) vendor.PartitionerConstructor {
	if partition < 0 && scheme == "manual" {
		printUsageErrorAndExit("-partition must not be -1 for -partitioning=manual")
	}
	switch scheme {
	case "manual":
		return vendor.NewManualPartitioner
	case "hash":
		return vendor.NewHashPartitioner
	case "random":
		return vendor.NewRandomPartitioner
	case "roundrobin":
		return vendor.NewRoundRobinPartitioner
	default:
...
func parseVersion(version string) vendor.KafkaVersion {
	result, err := vendor.ParseKafkaVersion(version)
	if err != nil {
...
func generateMessages(topic string, partition, messageLoad, messageSize int) []*vendor.ProducerMessage {
	messages := make([]*vendor.ProducerMessage, messageLoad)
	for i := 0; i < messageLoad; i++ {
		payload := make([]byte, messageSize)
		if _, err := rand.Read(payload); err != nil {
			printErrorAndExit(69, "Failed to generate message payload: %s", err)
		}
		messages[i] = &vendor.ProducerMessage{
			Topic:     topic,
			Partition: int32(partition),
			Value:     vendor.ByteEncoder(payload),
		}
	}
	return messages
}
...
func main() {
	...
	config.Producer.RequiredAcks = vendor.RequiredAcks(*requiredAcks)
...
func runAsyncProducer(topic string, partition, messageLoad, messageSize int,
	config *vendor.Config, brokers []string, throughput int) {
	producer, err := vendor.NewAsyncProducer(brokers, config)
	if err != nil {
...
func runSyncProducer(topic string, partition, messageLoad, messageSize, routines int,
	config *vendor.Config, brokers []string, throughput int) {
	producer, err := vendor.NewSyncProducer(brokers, config)
	if err != nil {
		printErrorAndExit(69, "Failed to create producer: %s", err)
	}
	defer func() {
	...
	messages := make([][]*vendor.ProducerMessage, routines)
...
func printMetrics(w io.Writer, r vendor.Registry) {
	if r.Get("record-send-rate") == nil || r.Get("request-latency-in-ms") == nil {
		return
	}
	recordSendRate := r.Get("record-send-rate").(vendor.Meter).Snapshot()
	requestLatency := r.Get("request-latency-in-ms").(vendor.Histogram).Snapshot()
