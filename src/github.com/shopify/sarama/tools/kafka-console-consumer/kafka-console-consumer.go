...
func main() {
	...
	switch *offset {
	case "oldest":
		initialOffset = vendor.OffsetOldest
	case "newest":
		initialOffset = vendor.OffsetNewest
	default:
	...
	c, err := vendor.NewConsumer(strings.Split(*brokerList, ","), config)
	if err != nil {
	...
	var (
		messages = make(chan *vendor.ConsumerMessage, *bufferSize)
...
func getPartitions(c vendor.Consumer) ([]int32, error) {
	if *partitions == "all" {
