...
func (b *ProduceResponseBlock) decode(pd vendor.packetDecoder, version int16) (err error) {
	tmp, err := pd.getInt16()
	if err != nil {
		return err
	}
...
func (b *ProduceResponseBlock) encode(pe vendor.packetEncoder, version int16) (err error) {
	...
	if version >= 2 {
		...
		if !b.Timestamp.Before(time.Unix(0, 0)) {
			...
			return vendor.PacketEncodingError{fmt.Sprintf("invalid timestamp (%v)", b.Timestamp)}
		}
...
func (r *ProduceResponse) decode(pd vendor.packetDecoder, version int16) (err error) {
	r.Version = version
...
func (r *ProduceResponse) encode(pe vendor.packetEncoder) error {
...
func (r *ProduceResponse) requiredVersion() vendor.KafkaVersion {
	switch r.Version {
	case 1:
...
func (r *ProduceResponse) AddTopicPartition(topic string, partition int32, err vendor.KError) {
	if r.Blocks == nil {
