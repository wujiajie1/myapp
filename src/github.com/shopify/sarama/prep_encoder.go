...
func (pe *prepEncoder) putArrayLength(in int) error {
	if in > math.MaxInt32 {
		return vendor.PacketEncodingError{fmt.Sprintf("array too long (%d)", in)}
	}
	pe.length += 4
	return nil
}
...
func (pe *prepEncoder) putRawBytes(in []byte) error {
	if len(in) > math.MaxInt32 {
		return vendor.PacketEncodingError{fmt.Sprintf("byteslice too long (%d)", len(in))}
	}
	pe.length += len(in)
	return nil
}
...
func (pe *prepEncoder) putString(in string) error {
	...
	if len(in) > math.MaxInt16 {
		return vendor.PacketEncodingError{fmt.Sprintf("string too long (%d)", len(in))}
	}
	pe.length += len(in)
	return nil
}
...
func (pe *prepEncoder) push(in vendor.pushEncoder) {
...
func (pe *prepEncoder) pop() error {
	...
	if dpe, ok := in.(vendor.dynamicPushEncoder); ok {
...
func (pe *prepEncoder) metricRegistry() vendor.Registry {
	return nil
}
