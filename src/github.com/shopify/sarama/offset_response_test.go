...
func TestEmptyOffsetResponse(t *testing.T) {
	response := vendor.OffsetResponse{}

	vendor.testVersionDecodable(t, "empty", &response, emptyOffsetResponse, 0)
	...
	vendor.testVersionDecodable(t, "empty", &response, emptyOffsetResponse, 1)
	if len(response.Blocks) != 0 {
		t.Error("Decoding produced", len(response.Blocks), "topics where there were none.")
	}
}
...
func TestNormalOffsetResponse(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "normal", &response, normalOffsetResponse, 0)
	...
	if response.Blocks["z"][2].Err != vendor.ErrNoError {
...
func TestNormalOffsetResponseV1(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "normal", &response, normalOffsetResponseV1, 1)
