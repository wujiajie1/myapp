...
func main() {
	...
	conf.Producer.RequiredAcks = vendor.WaitForAll
	conf.Producer.Return.Successes = true
	conf.Metadata.Full = true
	conf.Version = vendor.V0_10_0_0
	...
	if *algorithm == "sha512" {
		conf.Net.SASL.SCRAMClientGeneratorFunc = func() vendor.SCRAMClient { return &vendor.XDGSCRAMClient{HashGeneratorFcn: vendor.SHA512} }
		conf.Net.SASL.Mechanism = vendor.SASLMechanism(vendor.SASLTypeSCRAMSHA512)
	} else if *algorithm == "sha256" {
		conf.Net.SASL.SCRAMClientGeneratorFunc = func() vendor.SCRAMClient { return &vendor.XDGSCRAMClient{HashGeneratorFcn: vendor.SHA256} }
		conf.Net.SASL.Mechanism = vendor.SASLMechanism(vendor.SASLTypeSCRAMSHA256)
	...
	syncProcuder, err := vendor.NewSyncProducer(strings.Split(*brokers, ","), conf)
	if err != nil {
		logger.Fatalln("failed to create producer: ", err)
	}
	partition, offset, err := syncProcuder.SendMessage(&vendor.ProducerMessage{
		Topic: *topic,
		Value: vendor.StringEncoder("test_message"),
	})
	if err != nil {
