...
func TestCollectSuccessfully(t *testing.T) {
	dataCollectorMock := vendor.NewSyncProducer(t, nil)
	dataCollectorMock.ExpectSendMessageAndSucceed()

	accessLogProducerMock := vendor.NewAsyncProducer(t, nil)
...
func TestCollectionFailure(t *testing.T) {
	dataCollectorMock := vendor.NewSyncProducer(t, nil)
	dataCollectorMock.ExpectSendMessageAndFail(vendor.ErrRequestTimedOut)
