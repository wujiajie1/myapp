...
func (h *RecordHeader) encode(pe vendor.packetEncoder) error {
...
func (h *RecordHeader) decode(pd vendor.packetDecoder) (err error) {
...
type Record struct {
	...
	Key            []byte
	Value          []byte
	length         vendor.varintLengthField
}

func (r *Record) encode(pe vendor.packetEncoder) error {
	pe.push(&r.length)
...
func (r *Record) decode(pd vendor.packetDecoder) (err error) {
