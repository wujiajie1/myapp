...
func makeProduceSet() (*vendor.asyncProducer, *vendor.produceSet) {
	conf := vendor.NewConfig()
	txnmgr, _ := vendor.newTransactionManager(conf, nil)
	parent := &vendor.asyncProducer{
		conf:   conf,
		txnmgr: txnmgr,
	}
	return parent, vendor.newProduceSet(parent)
}

func safeAddMessage(t *testing.T, ps *vendor.produceSet, msg *vendor.ProducerMessage) {
...
func TestProduceSetAddingMessages(t *testing.T) {
	_, ps := makeProduceSet()
	msg := &vendor.ProducerMessage{Key: vendor.StringEncoder(vendor.TestMessage), Value: vendor.StringEncoder(vendor.TestMessage)}
...
func TestProduceSetPartitionTracking(t *testing.T) {
	...
	m1 := &vendor.ProducerMessage{Topic: "t1", Partition: 0}
	m2 := &vendor.ProducerMessage{Topic: "t1", Partition: 1}
	m3 := &vendor.ProducerMessage{Topic: "t2", Partition: 0}
	...
	ps.eachPartition(func(topic string, partition int32, pSet *vendor.partitionSet) {
		if len(pSet.msgs) != 1 {
...
func TestProduceSetRequestBuilding(t *testing.T) {
	parent, ps := makeProduceSet()
	parent.conf.Producer.RequiredAcks = vendor.WaitForAll
	parent.conf.Producer.Timeout = 10 * time.Second

	msg := &vendor.ProducerMessage{
		Topic:     "t1",
		Partition: 0,
		Key:       vendor.StringEncoder(vendor.TestMessage),
	...
	if req.RequiredAcks != vendor.WaitForAll {
...
func TestProduceSetCompressedRequestBuilding(t *testing.T) {
	...
	parent.conf.Producer.RequiredAcks = vendor.WaitForAll
	parent.conf.Producer.Timeout = 10 * time.Second
	parent.conf.Producer.Compression = vendor.CompressionGZIP
	parent.conf.Version = vendor.V0_10_0_0
...
func TestProduceSetV3RequestBuilding(t *testing.T) {
	...
	parent.conf.Producer.RequiredAcks = vendor.WaitForAll
	parent.conf.Producer.Timeout = 10 * time.Second
	parent.conf.Version = vendor.V0_11_0_0
...
func TestProduceSetIdempotentRequestBuilding(t *testing.T) {
	...
	parent := &vendor.asyncProducer{
		conf: config,
		txnmgr: &vendor.transactionManager{
			producerID:    pID,
			producerEpoch: pEpoch,
		},
	}
	ps := vendor.newProduceSet(parent)
