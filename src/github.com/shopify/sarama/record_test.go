...
var recordBatchTestCases = []struct {
	...
	},
	{
		...
		batch: vendor.RecordBatch{
			Version:          2,
			Codec:            vendor.CompressionGZIP,
			CompressionLevel: vendor.CompressionLevelDefault,
			FirstTimestamp:   time.Unix(1479847795, 0),
			MaxTimestamp:     time.Unix(0, 0),
			LastOffsetDelta:  0,
			Records: []*vendor.Record{{
	...
	},
	{
		...
		batch: vendor.RecordBatch{
			Version:         2,
			Codec:           vendor.CompressionSnappy,
			FirstTimestamp:  time.Unix(1479847795, 0),
			MaxTimestamp:    time.Unix(0, 0),
			LastOffsetDelta: 0,
			Records: []*vendor.Record{{
...
func TestRecordBatchEncoding(t *testing.T) {
	for _, tc := range recordBatchTestCases {
		if tc.oldGoEncoded != nil && isOldGo(t) {
			vendor.testEncodable(t, tc.name, &tc.batch, tc.oldGoEncoded)
		} else {
			vendor.testEncodable(t, tc.name, &tc.batch, tc.encoded)
		}
	}
}
...
func TestRecordBatchDecoding(t *testing.T) {
	for _, tc := range recordBatchTestCases {
		batch := vendor.RecordBatch{}
		vendor.testDecodable(t, tc.name, &batch, tc.encoded)
		for _, r := range batch.Records {
			r.length = vendor.varintLengthField{}
		}
		...
		if !reflect.DeepEqual(batch, tc.batch) {
			t.Errorf(vendor.Sprintf("invalid decode of %s\ngot %+v\nwanted %+v", tc.name, batch, tc.batch))
		}
	}
}
