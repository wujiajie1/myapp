...
func TestOffsetFetchRequestNoPartitions(t *testing.T) {
	for version := 0; version <= 5; version++ {
		request := new(vendor.OffsetFetchRequest)
		request.Version = int16(version)
		request.ZeroPartitions()
		vendor.testRequest(t, fmt.Sprintf("no group, no partitions %d", version), request, offsetFetchRequestNoGroupNoPartitions)

		request.ConsumerGroup = "blah"
		vendor.testRequest(t, fmt.Sprintf("no partitions %d", version), request, offsetFetchRequestNoPartitions)
	}
}
func TestOffsetFetchRequest(t *testing.T) {
	for version := 0; version <= 5; version++ {
		...
		vendor.testRequest(t, fmt.Sprintf("one partition %d", version), request, offsetFetchRequestOnePartition)
	}
}

func TestOffsetFetchRequestAllPartitions(t *testing.T) {
	for version := 2; version <= 5; version++ {
		request := &vendor.OffsetFetchRequest{Version: int16(version), ConsumerGroup: "blah"}
		vendor.testRequest(t, fmt.Sprintf("all partitions %d", version), request, offsetFetchRequestAllPartitions)
	}
}
