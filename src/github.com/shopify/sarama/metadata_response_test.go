...
func TestEmptyMetadataResponseV0(t *testing.T) {
	response := vendor.MetadataResponse{}

	vendor.testVersionDecodable(t, "empty, V0", &response, emptyMetadataResponseV0, 0)
...
func TestMetadataResponseWithBrokersV0(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "brokers, no topics, V0", &response, brokersNoTopicsMetadataResponseV0, 0)
...
func TestMetadataResponseWithTopicsV0(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "topics, no brokers, V0", &response, topicsNoBrokersMetadataResponseV0, 0)
	if len(response.Brokers) != 0 {
		t.Error("Decoding produced", len(response.Brokers), "brokers where there were none!")
	}
	...
	if response.Topics[0].Err != vendor.ErrNoError {
	...
	if response.Topics[0].Partitions[0].Err != vendor.ErrInvalidMessageSize {
	...
	if response.Topics[1].Err != vendor.ErrNoError {
...
func TestMetadataResponseWithBrokersV1(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "topics, V1", &response, brokersNoTopicsMetadataResponseV1, 1)
	if len(response.Brokers) != 2 {
...
func TestMetadataResponseWithTopicsV1(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "topics, V1", &response, topicsNoBrokersMetadataResponseV1, 1)
	if len(response.Brokers) != 0 {
		t.Error("Decoding produced", len(response.Brokers), "brokers where there were none!")
	}
...
func TestMetadataResponseWithThrottleTime(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "no topics, no brokers, throttle time and cluster Id V3", &response, noBrokersNoTopicsWithThrottleTimeAndClusterIDV3, 3)
...
func TestMetadataResponseWithOfflineReplicasV5(t *testing.T) {
	...
	vendor.testVersionDecodable(t, "no brokers, 1 topic with offline replica V5", &response, noBrokersOneTopicWithOfflineReplicasV5, 5)
