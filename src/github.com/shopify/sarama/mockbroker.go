...
type requestHandlerFunc func(req *vendor.request) (res vendor.encoder)
...
type MockBroker struct {
	...
	stopper      chan vendor.none
	expectations chan vendor.encoder
	listener     net.Listener
	t            vendor.TestReporter
...
type RequestResponse struct {
	Request  vendor.protocolBody
	Response vendor.encoder
}
...
func (b *MockBroker) SetHandlerByMap(handlerMap map[string]vendor.MockResponse) {
	b.setHandler(func(req *vendor.request) (res vendor.encoder) {
...
func (b *MockBroker) Close() {
	...
	if len(b.expectations) > 0 {
		...
		for e := range b.expectations {
			_, _ = buf.WriteString(vendor.Sdump(e))
		}
...
func (b *MockBroker) serverLoop() {
	...
	wg.Wait()
	vendor.Logger.Printf("*** mockbroker/%d: listener closed, err=%v", b.BrokerID(), err)
}
...
func (b *MockBroker) handleRequests(conn net.Conn, idx int, wg *sync.WaitGroup) {
	...
	vendor.Logger.Printf("*** mockbroker/%d/%d: connection opened", b.BrokerID(), idx)
	var err error

	abort := make(chan vendor.none)
	defer close(abort)
	go func() {
		select {
		case <-b.closing:
	...
	for {
		req, bytesRead, err := vendor.decodeRequest(conn)
		if err != nil {
			vendor.Logger.Printf("*** mockbroker/%d/%d: invalid request: err=%+v, %+v", b.brokerID, idx, err, vendor.Sdump(req))
			b.serverError(err)
			break
		}
		...
		if res == nil {
			vendor.Logger.Printf("*** mockbroker/%d/%d: ignored %v", b.brokerID, idx, vendor.Sdump(req))
			continue
		}
		vendor.Logger.Printf("*** mockbroker/%d/%d: served %v -> %v", b.brokerID, idx, req, res)

		encodedRes, err := vendor.encode(res, nil)
		if err != nil {
			b.serverError(err)
			break
		}
	...
	vendor.Logger.Printf("*** mockbroker/%d/%d: connection closed, err=%v", b.BrokerID(), idx, err)
}

func (b *MockBroker) defaultRequestHandler(req *vendor.request) (res vendor.encoder) {
	select {
...
func NewMockBroker(t vendor.TestReporter, brokerID int32) *MockBroker {
	return NewMockBrokerAddr(t, brokerID, "localhost:0")
}

// NewMockBrokerAddr behaves like newMockBroker but listens on the address you give
// it rather than just some ephemeral port.
func NewMockBrokerAddr(t vendor.TestReporter, brokerID int32, addr string) *MockBroker {
...
func NewMockBrokerListener(t vendor.TestReporter, brokerID int32, listener net.Listener) *MockBroker {
	var err error

	broker := &MockBroker{
		closing:      make(chan vendor.none),
		stopper:      make(chan vendor.none),
		t:            t,
		brokerID:     brokerID,
		expectations: make(chan vendor.encoder, 512),
		listener:     listener,
	}
	broker.handler = broker.defaultRequestHandler

	vendor.Logger.Printf("*** mockbroker/%d listening on %s\n", brokerID, broker.listener.Addr().String())
...
func (b *MockBroker) Returns(e vendor.encoder) {
	b.expectations <- e
}
