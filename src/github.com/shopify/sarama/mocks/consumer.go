...
func NewConsumer(t vendor.ErrorReporter, config *vendor.Config) *Consumer {
	if config == nil {
...
func (c *Consumer) ConsumePartition(topic string, partition int32, offset int64) (vendor.PartitionConsumer, error) {
	c.l.Lock()
	defer c.l.Unlock()
	...
	if c.partitionConsumers[topic] == nil || c.partitionConsumers[topic][partition] == nil {
		...
		return nil, vendor.errOutOfExpectations
	}

	pc := c.partitionConsumers[topic][partition]
	if pc.consumed {
		return nil, vendor.ConfigurationError("The topic/partition is already being consumed")
	}

	if pc.offset != vendor.AnyOffset && pc.offset != offset {
...
func (c *Consumer) Topics() ([]string, error) {
	...
	if c.metadata == nil {
		...
		return nil, vendor.ErrOutOfBrokers
	}
...
func (c *Consumer) ExpectConsumePartition(topic string, partition int32, offset int64) *PartitionConsumer {
	...
	if c.partitionConsumers[topic][partition] == nil {
		c.partitionConsumers[topic][partition] = &PartitionConsumer{
			...
			partition: partition,
			offset:    offset,
			messages:  make(chan *vendor.ConsumerMessage, c.config.ChannelBufferSize),
			errors:    make(chan *vendor.ConsumerError, c.config.ChannelBufferSize),
		}
	}
...
func (pc *PartitionConsumer) Close() error {
	if !pc.consumed {
		...
		return vendor.errPartitionConsumerNotStarted
	}
	...
	go func() {
		...
		var errs = make(vendor.ConsumerErrors, 0)
...
func (pc *PartitionConsumer) Errors() <-chan *vendor.ConsumerError {
	return pc.errors
}

// Messages implements the Messages method from the sarama.PartitionConsumer interface.
func (pc *PartitionConsumer) Messages() <-chan *vendor.ConsumerMessage {
	return pc.messages
}
...
func (pc *PartitionConsumer) YieldMessage(msg *vendor.ConsumerMessage) {
	pc.l.Lock()
	defer pc.l.Unlock()
...
func (pc *PartitionConsumer) YieldError(err error) {
	pc.errors <- &vendor.ConsumerError{
		Topic:     pc.topic,
