...
type AsyncProducer struct {
	l            sync.Mutex
	t            vendor.ErrorReporter
	expectations []*vendor.producerExpectation
	closed       chan struct{}
	input        chan *vendor.ProducerMessage
	successes    chan *vendor.ProducerMessage
	errors       chan *vendor.ProducerError
	lastOffset   int64
}
...
func NewAsyncProducer(t vendor.ErrorReporter, config *vendor.Config) *AsyncProducer {
	if config == nil {
	...
	mp := &AsyncProducer{
		...
		expectations: make([]*vendor.producerExpectation, 0),
		input:        make(chan *vendor.ProducerMessage, config.ChannelBufferSize),
		successes:    make(chan *vendor.ProducerMessage, config.ChannelBufferSize),
		errors:       make(chan *vendor.ProducerError, config.ChannelBufferSize),
	}
			...
			if mp.expectations == nil || len(mp.expectations) == 0 {
				...
				if expectation.CheckFunction != nil {
					if val, err := msg.Value.Encode(); err != nil {
						...
						mp.errors <- &vendor.ProducerError{Err: err, Msg: msg}
					} else {
				...
				if expectation.Result == vendor.errProduceSuccess {
					mp.lastOffset++
					...
					if config.Producer.Return.Errors {
						mp.errors <- &vendor.ProducerError{Err: expectation.Result, Msg: msg}
					}
				}
			}
			mp.l.Unlock()
...
// state object.
func (mp *AsyncProducer) Input() chan<- *vendor.ProducerMessage {
	return mp.input
}

// Successes corresponds with the Successes method of sarama's Producer implementation.
func (mp *AsyncProducer) Successes() <-chan *vendor.ProducerMessage {
	return mp.successes
}

// Errors corresponds with the Errors method of sarama's Producer implementation.
func (mp *AsyncProducer) Errors() <-chan *vendor.ProducerError {
	return mp.errors
}
...
func (mp *AsyncProducer) ExpectInputWithCheckerFunctionAndSucceed(cf vendor.ValueChecker) {
	mp.l.Lock()
	defer mp.l.Unlock()
	mp.expectations = append(mp.expectations, &vendor.producerExpectation{Result: vendor.errProduceSuccess, CheckFunction: cf})
}
...
func (mp *AsyncProducer) ExpectInputWithCheckerFunctionAndFail(cf vendor.ValueChecker, err error) {
	mp.l.Lock()
	defer mp.l.Unlock()
	mp.expectations = append(mp.expectations, &vendor.producerExpectation{Result: err, CheckFunction: cf})
}
