...
func NewSyncProducer(t vendor.ErrorReporter, config *vendor.Config) *SyncProducer {
	return &SyncProducer{
		t:            t,
...
func (sp *SyncProducer) SendMessage(msg *vendor.ProducerMessage) (partition int32, offset int64, err error) {
	...
	return -1, -1, vendor.errOutOfExpectations
}
...
func (sp *SyncProducer) SendMessages(msgs []*vendor.ProducerMessage) error {
	...
	return vendor.errOutOfExpectations
}
...
func (sp *SyncProducer) ExpectSendMessageWithCheckerFunctionAndSucceed(cf vendor.ValueChecker) {
	sp.l.Lock()
	defer sp.l.Unlock()
	sp.expectations = append(sp.expectations, &vendor.producerExpectation{Result: vendor.errProduceSuccess, CheckFunction: cf})
}
...
func (sp *SyncProducer) ExpectSendMessageWithCheckerFunctionAndFail(cf vendor.ValueChecker, err error) {
	sp.l.Lock()
	defer sp.l.Unlock()
	sp.expectations = append(sp.expectations, &vendor.producerExpectation{Result: err, CheckFunction: cf})
}
