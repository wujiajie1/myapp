...
func TestDescribeGroupsResponse(t *testing.T) {
	var response *vendor.DescribeGroupsResponse

	response = new(vendor.DescribeGroupsResponse)
	vendor.testVersionDecodable(t, "empty", response, describeGroupsResponseEmpty, 0)
	if len(response.Groups) != 0 {
		t.Error("Expected no groups")
	}

	response = new(vendor.DescribeGroupsResponse)
	vendor.testVersionDecodable(t, "populated", response, describeGroupsResponsePopulated, 0)
	if len(response.Groups) != 2 {
		t.Error("Expected two groups")
	}

	group0 := response.Groups[0]
	if group0.Err != vendor.ErrNoError {
	...
	if group1.Err != vendor.ErrGroupAuthorizationFailed {
