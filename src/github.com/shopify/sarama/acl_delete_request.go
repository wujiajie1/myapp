...
func (d *DeleteAclsRequest) encode(pe vendor.packetEncoder) error {
...
func (d *DeleteAclsRequest) decode(pd vendor.packetDecoder, version int16) (err error) {
	...
	d.Filters = make([]*vendor.AclFilter, n)
	for i := 0; i < n; i++ {
		d.Filters[i] = new(vendor.AclFilter)
...
func (d *DeleteAclsRequest) requiredVersion() vendor.KafkaVersion {
	switch d.Version {
	case 1:
