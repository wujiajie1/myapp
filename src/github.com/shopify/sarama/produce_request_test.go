...
func TestProduceRequest(t *testing.T) {
	request := new(vendor.ProduceRequest)
	vendor.testRequest(t, "empty", request, produceRequestEmpty)

	request.RequiredAcks = 0x123
	request.Timeout = 0x444
	vendor.testRequest(t, "header", request, produceRequestHeader)

	request.AddMessage("topic", 0xAD, &vendor.Message{Codec: vendor.CompressionNone, Key: nil, Value: []byte{0x00, 0xEE}})
	vendor.testRequest(t, "one message", request, produceRequestOneMessage)
	...
	packet := vendor.testRequestEncode(t, "one record", request, produceRequestOneRecord)
	...
	vendor.testRequestDecode(t, "one record", request, packet)
}
