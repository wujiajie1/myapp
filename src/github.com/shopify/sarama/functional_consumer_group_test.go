...
func TestFuncConsumerGroupPartitioning(t *testing.T) {
	vendor.checkKafkaVersion(t, "0.10.2")
...
func testFuncConsumerGroupFuzzySeed(topic string) error {
	...
	for pn := int32(0); pn < 4; pn++ {
		newest, err := client.GetOffset(topic, pn, vendor.OffsetNewest)
		if err != nil {
			return err
		}
		oldest, err := client.GetOffset(topic, pn, vendor.OffsetOldest)
		if err != nil {
			return err
		}
	...
	for i := total; i < 21000; i++ {
		producer.Input() <- &vendor.ProducerMessage{Topic: topic, Value: vendor.ByteEncoder([]byte("testdata"))}
	}
...
func (s *testFuncConsumerGroupSink) Push(clientID string, m *vendor.ConsumerMessage) {
	if s != nil {
...
func runTestFuncConsumerGroupMember(t *testing.T, groupID, clientID string, maxMessages int32, sink *testFuncConsumerGroupSink, topics ...string) *testFuncConsumerGroupMember {
	...
	group, err := vendor.NewConsumerGroup(vendor.kafkaBrokers, groupID, config)
	if err != nil {
		t.Fatal(err)
		return nil
	}
...
func (m *testFuncConsumerGroupMember) Setup(s vendor.ConsumerGroupSession) error {
	// store claims
	claims := make(map[string]int)
...
func (m *testFuncConsumerGroupMember) Cleanup(s vendor.ConsumerGroupSession) error {
	// enter post-cleanup state
	atomic.StoreInt32(&m.state, 3)
	return nil
}
func (m *testFuncConsumerGroupMember) ConsumeClaim(s vendor.ConsumerGroupSession, c vendor.ConsumerGroupClaim) error {
...
func (m *testFuncConsumerGroupMember) loop(topics []string) {
	...
	for {
		...
		if err := m.Consume(ctx, topics, m); err == vendor.ErrClosedConsumerGroup {
			return
		} else if err != nil {
			m.mu.Lock()
			m.errs = append(m.errs, err)
