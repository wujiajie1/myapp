...
type offsetManager struct {
	...
	closeOnce sync.Once
	closing   chan vendor.none
	closed    chan vendor.none
}

// NewOffsetManagerFromClient creates a new OffsetManager from the given client.
// It is still necessary to call Close() on the underlying client when finished with the partition manager.
func NewOffsetManagerFromClient(group string, client vendor.Client) (OffsetManager, error) {
	return newOffsetManagerFromClient(group, "", vendor.GroupGenerationUndefined, client)
}

func newOffsetManagerFromClient(group, memberID string, generation int32, client vendor.Client) (*offsetManager, error) {
	...
	om := &offsetManager{
		...
		closing: make(chan vendor.none),
		closed:  make(chan vendor.none),
	}
	go vendor.withRecover(om.mainLoop)
...
func (om *offsetManager) ManagePartition(topic string, partition int32) (PartitionOffsetManager, error) {
	...
	if topicManagers[partition] != nil {
		return nil, vendor.ConfigurationError("That topic/partition is already being managed")
	}
...
func (om *offsetManager) fetchInitialOffset(topic string, partition int32, retries int) (int64, string, error) {
	...
	req := new(vendor.OffsetFetchRequest)
	req.Version = 1
	...
	if block == nil {
		return 0, "", vendor.ErrIncompleteResponse
	}

	switch block.Err {
	case vendor.ErrNoError:
		return block.Offset, block.Metadata, nil
	case vendor.ErrNotCoordinatorForConsumer:
		if retries <= 0 {
			return 0, "", block.Err
		}
		om.releaseCoordinator(broker)
		return om.fetchInitialOffset(topic, partition, retries-1)
	case vendor.ErrOffsetsLoadInProgress:
		if retries <= 0 {
			return 0, "", block.Err
		}
...
func (om *offsetManager) coordinator() (*vendor.Broker, error) {
	om.brokerLock.RLock()
	broker := om.broker
...
func (om *offsetManager) releaseCoordinator(b *vendor.Broker) {
	om.brokerLock.Lock()
	if om.broker == b {
		om.broker = nil
	}
...
func (om *offsetManager) constructRequest() *vendor.OffsetCommitRequest {
	var r *vendor.OffsetCommitRequest
	var perPartitionTimestamp int64
	if om.conf.Consumer.Offsets.Retention == 0 {
		perPartitionTimestamp = vendor.ReceiveTime
		r = &vendor.OffsetCommitRequest{
			Version:                 1,
...
func (om *offsetManager) handleResponse(broker *vendor.Broker, req *vendor.OffsetCommitRequest, resp *vendor.OffsetCommitResponse) {
	om.pomsLock.RLock()
	defer om.pomsLock.RUnlock()
	...
	for _, topicManagers := range om.poms {
		for _, pom := range topicManagers {
			...
			var err vendor.KError
			var ok bool

			if resp.Errors[pom.topic] == nil {
				pom.handleError(vendor.ErrIncompleteResponse)
				continue
			}
			...
			case vendor.ErrNotLeaderForPartition, vendor.ErrLeaderNotAvailable,
				vendor.ErrConsumerCoordinatorNotAvailable, vendor.ErrNotCoordinatorForConsumer:
				// not a critical error, we just need to redispatch
				om.releaseCoordinator(broker)
			case vendor.ErrOffsetMetadataTooLarge, vendor.ErrInvalidCommitOffsetSize:
				// nothing we can do about this, just tell the user and carry on
				pom.handleError(err)
			case vendor.ErrOffsetsLoadInProgress:
				// nothing wrong but we didn't commit, we'll get it next time round
			case vendor.ErrUnknownTopicOrPartition:
...
type PartitionOffsetManager interface {
	...
	Errors() <-chan *vendor.ConsumerError
...
func (om *offsetManager) newPartitionOffsetManager(topic string, partition int32) (*partitionOffsetManager, error) {
	...
	return &partitionOffsetManager{
		...
		topic:     topic,
		partition: partition,
		errors:    make(chan *vendor.ConsumerError, om.conf.ChannelBufferSize),
		offset:    offset,
		metadata:  metadata,
	}, nil
}

func (pom *partitionOffsetManager) Errors() <-chan *vendor.ConsumerError {
	return pom.errors
}
...
func (pom *partitionOffsetManager) Close() error {
	...
	var errors vendor.ConsumerErrors
