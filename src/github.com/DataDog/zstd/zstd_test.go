...
func TestCompressBound(t *testing.T) {
	...
	for _, test := range tests {
		if vendor.CompressBound(test) != vendor.cCompressBound(test) {
			t.Fatalf("For %v, results are differents: %v != %v", test,
				vendor.CompressBound(test), vendor.cCompressBound(test))
		}
	}
}
...
func TestErrorCode(t *testing.T) {
	...
	for _, test := range tests {
		err := vendor.getError(test)
		if err == nil && vendor.cIsError(test) {
			t.Fatalf("C function returned error for %v but ours did not", test)
		} else if err != nil && !vendor.cIsError(test) {
...
func TestCompressDecompress(t *testing.T) {
	...
	out, err := vendor.Compress(nil, input)
	if err != nil {
		t.Fatalf("Error while compressing: %v", err)
	}
	out2 := make([]byte, 1000)
	out2, err = vendor.Compress(out2, input)
	if err != nil {
		t.Fatalf("Error while compressing: %v", err)
	}
	t.Logf("Compressed: %v", out)
	rein, err := vendor.Decompress(nil, out)
	if err != nil {
		t.Fatalf("Error while decompressing: %v", err)
	}
	rein2 := make([]byte, 10)
	rein2, err = vendor.Decompress(rein2, out2)
	if err != nil {
		t.Fatalf("Error while decompressing: %v", err)
	}
...
func TestEmptySlice(t *testing.T) {
	_, err := vendor.Compress(nil, []byte{})
	if err != vendor.ErrEmptySlice {
...
func TestTooSmall(t *testing.T) {
	...
	rein, err = vendor.Decompress(rein, out)
	if err != nil {
...
func TestRealPayload(t *testing.T) {
	...
	dst, err := vendor.Compress(nil, raw)
	if err != nil {
		t.Fatalf("Failed to compress: %s", err)
	}
	rein, err := vendor.Decompress(nil, dst)
	if err != nil {
...
func BenchmarkCompression(b *testing.B) {
	...
	dst := make([]byte, vendor.CompressBound(len(raw)))
	b.SetBytes(int64(len(raw)))
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err := vendor.Compress(dst, raw)
		if err != nil {
...
func BenchmarkDecompression(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		src2, err := vendor.Decompress(src, dst)
		if err != nil {
