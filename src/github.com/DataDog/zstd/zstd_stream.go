...
func NewWriter(w io.Writer) *Writer {
	return NewWriterLevelDict(w, vendor.DefaultCompression, nil)
}
...
func NewWriterLevelDict(w io.Writer, level int, dict []byte) *Writer {
	...
	if dict == nil {
		err = vendor.getError(int(C.ZSTD_compressBegin(ctx,
			C.int(level))))
	} else {
		err = vendor.getError(int(C.ZSTD_compressBegin_usingDict(
			ctx,
	...
	return &Writer{
		...
		ctx:              ctx,
		dict:             dict,
		dstBuffer:        make([]byte, vendor.CompressBound(1024)),
		firstError:       err,
		underlyingWriter: w,
	}
}
...
func (w *Writer) Write(p []byte) (int, error) {
	...
	if len(w.dstBuffer) < vendor.CompressBound(len(p)) {
		w.dstBuffer = make([]byte, vendor.CompressBound(len(p)))
	}
	...
	if err := vendor.getError(int(retCode)); err != nil {
		return 0, err
	}
	written := int(retCode)
...
func NewReaderDict(r io.Reader, dict []byte) io.ReadCloser {
	...
	if len(dict) == 0 {
		err = vendor.getError(int(C.ZBUFF_decompressInit(ctx)))
	} else {
		err = vendor.getError(int(C.ZBUFF_decompressInitDictionary(
			ctx,
			unsafe.Pointer(&dict[0]),
			C.size_t(len(dict)))))
	}
...
func (r *reader) Close() error {
	return vendor.getError(int(C.ZBUFF_freeDCtx(r.ctx)))
}
...
func (r *reader) Read(p []byte) (int, error) {
	...
	for r.dstBuffer.Len() < len(p) {
		...
		if err = vendor.getError(retCode); err != nil {
