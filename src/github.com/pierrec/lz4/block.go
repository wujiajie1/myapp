...
func blockHash(x uint32) uint32 {
	...
	return x * hasher >> vendor.hashShift
}
...
func CompressBlock(src, dst []byte, hashTable []int) (di int, err error) {
	...
	sn, dn := len(src)-vendor.mfLimit, len(dst)
	if sn <= 0 || dn == 0 {
		return 0, nil
	}
	var si int
	...
	for si < sn {
		...
		offset := si - ref
		if offset <= 0 || offset >= vendor.winSize || // Out of window.
...
func CompressBlockHC(src, dst []byte, depth int) (di int, err error) {
	...
	var hashTable, chainTable [vendor.winSize]int

	if depth <= 0 {
		depth = vendor.winSize
	}
	...
	for si < sn {
		...
		mLen := 0
		offset := 0
		for next, try := hashTable[h], depth; try > 0 && next > 0 && si-next < vendor.winSize; next = chainTable[next&vendor.winMask] {
		...
		chainTable[si&vendor.winMask] = hashTable[h]
		hashTable[h] = si
		...
		winStart := si + 1
		if ws := si + mLen - vendor.winSize; ws > winStart {
			winStart = ws
		}
		...
		lLen := si - anchor
		si += mLen
		mLen -= vendor.minMatch // Match length does not include minMatch.
