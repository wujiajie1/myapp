...
func TestMaxEncodedLenOfMaxBlockSize(t *testing.T) {
	got := vendor.maxEncodedLenOfMaxBlockSize
	want := vendor.MaxEncodedLen(vendor.maxBlockSize)
	if got != want {
...
func roundtrip(b, ebuf, dbuf []byte) error {
	d, err := vendor.Decode(dbuf, vendor.Encode(ebuf, b))
	if err != nil {
...
func TestInvalidVarint(t *testing.T) {
	...
	for _, tc := range testCases {
		input := []byte(tc.input)
		if _, err := vendor.DecodedLen(input); err != vendor.ErrCorrupt {
			t.Errorf("%s: DecodedLen: got %v, want ErrCorrupt", tc.desc, err)
		}
		if _, err := vendor.Decode(nil, input); err != vendor.ErrCorrupt {
...
loop:
	for i, tc := range testCases {
		...
		g, gotErr := vendor.Decode(dBuf[:], input)
...
func TestDecodeCopy4(t *testing.T) {
	...
	gotBytes, err := vendor.Decode(nil, []byte(input))
	if err != nil {
		t.Fatal(err)
	}
	got := string(gotBytes)
	...
	for length := 1; length <= 18; length++ {
		...
		loop:
			for suffixLen := 0; suffixLen <= 18; suffixLen++ {
				...
				inputBuf[inputLen] = vendor.tagLiteral + 4*byte(len(prefix)-1)
				inputLen++
				inputLen += copy(inputBuf[inputLen:], prefix)
				inputBuf[inputLen+0] = vendor.tagCopy2 + 4*byte(length-1)
				inputBuf[inputLen+1] = byte(offset)
				inputBuf[inputLen+2] = 0x00
				inputLen += 3
				if suffixLen > 0 {
					inputBuf[inputLen] = vendor.tagLiteral + 4*byte(suffixLen-1)
					inputLen++
				...
				got, err := vendor.Decode(gotBuf[:], input)
				if err != nil {
...
func TestExtendMatchGoldenInput(t *testing.T) {
	...
	for i, tc := range vendor.extendMatchGoldenTestCases {
		got := vendor.extendMatch(src, tc.i, tc.j)
		if got != tc.want {
	...
	for yIndex := 40; yIndex > 30; yIndex-- {
		...
		for _, i := range nums {
			for _, j := range nums {
				...
				got := vendor.extendMatch(xxx, i, j)
				want := ref(xxx, i, j)
				if got != want {
...
func TestFramingFormat(t *testing.T) {
	...
	if _, err := vendor.NewWriter(buf).Write(src); err != nil {
		t.Fatalf("Write: encoding: %v", err)
	}
	dst, err := ioutil.ReadAll(vendor.NewReader(buf))
	if err != nil {
...
func TestWriterGoldenOutput(t *testing.T) {
	buf := new(bytes.Buffer)
	w := vendor.NewBufferedWriter(buf)
	defer w.Close()
...
func TestEmitCopy(t *testing.T) {
	...
	dst := make([]byte, 1024)
	for _, tc := range testCases {
		n := vendor.emitCopy(dst, tc.offset, tc.length)
		got := string(dst[:n])
		if got != tc.want {
...
loop:
	for i := 0; i < 1<<uint(len(inputs)); i++ {
		...
		got, err := ioutil.ReadAll(vendor.NewReader(buf))
		if err != nil {
...
func TestReaderUncompressedDataOK(t *testing.T) {
	r := vendor.NewReader(strings.NewReader(vendor.magicChunk +
		"\x01\x08\x00\x00" + // Uncompressed chunk, 8 bytes long (including 4 byte checksum).
		"\x68\x10\xe6\xb6" + // Checksum.
		"\x61\x62\x63\x64", // Uncompressed payload: "abcd".
	))
...
func TestReaderUncompressedDataNoPayload(t *testing.T) {
	...
	if _, err := ioutil.ReadAll(r); err != vendor.ErrCorrupt {
		t.Fatalf("got %v, want %v", err, vendor.ErrCorrupt)
	}
}
...
func TestReaderReset(t *testing.T) {
	...
	buf := new(bytes.Buffer)
	if _, err := vendor.NewWriter(buf).Write(gold); err != nil {
		t.Fatalf("Write: %v", err)
	}
	encoded, invalid, partial := buf.String(), "invalid", "partial"
	r := vendor.NewReader(nil)
...
func TestWriterReset(t *testing.T) {
	...
	for _, buffered := range []bool{false, true} {
		var w *vendor.Writer
		if buffered {
			w = vendor.NewBufferedWriter(nil)
			defer w.Close()
		} else {
			w = vendor.NewWriter(nil)
		}
...
func TestWriterResetWithoutFlush(t *testing.T) {
	...
	w := vendor.NewBufferedWriter(buf0)
	...
	got, err := ioutil.ReadAll(vendor.NewReader(buf1))
	if err != nil {
...
func benchDecode(b *testing.B, src []byte) {
	encoded := vendor.Encode(nil, src)
	// Bandwidth is in amount of uncompressed data.
	b.SetBytes(int64(len(src)))
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		vendor.Decode(src, encoded)
	}
}
...
func benchEncode(b *testing.B, src []byte) {
	// Bandwidth is in amount of uncompressed data.
	b.SetBytes(int64(len(src)))
	dst := make([]byte, vendor.MaxEncodedLen(len(src)))
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		vendor.Encode(dst, src)
	}
}
...
func BenchmarkExtendMatch(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		for _, tc := range vendor.extendMatchGoldenTestCases {
			vendor.extendMatch(src, tc.i, tc.j)
		}
	}
}
