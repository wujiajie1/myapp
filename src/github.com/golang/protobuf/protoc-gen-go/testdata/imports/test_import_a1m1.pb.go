// Code generated by protoc-gen-go. DO NOT EDIT.
// source: imports/test_import_a1m1.proto

package imports

import (
	"fmt"
	"math"
	"vendor"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = vendor.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = vendor.ProtoPackageIsVersion3 // please upgrade the proto package

type A1M1 struct {
	F                    *vendor.M1 `protobuf:"bytes,1,opt,name=f,proto3" json:"f,omitempty"`
	XXX_NoUnkeyedLiteral struct{}   `json:"-"`
	XXX_unrecognized     []byte     `json:"-"`
	XXX_sizecache        int32      `json:"-"`
}

func (m *A1M1) Reset()         { *m = A1M1{} }
func (m *A1M1) String() string { return vendor.CompactTextString(m) }
func (*A1M1) ProtoMessage()    {}
func (*A1M1) Descriptor() ([]byte, []int) {
	return fileDescriptor_3b904a47327455f3, []int{0}
}

func (m *A1M1) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_A1M1.Unmarshal(m, b)
}
func (m *A1M1) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_A1M1.Marshal(b, m, deterministic)
}
func (m *A1M1) XXX_Merge(src vendor.Message) {
	xxx_messageInfo_A1M1.Merge(m, src)
}
func (m *A1M1) XXX_Size() int {
	return xxx_messageInfo_A1M1.Size(m)
}
func (m *A1M1) XXX_DiscardUnknown() {
	xxx_messageInfo_A1M1.DiscardUnknown(m)
}

var xxx_messageInfo_A1M1 vendor.InternalMessageInfo

func (m *A1M1) GetF() *vendor.M1 {
	if m != nil {
		return m.F
	}
	return nil
}

func init() {
	vendor.RegisterType((*A1M1)(nil), "test.A1M1")
}

func init() { vendor.RegisterFile("imports/test_import_a1m1.proto", fileDescriptor_3b904a47327455f3) }

var fileDescriptor_3b904a47327455f3 = []byte{
	// 149 bytes of a gzipped FileDescriptorProto
	,
}
