// Code generated by protoc-gen-go. DO NOT EDIT.
// source: import_public/b.proto

package import_public

import (
	"fmt"
	"math"
	"vendor"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = vendor.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = vendor.ProtoPackageIsVersion3 // please upgrade the proto package

type Local struct {
	M                    *vendor.M `protobuf:"bytes,1,opt,name=m" json:"m,omitempty"`
	E                    *vendor.E `protobuf:"varint,2,opt,name=e,enum=goproto.test.import_public.sub.E" json:"e,omitempty"`
	XXX_NoUnkeyedLiteral struct{}  `json:"-"`
	XXX_unrecognized     []byte    `json:"-"`
	XXX_sizecache        int32     `json:"-"`
}

func (m *Local) Reset()         { *m = Local{} }
func (m *Local) String() string { return vendor.CompactTextString(m) }
func (*Local) ProtoMessage()    {}
func (*Local) Descriptor() ([]byte, []int) {
	return fileDescriptor_84995586b3d09710, []int{0}
}

func (m *Local) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_Local.Unmarshal(m, b)
}
func (m *Local) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_Local.Marshal(b, m, deterministic)
}
func (m *Local) XXX_Merge(src vendor.Message) {
	xxx_messageInfo_Local.Merge(m, src)
}
func (m *Local) XXX_Size() int {
	return xxx_messageInfo_Local.Size(m)
}
func (m *Local) XXX_DiscardUnknown() {
	xxx_messageInfo_Local.DiscardUnknown(m)
}

var xxx_messageInfo_Local vendor.InternalMessageInfo

func (m *Local) GetM() *vendor.M {
	if m != nil {
		return m.M
	}
	return nil
}

func (m *Local) GetE() vendor.E {
	if m != nil && m.E != nil {
		return *m.E
	}
	return vendor.E_ZERO
}

func init() {
	vendor.RegisterType((*Local)(nil), "goproto.test.import_public.Local")
}

func init() { vendor.RegisterFile("import_public/b.proto", fileDescriptor_84995586b3d09710) }

var fileDescriptor_84995586b3d09710 = []byte{
	// 169 bytes of a gzipped FileDescriptorProto
	,
}
