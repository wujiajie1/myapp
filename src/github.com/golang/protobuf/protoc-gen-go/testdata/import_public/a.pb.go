// Code generated by protoc-gen-go. DO NOT EDIT.
// source: import_public/a.proto

package import_public

import (
	"fmt"
	"math"
	"vendor"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = vendor.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = vendor.ProtoPackageIsVersion3 // please upgrade the proto package

const Default_M_DefaultField = vendor.Default_M_DefaultField

// M from public import import_public/sub/a.proto
type M = vendor.M
type M_OneofInt32 = vendor.M_OneofInt32
type M_OneofInt64 = vendor.M_OneofInt64

// M_Grouping from public import import_public/sub/a.proto
type M_Grouping = vendor.M_Grouping

// M_Submessage from public import import_public/sub/a.proto
type M_Submessage = vendor.M_Submessage
type M_Submessage_SubmessageOneofInt32 = vendor.M_Submessage_SubmessageOneofInt32
type M_Submessage_SubmessageOneofInt64 = vendor.M_Submessage_SubmessageOneofInt64

// E from public import import_public/sub/a.proto
type E = vendor.E

var E_name = vendor.E_name
var E_value = vendor.E_value

const E_ZERO = E(vendor.E_ZERO)

// M_Subenum from public import import_public/sub/a.proto
type M_Subenum = vendor.M_Subenum

var M_Subenum_name = vendor.M_Subenum_name
var M_Subenum_value = vendor.M_Subenum_value

const M_M_ZERO = M_Subenum(vendor.M_M_ZERO)

// M_Submessage_Submessage_Subenum from public import import_public/sub/a.proto
type M_Submessage_Submessage_Subenum = vendor.M_Submessage_Submessage_Subenum

var M_Submessage_Submessage_Subenum_name = vendor.M_Submessage_Submessage_Subenum_name
var M_Submessage_Submessage_Subenum_value = vendor.M_Submessage_Submessage_Subenum_value

const M_Submessage_M_SUBMESSAGE_ZERO = M_Submessage_Submessage_Subenum(vendor.M_Submessage_M_SUBMESSAGE_ZERO)

var E_ExtensionField = vendor.E_ExtensionField

type Public struct {
	M                    *vendor.M     `protobuf:"bytes,1,opt,name=m" json:"m,omitempty"`
	E                    *vendor.E     `protobuf:"varint,2,opt,name=e,enum=goproto.test.import_public.sub.E" json:"e,omitempty"`
	Local                *vendor.Local `protobuf:"bytes,3,opt,name=local" json:"local,omitempty"`
	XXX_NoUnkeyedLiteral struct{}      `json:"-"`
	XXX_unrecognized     []byte        `json:"-"`
	XXX_sizecache        int32         `json:"-"`
}

func (m *Public) Reset()         { *m = Public{} }
func (m *Public) String() string { return vendor.CompactTextString(m) }
func (*Public) ProtoMessage()    {}
func (*Public) Descriptor() ([]byte, []int) {
	return fileDescriptor_73b7577c95fa6b70, []int{0}
}

func (m *Public) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_Public.Unmarshal(m, b)
}
func (m *Public) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_Public.Marshal(b, m, deterministic)
}
func (m *Public) XXX_Merge(src vendor.Message) {
	xxx_messageInfo_Public.Merge(m, src)
}
func (m *Public) XXX_Size() int {
	return xxx_messageInfo_Public.Size(m)
}
func (m *Public) XXX_DiscardUnknown() {
	xxx_messageInfo_Public.DiscardUnknown(m)
}

var xxx_messageInfo_Public vendor.InternalMessageInfo

func (m *Public) GetM() *vendor.M {
	if m != nil {
		return m.M
	}
	return nil
}

func (m *Public) GetE() vendor.E {
	if m != nil && m.E != nil {
		return *m.E
	}
	return vendor.E_ZERO
}

func (m *Public) GetLocal() *vendor.Local {
	if m != nil {
		return m.Local
	}
	return nil
}

func init() {
	vendor.RegisterType((*Public)(nil), "goproto.test.import_public.Public")
}

func init() { vendor.RegisterFile("import_public/a.proto", fileDescriptor_73b7577c95fa6b70) }

var fileDescriptor_73b7577c95fa6b70 = []byte{
	// 195 bytes of a gzipped FileDescriptorProto
	,
}
