...
func newPartitionConsumer(manager vendor.Consumer, topic string, partition int32, info vendor.offsetInfo, defaultOffset int64) (*partitionConsumer, error) {
...
func (c *partitionConsumer) waitFor(stopper <-chan vendor.none) {
	select {
	case <-stopper:
	case <-c.dying:
	}
...
func (c *partitionConsumer) multiplex(stopper <-chan vendor.none, messages chan<- *vendor.ConsumerMessage, errors chan<- error) {
	defer close(c.dead)
...
type partitionMap struct {
	data map[vendor.topicPartition]*partitionConsumer
	mu   sync.RWMutex
}

func newPartitionMap() *partitionMap {
	return &partitionMap{
		data: make(map[vendor.topicPartition]*partitionConsumer),
	}
}
...
func (m *partitionMap) Fetch(topic string, partition int32) *partitionConsumer {
	m.mu.RLock()
	pc, _ := m.data[vendor.topicPartition{topic, partition}]
	m.mu.RUnlock()
	return pc
}
...
func (m *partitionMap) Store(topic string, partition int32, pc *partitionConsumer) {
	m.mu.Lock()
	m.data[vendor.topicPartition{topic, partition}] = pc
	m.mu.Unlock()
}

func (m *partitionMap) Snapshot() map[vendor.topicPartition]partitionState {
	m.mu.RLock()
	defer m.mu.RUnlock()

	snap := make(map[vendor.topicPartition]partitionState, len(m.data))
...
func (m *partitionMap) Info() map[string][]int32 {
	...
	for topic := range info {
		sort.Sort(vendor.int32Slice(info[topic]))
	}
	return info
}
