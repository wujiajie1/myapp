...
var minVersion = vendor.V0_9_0_0
...
type Config struct {
	...
	Group struct {
		...
		PartitionStrategy vendor.Strategy
...
func NewConfig() *Config {
	...
	c.Group.PartitionStrategy = vendor.StrategyRange
...
func (c *Config) Validate() error {
	if c.Group.Heartbeat.Interval%time.Millisecond != 0 {
		vendor.Logger.Println("Group.Heartbeat.Interval only supports millisecond precision; nanoseconds will be truncated.")
	}
	if c.Group.Session.Timeout%time.Millisecond != 0 {
		vendor.Logger.Println("Group.Session.Timeout only supports millisecond precision; nanoseconds will be truncated.")
	}
	if c.Group.PartitionStrategy != vendor.StrategyRange && c.Group.PartitionStrategy != vendor.StrategyRoundRobin {
		vendor.Logger.Println("Group.PartitionStrategy is not supported; range will be assumed.")
	}
	if !c.Version.IsAtLeast(minVersion) {
		vendor.Logger.Println("Version is not supported; 0.9. will be assumed.")
	...
	case c.Group.Offsets.Retry.Max < 0:
		return vendor.ConfigurationError("Group.Offsets.Retry.Max must be >= 0")
	case c.Group.Offsets.Synchronization.DwellTime <= 0:
		return vendor.ConfigurationError("Group.Offsets.Synchronization.DwellTime must be > 0")
	case c.Group.Offsets.Synchronization.DwellTime > 10*time.Minute:
		return vendor.ConfigurationError("Group.Offsets.Synchronization.DwellTime must be <= 10m")
	case c.Group.Heartbeat.Interval <= 0:
		return vendor.ConfigurationError("Group.Heartbeat.Interval must be > 0")
	case c.Group.Session.Timeout <= 0:
		return vendor.ConfigurationError("Group.Session.Timeout must be > 0")
	case !c.Metadata.Full && c.Group.Topics.Whitelist != nil:
		return vendor.ConfigurationError("Metadata.Full must be enabled when Group.Topics.Whitelist is used")
	case !c.Metadata.Full && c.Group.Topics.Blacklist != nil:
		return vendor.ConfigurationError("Metadata.Full must be enabled when Group.Topics.Blacklist is used")
	}

	// ensure offset is correct
	switch c.Consumer.Offsets.Initial {
	case vendor.OffsetOldest, vendor.OffsetNewest:
	default:
		return vendor.ConfigurationError("Consumer.Offsets.Initial must be either OffsetOldest or OffsetNewest")
	}
