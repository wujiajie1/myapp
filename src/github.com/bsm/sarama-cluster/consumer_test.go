...
var _ = Describe("Consumer", func() {
	...
	var newConsumerOf = func(group string, topics ...string) (*vendor.Consumer, error) {
		config := vendor.NewConfig()
		config.Consumer.Return.Errors = true
		config.Consumer.Offsets.Initial = vendor.OffsetOldest
		return vendor.NewConsumer(vendor.testKafkaAddrs, group, topics, config)
	}

	var subscriptionsOf = func(c *vendor.Consumer) GomegaAsyncAssertion {
	...
	It("should init and share", func() {
		// start CS1
		cs1, err := newConsumerOf(vendor.testGroup, vendor.testTopics...)
		...
		// start CS2
		cs2, err := newConsumerOf(vendor.testGroup, vendor.testTopics...)
		Expect(err).NotTo(HaveOccurred())
		defer cs2.Close()
	...
	It("should allow more consumers than partitions", func() {
		cs1, err := newConsumerOf(vendor.testGroup, "topic-a")
		Expect(err).NotTo(HaveOccurred())
		defer cs1.Close()
		cs2, err := newConsumerOf(vendor.testGroup, "topic-a")
		Expect(err).NotTo(HaveOccurred())
		defer cs2.Close()
		cs3, err := newConsumerOf(vendor.testGroup, "topic-a")
		Expect(err).NotTo(HaveOccurred())
		defer cs3.Close()
		cs4, err := newConsumerOf(vendor.testGroup, "topic-a")
		Expect(err).NotTo(HaveOccurred())
		...
		// add a 5th consumer
		cs5, err := newConsumerOf(vendor.testGroup, "topic-a")
		Expect(err).NotTo(HaveOccurred())
		defer cs5.Close()
	...
	It("should be allowed to subscribe to partitions via white/black-lists", func() {
		...
		cs, err := vendor.NewConsumer(vendor.testKafkaAddrs, vendor.testGroup, nil, config)
		Expect(err).NotTo(HaveOccurred())
		defer cs.Close()
	...
	It("should receive rebalance notifications", func() {
		...
		cs, err := vendor.NewConsumer(vendor.testKafkaAddrs, vendor.testGroup, vendor.testTopics, config)
		Expect(err).NotTo(HaveOccurred())
		defer cs.Close()
	...
	It("should support manual mark/commit", func() {
		cs, err := newConsumerOf(vendor.testGroup, "topic-a")
		Expect(err).NotTo(HaveOccurred())
		defer cs.Close()
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(offsets).To(Equal(map[string]map[int32]vendor.offsetInfo{
	...
	It("should not commit unprocessed offsets", func() {
		...
		bk, err := vendor.testClient.Coordinator(groupID)
		Expect(err).NotTo(HaveOccurred())
		...
		Expect(bk.FetchOffset(req)).To(Equal(&vendor.OffsetFetchResponse{
			Blocks: map[string]map[int32]*vendor.OffsetFetchResponseBlock{
	...
	It("should consume partitions", func() {
		...
		consume := func(consumerID string) {
			...
			config := vendor.NewConfig()
			config.Group.Mode = vendor.ConsumerModePartitions
			config.Consumer.Offsets.Initial = vendor.OffsetOldest

			cs, err := vendor.NewConsumer(vendor.testKafkaAddrs, "partitions", vendor.testTopics, config)
			Expect(err).NotTo(HaveOccurred())
			defer cs.Close()

			for pc := range cs.Partitions() {
				go func(pc vendor.PartitionConsumer) {
					defer pc.Close()
	...
	It("should not lock on rebalance while dying", func() {
		count := int32(0)
		consume := func(consumerID string, closeChan, doneCh chan vendor.none) {
			defer GinkgoRecover()
			...
			cs, err := vendor.NewConsumer(vendor.testKafkaAddrs, "partitions-no-lock", vendor.testTopics, config)
			Expect(err).NotTo(HaveOccurred())
			defer func() {
				cs.Close()
				close(doneCh)
		...
		chanA := make(chan vendor.none)
		chanADone := make(chan vendor.none)
		go consume("A", chanA, chanADone)
		chanB := make(chan vendor.none)
		chanBDone := make(chan vendor.none)
		...
		Eventually(func() chan vendor.none {
			return chanADone
	...
	It("should consume/commit/resume", func() {
		acc := make(chan *vendor.testConsumerMessage, 20000)
		consume := func(consumerID string, max int32) {
			defer GinkgoRecover()

			cs, err := vendor.NewConsumer(vendor.testKafkaAddrs, "fuzzing", vendor.testTopics, nil)
			Expect(err).NotTo(HaveOccurred())
			defer cs.Close()
			cs.consumerID = consumerID

			for msg := range cs.Messages() {
				acc <- &vendor.testConsumerMessage{*msg, consumerID}
				cs.MarkOffset(msg, "")
		...
		Expect(vendor.testSeed(5000, vendor.testTopics)).NotTo(HaveOccurred())
	...
	It("should allow close to be called multiple times", func() {
		cs, err := newConsumerOf(vendor.testGroup, vendor.testTopics...)
		Expect(err).NotTo(HaveOccurred())
