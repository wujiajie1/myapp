...
var _ = Describe("OffsetStash", func() {
	var subject *vendor.OffsetStash

	BeforeEach(func() {
		subject = vendor.NewOffsetStash()
	})
	...
	It("should update", func() {
		...
		Expect(subject.offsets).To(HaveKeyWithValue(
			vendor.topicPartition{Topic: "topic", Partition: 0},
			vendor.offsetInfo{Offset: 0, Metadata: "m3ta"},
		))
		...
		Expect(subject.offsets).To(HaveKeyWithValue(
			vendor.topicPartition{Topic: "topic", Partition: 0},
			vendor.offsetInfo{Offset: 200, Metadata: "m3ta"},
		))
		...
		Expect(subject.offsets).To(HaveKeyWithValue(
			vendor.topicPartition{Topic: "topic", Partition: 1},
			vendor.offsetInfo{Offset: 300, Metadata: ""},
		))
	})
	...
	It("should reset", func() {
		...
		Expect(subject.offsets).To(HaveKeyWithValue(
			vendor.topicPartition{Topic: "topic", Partition: 0},
			vendor.offsetInfo{Offset: 199, Metadata: "m3t@"},
		))
		...
		Expect(subject.offsets).To(HaveKeyWithValue(
			vendor.topicPartition{Topic: "topic", Partition: 1},
			vendor.offsetInfo{Offset: 200, Metadata: "m3t@"},
		))
