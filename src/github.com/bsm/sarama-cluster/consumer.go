...
type Consumer struct {
	...
	consumer vendor.Consumer
	subs     *vendor.partitionMap
	...
	dying, dead chan vendor.none
	closeOnce   sync.Once

	consuming     int32
	messages      chan *vendor.ConsumerMessage
	errors        chan error
	partitions    chan vendor.PartitionConsumer
	notifications chan *vendor.Notification

	commitMu sync.Mutex
}

// NewConsumer initializes a new consumer
func NewConsumer(addrs []string, groupID string, topics []string, config *vendor.Config) (*Consumer, error) {
	client, err := vendor.NewClient(addrs, config)
	if err != nil {
		return nil, err
	}
...
func NewConsumerFromClient(client *vendor.Client, groupID string, topics []string) (*Consumer, error) {
	if !client.claim() {
		return nil, vendor.errClientInUse
	}

	consumer, err := vendor.NewConsumerFromClient(client.Client)
	if err != nil {
		client.release()
		return nil, err
	}
	...
	c := &Consumer{
		client:   client,
		consumer: consumer,
		subs:     vendor.newPartitionMap(),
		groupID:  groupID,

		coreTopics: topics,

		dying: make(chan vendor.none),
		dead:  make(chan vendor.none),

		messages:      make(chan *vendor.ConsumerMessage),
		errors:        make(chan error, client.config.ChannelBufferSize),
		partitions:    make(chan vendor.PartitionConsumer, 1),
		notifications: make(chan *vendor.Notification),
	}
...
func (c *Consumer) Messages() <-chan *vendor.ConsumerMessage { return c.messages }
...
func (c *Consumer) Partitions() <-chan vendor.PartitionConsumer { return c.partitions }
...
func (c *Consumer) Notifications() <-chan *vendor.Notification { return c.notifications }
...
func (c *Consumer) MarkOffset(msg *vendor.ConsumerMessage, metadata string) {
...
// See MarkOffset for additional explanation.
func (c *Consumer) MarkOffsets(s *vendor.OffsetStash) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (c *Consumer) ResetOffset(msg *vendor.ConsumerMessage, metadata string) {
	if sub := c.subs.Fetch(msg.Topic, msg.Partition); sub != nil {
...
// See ResetOffset for additional explanation.
func (c *Consumer) ResetOffsets(s *vendor.OffsetStash) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (c *Consumer) CommitOffsets() error {
	...
	req := &vendor.OffsetCommitRequest{
		Version:                 2,
	...
	for topic, errs := range resp.Errors {
		for partition, kerr := range errs {
			if kerr != vendor.ErrNoError {
				err = kerr
			} else if state, ok := snap[vendor.topicPartition{topic, partition}]; ok {
				if sub := c.subs.Fetch(topic, partition); sub != nil {
...
func (c *Consumer) nextTick() {
	...
	var notification *vendor.Notification
	if c.client.config.Group.Return.Notifications {
		notification = vendor.newNotification(c.subs.Info())
	}
	...
	if c.client.config.Group.Return.Notifications {
		c.handleNotification(vendor.newNotification(c.subs.Info()))
	}
...
func (c *Consumer) hbLoop(stopped <-chan vendor.none) {
	...
	for {
		...
		case <-ticker.C:
			...
			case nil, vendor.ErrNoError:
			case vendor.ErrNotCoordinatorForConsumer, vendor.ErrRebalanceInProgress:
				return
			default:
				c.handleError(&vendor.Error{Ctx: "heartbeat", error: err})
				return
			}
		case <-stopped:
			return
...
func (c *Consumer) twLoop(stopped <-chan vendor.none) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				c.handleError(&vendor.Error{Ctx: "topics", error: err})
				return
			}
...
func (c *Consumer) cmLoop(stopped <-chan vendor.none) {
	...
	for {
		...
		case <-ticker.C:
			if err := c.commitOffsetsWithRetry(c.client.config.Group.Offsets.Retry.Max); err != nil {
				c.handleError(&vendor.Error{Ctx: "commit", error: err})
				return
			}
		case <-stopped:
			return
...
func (c *Consumer) rebalanceError(err error, n *vendor.Notification) {
	if n != nil {
	...
	switch err {
	case vendor.ErrRebalanceInProgress:
	default:
		c.handleError(&vendor.Error{Ctx: "rebalance", error: err})
	}
...
func (c *Consumer) handleNotification(n *vendor.Notification) {
	if c.client.config.Group.Return.Notifications {
		select {
...
func (c *Consumer) handleError(e *vendor.Error) {
	if c.client.config.Consumer.Return.Errors {
		...
		vendor.Logger.Printf("%s error: %s\n", e.Ctx, e.Error())
	}
}
...
func (c *Consumer) heartbeat() error {
	...
	memberID, generationID := c.membership()
	resp, err := broker.Heartbeat(&vendor.HeartbeatRequest{
		GroupId:      c.groupID,
		MemberId:     memberID,
...
func (c *Consumer) rebalance() (map[string][]int32, error) {
	...
	vendor.Logger.Printf("cluster/consumer %s rebalance\n", memberID)
	...
	switch {
	case err == vendor.ErrUnknownMemberId:
		c.membershipMu.Lock()
		c.memberID = ""
	...
	switch {
	case err == vendor.ErrRebalanceInProgress:
		return nil, err
	case err != nil:
		_ = c.leaveGroup()
		return nil, err
...
func (c *Consumer) subscribe(tomb *vendor.loopTomb, subs map[string][]int32) error {
	// fetch offsets
...
func (c *Consumer) joinGroup() (*vendor.balancer, error) {
	memberID, _ := c.membership()
	req := &vendor.JoinGroupRequest{
		GroupId:        c.groupID,
		MemberId:       memberID,
	...
	err := req.AddGroupProtocolMetadata(string(vendor.StrategyRange), meta)
	if err != nil {
		return nil, err
	}
	err = req.AddGroupProtocolMetadata(string(vendor.StrategyRoundRobin), meta)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		c.closeCoordinator(broker, err)
		return nil, err
	} else if resp.Err != vendor.ErrNoError {
		c.closeCoordinator(broker, resp.Err)
		return nil, resp.Err
	}

	var strategy *vendor.balancer
	if resp.LeaderId == resp.MemberId {
		...
		strategy, err = vendor.newBalancerFromMeta(c.client, vendor.Strategy(resp.GroupProtocol), members)
		if err != nil {
			return nil, err
		}
	}
...
func (c *Consumer) syncGroup(strategy *vendor.balancer) (map[string][]int32, error) {
	memberID, generationID := c.membership()
	req := &vendor.SyncGroupRequest{
		GroupId:      c.groupID,
		MemberId:     memberID,
		GenerationId: generationID,
	}
	...
	if strategy != nil {
		for memberID, topics := range strategy.Perform() {
			if err := req.AddGroupAssignmentMember(memberID, &vendor.ConsumerGroupMemberAssignment{
				Topics: topics,
			}); err != nil {
				return nil, err
			}
	...
	for topic := range members.Topics {
		sort.Sort(vendor.int32Slice(members.Topics[topic]))
	}
	return members.Topics, nil
}

// Fetches latest committed offsets for all subscriptions
func (c *Consumer) fetchOffsets(subs map[string][]int32) (map[string]map[int32]vendor.offsetInfo, error) {
	offsets := make(map[string]map[int32]vendor.offsetInfo, len(subs))
	req := &vendor.OffsetFetchRequest{
		Version:       1,
		ConsumerGroup: c.groupID,
	}

	for topic, partitions := range subs {
		offsets[topic] = make(map[int32]vendor.offsetInfo, len(partitions))
		for _, partition := range partitions {
			offsets[topic][partition] = vendor.offsetInfo{Offset: -1}
	...
	for topic, partitions := range subs {
		for _, partition := range partitions {
			...
			if block == nil {
				return nil, vendor.ErrIncompleteResponse
			}

			if block.Err == vendor.ErrNoError {
				offsets[topic][partition] = vendor.offsetInfo{Offset: block.Offset, Metadata: block.Metadata}
			} else {
...
func (c *Consumer) leaveGroup() error {
	...
	memberID, _ := c.membership()
	if _, err = broker.LeaveGroup(&vendor.LeaveGroupRequest{
		GroupId:  c.groupID,
		MemberId: memberID,
	}); err != nil {
		c.closeCoordinator(broker, err)
...
func (c *Consumer) createConsumer(tomb *vendor.loopTomb, topic string, partition int32, info vendor.offsetInfo) error {
	memberID, _ := c.membership()
	vendor.Logger.Printf("cluster/consumer %s consume %s/%d from %d\n", memberID, topic, partition, info.NextOffset(c.client.config.Consumer.Offsets.Initial))

	// Create partitionConsumer
	pc, err := vendor.newPartitionConsumer(c.consumer, topic, partition, info, c.client.config.Consumer.Offsets.Initial)
	if err != nil {
		return err
	}
	...
	tomb.Go(func(stopper <-chan vendor.none) {
		if c.client.config.Group.Mode == vendor.ConsumerModePartitions {
			pc.waitFor(stopper)
		} else {
...
func (c *Consumer) closeCoordinator(broker *vendor.Broker, err error) {
	if broker != nil {
		_ = broker.Close()
	}

	switch err {
	case vendor.ErrConsumerCoordinatorNotAvailable, vendor.ErrNotCoordinatorForConsumer:
...
func (c *Consumer) refreshMetadata() (err error) {
	...
	switch err {
	case vendor.ErrTopicAuthorizationFailed:
