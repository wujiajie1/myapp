...
func TestCompare(t *testing.T) {
	for _, v := range fixtures {
		gt, err := coreos.NewVersion(v.GreaterVersion)
		if err != nil {
			t.Error(err)
		}

		lt, err := coreos.NewVersion(v.LesserVersion)
		if err != nil {
			t.Error(err)
		}
...
func testString(t *testing.T, orig string, version *coreos.Version) {
...
func TestSort(t *testing.T) {
	...
	semvers := []*coreos.Version{}
	for _, v := range unsortedVersions {
		sv, err := coreos.NewVersion(v)
		if err != nil {
			t.Fatal(err)
		}
...
func TestBumpMajor(t *testing.T) {
	version, _ := coreos.NewVersion("1.0.0")
	version.BumpMajor()
	if version.Major != 2 {
		t.Fatalf("bumping major on 1.0.0 resulted in %v", version)
	}

	version, _ = coreos.NewVersion("1.5.2")
	version.BumpMajor()
	if version.Minor != 0 && version.Patch != 0 {
		t.Fatalf("bumping major on 1.5.2 resulted in %v", version)
	}

	version, _ = coreos.NewVersion("1.0.0+build.1-alpha.1")
	version.BumpMajor()
...
func TestMust(t *testing.T) {
	tests := []struct {
		...
		{
			versionStr: "1.0.0",
			version:    &coreos.Version{Major: 1},
		},
		{
	...
	for _, tt := range tests {
		func() {
			...
			version := coreos.Must(coreos.NewVersion(tt.versionStr))
...
type fixtureJSON struct {
	GreaterVersion *coreos.Version
	LesserVersion  *coreos.Version
}
...
func TestJSON(t *testing.T) {
	...
	for i, v := range fixtures {
		var err error
		fj[i].GreaterVersion, err = coreos.NewVersion(v.GreaterVersion)
		if err != nil {
			t.Fatal(err)
		}
		fj[i].LesserVersion, err = coreos.NewVersion(v.LesserVersion)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestYAML(t *testing.T) {
	...
	for i, v := range fixtures {
		var err error
		expected[i].GreaterVersion, err = coreos.NewVersion(v.GreaterVersion)
		if err != nil {
			t.Fatal(err)
		}
		expected[i].LesserVersion, err = coreos.NewVersion(v.LesserVersion)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func TestBadInput(t *testing.T) {
	...
	for _, b := range bad {
		if _, err := coreos.NewVersion(b); err == nil {
