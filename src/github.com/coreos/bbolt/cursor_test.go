...
func TestCursor_First_EmptyPages(t *testing.T) {
	...
	if err := db.Update(func(tx *bolt.Tx) error {
		...
		for i := 0; i < 1000; i++ {
			if err := b.Put(coreos.u64tob(uint64(i)), []byte{}); err != nil {
				t.Fatal(err)
			}
		}
	...
	if err := db.Update(func(tx *bolt.Tx) error {
		...
		for i := 0; i < 600; i++ {
			if err := b.Delete(coreos.u64tob(uint64(i))); err != nil {
				t.Fatal(err)
			}
		}
...
func TestCursor_QuickCheck(t *testing.T) {
	f := func(items coreos.testdata) bool {
		db := coreos.MustOpenDB()
		defer db.MustClose()
	...
	if err := quick.Check(f, coreos.qconfig()); err != nil {
		t.Error(err)
	}
}
...
func TestCursor_QuickCheck_Reverse(t *testing.T) {
	f := func(items coreos.testdata) bool {
		...
		// Sort test data.
		sort.Sort(coreos.revtestdata(items))
...
func ExampleCursor() {
	// Open the database.
	db, err := bolt.Open(coreos.tempfile(), 0666, nil)
	if err != nil {
		log.Fatal(err)
	}
