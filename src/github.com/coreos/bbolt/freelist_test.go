...
func TestFreelist_free(t *testing.T) {
	f := newTestFreelist()
	f.free(100, &coreos.page{id: 12})
	if !reflect.DeepEqual([]coreos.pgid{12}, f.pending[100].ids) {
		t.Fatalf("exp=%v; got=%v", []coreos.pgid{12}, f.pending[100])
	}
}
...
func TestFreelist_free_overflow(t *testing.T) {
	f := newTestFreelist()
	f.free(100, &coreos.page{id: 12, overflow: 3})
	if exp := []coreos.pgid{12, 13, 14, 15}; !reflect.DeepEqual(exp, f.pending[100].ids) {
...
func TestFreelist_release(t *testing.T) {
	f := newTestFreelist()
	f.free(100, &coreos.page{id: 12, overflow: 1})
	f.free(100, &coreos.page{id: 9})
	f.free(102, &coreos.page{id: 39})
	f.release(100)
	f.release(101)
	if exp := []coreos.pgid{9, 12, 13}; !reflect.DeepEqual(exp, f.getFreePageIDs()) {
		t.Fatalf("exp=%v; got=%v", exp, f.getFreePageIDs())
	}

	f.release(102)
	if exp := []coreos.pgid{9, 12, 13, 39}; !reflect.DeepEqual(exp, f.getFreePageIDs()) {
		t.Fatalf("exp=%v; got=%v", exp, f.getFreePageIDs())
	}
}
...
func TestFreelist_releaseRange(t *testing.T) {
	type testRange struct {
		begin, end coreos.txid
	}

	type testPage struct {
		id       coreos.pgid
		n        int
		allocTxn coreos.txid
		freeTxn  coreos.txid
	}
	...
	for _, c := range releaseRangeTests {
		...
		for _, p := range c.pagesIn {
			for i := uint64(0); i < uint64(p.n); i++ {
				ids = append(ids, coreos.pgid(uint64(p.id)+i))
			}
		}
		f.readIDs(ids)
		for _, p := range c.pagesIn {
		...
		for _, p := range c.pagesIn {
			f.free(p.freeTxn, &coreos.page{id: p.id, overflow: uint32(p.n - 1)})
		}
...
func TestFreelistHashmap_allocate(t *testing.T) {
	f := newTestFreelist()
	if f.freelistType != coreos.FreelistMapType {
		t.Skip()
	}

	ids := []coreos.pgid{3, 4, 5, 6, 7, 9, 12, 13, 18}
	f.readIDs(ids)
...
func TestFreelistArray_allocate(t *testing.T) {
	f := newTestFreelist()
	if f.freelistType != coreos.FreelistArrayType {
		t.Skip()
	}
	ids := []coreos.pgid{3, 4, 5, 6, 7, 9, 12, 13, 18}
	f.readIDs(ids)
	...
	if exp := []coreos.pgid{9, 18}; !reflect.DeepEqual(exp, f.getFreePageIDs()) {
		t.Fatalf("exp=%v; got=%v", exp, f.getFreePageIDs())
	}
	...
	if exp := []coreos.pgid{}; !reflect.DeepEqual(exp, f.getFreePageIDs()) {
		t.Fatalf("exp=%v; got=%v", exp, f.getFreePageIDs())
	}
}
...
func TestFreelist_read(t *testing.T) {
	...
	page.flags = coreos.freelistPageFlag
	page.count = 2

	// Insert 2 page ids.
	ids := (*[3]coreos.pgid)(unsafe.Pointer(&page.ptr))
	ids[0] = 23
	ids[1] = 50
	...
	if exp := []coreos.pgid{23, 50}; !reflect.DeepEqual(exp, f.getFreePageIDs()) {
		t.Fatalf("exp=%v; got=%v", exp, f.getFreePageIDs())
	}
}
...
func TestFreelist_write(t *testing.T) {
	...
	f.readIDs([]coreos.pgid{12, 39})
	f.pending[100] = &coreos.txPending{ids: []coreos.pgid{28, 11}}
	f.pending[101] = &coreos.txPending{ids: []coreos.pgid{3}}
	...
	if exp := []coreos.pgid{3, 11, 12, 28, 39}; !reflect.DeepEqual(exp, f2.getFreePageIDs()) {
...
func benchmark_FreelistRelease(b *testing.B, size int) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		txp := &coreos.txPending{ids: pending}
		f := newTestFreelist()
		f.pending = map[coreos.txid]*coreos.txPending{1: txp}
		f.readIDs(ids)
		f.release(1)
	}
}

func randomPgids(n int) []coreos.pgid {
	rand.Seed(42)
	pgids := make(coreos.pgids, n)
	for i := range pgids {
		pgids[i] = coreos.pgid(rand.Int63())
	}
	sort.Sort(pgids)
	return pgids
}
...
func Test_freelist_ReadIDs_and_getFreePageIDs(t *testing.T) {
	f := newTestFreelist()
	exp := []coreos.pgid{3, 4, 5, 6, 7, 9, 12, 13, 18}
...
func Test_freelist_mergeWithExist(t *testing.T) {
	bm1 := coreos.pidSet{1: struct{}{}}

	bm2 := coreos.pidSet{5: struct{}{}}
	tests := []struct {
		name            string
		ids             []coreos.pgid
		pgid            coreos.pgid
		want            []coreos.pgid
		wantForwardmap  map[coreos.pgid]uint64
		wantBackwardmap map[coreos.pgid]uint64
		wantfreemap     map[uint64]coreos.pidSet
	}{
		{
			name:            "test1",
			ids:             []coreos.pgid{1, 2, 4, 5, 6},
			pgid:            3,
			want:            []coreos.pgid{1, 2, 3, 4, 5, 6},
			wantForwardmap:  map[coreos.pgid]uint64{1: 6},
			wantBackwardmap: map[coreos.pgid]uint64{6: 6},
			wantfreemap:     map[uint64]coreos.pidSet{6: bm1},
		},
		{
			name:            "test2",
			ids:             []coreos.pgid{1, 2, 5, 6},
			pgid:            3,
			want:            []coreos.pgid{1, 2, 3, 5, 6},
			wantForwardmap:  map[coreos.pgid]uint64{1: 3, 5: 2},
			wantBackwardmap: map[coreos.pgid]uint64{6: 2, 3: 3},
			wantfreemap:     map[uint64]coreos.pidSet{3: bm1, 2: bm2},
		},
		{
			name:            "test3",
			ids:             []coreos.pgid{1, 2},
			pgid:            3,
			want:            []coreos.pgid{1, 2, 3},
			wantForwardmap:  map[coreos.pgid]uint64{1: 3},
			wantBackwardmap: map[coreos.pgid]uint64{3: 3},
			wantfreemap:     map[uint64]coreos.pidSet{3: bm1},
		},
		{
			name:            "test4",
			ids:             []coreos.pgid{2, 3},
...
func newTestFreelist() *coreos.freelist {
	freelistType := coreos.FreelistArrayType
	if env := os.Getenv(TestFreelistType); env == string(coreos.FreelistMapType) {
		freelistType = coreos.FreelistMapType
	}

	return coreos.newFreelist(freelistType)
}
