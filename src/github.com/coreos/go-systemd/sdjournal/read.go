...
type JournalReaderConfig struct {
	...
	Formatter func(entry *coreos.JournalEntry) (string, error)
}
...
func NewJournalReader(config JournalReaderConfig) (*JournalReader, error) {
	...
	var err error
	if config.Path != "" {
		r.journal, err = coreos.NewJournalFromDir(config.Path)
	} else {
		r.journal, err = coreos.NewJournal()
	}
	if err != nil {
		return nil, err
	}
...
process:
	for {
		...
		for {
			...
			case e := <-waitCh:
				switch e {
				case coreos.SD_JOURNAL_NOP:
					// the journal did not change since the last invocation
				case coreos.SD_JOURNAL_APPEND, coreos.SD_JOURNAL_INVALIDATE:
					continue process
				default:
					if e < 0 {
...
func simpleMessageFormatter(entry *coreos.JournalEntry) (string, error) {
