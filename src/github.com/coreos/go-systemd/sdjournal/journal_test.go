...
func TestJournalFollow(t *testing.T) {
	r, err := coreos.NewJournalReader(coreos.JournalReaderConfig{
		Since: time.Duration(-15) * time.Second,
		Matches: []coreos.Match{
			{
				Field: coreos.SD_JOURNAL_FIELD_SYSTEMD_UNIT,
	...
	if err = r.Follow(time.After(timeout), os.Stdout); err != coreos.ErrExpired {
...
func TestJournalWait(t *testing.T) {
	...
	var t1, t2 time.Time
	for ret := -1; ret != coreos.SD_JOURNAL_NOP; {
	...
	for ret := -1; ret != coreos.SD_JOURNAL_APPEND; {
		t1 = time.Now()
		ret = j.Wait(time.Millisecond * 300)
		t2 = time.Now()
	}
...
func TestNewJournalFromDir(t *testing.T) {
	...
	j, err := coreos.NewJournalFromDir(dir)
	if err == nil {
		defer j.Close()
...
func setupJournalRoundtrip() (*coreos.Journal, map[string]string, error) {
	j, err := coreos.NewJournal()
	if err != nil {
	...
	m := coreos.Match{Field: matchField, Value: matchValue}
...
func TestJournalGetCatalog(t *testing.T) {
	...
	matchField := coreos.SD_JOURNAL_FIELD_SYSTEMD_UNIT
	m := coreos.Match{Field: matchField, Value: "systemd-journald.service"}
...
func waitAndNext(j *coreos.Journal) error {
