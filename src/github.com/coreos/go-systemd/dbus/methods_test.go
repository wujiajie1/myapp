...
type TrUnitProp struct {
	name  string
	props []coreos.Property
}

func setupConn(t *testing.T) *coreos.Conn {
	conn, err := coreos.New()
	if err != nil {
		t.Fatal(err)
	}
...
func setupUnit(target string, conn *coreos.Conn, t *testing.T) {
...
func linkUnit(target string, conn *coreos.Conn, t *testing.T) {
...
func getUnitStatus(units []coreos.UnitStatus, name string) *coreos.UnitStatus {
	for _, u := range units {
		if u.Name == name {
			return &u
		}
...
func getUnitStatusSingle(conn *coreos.Conn, name string) *coreos.UnitStatus {
...
func getUnitFile(units []coreos.UnitFile, name string) *coreos.UnitFile {
	for _, u := range units {
...
func runStartTrUnit(t *testing.T, conn *coreos.Conn, trTarget TrUnitProp) error {
...
func runStopUnit(t *testing.T, conn *coreos.Conn, trTarget TrUnitProp) error {
	reschan := make(chan string)
...
func TestSetUnitProperties(t *testing.T) {
	...
	if err := conn.SetUnitProperties(unit, true, coreos.Property{"CPUShares", dbus.MakeVariant(uint64(1023))}); err != nil {
		t.Fatal(err)
	}
	...
	testCases := []struct {
	...
	}{
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/sleep", "400"}, false),
				},
			},
		...
		},
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/true"}, false),
					coreos.PropType("oneshot"),
					coreos.PropRemainAfterExit(true),
				},
			},
			trDep:     TrUnitProp{"", nil},
		...
		},
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/sleep", "400"}, false),
					coreos.PropRequires("testing-transient-requiresdep.service"),
				},
			},
			trDep: TrUnitProp{
		...
		},
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/sleep", "400"}, false),
					coreos.PropRequires("testing-transient-requiresdep-ov.service"),
				},
			},
			trDep: TrUnitProp{
		...
		},
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/sleep", "400"}, false),
					coreos.PropRequisite("testing-transient-requisitedep.service"),
				},
			},
			trDep: TrUnitProp{
		...
		},
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/sleep", "400"}, false),
					coreos.PropRequisiteOverridable("testing-transient-requisitedep-ov.service"),
				},
			},
			trDep: TrUnitProp{
		...
		},
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/sleep", "400"}, false),
					coreos.PropWants("testing-transient-wantsdep.service"),
				},
			},
			trDep: TrUnitProp{
		...
		},
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/sleep", "400"}, false),
					coreos.PropBindsTo("testing-transient-bindstodep.service"),
				},
			},
			trDep: TrUnitProp{
		...
		},
		{
			trTarget: TrUnitProp{
				...
				props: []coreos.Property{
					coreos.PropExecStart([]string{"/bin/sleep", "400"}, false),
					coreos.PropConflicts("testing-transient-conflictsdep.service"),
				},
			},
			trDep: TrUnitProp{
...
func TestStartStopTransientScope(t *testing.T) {
	...
	props := []coreos.Property{
		coreos.PropPids(uint32(cmd.Process.Pid)),
	}
...
func TestUnitName(t *testing.T) {
	for _, unit := range []string{
		...
		got := coreos.unitName(coreos.unitPath(unit))
		if got != unit {
