...
func (c *coreos.Conn) Subscribe() error {
...
func (c *coreos.Conn) Unsubscribe() error {
	return c.sigobj.Call("org.freedesktop.systemd1.Manager.Unsubscribe", 0).Store()
}

func (c *coreos.Conn) dispatch() {
	ch := make(chan *dbus.Signal, coreos.signalBuffer)
...
func (c *coreos.Conn) SubscribeUnits(interval time.Duration) (<-chan map[string]*coreos.UnitStatus, <-chan error) {
	return c.SubscribeUnitsCustom(interval, 0, func(u1, u2 *coreos.UnitStatus) bool { return *u1 != *u2 }, nil)
}

// SubscribeUnitsCustom is like SubscribeUnits but lets you specify the buffer
// size of the channels, the comparison function for detecting changes and a filter
// function for cutting down on the noise that your channel receives.
func (c *coreos.Conn) SubscribeUnitsCustom(interval time.Duration, buffer int, isChanged func(*coreos.UnitStatus, *coreos.UnitStatus) bool, filterUnit func(string) bool) (<-chan map[string]*coreos.UnitStatus, <-chan error) {
	old := make(map[string]*coreos.UnitStatus)
	statusChan := make(chan map[string]*coreos.UnitStatus, buffer)
	...
	go func() {
		for {
			...
			if err == nil {
				cur := make(map[string]*coreos.UnitStatus)
				for i := range units {
				...
				changed := make(map[string]*coreos.UnitStatus)
				for n, u := range cur {
...
// passes silently.
func (c *coreos.Conn) SetSubStateSubscriber(updateCh chan<- *SubStateUpdate, errCh chan<- error) {
	if c == nil {
		msg := "nil receiver"
		select {
...
func (c *coreos.Conn) sendSubStateUpdate(unitPath dbus.ObjectPath) {
	c.subStateSubscriber.Lock()
	defer c.subStateSubscriber.Unlock()
...
func (c *coreos.Conn) shouldIgnore(path dbus.ObjectPath) bool {
	t, ok := c.subStateSubscriber.ignore[path]
	return ok && t >= time.Now().UnixNano()
}

func (c *coreos.Conn) updateIgnore(path dbus.ObjectPath, info map[string]interface{}) {
...
func (c *coreos.Conn) cleanIgnore() {
...
func (c *coreos.Conn) SetPropertiesSubscriber(updateCh chan<- *PropertiesUpdate, errCh chan<- error) {
...
func (c *coreos.Conn) sendPropertiesUpdate(unitPath dbus.ObjectPath, changedProps map[string]dbus.Variant) {
	c.propertiesSubscriber.Lock()
	defer c.propertiesSubscriber.Unlock()
	...
	update := &PropertiesUpdate{coreos.unitName(unitPath), changedProps}
