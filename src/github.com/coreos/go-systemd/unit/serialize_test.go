	...
	tests := []struct {
		...
		// options with same section share the header
		{
			[]*coreos.UnitOption{
				...
				&coreos.UnitOption{"Unit", "BindsTo", "bar.service"},
			},
			`[Unit]
Description=Foo
BindsTo=bar.service
...
`,
		...
		// options are ordered within groups, and sections are ordered in the order in which they were first seen
		{
			[]*coreos.UnitOption{
				...
				&coreos.UnitOption{"Service", "ExecStart", "/usr/bin/sleep infinity"},
				&coreos.UnitOption{"Unit", "BindsTo", "bar.service"},
				&coreos.UnitOption{"X-Foo", "Bar", "baz"},
				&coreos.UnitOption{"Service", "ExecStop", "/usr/bin/sleep 1"},
				&coreos.UnitOption{"Unit", "Documentation", "https://foo.com"},
			},
			`[Unit]
Description=Foo
BindsTo=bar.service
...
`,
		...
		{
			[]*coreos.UnitOption{
				&coreos.UnitOption{"Unit", "Description", "Fo\no"},
			},
			`[Unit]
Description=Fo
o
`,
	...
	for i, tt := range tests {
		outReader := coreos.Serialize(tt.input)
