...
func Deserialize(f io.Reader) (opts []*coreos.UnitOption, err error) {
...
func newLexer(f io.Reader) (*lexer, <-chan *coreos.UnitOption, <-chan error) {
	optchan := make(chan *coreos.UnitOption)
...
type lexer struct {
	buf     *bufio.Reader
	optchan chan *coreos.UnitOption
	errchan chan error
	section string
}
...
func (l *lexer) lexOptionValueFunc(section, name string, partial bytes.Buffer) lexStep {
	return func() (lexStep, error) {
		...
		l.optchan <- &coreos.UnitOption{Section: section, Name: name, Value: val}
