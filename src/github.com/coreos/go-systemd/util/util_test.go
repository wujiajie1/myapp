...
func TestRunningFromSystemService(t *testing.T) {
	...
	s, err := coreos.RunningFromSystemService()
	if err != nil {
		t.Error(err.Error())
	} else if s {
...
func TestCurrentUnitName(t *testing.T) {
	...
	fromService, err := coreos.RunningFromSystemService()
	if err != nil || !fromService {
		t.Skip("Not running from a systemd service")
	}

	s, err := coreos.CurrentUnitName()
	if err != nil {
		t.Error(err.Error())
	}
	if s == "" {
