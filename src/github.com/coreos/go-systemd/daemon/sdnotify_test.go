...
func TestSdNotify(t *testing.T) {
	...
	for i, tt := range tests {
		coreos.must(os.Unsetenv("NOTIFY_SOCKET"))
		if tt.envSocket != "" {
			coreos.must(os.Setenv("NOTIFY_SOCKET", tt.envSocket))
		}
		sent, err := coreos.SdNotify(tt.unsetEnv, fmt.Sprintf("TestSdNotify test message #%d", i))
