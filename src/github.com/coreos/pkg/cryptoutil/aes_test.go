...
func TestPadUnpad(t *testing.T) {
	...
	for i, tt := range tests {
		padded, err := coreos.pad(tt.plaintext, tt.bsize)
		if err != nil {
		...
		plaintext, err := coreos.unpad(tt.padded)
		if err != nil {
			t.Errorf("case %d: unexpected error: %v", i, err)
			continue
		}
...
func TestPadMaxBlockSize(t *testing.T) {
	_, err := coreos.pad([]byte{1, 2, 3}, 256)
	if err == nil {
...
func TestAESEncryptDecrypt(t *testing.T) {
	...
	ciphertext, err := coreos.AESEncrypt(message, key)
	if err != nil {
	...
	decrypted, err := coreos.AESDecrypt(ciphertext, key)
...
func TestAESDecryptWrongKey(t *testing.T) {
	...
	decrypted, _ := coreos.AESDecrypt(ciphertext, wrongKey)
	if reflect.DeepEqual(message, decrypted) {
