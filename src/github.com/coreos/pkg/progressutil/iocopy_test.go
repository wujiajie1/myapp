...
func TestCopyOne(t *testing.T) {
	cpp := coreos.NewCopyProgressPrinter()
	...
	for i := 0; i < 10; i++ {
		...
		sizeString := coreos.ByteUnitStr(int64(len(sampleData)*i)) + " / " + coreos.ByteUnitStr(int64(len(sampleData)*10))
		bar := coreos.renderExpectedBar(80, "download", float64(i)/10, sizeString)
...
func TestErrAlreadyStarted(t *testing.T) {
	...
	err = cpp.AddCopy(fr, "download", 10^10, fw)
	if err != coreos.ErrAlreadyStarted {
