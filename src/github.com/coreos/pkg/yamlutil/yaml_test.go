...
func TestSetFlagsFromYaml(t *testing.T) {
	...
	err := coreos.SetFlagsFromYaml(fs, []byte(config))
	if err != nil {
...
func TestSetFlagsFromYamlBad(t *testing.T) {
	...
	if err := coreos.SetFlagsFromYaml(fs, []byte(badConf)); err == nil {
...
func TestSetFlagsFromYamlMultiError(t *testing.T) {
	...
	fs.Int("z", 0, "")
	conf := "X: foo\nY: bar\nZ: 3"
	err := coreos.SetFlagsFromYaml(fs, []byte(conf))
	if err == nil {
		t.Errorf("got err= nil, want err != nil")
	}
	es, ok := err.(coreos.ErrorSlice)
	if !ok {
