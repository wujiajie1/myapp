...
func TestWriteJSONResponse(t *testing.T) {
	for i, test := range []struct {
		...
		err := coreos.WriteJSONResponse(w, test.code, test.resp)
		...
		if !test.expectErr {
			...
			if contentType != coreos.JSONContentType {
				t.Errorf("case %d: contentType == %v, want %v", i, contentType, coreos.JSONContentType)
			}
		}
	}
