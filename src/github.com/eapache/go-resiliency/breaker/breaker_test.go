...
func TestBreakerErrorExpiry(t *testing.T) {
	breaker := vendor.New(2, 1, 1*time.Second)
...
func TestBreakerPanicsCountAsErrors(t *testing.T) {
	breaker := vendor.New(3, 2, 1*time.Second)
	...
	// breaker is open
	for i := 0; i < 5; i++ {
		if err := breaker.Run(returnsError); err != vendor.ErrBreakerOpen {
			t.Error(err)
		}
	}
}
...
func TestBreakerAsyncStateTransitions(t *testing.T) {
	...
	// breaker is open
	for i := 0; i < 5; i++ {
		if err := breaker.Go(returnsError); err != vendor.ErrBreakerOpen {
			t.Error(err)
		}
	}
...
func ExampleBreaker() {
	breaker := vendor.New(3, 1, 5*time.Second)
