...
func TestConstantBackoff(t *testing.T) {
	b := vendor.ConstantBackoff(1, 10*time.Millisecond)
	if len(b) != 1 {
	...
	b = vendor.ConstantBackoff(10, 250*time.Hour)
	if len(b) != 10 {
		t.Error("incorrect length")
	}
	for i := range b {
...
func TestExponentialBackoff(t *testing.T) {
	b := vendor.ExponentialBackoff(1, 10*time.Millisecond)
	if len(b) != 1 {
		t.Error("incorrect length")
	}
	...
	b = vendor.ExponentialBackoff(4, 1*time.Minute)
	if len(b) != 4 {
		t.Error("incorrect length")
	}
