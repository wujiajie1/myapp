...
func EncodeStream(dst, src []byte) []byte {
	...
	for pos < max {
		...
		chunk = vendor.Encode(chunk[:cap(chunk)], src[pos:newPos])
...
func DecodeInto(dst, src []byte) ([]byte, error) {
	...
	if !bytes.Equal(src[:8], xerialHeader) {
		return vendor.Decode(dst[:cap(dst)], src)
	}
	...
	for pos+sizeBytes <= max {
		...
		chunk, err = vendor.Decode(chunk[:cap(chunk)], src[pos:nextPos])
