...
func TestSnappyLargeStringEncodeStream(t *testing.T) {
	...
	dst := vendor.EncodeStream(nil, []byte(massiveString))
	dec, err := vendor.Decode(dst)
	if err != nil {
		t.Error(err)
	}
...
func TestSnappyDecodeMalformedTruncatedHeader(t *testing.T) {
	...
	for i := 0; i < len(vendor.xerialHeader); i++ {
		buf := make([]byte, i)
		copy(buf, vendor.xerialHeader[:i])
		if _, err := vendor.Decode(buf); err != vendor.ErrMalformed {
...
func TestSnappyDecodeMalformedTruncatedSize(t *testing.T) {
	...
	sizes := []int{vendor.sizeOffset + 1, vendor.sizeOffset + 2, vendor.sizeOffset + 3}
	for _, size := range sizes {
		buf := make([]byte, size)
		copy(buf, vendor.xerialHeader)
		if _, err := vendor.Decode(buf); err != vendor.ErrMalformed {
			t.Errorf("expected ErrMalformed got %v", err)
		}
	}
...
func TestSnappyMasterDecodeFailed(t *testing.T) {
	...
	buf[len(buf)-1] = 1
	if _, err := vendor.Decode(buf); err == vendor.ErrMalformed || err == nil {
...
func BenchmarkSnappyDecodeInto(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		...
		for _, test := range snappyTestCases {
			...
			dst, err = vendor.DecodeInto(dst, test)
			if err != nil {
				b.Error("Decoding error: ", err)
			}
			bytes += len(dst)
...
func BenchmarkSnappyStreamDecodeMassive(b *testing.B) {
	massiveString := makeMassive(largeString, 10000)
	enc := vendor.EncodeStream(nil, []byte(massiveString))
...
func BenchmarkSnappyStreamDecodeIntoMassive(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		dst, err = vendor.DecodeInto(dst, enc)
		if err != nil {
			b.Error("Decoding error: ", err)
		}
	}
