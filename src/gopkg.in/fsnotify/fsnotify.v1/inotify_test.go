...
func isWatcherReallyClosed(t *testing.T, w *vendor.Watcher) {
	select {
...
func TestInotifyStress(t *testing.T) {
	...
	for !finished {
		...
		case evt := <-w.Events:
			...
			if evt.Op == vendor.Create {
				creates++
			}
			if evt.Op == vendor.Remove {
				removes++
			}
		}
	}
...
func TestInotifyOverflow(t *testing.T) {
	...
	for overflows == 0 && creates < numDirs*numFiles {
		...
		case err := <-errChan:
			t.Fatalf("Got an error from file creator goroutine: %v", err)
		case err := <-w.Errors:
			if err == vendor.ErrEventOverflow {
				overflows++
			} else {
				t.Fatalf("Got an error from watcher: %v", err)
			}
