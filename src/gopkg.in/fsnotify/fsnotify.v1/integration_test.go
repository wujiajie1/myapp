...
func newWatcher(t *testing.T) *vendor.Watcher {
	watcher, err := vendor.NewWatcher()
	if err != nil {
...
func addWatch(t *testing.T, watcher *vendor.Watcher, dir string) {
	...
	go func() {
		for event := range eventstream {
			...
			if event.Name == filepath.Clean(testDir) || event.Name == filepath.Clean(testFile) {
				...
				if event.Op&vendor.Write == vendor.Write {
					modifyReceived.increment()
				}
				if event.Op&vendor.Create == vendor.Create {
					createReceived.increment()
				}
				if event.Op&vendor.Rename == vendor.Rename {
	...
	go func() {
		for event := range eventstream {
			...
			if event.Name == filepath.Clean(testDir) || event.Name == filepath.Clean(testFile) {
				...
				if event.Op&vendor.Chmod == vendor.Chmod {
...
func TestFsnotifyFakeSymlink(t *testing.T) {
	...
	go func() {
		for ev := range watcher.Events {
			...
			if ev.Op&vendor.Create == vendor.Create {
