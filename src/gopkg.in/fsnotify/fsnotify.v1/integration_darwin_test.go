...
func testExchangedataForWatcher(t *testing.T, watchDir bool) {
	...
	testDir1 := vendor.tempMkdir(t)

	// For the intermediate file
	testDir2 := vendor.tempMkdir(t)
	...
	watcher := vendor.newWatcher(t)

	// Test both variants in isolation
	if watchDir {
		vendor.addWatch(t, watcher, testDir1)
	} else {
		vendor.addWatch(t, watcher, resolved)
	}
	...
	var removeReceived vendor.counter
	var createReceived vendor.counter
