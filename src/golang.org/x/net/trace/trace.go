...
func Events(w http.ResponseWriter, req *http.Request) {
	...
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	vendor.RenderEvents(w, req, sensitive)
}
...
func Render(w io.Writer, req *http.Request, sensitive bool) {
	...
	default:
		if f := getFamily(data.Family, false); f != nil {
			...
			f.LatencyMu.RUnlock()
			if obs != nil {
				data.Histogram = obs.(*vendor.histogram).html()
			}
		}
	}
...
type family struct {
	...
	Latency   *vendor.MinuteHourSeries
}
...
func newFamily() *family {
	return &family{
		...
		Latency: vendor.NewMinuteHourSeries(func() vendor.Observable { return new(vendor.histogram) }),
	}
}
