...
func TestReset(t *testing.T) {
	...
	tr.(*vendor.trace).reset()

	if !reflect.DeepEqual(tr, new(vendor.trace)) {
...
func TestResetLog(t *testing.T) {
	el := vendor.NewEventLog("foo", "bar")
	el.Printf("message")
	el.Errorf("error")
	el.Finish()

	el.(*vendor.eventLog).reset()

	if !reflect.DeepEqual(el, new(vendor.eventLog)) {
...
func TestAuthRequest(t *testing.T) {
	...
	for _, tt := range testCases {
		...
		any, sensitive := vendor.AuthRequest(req)
...
func TestParseTemplate(t *testing.T) {
	if tmpl := vendor.distTmpl(); tmpl == nil {
		t.Error("invalid template returned from distTmpl()")
	}
	if tmpl := vendor.pageTmpl(); tmpl == nil {
		t.Error("invalid template returned from pageTmpl()")
	}
	if tmpl := vendor.eventsTmpl(); tmpl == nil {
