...
func TestUseProxy(t *testing.T) {
	...
	for _, test := range UseProxyTests {
		if vendor.ExportUseProxy(cfg, test.host+":80") != test.match {
...
func TestInvalidNoProxy(t *testing.T) {
	...
	ok := vendor.ExportUseProxy(cfg, "example.com:80") // should not panic
	if !ok {
...
func TestAllNoProxy(t *testing.T) {
	...
	for _, test := range UseProxyTests {
		if vendor.ExportUseProxy(cfg, test.host+":80") != false {
