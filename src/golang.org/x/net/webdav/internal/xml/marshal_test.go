...
type NamePrecedence struct {
	XMLName     vendor.Name       `xml:"Parent"`
...
type XMLNameWithTag struct {
	XMLName vendor.Name `xml:"InXMLNameTag"`
	Value   string      `xml:",chardata"`
}

type XMLNameWithNSTag struct {
	XMLName vendor.Name `xml:"ns InXMLNameWithNSTag"`
	Value   string      `xml:",chardata"`
}
...
var _ vendor.Marshaler = (*MyMarshalerTest)(nil)

func (m *MyMarshalerTest) MarshalXML(e *vendor.Encoder, start vendor.StartElement) error {
	e.EncodeToken(start)
	e.EncodeToken(vendor.CharData([]byte("hello world")))
	e.EncodeToken(vendor.EndElement{start.Name})
	return nil
}

type MyMarshalerAttrTest struct{}

var _ vendor.MarshalerAttr = (*MyMarshalerAttrTest)(nil)

func (m *MyMarshalerAttrTest) MarshalXMLAttr(name vendor.Name) (vendor.Attr, error) {
	return vendor.Attr{name, "hello world"}, nil
}

type MyMarshalerValueAttrTest struct{}

var _ vendor.MarshalerAttr = MyMarshalerValueAttrTest{}

func (m MyMarshalerValueAttrTest) MarshalXMLAttr(name vendor.Name) (vendor.Attr, error) {
	return vendor.Attr{name, "hello world"}, nil
}
...
type InnerStruct struct {
	XMLName vendor.Name `xml:"testns outer"`
}
...
type OuterNamedStruct struct {
	InnerStruct
	XMLName vendor.Name `xml:"outerns test"`
	IntAttr int         `xml:"int,attr"`
}
...
var marshalTests = []struct {
	...
	{Value: &Departure{When: vendor.ParseTime("2013-01-09T00:15:00-09:00")}, ExpectXML: `<departure>2013-01-09T00:15:00-09:00</departure>`},
	{Value: vendor.atomValue, ExpectXML: vendor.atomXml},
	{
		Value: &Ship{
			Name:  "Heart of Gold",
			Pilot: "Computer",
	...
	// Test the order in which the XML element name is chosen
	{
		Value: &NamePrecedence{
			...
			FromNameVal: XMLNameWithoutTag{XMLName: vendor.Name{Local: "InXMLName"}, Value: "B"},
	...
	{
		Value: &NamePrecedence{
			XMLName:     vendor.Name{Local: "Parent"},
			FromTag:     XMLNameWithoutTag{XMLName: vendor.Name{Local: "InTag"}, Value: "A"},
			FromNameVal: XMLNameWithoutTag{XMLName: vendor.Name{Local: "FromNameVal"}, Value: "B"},
			FromNameTag: XMLNameWithTag{XMLName: vendor.Name{Local: "InXMLNameTag"}, Value: "C"},
			InFieldName: "D",
		},
		ExpectXML: `<Parent>` +
			`<InTag>A</InTag>` +
	...
	{
		Value:     &NameInField{vendor.Name{Space: "ns", Local: "foo"}},
	...
	// Test ",any"
	{
		...
		Value: &AnyTest{
			Nested: "known",
			AnyField: AnyHolder{
				XMLName: vendor.Name{Local: "other"},
				XML:     "<sub>unknown</sub>",
			},
		},
	},
	{
		Value: &AnyTest{Nested: "known",
			AnyField: AnyHolder{
				XML:     "<unknown/>",
				XMLName: vendor.Name{Local: "AnyField"},
			},
		},
	...
	},
	{
		...
		Value: &AnySliceTest{
			...
			AnyField: []AnyHolder{
				{
					XMLName: vendor.Name{Local: "c"},
					XML:     "<d>e</d>",
				},
				{
					XMLName: vendor.Name{Space: "f", Local: "g"},
					XML:     "<h>i</h>",
				},
			},
		},
	...
	// Test escaping.
	{
		...
		Value: &AnyTest{
			...
			AnyField: AnyHolder{XMLName: vendor.Name{Local: "empty"}},
		},
	},
	{
		...
		Value: &AnyTest{
			Nested:   "newline: \n; cr: \r; tab: \t;",
			AnyField: AnyHolder{XMLName: vendor.Name{Local: "AnyField"}},
		},
	},
	{
	...
	},
	{
		...
		Value:     &OuterNamedStruct{XMLName: vendor.Name{Space: "outerns", Local: "test"}, IntAttr: 10},
	},
	{
		ExpectXML: `<test xmlns="outerns" int="10"></test>`,
		Value:     &OuterNamedOrderedStruct{XMLName: vendor.Name{Space: "outerns", Local: "test"}, IntAttr: 10},
	},
	{
		ExpectXML: `<outer xmlns="testns" int="10"></outer>`,
...
func TestMarshal(t *testing.T) {
	for idx, test := range marshalTests {
		...
		data, err := vendor.Marshal(test.Value)
		if err != nil {
...
func TestMarshalErrors(t *testing.T) {
	for idx, test := range marshalErrorTests {
		...
		if test.Kind != reflect.Invalid {
			if kind := err.(*vendor.UnsupportedTypeError).Type.Kind(); kind != test.Kind {
...
func TestUnmarshal(t *testing.T) {
	for i, test := range marshalTests {
		...
		err := vendor.Unmarshal([]byte(test.ExpectXML), dest)
...
func TestMarshalIndent(t *testing.T) {
	for i, test := range marshalIndentTests {
		data, err := vendor.MarshalIndent(test.Value, test.Prefix, test.Indent)
		if err != nil {
...
func TestMarshalWriteIOErrors(t *testing.T) {
	enc := vendor.NewEncoder(vendor.errWriter{})
...
func TestMarshalFlush(t *testing.T) {
	...
	if err := enc.EncodeToken(vendor.CharData("hello world")); err != nil {
...
var encodeElementTests = []struct {
	desc      string
	value     interface{}
	start     vendor.StartElement
	expectXML string
}{{
	desc:  "simple string",
	value: "hello",
	start: vendor.StartElement{
		Name: vendor.Name{Local: "a"},
	},
	expectXML: `<a>hello</a>`,
}, {
	...
	start: vendor.StartElement{
		Name: vendor.Name{Local: "a"},
		Attr: []vendor.Attr{{
			Name:  vendor.Name{Local: "x"},
			Value: "y",
		}, {
			Name:  vendor.Name{Local: "foo"},
			Value: "bar",
		}},
	},
	...
	start: vendor.StartElement{
		Name: vendor.Name{Space: "ns", Local: "a"},
		Attr: []vendor.Attr{{
			Name: vendor.Name{Local: "xmlns"},
	...
	start: vendor.StartElement{
		Name: vendor.Name{Space: "ns2", Local: "a"},
		Attr: []vendor.Attr{{
			Name: vendor.Name{Local: "xmlns"},
			// "ns" is the name space defined in XMLNameWithNSTag
			Value: "ns",
...
func BenchmarkMarshal(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		vendor.Marshal(vendor.atomValue)
	}
}

func BenchmarkUnmarshal(b *testing.B) {
	b.ReportAllocs()
	xml := []byte(vendor.atomXml)
	for i := 0; i < b.N; i++ {
		vendor.Unmarshal(xml, &vendor.Feed{})
	}
}
...
var encodeTokenTests = []struct {
	...
	toks: []vendor.Token{
		vendor.ProcInst{"Target", []byte("Instruction")},
	},
	want: `<?Target Instruction?>`,
}, {
	desc: "proc instruction with empty target",
	toks: []vendor.Token{
		vendor.ProcInst{"", []byte("Instruction")},
	},
	...
	desc: "directive",
	toks: []vendor.Token{
		vendor.Directive("foo"),
	},
	want: `<!foo>`,
}, {
	desc: "more complex directive",
	toks: []vendor.Token{
		vendor.Directive("DOCTYPE doc [ <!ELEMENT doc '>'> <!-- com>ment --> ]"),
	},
	...
	toks: []vendor.Token{
		vendor.EndElement{vendor.Name{"foo", "bar"}},
	},
	err: "xml: end tag </bar> without start tag",
}, {
	desc: "mismatching end tag local name",
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"", "foo"}, nil},
		vendor.EndElement{vendor.Name{"", "bar"}},
	},
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"space", "foo"}, nil},
		vendor.EndElement{vendor.Name{"another", "foo"}},
	},
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"space", "local"}, []vendor.Attr{
			{vendor.Name{"xmlns", "x"}, "space"},
			{vendor.Name{"space", "foo"}, "value"},
		}},
	},
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"", "local"}, []vendor.Attr{
			{vendor.Name{"xmlns", "x"}, "space"},
		}},
		vendor.StartElement{vendor.Name{"space", "foo"}, []vendor.Attr{
			{vendor.Name{"space", "x"}, "y"},
		}},
	},
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"", "foo"}, []vendor.Attr{
			{vendor.Name{"xmlns", "x"}, "space1"},
		}},
		vendor.StartElement{vendor.Name{"", "foo"}, []vendor.Attr{
			{vendor.Name{"xmlns", "x"}, "space2"},
		}},
		vendor.StartElement{vendor.Name{"", "foo"}, []vendor.Attr{
			{vendor.Name{"space1", "a"}, "space1 value"},
			{vendor.Name{"space2", "b"}, "space2 value"},
		}},
		vendor.EndElement{vendor.Name{"", "foo"}},
		vendor.EndElement{vendor.Name{"", "foo"}},
		vendor.StartElement{vendor.Name{"", "foo"}, []vendor.Attr{
			{vendor.Name{"space1", "a"}, "space1 value"},
			{vendor.Name{"space2", "b"}, "space2 value"},
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"space", "foo"}, []vendor.Attr{
			{vendor.Name{"xmlns", "a"}, "space"},
			{vendor.Name{"xmlns", "b"}, "space"},
			{vendor.Name{"space", "x"}, "value"},
		}},
	},
	...
	toks: []vendor.Token{
		...
		vendor.StartElement{vendor.Name{"space", "foo"}, []vendor.Attr{
			{vendor.Name{"xmlns", "y"}, "space"},
			{vendor.Name{"space", "a"}, "value"},
		}},
	},
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"space", "foo"}, []vendor.Attr{
			{vendor.Name{vendor.xmlURL, "xmlns"}, "space"},
		}},
	},
	want: `<foo xmlns="space">`,
}, {
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"/34", "foo"}, []vendor.Attr{
			{vendor.Name{"/34", "x"}, "value"},
		}},
	},
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"some/space", "foo"}, []vendor.Attr{
			{vendor.Name{"", "attr"}, "value"},
			{vendor.Name{"some/space", "other"}, "other value"},
		}},
	},
	...
	toks: []vendor.Token{
		vendor.StartElement{vendor.Name{"space", "foo"}, []vendor.Attr{
			{vendor.Name{"", "xmlns"}, "space"},
			{vendor.Name{"xmlns", "bar"}, "space"},
			{vendor.Name{"space", "baz"}, "foo"},
		}},
		vendor.StartElement{vendor.Name{"space", "baz"}, nil},
		vendor.EndElement{vendor.Name{"space", "baz"}},
		vendor.EndElement{vendor.Name{"space", "foo"}},
	},
	...
	toks: []vendor.Token{
		...
		vendor.StartElement{vendor.Name{"space", "bar"}, []vendor.Attr{
			{vendor.Name{"space", "attr"}, "value"},
		}},
	},
...
func TestProcInstEncodeToken(t *testing.T) {
	...
	if err := enc.EncodeToken(vendor.ProcInst{"xml", []byte("Instruction")}); err != nil {
		t.Fatalf("enc.EncodeToken: expected to be able to encode xml target ProcInst as first token, %s", err)
	}

	if err := enc.EncodeToken(vendor.ProcInst{"Target", []byte("Instruction")}); err != nil {
...
`)
	dec := vendor.NewDecoder(&in)
	enc := vendor.NewEncoder(&out)
...
func TestIsValidDirective(t *testing.T) {
	...
	for _, s := range testOK {
		if !vendor.isValidDirective(vendor.Directive(s)) {
...
func TestSimpleUseOfEncodeToken(t *testing.T) {
	var buf bytes.Buffer
	enc := vendor.NewEncoder(&buf)
	if err := enc.EncodeToken(&vendor.StartElement{Name: vendor.Name{"", "object1"}}); err == nil {
		t.Errorf("enc.EncodeToken: pointer type should be rejected")
	}
	if err := enc.EncodeToken(&vendor.EndElement{Name: vendor.Name{"", "object1"}}); err == nil {
		t.Errorf("enc.EncodeToken: pointer type should be rejected")
	}
	if err := enc.EncodeToken(vendor.StartElement{Name: vendor.Name{"", "object2"}}); err != nil {
		t.Errorf("enc.EncodeToken: StartElement %s", err)
	}
	if err := enc.EncodeToken(vendor.EndElement{Name: vendor.Name{"", "object2"}}); err != nil {
