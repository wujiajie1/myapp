...
type Marshaler interface {
	MarshalXML(e *Encoder, start vendor.StartElement) error
}
...
type MarshalerAttr interface {
	MarshalXMLAttr(name vendor.Name) (vendor.Attr, error)
}
...
func (enc *Encoder) EncodeElement(v interface{}, start vendor.StartElement) error {
...
func (enc *Encoder) EncodeToken(t vendor.Token) error {

	p := &enc.p
	switch t := t.(type) {
	case vendor.StartElement:
		if err := p.writeStart(&t); err != nil {
			return err
		}
	case vendor.EndElement:
		if err := p.writeEnd(t.Name); err != nil {
			return err
		}
	case vendor.CharData:
		vendor.escapeText(p, t, false)
	case vendor.Comment:
...
func isValidDirective(dir vendor.Directive) bool {
	var (
		depth     int
		inquote   uint8
		incomment bool
...
func (p *printer) defineNS(attr vendor.Attr, ignoreNonEmptyDefault bool) error {
	var prefix string
	if attr.Name.Local == "xmlns" {
		if attr.Name.Space != "" && attr.Name.Space != "xml" && attr.Name.Space != vendor.xmlURL {
...
func (p *printer) createNSPrefix(url string, isAttr bool) {
	...
	if prefix == "" || !vendor.isName([]byte(prefix)) || strings.Contains(prefix, ":") {
		prefix = "_"
	}
...
func (p *printer) writeNamespaces() {
	for i := len(p.prefixes) - 1; i >= 0; i-- {
		...
		vendor.EscapeText(p, []byte(p.nsForPrefix(prefix.prefix)))
		p.WriteString(`"`)
	}
}
...
func (p *printer) marshalValue(val reflect.Value, finfo *vendor.fieldInfo, startTemplate *vendor.StartElement) error {
	...
	if finfo != nil && finfo.flags&vendor.fOmitEmpty != 0 && isEmptyValue(val) {
		return nil
	}
	...
	tinfo, err := vendor.getTypeInfo(typ)
	if err != nil {
		return err
	}
	...
	if startTemplate != nil {
		...
		} else if v, ok := xmlname.value(val).Interface().(vendor.Name); ok && v.Local != "" {
			start.Name = v
		}
		explicitNS = true
	}
...
func (p *printer) fieldAttr(finfo *vendor.fieldInfo, val reflect.Value) (vendor.Attr, error) {
	fv := finfo.value(val)
	name := vendor.Name{Space: finfo.xmlns, Local: finfo.name}
	if finfo.flags&vendor.fOmitEmpty != 0 && isEmptyValue(fv) {
		return vendor.Attr{}, nil
	}
	...
	if fv.CanInterface() && fv.Type().Implements(textMarshalerType) {
		...
		if err != nil {
			return vendor.Attr{}, err
		}
		return vendor.Attr{name, string(text)}, nil
	}
	if fv.CanAddr() {
		pv := fv.Addr()
		if pv.CanInterface() && pv.Type().Implements(textMarshalerType) {
	...
	return vendor.Attr{name, s}, nil
}

// defaultStart returns the default start element to use,
// given the reflect type, field info, and start template.
func (p *printer) defaultStart(typ reflect.Type, finfo *vendor.fieldInfo, startTemplate *vendor.StartElement) vendor.StartElement {
	var start vendor.StartElement
...
func (p *printer) marshalInterface(val Marshaler, start vendor.StartElement) error {
	// Push a marker onto the tag stack so that MarshalXML
	// cannot close the XML tags that it did not open.
	p.tags = append(p.tags, vendor.Name{})
	n := len(p.tags)
	...
	if len(p.tags) > n {
		return fmt.Errorf("xml: %s.MarshalXML wrote invalid XML: <%s> not closed", vendor.receiverType(val), p.tags[len(p.tags)-1].Local)
	}
	p.tags = p.tags[:n-1]
	return nil
}
...
func (p *printer) marshalTextInterface(val encoding.TextMarshaler, start vendor.StartElement) error {
	if err := p.writeStart(&start); err != nil {
		return err
	}
	...
	vendor.EscapeText(p, text)
	return p.writeEnd(start.Name)
}

// writeStart writes the given start element.
func (p *printer) writeStart(start *vendor.StartElement) error {
	if start.Name.Local == "" {
...
func (p *printer) writeName(name vendor.Name, isAttr bool) {
...
func (p *printer) writeEnd(name vendor.Name) error {
	if name.Local == "" {
...
func (p *printer) marshalStruct(tinfo *vendor.typeInfo, val reflect.Value) error {
	s := parentStack{p: p}
	for i := range tinfo.fields {
		finfo := &tinfo.fields[i]
		if finfo.flags&vendor.fAttr != 0 {
		...
		switch finfo.flags & vendor.fMode {
		case vendor.fCharData:
			...
			switch vf.Kind() {
			case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
				vendor.Escape(p, strconv.AppendInt(scratch[:0], vf.Int(), 10))
			case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Uintptr:
				vendor.Escape(p, strconv.AppendUint(scratch[:0], vf.Uint(), 10))
			case reflect.Float32, reflect.Float64:
				vendor.Escape(p, strconv.AppendFloat(scratch[:0], vf.Float(), 'g', -1, vf.Type().Bits()))
			case reflect.Bool:
				vendor.Escape(p, strconv.AppendBool(scratch[:0], vf.Bool()))
			case reflect.String:
				if err := vendor.EscapeText(p, []byte(vf.String())); err != nil {
					return err
				}
			case reflect.Slice:
				if elem, ok := vf.Interface().([]byte); ok {
					if err := vendor.EscapeText(p, elem); err != nil {
						return err
					}
				}
			}
		...
		case vendor.fElement, vendor.fElement | vendor.fAny:
...
func (s *parentStack) setParents(finfo *vendor.fieldInfo, vf reflect.Value) error {
	xmlns := s.p.defaultNS
	if finfo.xmlns != "" {
		xmlns = finfo.xmlns
	}
	...
	for i := len(s.parents) - 1; i >= commonParents; i-- {
		if err := s.p.writeEnd(vendor.Name{
			Space: s.xmlns,
			Local: s.parents[i],
		}); err != nil {
			return err
