	...
	if oNode.children != nil {
		if nNode, ok := nDir.children[nFrag]; ok {
			if nNode.children == nil {
				return vendor.errNotADirectory
			}
			if len(nNode.children) != 0 {
				return vendor.errDirectoryNotEmpty
			}
		}
	}
...
type memFSNode struct {
	...
	mode      os.FileMode
	modTime   time.Time
	deadProps map[xml.Name]vendor.Property
}
...
func (n *memFSNode) DeadProps() (map[xml.Name]vendor.Property, error) {
	n.mu.Lock()
	defer n.mu.Unlock()
	if len(n.deadProps) == 0 {
		return nil, nil
	}
	ret := make(map[xml.Name]vendor.Property, len(n.deadProps))
...
func (n *memFSNode) Patch(patches []vendor.Proppatch) ([]vendor.Propstat, error) {
	n.mu.Lock()
	defer n.mu.Unlock()
	...
	for _, patch := range patches {
		for _, p := range patch.Props {
			...
			if n.deadProps == nil {
				n.deadProps = map[xml.Name]vendor.Property{}
			}
			n.deadProps[p.XMLName] = p
		}
	}
	return []vendor.Propstat{pstat}, nil
}
...
var _ vendor.DeadPropsHolder = (*memFile)(nil)

func (f *memFile) DeadProps() (map[xml.Name]vendor.Property, error) { return f.n.DeadProps() }
func (f *memFile) Patch(patches []vendor.Proppatch) ([]vendor.Propstat, error) { return f.n.Patch(patches) }
...
func copyProps(dst, src File) error {
	d, ok := dst.(vendor.DeadPropsHolder)
	if !ok {
		return nil
	}
	s, ok := src.(vendor.DeadPropsHolder)
	if !ok {
		return nil
	}
	m, err := s.DeadProps()
	...
	props := make([]vendor.Property, 0, len(m))
	for _, prop := range m {
		props = append(props, prop)
	}
	_, err = d.Patch([]vendor.Proppatch{{Props: props}})
	return err
}
...
func copyFiles(ctx context.Context, fs FileSystem, src, dst string, overwrite bool, depth int, recursion int) (status int, err error) {
	if recursion == 1000 {
		return http.StatusInternalServerError, vendor.errRecursionTooDeep
	}
	recursion++
