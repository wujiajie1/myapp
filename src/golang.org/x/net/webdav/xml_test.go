...
func TestReadLockInfo(t *testing.T) {
	...
	testCases := []struct {
		...
		"" +
			...
			"</D:lockinfo>",
		vendor.lockInfo{
			XMLName:   vendor.Name{Space: "DAV:", Local: "lockinfo"},
	...
	for _, tc := range testCases {
		li, status, err := vendor.readLockInfo(strings.NewReader(tc.input))
		if tc.wantStatus != 0 {
			if err == nil {
...
func TestReadPropfind(t *testing.T) {
	testCases := []struct {
		...
		input: "" +
			...
			"</A:propfind>",
		wantPF: vendor.propfind{
			XMLName:  vendor.Name{Space: "DAV:", Local: "propfind"},
			Propname: new(struct{}),
		},
	}, {
		...
		wantPF: vendor.propfind{
			XMLName: vendor.Name{Space: "DAV:", Local: "propfind"},
			Allprop: new(struct{}),
			Include: vendor.propfindProps{xml.Name{Space: "DAV:", Local: "displayname"}},
		},
	}, {
		...
		wantPF: vendor.propfind{
			XMLName: vendor.Name{Space: "DAV:", Local: "propfind"},
			Prop:    vendor.propfindProps{xml.Name{Space: "DAV:", Local: "displayname"}},
		},
	}, {
	...
	for _, tc := range testCases {
		pf, status, err := vendor.readPropfind(strings.NewReader(tc.input))
		if tc.wantStatus != 0 {
			if err == nil {
				t.Errorf("%s: got nil error, want non-nil", tc.desc)
				continue
...
func TestMultistatusWriter(t *testing.T) {
	...
	testCases := []struct {
		...
		responses: []vendor.response{{
			...
			Propstat: []vendor.propstat{{
				Prop: []vendor.Property{{
					XMLName: xml.Name{
		...
		wantErr: vendor.errInvalidResponse,
		// default of http.responseWriter
		wantCode: http.StatusOK,
	}, {
...
loop:
	for _, tc := range testCases {
		...
		w := vendor.multistatusWriter{w: rec, responseDescription: tc.respdesc}
		if tc.writeHeader {
...
func TestReadProppatch(t *testing.T) {
	ppStr := func(pps []vendor.Proppatch) string {
		var outer []string
		for _, pp := range pps {
			var inner []string
	...
	for _, tc := range testCases {
		pp, status, err := vendor.readProppatch(strings.NewReader(tc.input))
		if tc.wantStatus != 0 {
			if err == nil {
				t.Errorf("%s: got nil error, want non-nil", tc.desc)
				continue
...
func TestUnmarshalXMLValue(t *testing.T) {
	...
	var n xmlNormalizer
	for _, tc := range testCases {
		d := vendor.NewDecoder(strings.NewReader(tc.input))
		var v vendor.xmlValue
...
func (n *xmlNormalizer) normalize(w io.Writer, r io.Reader) error {
	d := vendor.NewDecoder(r)
	e := vendor.NewEncoder(w)
	for {
		t, err := d.Token()
		if err != nil {
			if t == nil && err == io.EOF {
		...
		switch val := t.(type) {
		case vendor.Directive, vendor.ProcInst:
			continue
		case vendor.Comment:
			if n.omitComments {
				continue
		...
		case vendor.StartElement:
			start, _ := vendor.CopyToken(val).(vendor.StartElement)
			attr := start.Attr[:0]
