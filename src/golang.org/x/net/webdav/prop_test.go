...
func TestMemPS(t *testing.T) {
	...
	calcProps := func(name string, fs vendor.FileSystem, ls vendor.LockSystem, pstats []vendor.Propstat) error {
		...
		for _, pst := range pstats {
			for i, p := range pst.Props {
				...
				case xml.Name{Space: "DAV:", Local: "getetag"}:
					...
					etag, err := vendor.findETag(ctx, fs, ls, name, fi)
					if err != nil {
						return err
					}
	...
	type propOp struct {
		...
		name          string
		pnames        []xml.Name
		patches       []vendor.Proppatch
		wantPnames    []xml.Name
		wantPropstats []vendor.Propstat
	}
	...
	for _, tc := range testCases {
		fs, err := vendor.buildTestFS(tc.buildfs)
		if err != nil {
		...
		for _, op := range tc.propOp {
			...
			var propstats []vendor.Propstat
			switch op.op {
			case "propname":
				pnames, err := vendor.propnames(ctx, fs, ls, op.name)
				if err != nil {
				...
				continue
			case "allprop":
				propstats, err = vendor.allprop(ctx, fs, ls, op.name, op.pnames)
			case "propfind":
				propstats, err = vendor.props(ctx, fs, ls, op.name, op.pnames)
			case "proppatch":
				propstats, err = vendor.patch(ctx, fs, ls, op.name, op.patches)
			default:
...
type byPropname []vendor.Property
...
func (fs noDeadPropsFS) OpenFile(ctx context.Context, name string, flag int, perm os.FileMode) (vendor.File, error) {
...
func TestFindContentTypeOverride(t *testing.T) {
	fs, err := vendor.buildTestFS([]string{"touch /file"})
	if err != nil {
	...
	originalContentType, err := vendor.findContentType(ctx, fs, nil, "/file", fi)
	if err != nil {
	...
	ContentType, err := vendor.findContentType(ctx, fs, nil, "/file", o)
	if err != nil {
		t.Fatalf("findContentType /file failed: %v", err)
	}
	...
	o = &overrideContentType{fi, "OverriddenContentType", vendor.ErrNotImplemented}
	ContentType, err = vendor.findContentType(ctx, fs, nil, "/file", o)
	if err != nil {
		t.Fatalf("findContentType /file failed: %v", err)
	}
...
func TestFindETagOverride(t *testing.T) {
	...
	// Check non overridden case
	originalETag, err := vendor.findETag(ctx, fs, nil, "/file", fi)
	if err != nil {
	...
	ETag, err := vendor.findETag(ctx, fs, nil, "/file", o)
	if err != nil {
		t.Fatalf("findETag /file failed: %v", err)
	}
	if ETag != o.eTag {
	...
	o = &overrideETag{fi, `"OverriddenETag"`, vendor.ErrNotImplemented}
	ETag, err = vendor.findETag(ctx, fs, nil, "/file", o)
	if err != nil {
		t.Fatalf("findETag /file failed: %v", err)
	}
