...
func TestDirResolve(t *testing.T) {
	...
	for _, tc := range testCases {
		d := vendor.Dir(filepath.FromSlash(tc.dir))
...
func TestWalk(t *testing.T) {
	...
	for _, tc := range testCases {
		fs := vendor.NewMemFS().(*vendor.memFS)
		...
		i, prevFrag := 0, ""
		err := fs.walk("test", tc.dir, func(dir *vendor.memFSNode, frag string, final bool) error {
			got := walkStep{
				name:  prevFrag,
				frag:  frag,
				final: final,
...
func find(ctx context.Context, ss []string, fs vendor.FileSystem, name string) ([]string, error) {
...
func testFS(t *testing.T, fs vendor.FileSystem) {
	...
	for i, tc := range testCases {
		...
		case "copy__", "mk-dir", "move__", "rm-all", "stat":
			...
			case "copy__":
				depth := 0
				if parts[1] == "d=∞" {
					depth = vendor.infiniteDepth
				}
				_, opErr = vendor.copyFiles(ctx, fs, parts[2], parts[3], parts[0] == "o=T", depth, 0)
			case "mk-dir":
				opErr = fs.Mkdir(ctx, parts[0], 0777)
			case "move__":
				_, opErr = vendor.moveFiles(ctx, fs, parts[1], parts[2], parts[0] == "o=T")
			case "rm-all":
...
func TestDir(t *testing.T) {
	...
	testFS(t, vendor.Dir(td))
}

func TestMemFS(t *testing.T) {
	testFS(t, vendor.NewMemFS())
}
...
func TestCopyMoveProps(t *testing.T) {
	...
	patch := func(name string, patches ...vendor.Proppatch) error {
		f, err := fs.OpenFile(ctx, name, os.O_RDWR, 0666)
		if err != nil {
			return err
		}
		_, pErr := f.(vendor.DeadPropsHolder).Patch(patches)
		cErr := f.Close()
		if pErr != nil {
			return pErr
		}
	...
	props := func(name string) (map[xml.Name]vendor.Property, error) {
		f, err := fs.OpenFile(ctx, name, os.O_RDWR, 0666)
		if err != nil {
			return nil, err
		}
		m, pErr := f.(vendor.DeadPropsHolder).DeadProps()
		cErr := f.Close()
		if pErr != nil {
			return nil, pErr
		}
	...
	if err := patch("/src", vendor.Proppatch{Props: []vendor.Property{p0, p1}}); err != nil {
		t.Fatalf("patch /src +p0 +p1: %v", err)
	}
	if _, err := vendor.copyFiles(ctx, fs, "/src", "/tmp", true, vendor.infiniteDepth, 0); err != nil {
		t.Fatalf("copyFiles /src /tmp: %v", err)
	}
	if _, err := vendor.moveFiles(ctx, fs, "/tmp", "/dst", true); err != nil {
		t.Fatalf("moveFiles /tmp /dst: %v", err)
	}
	if err := patch("/src", vendor.Proppatch{Props: []vendor.Property{p0}, Remove: true}); err != nil {
		t.Fatalf("patch /src -p0: %v", err)
	}
	if err := patch("/src", vendor.Proppatch{Props: []vendor.Property{p2}}); err != nil {
		t.Fatalf("patch /src +p2: %v", err)
	}
	if err := patch("/dst", vendor.Proppatch{Props: []vendor.Property{p1}, Remove: true}); err != nil {
		t.Fatalf("patch /dst -p1: %v", err)
	}
	if err := patch("/dst", vendor.Proppatch{Props: []vendor.Property{p3}}); err != nil {
	...
	wantSrc := map[xml.Name]vendor.Property{
		p1.XMLName: p1,
		p2.XMLName: p2,
	}
	...
	wantDst := map[xml.Name]vendor.Property{
		p0.XMLName: p0,
		p3.XMLName: p3,
	}
...
func TestWalkFS(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		err = vendor.walkFS(ctx, fs, tc.depth, tc.startAt, fi, traceFn)
		if err != nil {
...
func buildTestFS(buildfs []string) (vendor.FileSystem, error) {
