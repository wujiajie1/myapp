...
func TestWalkToRoot(t *testing.T) {
	...
	for _, tc := range testCases {
		var got []string
		if !vendor.walkToRoot(tc.name, func(name0 string, first bool) bool {
			if first != (len(got) == 0) {
...
var lockTestDurations = []time.Duration{
	vendor.infiniteTimeout, // infiniteTimeout means to never expire.
...
func TestMemLSCanCreate(t *testing.T) {
	now := time.Unix(0, 0)
	m := vendor.NewMemLS().(*vendor.memLS)

	for _, name := range lockTestNames {
		_, err := m.Create(now, vendor.LockDetails{
			Root:      name,
...
func TestMemLSLookup(t *testing.T) {
	...
	for _, name := range lockTestNames {
		token, err := m.Create(now, vendor.LockDetails{
			Root:      name,
			Duration:  vendor.infiniteTimeout,
			ZeroDepth: lockTestZeroDepth(name),
		})
	...
	for _, baseName := range baseNames {
		for _, suffix := range []string{"", "/0", "/1/2/3"} {
			...
			for _, token := range []string{badToken, goodToken} {
				...
				got := m.lookup(name, vendor.Condition{Token: token})
				want := base
				if token == badToken {
					want = nil
				}
...
func TestMemLSConfirm(t *testing.T) {
	now := time.Unix(0, 0)
	m := vendor.NewMemLS().(*vendor.memLS)
	alice, err := m.Create(now, vendor.LockDetails{
		Root:      "/alice",
		Duration:  vendor.infiniteTimeout,
		ZeroDepth: false,
	})
	tweedle, err := m.Create(now, vendor.LockDetails{
		Root:      "/tweedle",
		Duration:  vendor.infiniteTimeout,
		ZeroDepth: false,
	})
	...
	_, err = m.Confirm(now, "/tweedle/dee", "", vendor.Condition{Token: alice})
	...
	release, err := m.Confirm(now, "/tweedle/dee", "/tweedle/dum", vendor.Condition{Token: tweedle})
	if err != nil {
	...
	releaseDee, err := m.Confirm(now, "/tweedle/dee", "", vendor.Condition{Token: tweedle})
	if err != nil {
	...
	_, err = m.Confirm(now, "/tweedle/dum", "", vendor.Condition{Token: tweedle})
	if err != vendor.ErrConfirmationFailed {
	...
	releaseDum, err := m.Confirm(now, "/tweedle/dum", "", vendor.Condition{Token: tweedle})
	if err != nil {
	...
	for i := 0; i < N; i++ {
		...
		if token != "" {
			...
			case 0:
				confirmed = true
				nConfirm++
				release, err := m.Confirm(now, name, "", vendor.Condition{Token: token})
				if err != nil {
...
func (m *vendor.memLS) consistent() error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func TestParseTimeout(t *testing.T) {
	...
	for _, tc := range testCases {
		got, gotErr := vendor.parseTimeout(tc.s)
