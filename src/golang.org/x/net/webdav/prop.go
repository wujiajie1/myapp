...
type DeadPropsHolder interface {
	...
	DeadProps() (map[xml.Name]vendor.Property, error)
...
var liveProps = map[xml.Name]struct {
	...
	findFn func(context.Context, vendor.FileSystem, vendor.LockSystem, string, os.FileInfo) (string, error)
...
func props(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, pnames []xml.Name) ([]Propstat, error) {
	...
	var deadProps map[xml.Name]vendor.Property
	...
	for _, pn := range pnames {
		...
		if prop := liveProps[pn]; prop.findFn != nil && (prop.dir || !isDir) {
			...
			pstatOK.Props = append(pstatOK.Props, vendor.Property{
				XMLName:  pn,
				InnerXML: []byte(innerXML),
			})
		} else {
			pstatNotFound.Props = append(pstatNotFound.Props, vendor.Property{
				XMLName: pn,
			})
		}
	}
...
func propnames(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string) ([]xml.Name, error) {
	f, err := fs.OpenFile(ctx, name, os.O_RDONLY, 0)
	if err != nil {
		return nil, err
	}
...
func allprop(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, include []xml.Name) ([]Propstat, error) {
...
func patch(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, patches []Proppatch) ([]Propstat, error) {
	conflict := false
loop:
	...
	if conflict {
		...
		pstatFailedDep := Propstat{
			Status: vendor.StatusFailedDependency,
		}
		for _, patch := range patches {
			for _, p := range patch.Props {
				if _, ok := liveProps[p.XMLName]; ok {
					pstatForbidden.Props = append(pstatForbidden.Props, vendor.Property{XMLName: p.XMLName})
				} else {
					pstatFailedDep.Props = append(pstatFailedDep.Props, vendor.Property{XMLName: p.XMLName})
				}
			}
		}
	...
	if dph, ok := f.(DeadPropsHolder); ok {
		...
		for _, pstat := range ret {
			for i, p := range pstat.Props {
				pstat.Props[i] = vendor.Property{XMLName: p.XMLName}
			}
		}
		return ret, nil
	}
	...
	for _, patch := range patches {
		for _, p := range patch.Props {
			pstat.Props = append(pstat.Props, vendor.Property{XMLName: p.XMLName})
		}
	}
...
func findResourceType(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, fi os.FileInfo) (string, error) {
	if fi.IsDir() {
...
func findDisplayName(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, fi os.FileInfo) (string, error) {
	if vendor.slashClean(name) == "/" {
...
func findContentLength(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, fi os.FileInfo) (string, error) {
	return strconv.FormatInt(fi.Size(), 10), nil
}

func findLastModified(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, fi os.FileInfo) (string, error) {
...
func findContentType(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, fi os.FileInfo) (string, error) {
...
func findETag(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, fi os.FileInfo) (string, error) {
...
func findSupportedLock(ctx context.Context, fs vendor.FileSystem, ls vendor.LockSystem, name string, fi os.FileInfo) (string, error) {
	return `` +
