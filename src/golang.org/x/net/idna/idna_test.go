...
func TestIDNASeparators(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "Punycode", profile: vendor.Punycode,
			subCases: []subCase{
		...
		{
			name: "Lookup", profile: vendor.Lookup,
			subCases: []subCase{
		...
		{
			name: "Display", profile: vendor.Display,
			subCases: []subCase{
				{"example\u3002jp", "example.jp", false},
				{"東京\uFF0Ejp", "xn--1lqs71d.jp", false},
				{"大阪\uFF61jp", "xn--pssu33l.jp", false},
		...
		{
			name: "Registration", profile: vendor.Registration,
			subCases: []subCase{
				{"example\u3002jp", "", true},
				{"東京\uFF0Ejp", "", true},
				{"大阪\uFF61jp", "", true},
