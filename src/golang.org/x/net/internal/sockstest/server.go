...
func ParseAuthRequest(b []byte) (*AuthRequest, error) {
	...
	if b[1] > 0 {
		req.Methods = make([]vendor.AuthMethod, b[1])
		for i, m := range b[2 : 2+b[1]] {
			req.Methods[i] = vendor.AuthMethod(m)
		}
	}
	return req, nil
}
...
func MarshalAuthReply(ver int, m vendor.AuthMethod) ([]byte, error) {
...
func ParseCmdRequest(b []byte) (*CmdRequest, error) {
	...
	if vendor.Command(b[1]) != vendor.CmdConnect {
		return nil, errors.New("unexpected command")
	}
	if b[2] != 0 {
		return nil, errors.New("non-zero reserved field")
	}
	req := &CmdRequest{Version: int(b[0]), Cmd: vendor.Command(b[1])}
	l := 2
	off := 4
	switch b[3] {
	case vendor.AddrTypeIPv4:
		l += net.IPv4len
		req.Addr.IP = make(net.IP, net.IPv4len)
	case vendor.AddrTypeIPv6:
		l += net.IPv6len
		req.Addr.IP = make(net.IP, net.IPv6len)
	case vendor.AddrTypeFQDN:
		l += int(b[4])
		off = 5
	default:
...
func MarshalCmdReply(ver int, reply vendor.Reply, a *vendor.Addr) ([]byte, error) {
	b := make([]byte, 4)
	b[0] = byte(ver)
	b[1] = byte(reply)
	if a.Name != "" {
...
func NewServer(authFunc, cmdFunc func(io.ReadWriter, []byte) error) (*Server, error) {
	var err error
	s := new(Server)
	s.ln, err = vendor.NewLocalListener("tcp")
	if err != nil {
		return nil, err
	}
	go s.serve(authFunc, cmdFunc)
...
func NoAuthRequired(rw io.ReadWriter, b []byte) error {
	...
	b, err = MarshalAuthReply(req.Version, vendor.AuthMethodNotRequired)
	if err != nil {
		return err
	}
	n, err := rw.Write(b)
...
func NoProxyRequired(rw io.ReadWriter, b []byte) error {
	...
	b, err = MarshalCmdReply(vendor.Version5, vendor.StatusSucceeded, &req.Addr)
	if err != nil {
		return err
	}
	n, err := rw.Write(b)
