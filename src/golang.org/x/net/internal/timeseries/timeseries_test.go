...
func isNear(x *vendor.Float, y float64, tolerance float64) bool {
	return math.Abs(x.Value()-y) < tolerance
}

func isApproximate(x *vendor.Float, y float64) bool {
	return isNear(x, y, 1e-2)
}

func checkApproximate(t *testing.T, o vendor.Observable, y float64) {
	x := o.(*vendor.Float)
...
func checkNear(t *testing.T, o vendor.Observable, y, tolerance float64) {
	x := o.(*vendor.Float)
...
func TestBasicTimeSeries(t *testing.T) {
	ts := vendor.NewTimeSeries(vendor.NewFloat)
	fo := new(vendor.Float)
	*fo = vendor.Float(10)
...
func TestExpectedErrorRate(t *testing.T) {
	...
	fake := new(mockClock)
	fake.Set(time.Now())
	ts.timeSeries.init(testResolutions, vendor.NewFloat, buckets, fake)
	for i := 1; i <= 61*61; i++ {
