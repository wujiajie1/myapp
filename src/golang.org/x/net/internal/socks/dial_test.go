...
func TestDial(t *testing.T) {
	...
	t.Run("ConnectWithConn", func(t *testing.T) {
		...
		if _, ok := a.(*vendor.Addr); !ok {
			t.Fatalf("got %+v; want socks.Addr", a)
		}
	})
	t.Run("Cancel", func(t *testing.T) {
		ss, err := vendor.NewServer(vendor.NoAuthRequired, blackholeCmdFunc)
		if err != nil {
			t.Fatal(err)
		}
		defer ss.Close()
	...
	t.Run("WithRogueServer", func(t *testing.T) {
		ss, err := vendor.NewServer(vendor.NoAuthRequired, rogueCmdFunc)
		if err != nil {
			t.Fatal(err)
		}
		defer ss.Close()
...
func blackholeCmdFunc(rw io.ReadWriter, b []byte) error {
	if _, err := vendor.ParseCmdRequest(b); err != nil {
		return err
	}
	var bb [1]byte
	for {
