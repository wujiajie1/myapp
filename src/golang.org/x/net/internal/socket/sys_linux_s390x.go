...
func getsockopt(s uintptr, level, name int, b []byte) (int, error) {
	...
	return int(l), vendor.errnoErr(errno)
}

func setsockopt(s uintptr, level, name int, b []byte) error {
	_, errno := socketcall(sysSETSOCKOPT, s, uintptr(level), uintptr(name), uintptr(unsafe.Pointer(&b[0])), uintptr(len(b)), 0)
	return vendor.errnoErr(errno)
}

func recvmsg(s uintptr, h *vendor.msghdr, flags int) (int, error) {
	n, errno := socketcall(sysRECVMSG, s, uintptr(unsafe.Pointer(h)), uintptr(flags), 0, 0, 0)
	return int(n), vendor.errnoErr(errno)
}

func sendmsg(s uintptr, h *vendor.msghdr, flags int) (int, error) {
