...
func TestSocket(t *testing.T) {
	t.Run("Option", func(t *testing.T) {
		testSocketOption(t, &vendor.Option{Level: syscall.SOL_SOCKET, Name: syscall.SO_RCVBUF, Len: 4})
	})
}

func testSocketOption(t *testing.T, so *vendor.Option) {
	c, err := vendor.NewLocalPacketListener("udp")
	if err != nil {
...
func TestControlMessage(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		var w []byte
		var tailPadLen int
		mm := vendor.NewControlMessage([]int{0})
		for i, c := range tt.cs {
			m := vendor.NewControlMessage([]int{len(c.Data)})
			l := len(m) - len(mm)
		...
		for _, w := range ws {
			ms, err := vendor.ControlMessage(w).Parse()
			if err != nil {
...
func BenchmarkUDP(b *testing.B) {
	...
	for M := 1; M <= 1<<9; M = M << 1 {
		...
		switch runtime.GOOS {
		case "android", "linux":
			wms := make([]vendor.Message, M)
			for i := range wms {
				wms[i].Buffers = [][]byte{data}
				wms[i].Addr = c.LocalAddr()
			}
			rms := make([]vendor.Message, M)
			for i := range rms {
