...
func marshalSockaddr(ip net.IP, port int, zone string) []byte {
	if ip4 := ip.To4(); ip4 != nil {
		b := make([]byte, vendor.sizeofSockaddrInet)
		switch runtime.GOOS {
		case "android", "linux", "solaris", "windows":
			vendor.NativeEndian.PutUint16(b[:2], uint16(vendor.sysAF_INET))
		default:
			b[0] = vendor.sizeofSockaddrInet
			b[1] = vendor.sysAF_INET
		}
	...
	if ip6 := ip.To16(); ip6 != nil && ip.To4() == nil {
		b := make([]byte, vendor.sizeofSockaddrInet6)
		switch runtime.GOOS {
		case "android", "linux", "solaris", "windows":
			vendor.NativeEndian.PutUint16(b[:2], uint16(vendor.sysAF_INET6))
		default:
			b[0] = vendor.sizeofSockaddrInet6
			b[1] = vendor.sysAF_INET6
		}
		binary.BigEndian.PutUint16(b[2:4], uint16(port))
		copy(b[8:24], ip6)
		if zone != "" {
			vendor.NativeEndian.PutUint32(b[24:28], uint32(zoneCache.index(zone)))
		}
		return b
	}
	return nil
...
func parseInetAddr(b []byte, network string) (net.Addr, error) {
	...
	switch runtime.GOOS {
	case "android", "linux", "solaris", "windows":
		af = int(vendor.NativeEndian.Uint16(b[:2]))
	default:
		af = int(b[1])
	}
	var ip net.IP
	var zone string
	if af == vendor.sysAF_INET {
		if len(b) < vendor.sizeofSockaddrInet {
			return nil, errors.New("short address")
		}
		ip = make(net.IP, net.IPv4len)
		copy(ip, b[4:8])
	}
	if af == vendor.sysAF_INET6 {
		if len(b) < vendor.sizeofSockaddrInet6 {
			return nil, errors.New("short address")
		}
		ip = make(net.IP, net.IPv6len)
		copy(ip, b[8:24])
		if id := int(vendor.NativeEndian.Uint32(b[24:28])); id > 0 {
