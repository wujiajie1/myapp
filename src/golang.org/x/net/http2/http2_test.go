...
func init() {
	vendor.inTests = true
	vendor.DebugGoroutines = true
	flag.BoolVar(&vendor.VerboseLogs, "verboseh2", vendor.VerboseLogs, "Verbose HTTP/2 debug logging")
}
...
func TestSettingString(t *testing.T) {
	tests := []struct {
		...
		{vendor.Setting{vendor.SettingMaxFrameSize, 123}, "[MAX_FRAME_SIZE = 123]"},
		{vendor.Setting{1<<16 - 1, 123}, "[UNKNOWN_SETTING_65535 = 123]"},
	}
...
type twriter struct {
	t  testing.TB
	st *vendor.serverTester // optional
}
...
func encodeHeaderNoImplicit(t *testing.T, headers ...string) []byte {
	...
	for len(headers) > 0 {
		...
		headers = headers[2:]
		if err := enc.WriteField(vendor.HeaderField{Name: k, Value: v}); err != nil {
...
func TestConfigureServerIdleTimeout_Go18(t *testing.T) {
	...
	// With a zero http2.Server, verify that it copies IdleTimeout:
	{
		...
		s2 := &vendor.Server{}
		if err := vendor.ConfigureServer(s1, s2); err != nil {
			t.Fatal(err)
		}
