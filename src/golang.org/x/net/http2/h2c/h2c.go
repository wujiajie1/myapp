...
func NewHandler(h http.Handler, s *vendor.Server) http.Handler {
	return &h2cHandler{
		Handler: h,
		s:       s,
	}
...
func (s h2cHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "PRI" && len(r.Header) == 0 && r.URL.Path == "*" && r.Proto == "HTTP/2.0" {
		...
		s.s.ServeConn(conn, &vendor.ServeConnOpts{Handler: s.Handler})
		return
	}
...
func initH2CWithPriorKnowledge(w http.ResponseWriter) (net.Conn, error) {
	...
	if string(buf[:n]) == expectedBody {
		c := &rwConn{
			Conn:      conn,
			Reader:    io.MultiReader(strings.NewReader(vendor.ClientPreface), rw),
			BufWriter: rw.Writer,
		}
		return c, nil
	}
...
func drainClientPreface(r io.Reader) error {
	...
	prefaceLen := int64(len(vendor.ClientPreface))
	n, err := io.CopyN(&buf, r, prefaceLen)
	if err != nil {
		return err
	}
	if n != prefaceLen || buf.String() != vendor.ClientPreface {
		return fmt.Errorf("Client never sent: %s", vendor.ClientPreface)
	}
	return nil
}
...
func convertH1ReqToH2(r *http.Request) (*bytes.Buffer, []vendor.Setting, error) {
	h2Bytes := bytes.NewBuffer([]byte((vendor.ClientPreface)))
	framer := vendor.NewFramer(h2Bytes, nil)
	...
	err = framer.WriteHeaders(vendor.HeadersFrameParam{
		StreamID:      1,
...
func getMaxFrameSize(settings []vendor.Setting) uint32 {
	for _, setting := range settings {
		if setting.ID == vendor.SettingMaxFrameSize {
			return setting.Val
		}
	}
	return 16384
...
func getMaxHeaderTableSize(settings []vendor.Setting) uint32 {
	for _, setting := range settings {
		if setting.ID == vendor.SettingHeaderTableSize {
			return setting.Val
		}
	}
	return 4096
...
func (w *settingsAckSwallowWriter) Write(p []byte) (int, error) {
	if !w.didSwallow {
		...
		for {
			...
			fh, err := vendor.ReadFrameHeader(bytes.NewBuffer(w.buf))
			if err != nil {
			...
			if fh.Type == vendor.FrameSettings && fh.Flags.Has(vendor.FlagSettingsAck) {
...
func isH2CUpgrade(h http.Header) bool {
	return vendor.HeaderValuesContainsToken(h[textproto.CanonicalMIMEHeaderKey("Upgrade")], "h2c") &&
		vendor.HeaderValuesContainsToken(h[textproto.CanonicalMIMEHeaderKey("Connection")], "HTTP2-Settings")
}

// getH2Settings returns the []http2.Setting that are encoded in the
// HTTP2-Settings header.
func getH2Settings(h http.Header) ([]vendor.Setting, error) {
...
func decodeSettings(headerVal string) ([]vendor.Setting, error) {
	...
	settings := make([]vendor.Setting, 0)
	for i := 0; i < len(b)/6; i++ {
		settings = append(settings, vendor.Setting{
			ID:  vendor.SettingID(binary.BigEndian.Uint16(b[i*6 : i*6+2])),
...
func getH2HeaderBytes(r *http.Request, maxHeaderTableSize uint32) ([]byte, error) {
	...
	hpackEnc := vendor.NewEncoder(headerBytes)
	hpackEnc.SetMaxDynamicTableSize(maxHeaderTableSize)

	// Section 8.1.2.3
	err := hpackEnc.WriteField(vendor.HeaderField{
		Name:  ":method",
		Value: r.Method,
	})
	if err != nil {
