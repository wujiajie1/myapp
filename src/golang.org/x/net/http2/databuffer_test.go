...
func testDataBuffer(t *testing.T, wantBytes []byte, setup func(t *testing.T) *vendor.dataBuffer) {
...
func TestDataBufferAllocation(t *testing.T) {
	...
	testDataBuffer(t, wantRead.Bytes(), func(t *testing.T) *vendor.dataBuffer {
		b := &vendor.dataBuffer{}
		for _, p := range writes {
...
func TestDataBufferAllocationWithExpected(t *testing.T) {
	...
	testDataBuffer(t, wantRead.Bytes(), func(t *testing.T) *vendor.dataBuffer {
		b := &vendor.dataBuffer{expected: 32 * 1024}
		for _, p := range writes {
			if n, err := b.Write(p); n != len(p) || err != nil {
				t.Fatalf("Write(%q x %d)=%v,%v want %v,nil", p[:1], len(p), n, err, len(p))
			}
...
func TestDataBufferWriteAfterPartialRead(t *testing.T) {
	testDataBuffer(t, []byte("cdxyz"), func(t *testing.T) *vendor.dataBuffer {
		b := &vendor.dataBuffer{}
