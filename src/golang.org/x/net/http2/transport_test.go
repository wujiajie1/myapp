...
func TestTransportExternal(t *testing.T) {
	...
	rt := &vendor.Transport{TLSClientConfig: tlsConfigInsecure}
...
func (c *fakeTLSConn) ConnectionState() tls.ConnectionState {
	return tls.ConnectionState{
		...
		CipherSuite: vendor.cipher_TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
	}
}

func startH2cServer(t *testing.T) net.Listener {
	h2Server := &vendor.Server{}
	...
	go func() {
		...
		h2Server.ServeConn(&fakeTLSConn{conn}, &vendor.ServeConnOpts{Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestTransport(t *testing.T) {
	const body = "sup"
	st := vendor.newServerTester(t, func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, body)
	}, vendor.optOnlyServer)
	defer st.Close()

	tr := &vendor.Transport{TLSClientConfig: tlsConfigInsecure}
...
func onSameConn(t *testing.T, modReq func(*http.Request)) bool {
	...
	}, vendor.optOnlyServer, func(c net.Conn, st http.ConnState) {
...
func TestTransportGroupsPendingDials(t *testing.T) {
	...
	if err := retry(50, 10*time.Millisecond, func() error {
		cp, ok := tr.connPool().(*vendor.clientConnPool)
		if !ok {
...
func TestActualContentLength(t *testing.T) {
	...
	for i, tt := range tests {
		got := vendor.actualContentLength(tt.req)
		if got != tt.want {
...
func TestConfigureTransport(t *testing.T) {
	t1 := &http.Transport{}
	err := vendor.ConfigureTransport(t1)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := vendor.ConfigureTransport(t1); err == nil {
...
type clientTester struct {
	...
	fr     *vendor.Framer // server's framer
	client func() error
	server func() error
}
...
func (ct *clientTester) greet(settings ...vendor.Setting) {
	...
	if sf, ok := f.(*vendor.SettingsFrame); !ok {
...
func (ct *clientTester) readNonSettingsFrame() (vendor.Frame, error) {
	for {
		f, err := ct.fr.ReadFrame()
		if err != nil {
			return nil, err
...
func (ct *clientTester) readFrame() (vendor.Frame, error) {
	return vendor.readFrameTimeout(ct.fr, 2*time.Second)
}

func (ct *clientTester) firstHeaders() (*vendor.HeadersFrame, error) {
	for {
		f, err := ct.readFrame()
		if err != nil {
		...
		switch f.(type) {
		case *vendor.WindowUpdateFrame, *vendor.SettingsFrame:
			continue
		}
		hf, ok := f.(*vendor.HeadersFrame)
		if !ok {
	...
	ct.server = func() error {
		...
		for {
			...
			case *vendor.DataFrame:
				...
				if dataLen > 0 {
					if dataRecv == 0 {
						enc.WriteField(vendor.HeaderField{Name: ":status", Value: strconv.Itoa(status)})
						ct.fr.WriteHeaders(vendor.HeadersFrameParam{
							StreamID:      f.StreamID,
							EndHeaders:    true,
							EndStream:     false,
	...
	ct.server = func() error {
		...
		for {
			...
			case *vendor.DataFrame:
				...
				{
					buf.Reset()
					enc.WriteField(vendor.HeaderField{Name: ":status", Value: "200"})
					enc.WriteField(vendor.HeaderField{Name: "x-foo", Value: "blah"})
					enc.WriteField(vendor.HeaderField{Name: "x-bar", Value: "more"})
					if trailers != noHeader {
						enc.WriteField(vendor.HeaderField{Name: "trailer", Value: "some-trailer"})
					}
				...
				if trailers != noHeader {
					endStream = true
					buf.Reset()
					enc.WriteField(vendor.HeaderField{Name: "some-trailer", Value: "some-value"})
					send(trailers)
				}
				if endStream {
					return nil
			...
			case *vendor.HeadersFrame:
				if expect100Continue != noHeader {
					buf.Reset()
					enc.WriteField(vendor.HeaderField{Name: ":status", Value: "100"})
...
func TestTransportUnknown1xx(t *testing.T) {
	var buf bytes.Buffer
	defer func() { vendor.got1xxFuncForTests = nil }()
	vendor.got1xxFuncForTests = func(code int, header textproto.MIMEHeader) error {
	...
	ct.server = func() error {
		...
		for {
			...
			case *vendor.HeadersFrame:
				for i := 110; i <= 114; i++ {
					buf.Reset()
					enc.WriteField(vendor.HeaderField{Name: ":status", Value: fmt.Sprint(i)})
					enc.WriteField(vendor.HeaderField{Name: "foo-bar", Value: fmt.Sprint(i)})
					ct.fr.WriteHeaders(vendor.HeadersFrameParam{
						StreamID:      f.StreamID,
						EndHeaders:    true,
						EndStream:     false,
				...
				buf.Reset()
				enc.WriteField(vendor.HeaderField{Name: ":status", Value: "204"})
				ct.fr.WriteHeaders(vendor.HeadersFrameParam{
					StreamID:      f.StreamID,
					EndHeaders:    true,
					EndStream:     false,
...
func TestTransportReceiveUndeclaredTrailer(t *testing.T) {
	...
	ct.server = func() error {
		...
		var n int
		var hf *vendor.HeadersFrame
		for hf == nil && n < 10 {
			f, err := ct.fr.ReadFrame()
			if err != nil {
				return err
			}
			hf, _ = f.(*vendor.HeadersFrame)
			n++
		}
		...
		// send trailers
		buf.Reset()
		enc.WriteField(vendor.HeaderField{Name: "some-trailer", Value: "I'm an undeclared Trailer!"})
		ct.fr.WriteHeaders(vendor.HeadersFrameParam{
			StreamID:      hf.StreamID,
			EndHeaders:    true,
			EndStream:     true,
...
func testTransportInvalidTrailer_Pseudo(t *testing.T, trailers headerType) {
	testInvalidTrailer(t, trailers, vendor.pseudoHeaderError(":colon"), func(enc *vendor.Encoder) {
		enc.WriteField(vendor.HeaderField{Name: ":colon", Value: "foo"})
		enc.WriteField(vendor.HeaderField{Name: "foo", Value: "bar"})
	})
}
...
func testTransportInvalidTrailer_Capital(t *testing.T, trailers headerType) {
	testInvalidTrailer(t, trailers, vendor.headerFieldNameError("Capital"), func(enc *vendor.Encoder) {
		enc.WriteField(vendor.HeaderField{Name: "foo", Value: "bar"})
		enc.WriteField(vendor.HeaderField{Name: "Capital", Value: "bad"})
	})
}
func TestTransportInvalidTrailer_EmptyFieldName(t *testing.T) {
	testInvalidTrailer(t, oneHeader, vendor.headerFieldNameError(""), func(enc *vendor.Encoder) {
		enc.WriteField(vendor.HeaderField{Name: "", Value: "bad"})
	})
}
func TestTransportInvalidTrailer_BinaryFieldValue(t *testing.T) {
	testInvalidTrailer(t, oneHeader, vendor.headerFieldValueError("has\nnewline"), func(enc *vendor.Encoder) {
		enc.WriteField(vendor.HeaderField{Name: "x", Value: "has\nnewline"})
	})
}

func testInvalidTrailer(t *testing.T, trailers headerType, wantErr error, writeTrailer func(*vendor.Encoder)) {
	ct := newClientTester(t)
	ct.client = func() error {
		req, _ := http.NewRequest("GET", "https://dummy.tld/", nil)
		res, err := ct.tr.RoundTrip(req)
		...
		for {
			...
			case *vendor.HeadersFrame:
				...
				// Response headers (1+ frames; 1 or 2 in this test, but never 0)
				{
					buf.Reset()
					enc.WriteField(vendor.HeaderField{Name: ":status", Value: "200"})
					enc.WriteField(vendor.HeaderField{Name: "trailer", Value: "declared"})
					endStream = false
					send(oneHeader)
				}
				// Trailers:
...
func headerListSize(h http.Header) (size uint32) {
	for k, vv := range h {
		for _, v := range vv {
			hf := vendor.HeaderField{Name: k, Value: v}
			size += hf.Size()
		}
	}
	return size
...
func padHeaders(t *testing.T, h http.Header, limit uint64, filler string) {
	...
	hf := vendor.HeaderField{Name: "Pad-Headers", Value: ""}
	...
	hf = vendor.HeaderField{Name: fmt.Sprintf(nameFmt, 1), Value: filler}
...
func TestTransportChecksRequestHeaderListSize(t *testing.T) {
	...
	headerListSizeForRequest := func(req *http.Request) (size uint64) {
		contentLen := vendor.actualContentLength(req)
		trailers, err := vendor.commaSeparatedTrailers(req)
		if err != nil {
			t.Fatalf("headerListSizeForRequest: %v", err)
		}
		cc := &vendor.ClientConn{peerMaxHeaderListSize: 0xffffffffffffffff}
		cc.henc = vendor.NewEncoder(&cc.hbuf)
		cc.mu.Lock()
		...
		hpackDec := vendor.NewDecoder(vendor.initialHeaderTableSize, func(hf vendor.HeaderField) {
			size += uint64(hf.Size())
		})
		if len(hdrs) > 0 {
	...
	addr := vendor.authorityAddr(req.URL.Scheme, req.URL.Host)
	...
	checkRoundTrip(req, vendor.errRequestHeaderListSize, "Headers over limit")

	// Push trailers over the limit.
	req = newRequest()
	req.Trailer = make(http.Header)
	padHeaders(t, req.Trailer, peerSize+1, filler)
	checkRoundTrip(req, vendor.errRequestHeaderListSize, "Trailers over limit")
	...
	checkRoundTrip(req, vendor.errRequestHeaderListSize, "Single large header")

	// Send trailers with a single large value.
	req = newRequest()
	req.Trailer = make(http.Header)
	req.Trailer.Set("Big", filler)
	checkRoundTrip(req, vendor.errRequestHeaderListSize, "Single large trailer")
}
...
func TestTransportChecksResponseHeaderListSize(t *testing.T) {
	...
	ct.client = func() error {
		req, _ := http.NewRequest("GET", "https://dummy.tld/", nil)
		res, err := ct.tr.RoundTrip(req)
		if err != vendor.errResponseHeaderListSize {
			if res != nil {
				res.Body.Close()
			}
			size := int64(0)
	...
	ct.server = func() error {
		...
		for {
			...
			case *vendor.HeadersFrame:
				...
				for i := 0; i < 5042; i++ {
					enc.WriteField(vendor.HeaderField{Name: large, Value: large})
				}
...
func TestTransportBodyReadErrorType(t *testing.T) {
	...
	want := vendor.StreamError{StreamID: 0x1, Code: 0x2}
	...
	ct.server = func() error {
		...
		for {
			...
			case *vendor.RSTStreamFrame:
				if f.StreamID == 1 && f.ErrCode == vendor.ErrCodeCancel {
					return nil
				}
			}
		}
...
func TestTransportNewTLSConfig(t *testing.T) {
	tests := [...]struct {
		...
		0: {
			...
			want: &tls.Config{
				ServerName: "foo.com",
				NextProtos: []string{vendor.NextProtoTLS},
			},
		},
		...
		2: {
			...
			want: &tls.Config{
				...
				NextProtos: []string{vendor.NextProtoTLS, "foo", "bar"},
			},
		},
		...
		3: {
			conf: &tls.Config{
				NextProtos: []string{"foo", "bar", vendor.NextProtoTLS},
			},
			host: "example.com",
			want: &tls.Config{
				ServerName: "example.com",
	...
	for i, tt := range tests {
		...
		tr := &vendor.Transport{TLSClientConfig: tt.conf}
...
func TestTransportReadHeadResponse(t *testing.T) {
	...
	ct.server = func() error {
		...
		for {
			...
			enc := vendor.NewEncoder(&buf)
			enc.WriteField(vendor.HeaderField{Name: ":status", Value: "200"})
			enc.WriteField(vendor.HeaderField{Name: "content-length", Value: "123"})
			ct.fr.WriteHeaders(vendor.HeadersFrameParam{
				StreamID:      hf.StreamID,
				EndHeaders:    true,
...
func TestTransportReadHeadResponseWithBody(t *testing.T) {
	...
	ct.server = func() error {
		...
		for {
			...
			enc := vendor.NewEncoder(&buf)
			enc.WriteField(vendor.HeaderField{Name: ":status", Value: "200"})
			enc.WriteField(vendor.HeaderField{Name: "content-length", Value: strconv.Itoa(len(response))})
			ct.fr.WriteHeaders(vendor.HeadersFrameParam{
				StreamID:      hf.StreamID,
				EndHeaders:    true,
				EndStream:     false,
...
func TestTransportHandlerBodyClose(t *testing.T) {
	...
	if !vendor.waitCondition(5*time.Second, 100*time.Millisecond, func() bool {
...
func TestTransportFlowControl(t *testing.T) {
	...
	for {
		...
		const max = vendor.transportDefaultStreamFlow
...
func testTransportUsesGoAwayDebugError(t *testing.T, failMidBody bool) {
	...
	const goAwayErrCode = vendor.ErrCodeHTTP11Required // arbitrary
	...
	ct.server = func() error {
		...
		for {
			...
			ct.fr.WriteGoAway(5, vendor.ErrCodeNo, []byte(goAwayDebugData))
...
func testTransportReturnsUnusedFlowControl(t *testing.T, oneDataFrame bool) {
	...
	ct.server = func() error {
		...
		enc := vendor.NewEncoder(&buf)
		enc.WriteField(vendor.HeaderField{Name: ":status", Value: "200"})
		enc.WriteField(vendor.HeaderField{Name: "content-length", Value: "5000"})
		ct.fr.WriteHeaders(vendor.HeadersFrameParam{
			StreamID:      hf.StreamID,
			EndHeaders:    true,
			EndStream:     false,
		...
		for {
			...
			switch waitingFor {
			case "RSTStreamFrame":
				if rf, ok := f.(*vendor.RSTStreamFrame); !ok || rf.ErrCode != vendor.ErrCodeCancel {
					return fmt.Errorf("Expected a RSTStreamFrame with code cancel; got %v", vendor.summarizeFrame(f))
				}
				waitingFor = "WindowUpdateFrame"
			case "WindowUpdateFrame":
				if wuf, ok := f.(*vendor.WindowUpdateFrame); !ok || wuf.Increment != 4999 {
					return fmt.Errorf("Expected WindowUpdateFrame for 4999 bytes; got %v", vendor.summarizeFrame(f))
				}
				return nil
			}
		}
...
func TestTransportAdjustsFlowControl(t *testing.T) {
	...
	ct.server = func() error {
		_, err := io.ReadFull(ct.sc, make([]byte, len(vendor.ClientPreface)))
		if err != nil {
		...
		for {
			...
			case *vendor.DataFrame:
				...
				if gotBytes >= vendor.initialWindowSize/2 && !sentSettings {
					sentSettings = true

					ct.fr.WriteSettings(vendor.Setting{ID: vendor.SettingInitialWindowSize, Val: bodySize})
...
func TestTransportReturnsDataPaddingFlowControl(t *testing.T) {
	...
	ct.server = func() error {
		...
		if wuf, ok := f.(*vendor.WindowUpdateFrame); !ok || wuf.Increment != wantBack || wuf.StreamID != 0 {
			return fmt.Errorf("Expected conn WindowUpdateFrame for %d bytes; got %v", wantBack, vendor.summarizeFrame(f))
		}
		...
		if wuf, ok := f.(*vendor.WindowUpdateFrame); !ok || wuf.Increment != wantBack || wuf.StreamID == 0 {
			return fmt.Errorf("Expected stream WindowUpdateFrame for %d bytes; got %v", wantBack, vendor.summarizeFrame(f))
		}
		unblockClient <- true
		return nil
	}
...
func TestTransportReturnsErrorOnBadResponseHeaders(t *testing.T) {
	...
	ct.client = func() error {
		...
		want := vendor.StreamError{1, vendor.ErrCodeProtocol, vendor.headerFieldNameError("  content-type")}
		if !reflect.DeepEqual(want, err) {
	...
	ct.server = func() error {
		...
		enc := vendor.NewEncoder(&buf)
		enc.WriteField(vendor.HeaderField{Name: ":status", Value: "200"})
		enc.WriteField(vendor.HeaderField{Name: "  content-type", Value: "bogus"}) // bogus spaces
		ct.fr.WriteHeaders(vendor.HeadersFrameParam{
			StreamID:      hf.StreamID,
			EndHeaders:    true,
			EndStream:     false,
		...
		for {
			...
			if _, ok := fr.(*vendor.SettingsFrame); ok {
				continue
			}
			if rst, ok := fr.(*vendor.RSTStreamFrame); !ok || rst.StreamID != 1 || rst.ErrCode != vendor.ErrCodeProtocol {
				t.Errorf("Frame = %v; want RST_STREAM for stream 1 with ErrCodeProtocol", vendor.summarizeFrame(fr))
			}
			break
		}
...
func TestTransportRequestPathPseudo(t *testing.T) {
	...
	for i, tt := range tests {
		...
		cc.mu.Unlock()
		var got result
		hpackDec := vendor.NewDecoder(vendor.initialHeaderTableSize, func(f vendor.HeaderField) {
			if f.Name == ":path" {
				got.path = f.Value
			}
		})
...
func TestRoundTripDoesntConsumeRequestBodyEarly(t *testing.T) {
	...
	cc := &vendor.ClientConn{
		closed: true,
	}
	_, err := cc.RoundTrip(req)
	if err != vendor.errClientConnUnusable {
...
func TestClientConnPing(t *testing.T) {
	st := vendor.newServerTester(t, func(w http.ResponseWriter, r *http.Request) {}, vendor.optOnlyServer)
	defer st.Close()
	tr := &vendor.Transport{TLSClientConfig: tlsConfigInsecure}
	defer tr.CloseIdleConnections()
...
func TestTransportRetryAfterGOAWAY(t *testing.T) {
	...
	go func() {
		...
		if err := ct.fr.WriteGoAway(0 /*max id*/, vendor.ErrCodeNo, nil); err != nil {
	...
	go func() {
		...
		enc.WriteField(vendor.HeaderField{Name: ":status", Value: "200"})
		enc.WriteField(vendor.HeaderField{Name: "foo", Value: "bar"})
		err = ct.fr.WriteHeaders(vendor.HeadersFrameParam{
			StreamID:      hf.StreamID,
			EndHeaders:    true,
			EndStream:     false,
			BlockFragment: buf.Bytes(),
	...
	ct.server = func() error {
		...
		for {
			...
			case *vendor.HeadersFrame:
				...
				nreq++
				if nreq == 1 {
					ct.fr.WriteRSTStream(f.StreamID, vendor.ErrCodeRefusedStream)
				} else {
					enc.WriteField(vendor.HeaderField{Name: ":status", Value: "204"})
					ct.fr.WriteHeaders(vendor.HeadersFrameParam{
						StreamID:      f.StreamID,
...
func TestTransportResponseDataBeforeHeaders(t *testing.T) {
	...
	ct.client = func() error {
		...
		if err, ok := err.(vendor.StreamError); !ok || err.Code != vendor.ErrCodeProtocol {
...
func TestTransportRequestsStallAtServerLimit(t *testing.T) {
	...
	ct.server = func() error {
		...
		ct.greet(vendor.Setting{vendor.SettingMaxConcurrentStreams, maxConcurrent})
...
func TestAuthorityAddr(t *testing.T) {
	...
	for _, tt := range tests {
		got := vendor.authorityAddr(tt.scheme, tt.authority)
		if got != tt.want {
...
func TestTransportAllocationsAfterResponseBodyClose(t *testing.T) {
	...
	trb, ok := res.Body.(vendor.transportResponseBody)
	if !ok {
	...
	} else if gotErr != vendor.errStreamClosed {
...
func benchSimpleRoundTrip(b *testing.B, nHeaders int) {
	defer vendor.disableGoroutineTracking()()
	b.ReportAllocs()
	st := vendor.newServerTester(b,
		func(w http.ResponseWriter, r *http.Request) {
		},
		vendor.optOnlyServer,
		vendor.optQuiet,
...
func TestTransportHandlesInvalidStatuslessResponse(t *testing.T) {
	...
	ct.server = func() error {
		...
		for {
			...
			switch f := f.(type) {
			case *vendor.HeadersFrame:
				enc.WriteField(vendor.HeaderField{Name: "content-type", Value: "text/html"}) // no :status header
				ct.fr.WriteHeaders(vendor.HeadersFrameParam{
					StreamID:      f.StreamID,
					EndHeaders:    true,
...
func activeStreams(cc *vendor.ClientConn) int {
	cc.mu.Lock()
	defer cc.mu.Unlock()
...
func testClientConnClose(t *testing.T, closeMode closeMode) {
	...
	case shutdown:
		...
		vendor.shutdownEnterWaitStateHook = func() {
			close(wait)
			vendor.shutdownEnterWaitStateHook = func() {}
		}
		defer func() { vendor.shutdownEnterWaitStateHook = func() {} }()
...
func TestTransportUsesGetBodyWhenPresent(t *testing.T) {
	...
	req2, err := vendor.shouldRetryRequest(req, vendor.errClientConnUnusable, afterBodyWrite)
	if err != nil {
		t.Fatal(err)
	}
	if calls != 1 {
...
func TestNoDialH2RoundTripperType(t *testing.T) {
	...
	t2 := new(vendor.Transport)
	rt := vendor.noDialH2RoundTripper{t2}
	if err := vendor.registerHTTPSProtocol(t1, rt); err != nil {
		t.Fatal(err)
	}
	...
	v := rv.Field(0)
	if _, ok := v.Interface().(*vendor.Transport); !ok {
...
func testTransportBodyReadError(t *testing.T, body []byte) {
	...
	ct.client = func() error {
		...
		checkNoStreams := func() error {
			cp, ok := ct.tr.connPool().(*vendor.clientConnPool)
			if !ok {
