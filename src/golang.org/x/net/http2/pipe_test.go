...
func TestPipeCloseWithError(t *testing.T) {
	p := &vendor.pipe{b: new(bytes.Buffer)}
	const body = "foo"
	...
	if n, err := p.Write([]byte("abc")); err != vendor.errClosedPipeWrite || n != 0 {
		t.Errorf("Write(abc) after close\ngot %v, %v\nwant 0, %v", n, err, vendor.errClosedPipeWrite)
	}
	if n, err := p.Read(make([]byte, 1)); err == nil || n != 0 {
		t.Errorf("Read() after close\ngot %v, nil\nwant 0, %v", n, vendor.errClosedPipeWrite)
	}
}
