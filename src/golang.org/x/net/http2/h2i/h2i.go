...
var commands = map[string]command{
	...
	"settings": {
		...
		complete: func() []string {
			return []string{
				"ACK",
				vendor.SettingHeaderTableSize.String(),
				vendor.SettingEnablePush.String(),
				vendor.SettingMaxConcurrentStreams.String(),
				vendor.SettingInitialWindowSize.String(),
				vendor.SettingMaxFrameSize.String(),
				vendor.SettingMaxHeaderListSize.String(),
			}
		},
	},
...
type h2i struct {
	...
	peerSetting map[vendor.SettingID]uint32
	hdec        *vendor.Decoder
}
...
func main() {
	...
	app := &h2i{
		host:        host,
		peerSetting: make(map[vendor.SettingID]uint32),
	}
	app.henc = vendor.NewEncoder(&app.hbuf)
...
func (app *h2i) Main() error {
	...
	if _, err := io.WriteString(tc, vendor.ClientPreface); err != nil {
		return err
	}

	app.framer = vendor.NewFramer(tc, tc)
...
func settingByName(name string) (vendor.SettingID, bool) {
	for _, sid := range [...]vendor.SettingID{
		vendor.SettingHeaderTableSize,
		vendor.SettingEnablePush,
		vendor.SettingMaxConcurrentStreams,
		vendor.SettingInitialWindowSize,
		vendor.SettingMaxFrameSize,
		vendor.SettingMaxHeaderListSize,
	} {
...
func (app *h2i) cmdHeaders(args []string) error {
	...
	return app.framer.WriteHeaders(vendor.HeadersFrameParam{
	...
	for {
		...
		case *vendor.HeadersFrame:
			...
			if app.hdec == nil {
				...
				app.hdec = vendor.NewDecoder(tableSize, app.onNewHeaderField)
			}
...
func (app *h2i) onNewHeaderField(f vendor.HeaderField) {
	if f.Sensitive {
...
func (app *h2i) writeHeader(name, value string) {
	app.henc.WriteField(vendor.HeaderField{Name: name, Value: value})
