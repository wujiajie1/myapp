...
func TestServer_Push_Success(t *testing.T) {
	...
	checkPushPromise := func(f vendor.Frame, promiseID uint32, wantH [][2]string) error {
		pp, ok := f.(*vendor.PushPromiseFrame)
		if !ok {
	...
	checkHeaders := func(f vendor.Frame, wantH [][2]string) error {
	...
	checkData := func(f vendor.Frame, wantData string) error {
	...
	expected := map[uint32][]func(vendor.Frame) error{
		1: {
			func(f vendor.Frame) error {
...
func TestServer_Push_RejectRecursivePush(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) error {
		...
		case "/push1":
			if got, want := w.(http.Pusher).Push(baseURL+"/push2", nil), vendor.ErrRecursivePush; got != want {
...
func testServer_Push_RejectSingleRequest(t *testing.T, doPush func(http.Pusher, *http.Request) error, settings ...vendor.Setting) {
...
func TestServer_Push_RejectIfDisabled(t *testing.T) {
	testServer_Push_RejectSingleRequest(t,
		...
		vendor.Setting{vendor.SettingEnablePush, 0})
}

func TestServer_Push_RejectWhenNoConcurrentStreams(t *testing.T) {
	testServer_Push_RejectSingleRequest(t,
		func(p http.Pusher, r *http.Request) error {
			if got, want := p.Push("https://"+r.Host+"/pushed", nil), vendor.ErrPushLimitReached; got != want {
				return fmt.Errorf("Push()=%v, want %v", got, want)
			}
			return nil
		},
		vendor.Setting{vendor.SettingMaxConcurrentStreams, 0})
}
...
func TestServer_Push_StateTransitions(t *testing.T) {
	...
	if got, want := st.streamState(2), vendor.stateIdle; got != want {
		t.Fatalf("streamState(2)=%v, want %v", got, want)
	}
	vendor.getSlash(st)
	// After the PUSH_PROMISE is sent, the stream should be stateHalfClosedRemote.
	st.wantPushPromise()
	if got, want := st.streamState(2), vendor.stateHalfClosedRemote; got != want {
		t.Fatalf("streamState(2)=%v, want %v", got, want)
	}
	...
	if got, want := st.streamState(2), vendor.stateClosed; got != want {
		t.Fatalf("streamState(2)=%v, want %v", got, want)
	}
	close(finishedPush)
}
...
func TestServer_Push_RejectAfterGoAway(t *testing.T) {
	...
	st.fr.WriteGoAway(1, vendor.ErrCodeNo, nil)
	go func() {
		for {
			select {
			case <-ready:
