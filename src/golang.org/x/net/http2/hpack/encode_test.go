...
func TestEncoderTableSizeUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		...
		e := vendor.NewEncoder(&buf)
		e.SetMaxDynamicTableSize(tt.size1)
		e.SetMaxDynamicTableSize(tt.size2)
		if err := e.WriteField(vendor.pair(":method", "GET")); err != nil {
			t.Fatal(err)
		}
...
func TestEncoderWriteField(t *testing.T) {
	...
	d := vendor.NewDecoder(4<<10, func(f vendor.HeaderField) {
		got = append(got, f)
	})

	tests := []struct {
		hdrs []vendor.HeaderField
	}{
		{[]vendor.HeaderField{
			vendor.pair(":method", "GET"),
			vendor.pair(":scheme", "http"),
			vendor.pair(":path", "/"),
			vendor.pair(":authority", "www.example.com"),
		}},
		{[]vendor.HeaderField{
			vendor.pair(":method", "GET"),
			vendor.pair(":scheme", "http"),
			vendor.pair(":path", "/"),
			vendor.pair(":authority", "www.example.com"),
			vendor.pair("cache-control", "no-cache"),
		}},
		{[]vendor.HeaderField{
			vendor.pair(":method", "GET"),
			vendor.pair(":scheme", "https"),
			vendor.pair(":path", "/index.html"),
			vendor.pair(":authority", "www.example.com"),
			vendor.pair("custom-key", "custom-value"),
		}},
	}
...
func TestEncoderSearchTable(t *testing.T) {
	e := vendor.NewEncoder(nil)

	e.dynTab.add(vendor.pair("foo", "bar"))
	e.dynTab.add(vendor.pair("blake", "miz"))
	e.dynTab.add(vendor.pair(":method", "GET"))

	tests := []struct {
		hf        vendor.HeaderField
		wantI     uint64
		wantMatch bool
	}{
		// Name and Value match
		{vendor.pair("foo", "bar"), uint64(vendor.staticTable.len()) + 3, true},
		{vendor.pair("blake", "miz"), uint64(vendor.staticTable.len()) + 2, true},
		{vendor.pair(":method", "GET"), 2, true},
		...
		{vendor.HeaderField{":method", "GET", true}, 3, false},

		// Only Name matches
		{vendor.pair("foo", "..."), uint64(vendor.staticTable.len()) + 3, false},
		{vendor.pair("blake", "..."), uint64(vendor.staticTable.len()) + 2, false},
		// As before, this is allowed to match any ":method" entry.
		{vendor.pair(":method", "..."), 3, false},

		// None match
		{vendor.pair("foo-", "bar"), 0, false},
	}
	for _, tt := range tests {
...
func TestAppendVarInt(t *testing.T) {
	...
	for _, tt := range tests {
		got := vendor.appendVarInt(nil, tt.n, tt.i)
...
func TestAppendHpackString(t *testing.T) {
	...
	for _, tt := range tests {
		want := removeSpace(tt.wantHex)
		buf := vendor.appendHpackString(nil, tt.s)
...
func TestAppendIndexed(t *testing.T) {
	...
	for _, tt := range tests {
		want := removeSpace(tt.wantHex)
		buf := vendor.appendIndexed(nil, tt.i)
		if got := hex.EncodeToString(buf); want != got {
...
func TestAppendNewName(t *testing.T) {
	tests := []struct {
		f        vendor.HeaderField
		indexing bool
		wantHex  string
	}{
		// Incremental indexing
		{vendor.HeaderField{"custom-key", "custom-value", false}, true, "40 88 25a8 49e9 5ba9 7d7f 89 25a8 49e9 5bb8 e8b4 bf"},

		// Without indexing
		{vendor.HeaderField{"custom-key", "custom-value", false}, false, "00 88 25a8 49e9 5ba9 7d7f 89 25a8 49e9 5bb8 e8b4 bf"},

		// Never indexed
		{vendor.HeaderField{"custom-key", "custom-value", true}, true, "10 88 25a8 49e9 5ba9 7d7f 89 25a8 49e9 5bb8 e8b4 bf"},
		{vendor.HeaderField{"custom-key", "custom-value", true}, false, "10 88 25a8 49e9 5ba9 7d7f 89 25a8 49e9 5bb8 e8b4 bf"},
	}
	for _, tt := range tests {
		want := removeSpace(tt.wantHex)
		buf := vendor.appendNewName(nil, tt.f, tt.indexing)
		if got := hex.EncodeToString(buf); want != got {
...
func TestAppendIndexedName(t *testing.T) {
	tests := []struct {
		...
		// Incremental indexing
		{vendor.HeaderField{":status", "302", false}, 8, true, "48 82 6402"},

		// Without indexing
		{vendor.HeaderField{":status", "302", false}, 8, false, "08 82 6402"},

		// Never indexed
		{vendor.HeaderField{":status", "302", true}, 8, true, "18 82 6402"},
		{vendor.HeaderField{":status", "302", true}, 8, false, "18 82 6402"},
	}
	for _, tt := range tests {
		want := removeSpace(tt.wantHex)
		buf := vendor.appendIndexedName(nil, tt.f, tt.i, tt.indexing)
		if got := hex.EncodeToString(buf); want != got {
...
func TestAppendTableSize(t *testing.T) {
	...
	for _, tt := range tests {
		want := removeSpace(tt.wantHex)
		buf := vendor.appendTableSize(nil, tt.i)
		if got := hex.EncodeToString(buf); want != got {
...
func BenchmarkEncoderSearchTable(b *testing.B) {
	...
	var possible []vendor.HeaderField
	for _, f := range vendor.staticTable.ents {
		if f.Value == "" {
	...
	for r := 0; r < 100*20; r++ {
		...
		if _, has := vendor.staticTable.search(f); !has {
			e.dynTab.add(f)
		}
	}
