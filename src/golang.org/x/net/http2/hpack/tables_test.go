...
func TestHeaderFieldTable(t *testing.T) {
	table := &vendor.headerFieldTable{}
	table.init()
	table.addEntry(vendor.pair("key1", "value1-1"))
	table.addEntry(vendor.pair("key2", "value2-1"))
	table.addEntry(vendor.pair("key1", "value1-2"))
	table.addEntry(vendor.pair("key3", "value3-1"))
	table.addEntry(vendor.pair("key4", "value4-1"))
	table.addEntry(vendor.pair("key2", "value2-2"))
	...
	tests := []struct {
		...
		{vendor.HeaderField{"key1", "value1-1", false}, 1, true, 0, false},
		{vendor.HeaderField{"key1", "value1-2", false}, 3, true, 0, false},
		{vendor.HeaderField{"key1", "value1-3", false}, 3, false, 0, false},
		{vendor.HeaderField{"key2", "value2-1", false}, 2, true, 3, false},
		{vendor.HeaderField{"key2", "value2-2", false}, 6, true, 3, true},
		{vendor.HeaderField{"key2", "value2-3", false}, 6, false, 3, false},
		{vendor.HeaderField{"key4", "value4-1", false}, 5, true, 2, true},
		// Name match only, because sensitive.
		{vendor.HeaderField{"key4", "value4-1", true}, 5, false, 2, false},
		// Key not found.
		{vendor.HeaderField{"key5", "value5-x", false}, 0, false, 0, false},
	}
	...
	searchStatic := func(f vendor.HeaderField) (uint64, bool) {
		old := vendor.staticTable
		vendor.staticTable = table
		defer func() { vendor.staticTable = old }()
		return vendor.staticTable.search(f)
	}

	searchDynamic := func(f vendor.HeaderField) (uint64, bool) {
...
`
	...
	for bs.Scan() {
		...
		if i < 1 || i > vendor.staticTable.len() {
			t.Errorf("Bogus index %d on line %q", i, l)
			continue
		}
		if got, want := vendor.staticTable.ents[i-1].Name, m[2]; got != want {
			t.Errorf("header index %d name = %q; want %q", i, got, want)
		}
		if got, want := vendor.staticTable.ents[i-1].Value, m[3]; got != want {
