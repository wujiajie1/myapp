...
func (d *vendor.Decoder) mustAt(idx int) vendor.HeaderField {
...
func TestDynamicTableAt(t *testing.T) {
	...
	if got, want := at(vendor.staticTable.len()+1), (pair("blake", "miz")); got != want {
		t.Errorf("at(dyn 1) = %v; want %v", got, want)
	}
	if got, want := at(vendor.staticTable.len()+2), (pair("foo", "bar")); got != want {
...
func TestDynamicTableSizeEvict(t *testing.T) {
	...
	if got, want := d.mustAt(vendor.staticTable.len()+1), (pair("foo", "bar")); got != want {
		t.Errorf("at(dyn 1) = %v; want %v", got, want)
	}
...
func TestDecoderDecode(t *testing.T) {
	tests := []struct {
		name       string
		in         []byte
		want       []vendor.HeaderField
		wantDynTab []vendor.HeaderField // newest entry first
	}{
		// C.2.1 Literal Header Field with Indexing
		// http://http2.github.io/http2-spec/compression.html#rfc.section.C.2.1
		{"C.2.1", dehex("400a 6375 7374 6f6d 2d6b 6579 0d63 7573 746f 6d2d 6865 6164 6572"),
			[]vendor.HeaderField{pair("custom-key", "custom-header")},
			[]vendor.HeaderField{pair("custom-key", "custom-header")},
		},
		...
		{"C.2.2", dehex("040c 2f73 616d 706c 652f 7061 7468"),
			[]vendor.HeaderField{pair(":path", "/sample/path")},
			[]vendor.HeaderField{}},

		// C.2.3 Literal Header Field never Indexed
		// http://http2.github.io/http2-spec/compression.html#rfc.section.C.2.3
		{"C.2.3", dehex("1008 7061 7373 776f 7264 0673 6563 7265 74"),
			[]vendor.HeaderField{{"password", "secret", true}},
			[]vendor.HeaderField{}},

		// C.2.4 Indexed Header Field
		// http://http2.github.io/http2-spec/compression.html#rfc.section.C.2.4
		{"C.2.4", []byte("\x82"),
			[]vendor.HeaderField{pair(":method", "GET")},
			[]vendor.HeaderField{}},
	}
...
func (dt *vendor.dynamicTable) reverseCopy() (hf []vendor.HeaderField) {
	hf = make([]vendor.HeaderField, len(dt.table.ents))
	for i := range hf {
...
func TestHuffmanDecodeExcessPadding(t *testing.T) {
	...
	for i, in := range tests {
		...
		if _, err := vendor.HuffmanDecode(&buf, in); err != vendor.ErrInvalidHuffman {
...
func TestHuffmanDecodeMaxLengthOnTrailingByte(t *testing.T) {
	...
	var buf bytes.Buffer
	if err := vendor.huffmanDecode(&buf, 2, in); err != vendor.ErrStringLength {
...
func TestHuffmanDecode(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if _, err := vendor.HuffmanDecode(&buf, in); err != nil {
...
func BenchmarkHuffmanDecode(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		buf.Reset()
		if _, err := vendor.HuffmanDecode(&buf, enc); err != nil {
...
func TestAppendHuffmanString(t *testing.T) {
	...
	for i, tt := range tests {
		...
		buf = vendor.AppendHuffmanString(buf, tt.in)
...
func TestHuffmanMaxStrLen(t *testing.T) {
	...
	huff := vendor.AppendHuffmanString(nil, msg)

	testGood := func(max int) {
		var out bytes.Buffer
		if err := vendor.huffmanDecode(&out, max, huff); err != nil {
	...
	var out bytes.Buffer
	if err := vendor.huffmanDecode(&out, len(msg)-1, huff); err != vendor.ErrStringLength {
...
func TestHuffmanRoundtripStress(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		huff = vendor.AppendHuffmanString(huff[:0], string(input))
		encSize += int64(len(huff))
		output.Reset()
		if err := vendor.huffmanDecode(&output, 0, huff); err != nil {
...
func TestHuffmanDecodeFuzz(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		buf.Reset()
		if err := vendor.huffmanDecode(&buf, 0, zbuf.Bytes()); err != nil {
			if err == vendor.ErrInvalidHuffman {
				numFail++
				continue
			}
...
func TestReadVarInt(t *testing.T) {
	...
	tests := []struct {
		...
		{1, []byte{1}, res{0, 0, vendor.errNeedMore}},
		{2, []byte{3}, res{0, 0, vendor.errNeedMore}},
		{3, []byte{7}, res{0, 0, vendor.errNeedMore}},
		{4, []byte{15}, res{0, 0, vendor.errNeedMore}},
		{5, []byte{31}, res{0, 0, vendor.errNeedMore}},
		{6, []byte{63}, res{0, 0, vendor.errNeedMore}},
		{7, []byte{127}, res{0, 0, vendor.errNeedMore}},
		{8, []byte{255}, res{0, 0, vendor.errNeedMore}},
		...
		// Short a byte:
		{5, []byte{191, 154}, res{0, 0, vendor.errNeedMore}},

		// integer overflow:
		{1, []byte{255, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128, 128}, res{0, 0, vendor.errVarintOverflow}},
	}
	for _, tt := range tests {
		i, remain, err := vendor.readVarInt(tt.n, tt.p)
...
func TestHuffmanFuzzCrash(t *testing.T) {
	got, err := vendor.HuffmanDecodeToString([]byte("00\x91\xff\xff\xff\xff\xc8"))
	if got != "" {
...
func pair(name, value string) vendor.HeaderField {
	return vendor.HeaderField{Name: name, Value: value}
}
...
func TestEmitEnabled(t *testing.T) {
	...
	numCallback := 0
	var dec *vendor.Decoder
	dec = vendor.NewDecoder(8<<20, func(vendor.HeaderField) {
		numCallback++
...
func TestSaveBufLimit(t *testing.T) {
	const maxStr = 1 << 10
	var got []vendor.HeaderField
	dec := vendor.NewDecoder(vendor.initialHeaderTableSize, func(hf vendor.HeaderField) {
		got = append(got, hf)
	})
	dec.SetMaxStringLength(maxStr)
	var frag []byte
	frag = append(frag[:0], vendor.encodeTypeByte(false, false))
	frag = vendor.appendVarInt(frag, 7, 3)
	...
	want := []vendor.HeaderField{{Name: "foo", Value: "bar"}}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("After small writes, got %v; want %v", got, want)
	}

	frag = append(frag[:0], vendor.encodeTypeByte(false, false))
	frag = vendor.appendVarInt(frag, 7, maxStr*3)
	frag = append(frag, make([]byte, maxStr*3)...)

	_, err := dec.Write(frag)
	if err != vendor.ErrStringLength {
...
func TestDynamicSizeUpdate(t *testing.T) {
	...
	d := vendor.NewDecoder(4096, func(_ vendor.HeaderField) {})
