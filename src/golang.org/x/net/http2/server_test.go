...
type serverTester struct {
	...
	sc             *vendor.serverConn
	hpackDec       *vendor.Decoder
	...
	hpackEnc  *vendor.Encoder
}

func init() {
	vendor.testHookOnPanicMu = new(sync.Mutex)
	vendor.goAwayTimeout = 25 * time.Millisecond
}

func resetHooks() {
	vendor.testHookOnPanicMu.Lock()
	vendor.testHookOnPanic = nil
	vendor.testHookOnPanicMu.Unlock()
}
...
func newServerTester(t testing.TB, handler http.HandlerFunc, opts ...interface{}) *serverTester {
	...
	vendor.ConfigureServer(ts.Config, h2server)

	st := &serverTester{
		t:  t,
		ts: ts,
	}
	st.hpackEnc = vendor.NewEncoder(&st.headerBuf)
	st.hpackDec = vendor.NewDecoder(vendor.initialHeaderTableSize, st.onHeaderField)

	ts.TLS = ts.Config.TLSConfig // the httptest.Server has its own copy of this TLS config
	if quiet {
		ts.Config.ErrorLog = log.New(ioutil.Discard, "", 0)
	} else {
		ts.Config.ErrorLog = log.New(io.MultiWriter(stderrv(), vendor.twriter{t: t, st: st}, &st.serverLogBuf), "", log.LstdFlags)
	}
	ts.StartTLS()
	...
	vendor.testHookGetServerConn = func(v *vendor.serverConn) {
		st.scMu.Lock()
		defer st.scMu.Unlock()
		st.sc = v
	}
	log.SetOutput(io.MultiWriter(stderrv(), vendor.twriter{t: t, st: st}))
	if !onlyServer {
		...
		st.cc = cc
		st.fr = vendor.NewFramer(cc, cc)
		if framerReuseFrames {
			st.fr.SetReuseFrames()
		}
		if !vendor.logFrameReads && !vendor.logFrameWrites {
...
func (st *serverTester) stream(id uint32) *vendor.stream {
	ch := make(chan *vendor.stream, 1)
...
func (st *serverTester) streamState(id uint32) vendor.streamState {
	ch := make(chan vendor.streamState, 1)
	st.sc.serveMsgCh <- func(int) {
...
func (st *serverTester) greet() {
	st.greetAndCheckSettings(func(vendor.Setting) error { return nil })
}

func (st *serverTester) greetAndCheckSettings(checkSetting func(s vendor.Setting) error) {
	st.writePreface()
	...
	for i := 0; i < 2; i++ {
		...
		case *vendor.SettingsFrame:
			if !f.Header().Flags.Has(vendor.FlagSettingsAck) {
		...
		case *vendor.WindowUpdateFrame:
			...
			incr := uint32((&vendor.Server{}).initialConnRecvWindowSize() - vendor.initialWindowSize)
			if f.Increment != incr {
...
func (st *serverTester) writePreface() {
	n, err := st.cc.Write(vendor.clientPreface)
	if err != nil {
		st.t.Fatalf("Error writing client preface: %v", err)
	}
	if n != len(vendor.clientPreface) {
		st.t.Fatalf("Writing client preface, wrote %d bytes; want %d", n, len(vendor.clientPreface))
	}
}
...
func (st *serverTester) writeHeaders(p vendor.HeadersFrameParam) {
	if err := st.fr.WriteHeaders(p); err != nil {
		st.t.Fatalf("Error writing HEADERS: %v", err)
	}
}

func (st *serverTester) writePriority(id uint32, p vendor.PriorityParam) {
...
func (st *serverTester) encodeHeaderField(k, v string) {
	err := st.hpackEnc.WriteField(vendor.HeaderField{Name: k, Value: v})
	if err != nil {
...
func (st *serverTester) bodylessReq1(headers ...string) {
	st.writeHeaders(vendor.HeadersFrameParam{
...
func readFrameTimeout(fr *vendor.Framer, wait time.Duration) (vendor.Frame, error) {
...
func (st *serverTester) readFrame() (vendor.Frame, error) {
	return readFrameTimeout(st.fr, 2*time.Second)
}

func (st *serverTester) wantHeaders() *vendor.HeadersFrame {
	f, err := st.readFrame()
	if err != nil {
...
func (st *serverTester) wantContinuation() *vendor.ContinuationFrame {
	f, err := st.readFrame()
	if err != nil {
		st.t.Fatalf("Error while expecting a CONTINUATION frame: %v", err)
	}
	cf, ok := f.(*vendor.ContinuationFrame)
	if !ok {
...
func (st *serverTester) wantData() *vendor.DataFrame {
	f, err := st.readFrame()
	if err != nil {
		st.t.Fatalf("Error while expecting a DATA frame: %v", err)
	}
	df, ok := f.(*vendor.DataFrame)
	if !ok {
...
func (st *serverTester) wantSettings() *vendor.SettingsFrame {
	f, err := st.readFrame()
	if err != nil {
		st.t.Fatalf("Error while expecting a SETTINGS frame: %v", err)
	}
	sf, ok := f.(*vendor.SettingsFrame)
	if !ok {
...
func (st *serverTester) wantPing() *vendor.PingFrame {
	f, err := st.readFrame()
	if err != nil {
		st.t.Fatalf("Error while expecting a PING frame: %v", err)
	}
	pf, ok := f.(*vendor.PingFrame)
	if !ok {
...
func (st *serverTester) wantGoAway() *vendor.GoAwayFrame {
	f, err := st.readFrame()
	if err != nil {
		st.t.Fatalf("Error while expecting a GOAWAY frame: %v", err)
	}
	gf, ok := f.(*vendor.GoAwayFrame)
	if !ok {
...
func (st *serverTester) wantRSTStream(streamID uint32, errCode vendor.ErrCode) {
	f, err := st.readFrame()
	if err != nil {
		st.t.Fatalf("Error while expecting an RSTStream frame: %v", err)
	}
	rs, ok := f.(*vendor.RSTStreamFrame)
	if !ok {
...
func (st *serverTester) wantWindowUpdate(streamID, incr uint32) {
	...
	wu, ok := f.(*vendor.WindowUpdateFrame)
	if !ok {
...
func (st *serverTester) wantSettingsAck() {
	...
	if !sf.Header().Flags.Has(vendor.FlagSettingsAck) {
		st.t.Fatal("Settings Frame didn't have ACK set")
	}
}

func (st *serverTester) wantPushPromise() *vendor.PushPromiseFrame {
	f, err := st.readFrame()
	if err != nil {
		st.t.Fatal(err)
	}
	ppf, ok := f.(*vendor.PushPromiseFrame)
	if !ok {
...
func TestServer_Request_Reject_Pseudo_AfterRegular(t *testing.T) {
	...
	testRejectRequest(t, func(st *serverTester) {
		...
		enc.WriteField(vendor.HeaderField{Name: ":method", Value: "GET"})
		enc.WriteField(vendor.HeaderField{Name: "regular", Value: "foobar"})
		enc.WriteField(vendor.HeaderField{Name: ":path", Value: "/"})
		enc.WriteField(vendor.HeaderField{Name: ":scheme", Value: "https"})
		st.writeHeaders(vendor.HeadersFrameParam{
			StreamID:      1, // clients send odd numbers
...
func testRejectRequest(t *testing.T, send func(*serverTester)) {
	...
	st.greet()
	send(st)
	st.wantRSTStream(1, vendor.ErrCodeProtocol)
}
...
func testRejectRequestWithProtocolError(t *testing.T, send func(*serverTester)) {
	...
	send(st)
	gf := st.wantGoAway()
	if gf.ErrCode != vendor.ErrCodeProtocol {
		t.Errorf("err code = %v; want %v", gf.ErrCode, vendor.ErrCodeProtocol)
	}
}
...
func TestRejectFrameOnIdle_RSTStream(t *testing.T) {
	testRejectRequestWithProtocolError(t, func(st *serverTester) {
		st.fr.WriteRSTStream(123, vendor.ErrCodeCancel)
	})
}
...
func TestServer_Request_Connect_InvalidPath(t *testing.T) {
	testServerRejectsStream(t, vendor.ErrCodeProtocol, func(st *serverTester) {
		st.writeHeaders(vendor.HeadersFrameParam{
			StreamID: 1,
			BlockFragment: st.encodeHeaderRaw(
				":method", "CONNECT",
...
func TestServer_Ping(t *testing.T) {
	...
	pf := st.wantPing()
	if !pf.Flags.Has(vendor.FlagPingAck) {
...
func TestServer_RejectsLargeFrames(t *testing.T) {
	...
	st.fr.WriteRawFrame(0xff, 0, 0, make([]byte, vendor.defaultMaxReadFrameSize+1))

	gf := st.wantGoAway()
	if gf.ErrCode != vendor.ErrCodeFrameSize {
		t.Errorf("GOAWAY err = %v; want %v", gf.ErrCode, vendor.ErrCodeFrameSize)
	}
...
func TestServer_Handler_Sends_WindowUpdate(t *testing.T) {
	puppet := vendor.newHandlerPuppet()
	st := newServerTester(t, func(w http.ResponseWriter, r *http.Request) {
		puppet.act(w, r)
	})
	defer st.Close()
...
func TestServer_Send_GoAway_After_Bogus_WindowUpdate(t *testing.T) {
	...
	gf := st.wantGoAway()
	if gf.ErrCode != vendor.ErrCodeFlowControl {
		t.Errorf("GOAWAY err = %v; want %v", gf.ErrCode, vendor.ErrCodeFlowControl)
	}
...
func TestServer_Send_RstStream_After_Bogus_WindowUpdate(t *testing.T) {
	...
	st.wantRSTStream(1, vendor.ErrCodeFlowControl)
}
...
func TestServer_RSTStream_Unblocks_Read(t *testing.T) {
	testServerPostUnblock(t,
		...
		func(st *serverTester) {
			if err := st.fr.WriteRSTStream(1, vendor.ErrCodeCancel); err != nil {
				t.Fatal(err)
			}
		},
		func(err error) {
			want := vendor.StreamError{StreamID: 0x1, Code: 0x8}
...
func TestServer_StateTransitions(t *testing.T) {
	...
	st = newServerTester(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if got, want := st.streamState(1), vendor.stateOpen; got != want {
		...
		if got, want := st.streamState(1), vendor.stateHalfClosedRemote; got != want {
			t.Errorf("in handler, state is %v; want %v", got, want)
		}
	...
	if got := st.streamState(1); got != vendor.stateIdle {
	...
	if got, want := st.streamState(1), vendor.stateClosed; got != want {
...
func TestServer_Rejects_HeadersNoEnd_Then_Headers(t *testing.T) {
	testServerRejectsConn(t, func(st *serverTester) {
		...
		st.writeHeaders(vendor.HeadersFrameParam{ // Not a continuation.
...
func TestServer_Rejects_HeadersEnd_Then_Continuation(t *testing.T) {
	testServerRejectsConn(t, func(st *serverTester) {
		...
		st.wantHeaders()
		if err := st.fr.WriteContinuation(1, true, vendor.encodeHeaderNoImplicit(t, "foo", "bar")); err != nil {
			t.Fatal(err)
		}
	})
}
...
func TestServer_Rejects_HeadersNoEnd_Then_ContinuationWrongStream(t *testing.T) {
	testServerRejectsConn(t, func(st *serverTester) {
		...
		if err := st.fr.WriteContinuation(3, true, vendor.encodeHeaderNoImplicit(t, "foo", "bar")); err != nil {
			t.Fatal(err)
		}
	})
}
...
func TestServer_Rejects_Priority0(t *testing.T) {
	testServerRejectsConn(t, func(st *serverTester) {
		st.fr.AllowIllegalWrites = true
		st.writePriority(0, vendor.PriorityParam{StreamDep: 1})
	})
}
...
func TestServer_Rejects_HeadersSelfDependence(t *testing.T) {
	testServerRejectsStream(t, vendor.ErrCodeProtocol, func(st *serverTester) {
		...
		st.writeHeaders(vendor.HeadersFrameParam{
			...
			EndStream:     true,
			EndHeaders:    true,
			Priority:      vendor.PriorityParam{StreamDep: 1},
		})
	})
}
...
func TestServer_Rejects_PrioritySelfDependence(t *testing.T) {
	testServerRejectsStream(t, vendor.ErrCodeProtocol, func(st *serverTester) {
		st.fr.AllowIllegalWrites = true
		st.writePriority(1, vendor.PriorityParam{StreamDep: 1})
	})
}

func TestServer_Rejects_PushPromise(t *testing.T) {
	testServerRejectsConn(t, func(st *serverTester) {
		pp := vendor.PushPromiseParam{
			StreamID:  1,
			PromiseID: 3,
		}
...
func testServerRejectsStream(t *testing.T, code vendor.ErrCode, writeReq func(*serverTester)) {
	st := newServerTester(t, func(w http.ResponseWriter, r *http.Request) {})
	defer st.Close()
	st.greet()
	writeReq(st)
...
func TestServer_Response_LargeWrite(t *testing.T) {
	...
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := st.fr.WriteSettings(
			vendor.Setting{vendor.SettingInitialWindowSize, 0},
			vendor.Setting{vendor.SettingMaxFrameSize, maxFrameSize},
		); err != nil {
			t.Fatal(err)
		}
		st.wantSettingsAck()
...
func TestServer_Response_LargeWrite_FlowControlled(t *testing.T) {
	...
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := st.fr.WriteSettings(vendor.Setting{vendor.SettingInitialWindowSize, uint32(reads[0])}); err != nil {
			t.Fatal(err)
		}
		st.wantSettingsAck()
...
func TestServer_Response_Empty_Data_Not_FlowControlled(t *testing.T) {
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		if err := st.fr.WriteSettings(vendor.Setting{vendor.SettingInitialWindowSize, 0}); err != nil {
			t.Fatal(err)
		}
		st.wantSettingsAck()
...
func TestServer_Rejects_Too_Many_Streams(t *testing.T) {
	...
	leaveHandler := make(chan bool)
	st := newServerTester(t, func(w http.ResponseWriter, r *http.Request) {
		id := w.(*vendor.responseWriter).rws.stream.id
		inHandler <- id
		if id == 1+(vendor.defaultMaxStreams+1)*2 && r.URL.Path != testPath {
	...
	for i := 0; i < vendor.defaultMaxStreams; i++ {
		sendReq(streamID())
		<-inHandler
	}
	defer func() {
	...
	st.wantRSTStream(rejectID, vendor.ErrCodeProtocol)
...
func TestServer_NoCrash_HandlerClose_Then_ClientClose(t *testing.T) {
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		st.wantRSTStream(1, vendor.ErrCodeNo)
		...
		// first.
		st.wantRSTStream(1, vendor.ErrCodeStreamClosed)
		...
		vendor.testHookOnPanicMu.Lock()
		vendor.testHookOnPanic = func(sc *vendor.serverConn, pv interface{}) bool {
			panMu.Lock()
			panicVal = pv
			panMu.Unlock()
			return true
...
func testRejectTLS(t *testing.T, max uint16) {
	...
	defer st.Close()
	gf := st.wantGoAway()
	if got, want := gf.ErrCode, vendor.ErrCodeInadequateSecurity; got != want {
...
func TestServer_Rejects_TLSBadCipher(t *testing.T) {
	st := newServerTester(t, nil, func(c *tls.Config) {
		...
		c.CipherSuites = []uint16{
			...
			vendor.cipher_TLS_RSA_WITH_AES_128_CBC_SHA256,
		}
	})
	defer st.Close()
	gf := st.wantGoAway()
...
func (st *serverTester) onHeaderField(f vendor.HeaderField) {
	if f.Name == "date" {
		return
	}
...
func testServerWithCurl(t *testing.T, permitProhibitedCipherSuites bool) {
	...
	vendor.requireCurl(t)
	var gotConn int32
	vendor.testHookOnConn = func() { atomic.StoreInt32(&gotConn, 1) }
	...
	vendor.ConfigureServer(ts.Config, &vendor.Server{
	...
	container := vendor.curl(t, "--silent", "--http2", "--insecure", "-v", ts.URL)
	defer vendor.kill(container)
	resc := make(chan interface{}, 1)
	go func() {
		res, err := vendor.dockerLogs(container)
		if err != nil {
			resc <- err
		} else {
			resc <- res
...
func TestServerDoS_MaxHeaderListSize(t *testing.T) {
	...
	// shake hands
	frameSize := vendor.defaultMaxReadFrameSize
	var advHeaderListSize *uint32
	st.greetAndCheckSettings(func(s vendor.Setting) error {
		switch s.ID {
		case vendor.SettingMaxFrameSize:
			if s.Val < vendor.minMaxFrameSize {
				frameSize = vendor.minMaxFrameSize
			} else if s.Val > vendor.maxFrameSize {
				frameSize = vendor.maxFrameSize
			} else {
				frameSize = int(s.Val)
			}
...
func TestCompressionErrorOnWrite(t *testing.T) {
	...
	ga := st.wantGoAway()
	if ga.ErrCode != vendor.ErrCodeCompression {
...
func TestIssue53(t *testing.T) {
	...
	s := &http.Server{
		ErrorLog: log.New(io.MultiWriter(stderrv(), vendor.twriter{t: t}), "", log.LstdFlags),
	...
	s2.ServeConn(c, &vendor.ServeConnOpts{BaseConfig: s})
	if !c.closed {
...
func TestConfigureServer(t *testing.T) {
	...
	for _, tt := range tests {
		...
		err := vendor.ConfigureServer(srv, nil)
...
func disableGoroutineTracking() (restore func()) {
	old := vendor.DebugGoroutines
	vendor.DebugGoroutines = false
	return func() { vendor.DebugGoroutines = old }
}
...
func TestServerHandleCustomConn(t *testing.T) {
	...
	go func() {
		...
		defer c2.Close()
		fr := vendor.NewFramer(c2, c2)
		io.WriteString(c2, vendor.ClientPreface)
		fr.WriteSettings()
		fr.WriteSettingsAck()
		f, err := fr.ReadFrame()
		if err != nil {
		...
		if sf, ok := f.(*vendor.SettingsFrame); !ok || sf.IsAck() {
			t.Errorf("Got %v; want non-ACK SettingsFrame", vendor.summarizeFrame(f))
			return
		}
		f, err = fr.ReadFrame()
		if err != nil {
		...
		if sf, ok := f.(*vendor.SettingsFrame); !ok || !sf.IsAck() {
			t.Errorf("Got %v; want ACK SettingsFrame", vendor.summarizeFrame(f))
			return
		}
		var henc hpackEncoder
		fr.WriteHeaders(vendor.HeadersFrameParam{
			StreamID:      1,
	...
	go s.ServeConn(connStateConn{c1, fakeConnState}, &vendor.ServeConnOpts{
...
func (he *hpackEncoder) encodeHeaderRaw(t *testing.T, headers ...string) []byte {
	...
	he.buf.Reset()
	if he.enc == nil {
		he.enc = vendor.NewEncoder(&he.buf)
	}
	for len(headers) > 0 {
		k, v := headers[0], headers[1]
		err := he.enc.WriteField(vendor.HeaderField{Name: k, Value: v})
		if err != nil {
...
func TestCheckValidHTTP2Request(t *testing.T) {
	...
	for i, tt := range tests {
		got := vendor.checkValidHTTP2RequestHeaders(tt.h)
		if !vendor.equalError(got, tt.want) {
...
func TestExpect100ContinueAfterHandlerWrites(t *testing.T) {
	...
	tr := &vendor.Transport{TLSClientConfig: vendor.tlsConfigInsecure}
	defer tr.CloseIdleConnections()

	req, _ := http.NewRequest("POST", st.ts.URL, io.LimitReader(vendor.neverEnding('A'), 2<<20))
...
func TestUnreadFlowControlReturned_Server(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		body := io.MultiReader(
			io.LimitReader(vendor.neverEnding('A'), 16<<10),
...
func TestServerIdleTimeout(t *testing.T) {
	...
	st.greet()
	ga := st.wantGoAway()
	if ga.ErrCode != vendor.ErrCodeNo {
	...
	testServerResponse(t, func(w http.ResponseWriter, r *http.Request) error {
		...
		for {
			...
			case *vendor.GoAwayFrame:
				sawGoAway = true
				unblockHandler <- true
				if f.LastStreamID != 1 || f.ErrCode != vendor.ErrCodeNo {
					t.Errorf("unexpected GOAWAY frame: %v", vendor.summarizeFrame(f))
				}
			...
			case *vendor.DataFrame:
				if f.StreamID != 1 || !f.StreamEnded() || len(f.Data()) != 0 {
					t.Errorf("unexpected DATA frame: %v", vendor.summarizeFrame(f))
				}
			default:
				t.Logf("unexpected frame: %v", vendor.summarizeFrame(f))
			}
		}
		if !sawGoAway {
