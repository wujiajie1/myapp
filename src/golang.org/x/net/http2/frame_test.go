...
func testFramer() (*vendor.Framer, *bytes.Buffer) {
	buf := new(bytes.Buffer)
	return vendor.NewFramer(buf, buf), buf
}

func TestFrameSizes(t *testing.T) {
	// Catch people rearranging the FrameHeader fields.
	if got, want := int(unsafe.Sizeof(vendor.FrameHeader{})), 12; got != want {
...
func TestFrameTypeString(t *testing.T) {
	tests := []struct {
		...
		{vendor.FrameData, "DATA"},
		{vendor.FramePing, "PING"},
		{vendor.FrameGoAway, "GOAWAY"},
...
func TestWriteRST(t *testing.T) {
	...
	fr.WriteRSTStream(streamID, vendor.ErrCode(errCode))
	...
	want := &vendor.RSTStreamFrame{
		FrameHeader: vendor.FrameHeader{
			valid:    true,
			Type:     0x3,
			Flags:    0x0,
			Length:   0x4,
...
func TestWriteDataPadded(t *testing.T) {
	tests := [...]struct {
		...
		0: {
			...
			pad:       nil,
			wantHeader: vendor.FrameHeader{
				Type:     vendor.FrameData,
				Flags:    vendor.FlagDataEndStream,
				Length:   3,
				StreamID: 1,
			},
		},
		...
		1: {
			...
			wantHeader: vendor.FrameHeader{
				Type:     vendor.FrameData,
				Flags:    vendor.FlagDataEndStream | vendor.FlagDataPadded,
				Length:   4,
				StreamID: 1,
			},
		},
...
func TestWriteHeaders(t *testing.T) {
	tests := []struct {
		name      string
		p         vendor.HeadersFrameParam
		wantEnc   string
		wantFrame *vendor.HeadersFrame
	}{
		{
			"basic",
			vendor.HeadersFrameParam{
				StreamID:      42,
				BlockFragment: []byte("abc"),
				Priority:      vendor.PriorityParam{},
			},
		...
		},
		{
			...
			&vendor.HeadersFrame{
				FrameHeader: vendor.FrameHeader{
					...
					StreamID: 42,
					Type:     vendor.FrameHeaders,
					Flags:    vendor.FlagHeadersEndStream | vendor.FlagHeadersEndHeaders,
					Length:   uint32(len("abc")),
				},
				Priority:      vendor.PriorityParam{},
				headerFragBuf: []byte("abc"),
		...
		},
		{
			...
			&vendor.HeadersFrame{
				FrameHeader: vendor.FrameHeader{
					...
					StreamID: 42,
					Type:     vendor.FrameHeaders,
					Flags:    vendor.FlagHeadersEndStream | vendor.FlagHeadersEndHeaders | vendor.FlagHeadersPadded,
		...
		},
		{
			...
			&vendor.HeadersFrame{
				FrameHeader: vendor.FrameHeader{
					...
					StreamID: 42,
					Type:     vendor.FrameHeaders,
					Flags:    vendor.FlagHeadersEndStream | vendor.FlagHeadersEndHeaders | vendor.FlagHeadersPadded | vendor.FlagHeadersPriority,
...
func TestWriteInvalidStreamDep(t *testing.T) {
	fr, _ := testFramer()
	err := fr.WriteHeaders(vendor.HeadersFrameParam{
		StreamID: 42,
		Priority: vendor.PriorityParam{
			StreamDep: 1 << 31,
		},
	})
	if err != vendor.errDepStreamID {
		t.Errorf("header error = %v; want %q", err, vendor.errDepStreamID)
	}

	err = fr.WritePriority(2, vendor.PriorityParam{StreamDep: 1 << 31})
	if err != vendor.errDepStreamID {
		t.Errorf("priority error = %v; want %q", err, vendor.errDepStreamID)
	}
}
...
func TestWriteContinuation(t *testing.T) {
	...
	tests := []struct {
		...
		wantFrame *vendor.ContinuationFrame
	}{
		{
			"not end",
			false,
			[]byte("abc"),
			&vendor.ContinuationFrame{
				FrameHeader: vendor.FrameHeader{
					valid:    true,
					StreamID: streamID,
					Type:     vendor.FrameContinuation,
					Length:   uint32(len("abc")),
				},
				headerFragBuf: []byte("abc"),
			},
		},
		{
			...
			&vendor.ContinuationFrame{
				FrameHeader: vendor.FrameHeader{
					...
					StreamID: streamID,
					Type:     vendor.FrameContinuation,
					Flags:    vendor.FlagContinuationEndHeaders,
...
func TestWritePriority(t *testing.T) {
	...
	tests := []struct {
		name      string
		priority  vendor.PriorityParam
		wantFrame *vendor.PriorityFrame
	}{
		{
			"not exclusive",
			vendor.PriorityParam{
				StreamDep: 2,
				Exclusive: false,
				Weight:    127,
			},
			&vendor.PriorityFrame{
				vendor.FrameHeader{
					valid:    true,
					StreamID: streamID,
					Type:     vendor.FramePriority,
					Length:   5,
				},
				vendor.PriorityParam{
					StreamDep: 2,
...
func TestWriteSettings(t *testing.T) {
	fr, buf := testFramer()
	settings := []vendor.Setting{{1, 2}, {3, 4}}
	...
	var got []vendor.Setting
	sf.ForeachSetting(func(s vendor.Setting) error {
		got = append(got, s)
...
func testWritePing(t *testing.T, ack bool) {
	...
	var wantFlags vendor.Flags
	if ack {
		wantFlags = vendor.FlagPingAck
	}
...
func TestReadFrameHeader(t *testing.T) {
	tests := []struct {
		in   string
		want vendor.FrameHeader
	}{
		{in: "\x00\x00\x00" + "\x00" + "\x00" + "\x00\x00\x00\x00", want: vendor.FrameHeader{}},
		{in: "\x01\x02\x03" + "\x04" + "\x05" + "\x06\x07\x08\x09", want: vendor.FrameHeader{
			Length: 66051, Type: 4, Flags: 5, StreamID: 101124105,
		}},
		// Ignore high bit:
		{in: "\xff\xff\xff" + "\xff" + "\xff" + "\xff\xff\xff\xff", want: vendor.FrameHeader{
			Length: 16777215, Type: 255, Flags: 255, StreamID: 2147483647}},
		{in: "\xff\xff\xff" + "\xff" + "\xff" + "\x7f\xff\xff\xff", want: vendor.FrameHeader{
			Length: 16777215, Type: 255, Flags: 255, StreamID: 2147483647}},
	}
	for i, tt := range tests {
		got, err := vendor.readFrameHeader(make([]byte, 9), strings.NewReader(tt.in))
		if err != nil {
...
func TestReadWriteFrameHeader(t *testing.T) {
	...
	for _, tt := range tests {
		...
		fr.endWrite()
		fh, err := vendor.ReadFrameHeader(buf)
		if err != nil {
...
func TestWriteGoAway(t *testing.T) {
	...
	if got := string(f.(*vendor.GoAwayFrame).DebugData()); got != debug {
...
func TestWritePushPromise(t *testing.T) {
	...
	_, ok := f.(*vendor.PushPromiseFrame)
	if !ok {
		t.Fatalf("got %T; want *PushPromiseFrame", f)
	}
	want := &vendor.PushPromiseFrame{
		FrameHeader: vendor.FrameHeader{
			valid:    true,
			Type:     0x5,
			Flags:    0x0,
...
func TestReadFrameOrder(t *testing.T) {
	head := func(f *vendor.Framer, id uint32, end bool) {
		f.WriteHeaders(vendor.HeadersFrameParam{
			StreamID:      id,
			BlockFragment: []byte("foo"), // unused, but non-empty
			EndHeaders:    end,
		})
	}
	cont := func(f *vendor.Framer, id uint32, end bool) {
		f.WriteContinuation(id, end, []byte("foo"))
	}

	tests := [...]struct {
		name    string
		w       func(*vendor.Framer)
		atLeast int
		wantErr string
	}{
		0: {
			w: func(f *vendor.Framer) {
				head(f, 1, true)
			},
		},
		1: {
	...
	for i, tt := range tests {
		...
		if !ok && err != vendor.ConnectionError(vendor.ErrCodeProtocol) {
...
func TestMetaFrameHeader(t *testing.T) {
	write := func(f *vendor.Framer, frags ...[]byte) {
	...
	want := func(flags vendor.Flags, length uint32, pairs ...string) *vendor.MetaHeadersFrame {
		mh := &vendor.MetaHeadersFrame{
			HeadersFrame: &vendor.HeadersFrame{
				FrameHeader: vendor.FrameHeader{
					Type:     vendor.FrameHeaders,
					Flags:    flags,
					Length:   length,
			...
			Fields: []vendor.HeaderField(nil),
		}
		for len(pairs) > 0 {
			mh.Fields = append(mh.Fields, vendor.HeaderField{
				Name:  pairs[0],
				Value: pairs[1],
			})
			pairs = pairs[2:]
	...
	truncated := func(mh *vendor.MetaHeadersFrame) *vendor.MetaHeadersFrame {
		mh.Truncated = true
		return mh
	}

	const noFlags vendor.Flags = 0
	...
	tests := [...]struct {
		...
		0: {
			name: "single_headers",
			w: func(f *vendor.Framer) {
				var he vendor.hpackEncoder
				all := he.encodeHeaderRaw(t, ":method", "GET", ":path", "/")
				write(f, all)
			},
			want: want(vendor.FlagHeadersEndHeaders, 2, ":method", "GET", ":path", "/"),
		},
		1: {
		...
		4: {
			...
			want:              vendor.ConnectionError(vendor.ErrCodeCompression),
		},
		5: {
		...
		6: {
			...
			want:          vendor.streamError(1, vendor.ErrCodeProtocol),
		...
		10: {
			...
			w:    func(f *vendor.Framer) { write(f, encodeHeaderRaw(t, "foo", "bar")) },
			want: want(vendor.FlagHeadersEndHeaders, 8, "foo", "bar"),
		},
		11: {
			name:          "invalid_field_name",
			w:             func(f *vendor.Framer) { write(f, encodeHeaderRaw(t, "CapitalBad", "x")) },
			want:          vendor.streamError(1, vendor.ErrCodeProtocol),
			wantErrReason: "invalid header field name \"CapitalBad\"",
		},
		12: {
			name:          "invalid_field_value",
			w:             func(f *vendor.Framer) { write(f, encodeHeaderRaw(t, "key", "bad_null\x00")) },
			want:          vendor.streamError(1, vendor.ErrCodeProtocol),
	...
	for i, tt := range tests {
		buf := new(bytes.Buffer)
		f := vendor.NewFramer(buf, buf)
		f.ReadMetaHeaders = vendor.NewDecoder(vendor.initialHeaderTableSize, nil)
		...
		if err != nil {
			...
			if se, ok := err.(vendor.StreamError); ok && se.Cause != nil && se.Cause.Error() == tt.wantErrReason {
				se.Cause = nil
				got = se
			}
		}
		if !reflect.DeepEqual(got, tt.want) {
			if mhg, ok := got.(*vendor.MetaHeadersFrame); ok {
				if mhw, ok := tt.want.(*vendor.MetaHeadersFrame); ok {
					hg := mhg.HeadersFrame
					hw := mhw.HeadersFrame
...
func readAndVerifyDataFrame(data string, length byte, fr *vendor.Framer, buf *bytes.Buffer, t *testing.T) *vendor.DataFrame {
	var streamID uint32 = 1<<24 + 2<<16 + 3<<8 + 4
...
func TestSettingsDuplicates(t *testing.T) {
	tests := []struct {
		settings []vendor.Setting
		want     bool
	}{
		{nil, false},
		{[]vendor.Setting{{ID: 1}}, false},
		{[]vendor.Setting{{ID: 1}, {ID: 2}}, false},
		{[]vendor.Setting{{ID: 1}, {ID: 2}}, false},
		{[]vendor.Setting{{ID: 1}, {ID: 2}, {ID: 3}}, false},
		{[]vendor.Setting{{ID: 1}, {ID: 2}, {ID: 3}}, false},
		{[]vendor.Setting{{ID: 1}, {ID: 2}, {ID: 3}, {ID: 4}}, false},

		{[]vendor.Setting{{ID: 1}, {ID: 2}, {ID: 3}, {ID: 2}}, true},
		{[]vendor.Setting{{ID: 4}, {ID: 2}, {ID: 3}, {ID: 4}}, true},
