...
func serveProdTLS(autocertManager *autocert.Manager) error {
	...
	vendor.ConfigureServer(srv, &vendor.Server{
		NewWriteScheduler: func() vendor.WriteScheduler {
			return vendor.NewPriorityWriteScheduler(nil)
		},
	})
...
func main() {
	var srv http.Server
	flag.BoolVar(&vendor.VerboseLogs, "verbose", false, "Verbose HTTP/2 debugging.")
	flag.Parse()
	srv.Addr = *httpsAddr
