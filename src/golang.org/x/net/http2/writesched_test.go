...
func makeWriteNonStreamRequest() vendor.FrameWriteRequest {
	return vendor.FrameWriteRequest{vendor.writeSettingsAck{}, nil, nil}
}

func makeWriteHeadersRequest(streamID uint32) vendor.FrameWriteRequest {
	st := &vendor.stream{id: streamID}
	return vendor.FrameWriteRequest{&vendor.writeResHeaders{streamID: streamID, httpResCode: 200}, st, nil}
}

func checkConsume(wr vendor.FrameWriteRequest, nbytes int32, want []vendor.FrameWriteRequest) error {
	consumed, rest, n := wr.Consume(nbytes)
	var wantConsumed, wantRest vendor.FrameWriteRequest
	switch len(want) {
	case 0:
	case 1:
		wantConsumed = want[0]
...
func TestFrameWriteRequestNonData(t *testing.T) {
	...
	if err := checkConsume(wr, 0, []vendor.FrameWriteRequest{wr}); err != nil {
		t.Errorf("Consume:\n%v", err)
	}
}
...
func TestFrameWriteRequestData(t *testing.T) {
	...
	const size = 32
	wr := vendor.FrameWriteRequest{&vendor.writeData{st.id, make([]byte, size), true}, st, make(chan error)}
	if got, want := wr.DataSize(), size; got != want {
		t.Errorf("DataSize: got %v, want %v", got, want)
	}

	// No flow-control bytes available: cannot consume anything.
	if err := checkConsume(wr, math.MaxInt32, []vendor.FrameWriteRequest{}); err != nil {
	...
	st.flow.add(size)
	want := []vendor.FrameWriteRequest{
		{
			write:  &vendor.writeData{st.id, make([]byte, st.sc.maxFrameSize), false},
			stream: st,
			done:   nil,
		},
		{
			write:  &vendor.writeData{st.id, make([]byte, size-st.sc.maxFrameSize), true},
			stream: st,
			done:   wr.done,
		},
	}
	...
	want = []vendor.FrameWriteRequest{
		{
			write:  &vendor.writeData{st.id, make([]byte, 8), false},
			stream: st,
			done:   nil,
		},
		{
			write:  &vendor.writeData{st.id, make([]byte, size-st.sc.maxFrameSize-8), true},
			stream: st,
			done:   wr.done,
		},
	}
...
func TestFrameWriteRequest_StreamID(t *testing.T) {
	const streamID = 123
	wr := vendor.FrameWriteRequest{write: vendor.streamError(streamID, vendor.ErrCodeNo)}
