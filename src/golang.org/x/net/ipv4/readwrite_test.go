...
func BenchmarkReadWriteUnicast(b *testing.B) {
	...
	b.Run("IPv4UDP", func(b *testing.B) {
		...
		cf := vendor.FlagTTL | vendor.FlagInterface
		if err := p.SetControlMessage(cf, true); err != nil {
			b.Fatal(err)
		}
		cm := vendor.ControlMessage{TTL: 1}
		ifi, _ := vendor.RoutedInterface("ip4", net.FlagUp|net.FlagLoopback)
		if ifi != nil {
...
func BenchmarkPacketConnReadWriteUnicast(b *testing.B) {
	...
	iph, err := (&vendor.Header{
		Version:  vendor.Version,
		Len:      vendor.HeaderLen,
		TotalLen: vendor.HeaderLen + len(payload),
		TTL:      1,
		Protocol: vendor.ProtocolReserved,
	...
	b.Run("IP", func(b *testing.B) {
		...
		c, err := net.ListenPacket(fmt.Sprintf("ip4:%d", vendor.ProtocolGRE), "127.0.0.1")
		if err != nil {
			b.Skipf("not supported on %s/%s: %v", runtime.GOOS, runtime.GOARCH, err)
		}
		defer c.Close()
...
func TestPacketConnConcurrentReadWriteUnicastUDP(t *testing.T) {
	...
	dst := c.LocalAddr()
	ifi, _ := vendor.RoutedInterface("ip4", net.FlagUp|net.FlagLoopback)
	cf := vendor.FlagTTL | vendor.FlagSrc | vendor.FlagDst | vendor.FlagInterface
