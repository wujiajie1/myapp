...
func ExamplePacketConn_servingOneShotMulticastDNS() {
	...
	if err := p.SetControlMessage(vendor.FlagDst, true); err != nil {
		log.Fatal(err)
	}
...
func ExamplePacketConn_tracingIPPacketRoute() {
	...
	if err := p.SetControlMessage(vendor.FlagTTL|vendor.FlagSrc|vendor.FlagDst|vendor.FlagInterface, true); err != nil {
		log.Fatal(err)
	}
	wm := vendor.Message{
	...
	for i := 1; i <= 64; i++ { // up to 64 hops
		...
		rm, err := vendor.ParseMessage(1, rb[:n])
		if err != nil {
			log.Fatal(err)
		}
...
func ExampleRawConn_advertisingOSPFHello() {
	...
	iph := &vendor.Header{
		...
		TOS:      0xc0, // DSCP CS6
		TotalLen: vendor.HeaderLen + len(ospf),
		TTL:      1,
		Protocol: 89,
	...
	switch runtime.GOOS {
	case "darwin", "linux":
		cm = &vendor.ControlMessage{IfIndex: en0.Index}
	default:
