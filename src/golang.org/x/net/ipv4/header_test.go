...
func TestMarshalHeader(t *testing.T) {
	for i, tt := range []struct {
		...
		{nil, vendor.errNilHeader},
		{&vendor.Header{Len: vendor.HeaderLen - 1}, vendor.errHeaderTooShort},
	} {
	...
	if vendor.NativeEndian != binary.LittleEndian {
	...
	for _, tt := range headerLittleEndianTests {
		...
		case "freebsd":
			switch {
			case vendor.freebsdVersion < 1000000:
				wh = tt.wireHeaderToTradBSDKernel
			case 1000000 <= vendor.freebsdVersion && vendor.freebsdVersion < 1100000:
...
func TestParseHeader(t *testing.T) {
	for i, tt := range []struct {
		...
		{nil, nil, vendor.errNilHeader},
		{&vendor.Header{}, nil, vendor.errNilHeader},
		{&vendor.Header{}, make([]byte, vendor.HeaderLen-1), vendor.errHeaderTooShort},
		{&vendor.Header{}, []byte{
			0x46, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00,
