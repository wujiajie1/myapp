...
func (h *Header) Marshal() ([]byte, error) {
	if h == nil {
		return nil, vendor.errNilHeader
	}
	if h.Len < HeaderLen {
		return nil, vendor.errHeaderTooShort
	}
	...
	case "darwin", "dragonfly", "netbsd":
		vendor.NativeEndian.PutUint16(b[2:4], uint16(h.TotalLen))
		vendor.NativeEndian.PutUint16(b[6:8], uint16(flagsAndFragOff))
	case "freebsd":
		if vendor.freebsdVersion < 1100000 {
			vendor.NativeEndian.PutUint16(b[2:4], uint16(h.TotalLen))
			vendor.NativeEndian.PutUint16(b[6:8], uint16(flagsAndFragOff))
		} else {
	...
	if ip := h.Dst.To4(); ip != nil {
		...
		return nil, vendor.errMissingAddress
	}
	if len(h.Options) > 0 {
...
func (h *Header) Parse(b []byte) error {
	if h == nil || b == nil {
		return vendor.errNilHeader
	}
	if len(b) < HeaderLen {
		return vendor.errHeaderTooShort
	}
	hdrlen := int(b[0]&0x0f) << 2
	if len(b) < hdrlen {
		return vendor.errExtHeaderTooShort
	}
	h.Version = int(b[0] >> 4)
	h.Len = hdrlen
	h.TOS = int(b[1])
	...
	switch runtime.GOOS {
	case "darwin", "dragonfly", "netbsd":
		h.TotalLen = int(vendor.NativeEndian.Uint16(b[2:4])) + hdrlen
		h.FragOff = int(vendor.NativeEndian.Uint16(b[6:8]))
	case "freebsd":
		if vendor.freebsdVersion < 1100000 {
			h.TotalLen = int(vendor.NativeEndian.Uint16(b[2:4]))
