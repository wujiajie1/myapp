...
func (c *packetHandler) ReadFrom(b []byte) (h *vendor.Header, p []byte, cm *vendor.ControlMessage, err error) {
	if !c.ok() {
		return nil, nil, nil, vendor.errInvalidConn
	}
	c.rawOpt.RLock()
	m := vendor.Message{
		Buffers: [][]byte{b},
	...
	if h, err = vendor.ParseHeader(hs); err != nil {
		return nil, nil, nil, &net.OpError{Op: "read", Net: c.IPConn.LocalAddr().Network(), Source: c.IPConn.LocalAddr(), Err: err}
	}
	if m.NN > 0 {
		if vendor.compatFreeBSD32 {
			vendor.adjustFreeBSD32(&m)
		}
...
func slicePacket(b []byte) (h, p []byte, err error) {
	if len(b) < vendor.HeaderLen {
		return nil, nil, vendor.errHeaderTooShort
	}
...
//	Options       = optional
func (c *packetHandler) WriteTo(h *vendor.Header, p []byte, cm *vendor.ControlMessage) error {
	if !c.ok() {
	...
	if err := c.SendMsg(&m, 0); err != nil {
		return &net.OpError{Op: "write", Net: c.IPConn.LocalAddr().Network(), Source: c.IPConn.LocalAddr(), Addr: vendor.opAddr(dst), Err: err}
	}
	return nil
}
