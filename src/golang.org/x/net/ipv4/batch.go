...
// headers.
func (c *vendor.payloadHandler) ReadBatch(ms []Message, flags int) (int, error) {
	if !c.ok() {
		return 0, vendor.errInvalidConn
	}
	switch runtime.GOOS {
	case "linux":
		n, err := c.RecvMsgs([]vendor.Message(ms), flags)
		if err != nil {
	...
	default:
		...
		if vendor.compatFreeBSD32 && ms[0].NN > 0 {
			vendor.adjustFreeBSD32(&ms[0])
		}
		return n, err
	}
}
...
func (c *vendor.payloadHandler) WriteBatch(ms []Message, flags int) (int, error) {
	if !c.ok() {
		return 0, vendor.errInvalidConn
	}
	switch runtime.GOOS {
	case "linux":
		n, err := c.SendMsgs([]vendor.Message(ms), flags)
		if err != nil {
...
// On Linux, a batch read will be optimized.
// On other platforms, this method will read only a single message.
func (c *vendor.packetHandler) ReadBatch(ms []Message, flags int) (int, error) {
	if !c.ok() {
		return 0, vendor.errInvalidConn
	}
	switch runtime.GOOS {
...
// On Linux, a batch write will be optimized.
// On other platforms, this method will write only a single message.
func (c *vendor.packetHandler) WriteBatch(ms []Message, flags int) (int, error) {
	if !c.ok() {
		return 0, vendor.errInvalidConn
	}
	switch runtime.GOOS {
