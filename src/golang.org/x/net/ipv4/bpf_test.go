...
func TestBPF(t *testing.T) {
	...
	p := vendor.NewPacketConn(l)

	// This filter accepts UDP packets whose first payload byte is
	// even.
	prog, err := vendor.Assemble([]vendor.Instruction{
		// Load the first byte of the payload (skipping UDP header).
		vendor.LoadAbsolute{Off: 8, Size: 1},
		// Select LSB of the byte.
		vendor.ALUOpConstant{Op: vendor.ALUOpAnd, Val: 1},
		// Byte is even?
		vendor.JumpIf{Cond: vendor.JumpEqual, Val: 0, SkipFalse: 1},
		// Accept.
		vendor.RetConstant{Val: 4096},
		// Ignore.
