...
func TestPacketConnReadWriteMulticastUDP(t *testing.T) {
	...
	ifi, err := vendor.RoutedInterface("ip4", net.FlagUp|net.FlagMulticast|net.FlagLoopback)
	if err != nil {
	...
	for _, tt := range packetConnReadWriteMulticastICMPTests {
		...
		for i, toggle := range []bool{true, false, true} {
			...
			if n, _, _, err := p.ReadFrom(rb); err != nil {
				...
				switch {
				case m.Type == vendor.ICMPTypeEchoReply && m.Code == 0: // net.inet.icmp.bmcastecho=1
				case m.Type == vendor.ICMPTypeEcho && m.Code == 0: // net.inet.icmp.bmcastecho=0
				default:
	...
	for _, tt := range rawConnReadWriteMulticastICMPTests {
		...
		for i, toggle := range []bool{true, false, true} {
			...
			if rh, b, _, err := r.ReadFrom(rb); err != nil {
				...
				switch {
				case (rh.Dst.IsLoopback() || rh.Dst.IsLinkLocalUnicast() || rh.Dst.IsGlobalUnicast()) && m.Type == vendor.ICMPTypeEchoReply && m.Code == 0: // net.inet.icmp.bmcastecho=1
				case rh.Dst.IsMulticast() && m.Type == vendor.ICMPTypeEcho && m.Code == 0: // net.inet.icmp.bmcastecho=0
				default:
					t.Fatalf("got type=%v, code=%v; want type=%v, code=%v", m.Type, m.Code, vendor.ICMPTypeEchoReply, 0)
				}
			}
