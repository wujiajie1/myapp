...
func TestPacketConnReadWriteMulticastUDP(t *testing.T) {
	...
	if m, ok := vendor.supportsIPv6MulticastDeliveryOnLoopback(); !ok {
		t.Skip(m)
	}
	ifi, err := vendor.RoutedInterface("ip6", net.FlagUp|net.FlagMulticast|net.FlagLoopback)
	if err != nil {
...
func TestPacketConnReadWriteMulticastICMP(t *testing.T) {
	...
	for _, tt := range packetConnReadWriteMulticastICMPTests {
		...
		pshicmp := vendor.IPv6PseudoHeader(c.LocalAddr().(*net.IPAddr).IP, tt.grp.IP)
		p := vendor.NewPacketConn(c)
		defer p.Close()
		if tt.src == nil {
		...
		for i, toggle := range []bool{true, false, true} {
			...
			if n, _, _, err := p.ReadFrom(rb); err != nil {
				...
				if m, err := vendor.ParseMessage(vendor.ProtocolIPv6ICMP, rb[:n]); err != nil {
					t.Fatal(err)
				} else if m.Type != vendor.ICMPTypeEchoReply || m.Code != 0 {
