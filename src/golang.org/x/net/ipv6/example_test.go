...
func ExamplePacketConn_servingOneShotMulticastDNS() {
	...
	if err := p.SetControlMessage(vendor.FlagDst|vendor.FlagInterface, true); err != nil {
		log.Fatal(err)
	}

	var wcm vendor.ControlMessage
	b := make([]byte, 1500)
	for {
...
func ExamplePacketConn_tracingIPPacketRoute() {
	...
	if err := p.SetControlMessage(vendor.FlagHopLimit|vendor.FlagSrc|vendor.FlagDst|vendor.FlagInterface, true); err != nil {
		log.Fatal(err)
	}
	wm := vendor.Message{
	...
	for i := 1; i <= 64; i++ { // up to 64 hops
		...
		rm, err := vendor.ParseMessage(58, rb[:n])
		if err != nil {
			log.Fatal(err)
		}
