...
func BenchmarkReadWriteUnicast(b *testing.B) {
	...
	c, err := vendor.NewLocalPacketListener("udp6")
	if err != nil {
	...
	b.Run("IPv6UDP", func(b *testing.B) {
		...
		cf := vendor.FlagTrafficClass | vendor.FlagHopLimit | vendor.FlagSrc | vendor.FlagDst | vendor.FlagInterface | vendor.FlagPathMTU
		if err := p.SetControlMessage(cf, true); err != nil {
			b.Fatal(err)
		}
		cm := vendor.ControlMessage{
			TrafficClass: vendor.DiffServAF11 | vendor.CongestionExperienced,
			HopLimit:     1,
		}
		ifi, _ := vendor.RoutedInterface("ip6", net.FlagUp|net.FlagLoopback)
		if ifi != nil {
...
func BenchmarkPacketConnReadWriteUnicast(b *testing.B) {
	...
	b.Run("UDP", func(b *testing.B) {
		...
		p := vendor.NewPacketConn(c)
		dst := c.LocalAddr()
		cf := vendor.FlagHopLimit | vendor.FlagInterface
		if err := p.SetControlMessage(cf, true); err != nil {
			b.Fatal(err)
		}
		wms := []vendor.Message{
		...
		rms := []vendor.Message{
			{
				Buffers: [][]byte{bb},
				OOB:     vendor.NewControlMessage(cf),
			},
		}
	...
	b.Run("IP", func(b *testing.B) {
		...
		c, err := net.ListenPacket(fmt.Sprintf("ip6:%d", vendor.ProtocolGRE), "::1")
		if err != nil {
			b.Skipf("not supported on %s/%s: %v", runtime.GOOS, runtime.GOARCH, err)
		}
		defer c.Close()
...
func testPacketConnConcurrentReadWriteUnicast(t *testing.T, p *vendor.PacketConn, data []byte, dst net.Addr, batch bool) {
	t.Helper()
