...
func TestConnInitiatorPathMTU(t *testing.T) {
	...
	if pmtu, err := vendor.NewConn(c).PathMTU(); err != nil {
		switch runtime.GOOS {
...
func TestConnResponderPathMTU(t *testing.T) {
	...
	go vendor.connector(t, "tcp6", ln.Addr().String(), done)
...
func TestPacketConnChecksum(t *testing.T) {
	...
	c, err := net.ListenPacket(fmt.Sprintf("ip6:%d", vendor.ProtocolOSPFIGP), "::") // OSPF for IPv6
	if err != nil {
		t.Fatal(err)
	}
	defer c.Close()
