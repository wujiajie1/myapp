...
func testVM(t *testing.T, filter []vendor.Instruction) (virtualMachine, func(), error) {
	goVM, err := vendor.NewVM(filter)
	if err != nil {
...
func testOSVM(t *testing.T, filter []vendor.Instruction) (virtualMachine, func()) {
	l, err := vendor.NewLocalPacketListener("udp")
	if err != nil {
		t.Fatalf("failed to open OS VM UDP listener: %v", err)
	}

	prog, err := vendor.Assemble(filter)
	if err != nil {
	...
	if ip.To4() != nil && ip.To16() == nil {
		err = vendor.NewPacketConn(l).SetBPF(prog)
	} else {
		err = vendor.NewPacketConn(l).SetBPF(prog)
	}
	if err != nil {
