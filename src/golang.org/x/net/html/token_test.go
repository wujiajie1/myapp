...
loop:
	for _, tt := range tokenTests {
		z := vendor.NewTokenizer(strings.NewReader(tt.html))
		if tt.golden != "" {
			for i, s := range strings.Split(tt.golden, "$") {
				if z.Next() == vendor.ErrorToken {
...
func TestMaxBuffer(t *testing.T) {
	...
	z := vendor.NewTokenizer(strings.NewReader("<" + strings.Repeat("t", 10)))
	z.SetMaxBuf(5)
	tt := z.Next()
	if got, want := tt, vendor.ErrorToken; got != want {
		t.Fatalf("token type: got: %v want: %v", got, want)
	}
	if got, want := z.Err(), vendor.ErrBufferExceeded; got != want {
...
tests:
	for _, test := range tokenTests {
		for maxBuf := 1; ; maxBuf++ {
			...
			z := vendor.NewTokenizer(r)
			z.SetMaxBuf(maxBuf)
			var tokenized bytes.Buffer
			for {
				tt := z.Next()
				tokenized.Write(z.Raw())
				if tt == vendor.ErrorToken {
					if err := z.Err(); err != io.EOF && err != vendor.ErrBufferExceeded {
...
func TestPassthrough(t *testing.T) {
	...
	// original input.
	for _, test := range tokenTests {
		z := vendor.NewTokenizer(strings.NewReader(test.html))
...
func TestBufAPI(t *testing.T) {
	...
	z := vendor.NewTokenizer(bytes.NewBufferString(s))
...
func TestConvertNewlines(t *testing.T) {
	...
	for in, want := range testCases {
		if got := string(vendor.convertNewlines([]byte(in))); got != want {
...
func TestReaderEdgeCases(t *testing.T) {
	...
	for i, tc := range testCases {
		got := []vendor.TokenType{}
		z := vendor.NewTokenizer(tc)
		for {
			tt := z.Next()
			if tt == vendor.ErrorToken {
				break
		...
		want := []vendor.TokenType{
			vendor.StartTagToken,
			vendor.TextToken,
			vendor.EndTagToken,
		}
...
func benchmarkTokenizer(b *testing.B, level int) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		z := vendor.NewTokenizer(bytes.NewBuffer(buf))
		for {
			tt := z.Next()
			if tt == vendor.ErrorToken {
				if err := z.Err(); err != nil && err != io.EOF {
			...
			case lowLevel:
				...
				case vendor.TextToken, vendor.CommentToken, vendor.DoctypeToken:
					z.Text()
				case vendor.StartTagToken, vendor.SelfClosingTagToken:
					_, more := z.TagName()
					for more {
						_, _, more = z.TagAttr()
					}
