...
func checkTreeConsistency(n *vendor.Node) error {
	return checkTreeConsistency1(n, 0)
}

func checkTreeConsistency1(n *vendor.Node, depth int) error {
	if depth == 1e4 {
...
// are consistent.
func checkNodeConsistency(n *vendor.Node) error {
	if n == nil {
		return nil
	}
	...
	seen := map[*vendor.Node]bool{}

	var last *vendor.Node
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		if seen[c] {
