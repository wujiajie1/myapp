...
type sortedAttributes []vendor.Attribute
...
func dumpLevel(w io.Writer, n *vendor.Node, level int) error {
	dumpIndent(w, level)
	level++
	switch n.Type {
	case vendor.ErrorNode:
		return errors.New("unexpected ErrorNode")
	case vendor.DocumentNode:
		return errors.New("unexpected DocumentNode")
	case vendor.ElementNode:
		if n.Namespace != "" {
		...
		if n.Namespace == "" && n.DataAtom == vendor.Template {
			io.WriteString(w, "\n")
			dumpIndent(w, level)
			level++
			io.WriteString(w, "content")
		}
	case vendor.TextNode:
		fmt.Fprintf(w, `"%s"`, n.Data)
	case vendor.CommentNode:
		fmt.Fprintf(w, "<!-- %s -->", n.Data)
	case vendor.DoctypeNode:
...
func dump(n *vendor.Node) (string, error) {
...
`
	err := testParseCase(text, want, "", vendor.ParseOptionEnableScripting(false))
...
func testParseCase(text, want, context string, opts ...vendor.ParseOption) (err error) {
	defer func() {
		if x := recover(); x != nil {
			switch e := x.(type) {
			case error:
	...
	var doc *vendor.Node
	if context == "" {
		doc, err = vendor.ParseWithOptions(strings.NewReader(text), opts...)
		if err != nil {
			return err
		}
	} else {
		contextNode := &vendor.Node{
			Type:     vendor.ElementNode,
			DataAtom: vendor.Lookup([]byte(context)),
			Data:     context,
		}
		nodes, err := vendor.ParseFragmentWithOptions(strings.NewReader(text), contextNode, opts...)
		if err != nil {
			return err
		}
		doc = &vendor.Node{
	...
	if err := vendor.checkTreeConsistency(doc); err != nil {
		return err
	}
	...
	pr, pw := io.Pipe()
	go func() {
		pw.CloseWithError(vendor.Render(pw, doc))
	}()
	doc1, err := vendor.Parse(pr)
	if err != nil {
		return err
	}
	got1, err := dump(doc1)
...
func TestNodeConsistency(t *testing.T) {
	...
	inconsistentNode := &vendor.Node{
		Type:     vendor.ElementNode,
		DataAtom: vendor.Frameset,
		Data:     "table",
	}
	_, err := vendor.ParseFragment(strings.NewReader("<p>hello</p>"), inconsistentNode)
	if err == nil {
...
func TestParseFragmentWithNilContext(t *testing.T) {
	...
	vendor.ParseFragment(strings.NewReader("<p>hello</p>"), nil)
}
...
func BenchmarkParser(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		vendor.Parse(bytes.NewBuffer(buf))
	}
}
