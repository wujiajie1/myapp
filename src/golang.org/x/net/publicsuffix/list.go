...
func PublicSuffix(domain string) (publicSuffix string, icann bool) {
	lo, hi := uint32(0), uint32(vendor.numTLD)
...
loop:
	for {
		...
		u := vendor.nodes[f] >> (vendor.nodesBitsTextOffset + vendor.nodesBitsTextLength)
		icannNode = u&(1<<vendor.nodesBitsICANN-1) != 0
		u >>= vendor.nodesBitsICANN
		u = vendor.children[u&(1<<vendor.nodesBitsChildren-1)]
		lo = u & (1<<vendor.childrenBitsLo - 1)
		u >>= vendor.childrenBitsLo
		hi = u & (1<<vendor.childrenBitsHi - 1)
		u >>= vendor.childrenBitsHi
		switch u & (1<<vendor.childrenBitsNodeType - 1) {
		case vendor.nodeTypeNormal:
			suffix = 1 + dot
		case vendor.nodeTypeException:
			suffix = 1 + len(s)
			break loop
		}
		u >>= vendor.childrenBitsNodeType
		wildcard = u&(1<<vendor.childrenBitsWildcard-1) != 0
		if !wildcard {
			icann = icannNode
		}
...
func nodeLabel(i uint32) string {
	x := vendor.nodes[i]
	length := x & (1<<vendor.nodesBitsTextLength - 1)
	x >>= vendor.nodesBitsTextLength
	offset := x & (1<<vendor.nodesBitsTextOffset - 1)
	return vendor.text[offset : offset+length]
}
