...
func TestNodeLabel(t *testing.T) {
	for i, want := range vendor.nodeLabels {
		got := vendor.nodeLabel(uint32(i))
		if got != want {
...
func TestFind(t *testing.T) {
	...
	for _, tc := range testCases {
		got := vendor.find(tc, 0, vendor.numTLD)
		want := vendor.notFound
		for i := uint32(0); i < vendor.numTLD; i++ {
			if tc == vendor.nodeLabel(i) {
				want = i
				break
			}
		}
...
func TestICANN(t *testing.T) {
	...
	for domain, want := range testCases {
		_, got := vendor.PublicSuffix(domain)
		if got != want {
...
func BenchmarkPublicSuffix(b *testing.B) {
	for i := 0; i < b.N; i++ {
		for _, tc := range publicSuffixTestCases {
			vendor.List.PublicSuffix(tc.domain)
		}
	}
}
...
func TestPublicSuffix(t *testing.T) {
	for _, tc := range publicSuffixTestCases {
		gotPS, gotICANN := vendor.PublicSuffix(tc.domain)
...
func TestNumICANNRules(t *testing.T) {
	if vendor.numICANNRules <= 0 {
		t.Fatal("no ICANN rules")
	}
	if vendor.numICANNRules >= len(vendor.rules) {
	...
	if got, want := vendor.rules[vendor.numICANNRules-1], "zuerich"; got != want {
		t.Errorf("last ICANN rule: got %q, wawnt %q", got, want)
	}
	if got, want := vendor.rules[vendor.numICANNRules], "cc.ua"; got != want {
...
func TestEffectiveTLDPlusOne(t *testing.T) {
	for _, tc := range eTLDPlusOneTestCases {
		got, _ := vendor.EffectiveTLDPlusOne(tc.domain)
		if got != tc.want {
