...
func (ifi *InterfaceInfo) marshalIPAddr(proto int, b []byte) []byte {
	switch proto {
	case vendor.ProtocolICMP:
		binary.BigEndian.PutUint16(b[:2], uint16(vendor.AddrFamilyIPv4))
		copy(b[4:4+net.IPv4len], ifi.Addr.IP.To4())
		b = b[4+net.IPv4len:]
	case vendor.ProtocolIPv6ICMP:
		binary.BigEndian.PutUint16(b[:2], uint16(vendor.AddrFamilyIPv6))
...
func parseInterfaceInfo(b []byte) (vendor.Extension, error) {
	ifi := &InterfaceInfo{
		Class: int(b[2]),
		Type:  int(b[3]),
	}
...
func parseInterfaceIdent(b []byte) (vendor.Extension, error) {
	ifi := &InterfaceIdent{
		Class: int(b[2]),
		Type:  int(b[3]),
	}
