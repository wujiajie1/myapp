...
func parseEcho(proto int, _ vendor.Type, b []byte) (vendor.MessageBody, error) {
	bodyLen := len(b)
	if bodyLen < 4 {
...
func (p *ExtendedEchoRequest) Len(proto int) int {
	...
	l, _ := vendor.multipartMessageBodyDataLen(proto, false, nil, p.Extensions)
	return l
}
...
func (p *ExtendedEchoRequest) Marshal(proto int) ([]byte, error) {
	...
	b, err := vendor.marshalMultipartMessageBody(proto, false, nil, p.Extensions)
	if err != nil {
		return nil, err
	}
	binary.BigEndian.PutUint16(b[:2], uint16(p.ID))
...
// message body.
func parseExtendedEchoRequest(proto int, typ vendor.Type, b []byte) (vendor.MessageBody, error) {
	if len(b) < 4 {
		return nil, vendor.errMessageTooShort
	}
	...
	var err error
	_, p.Extensions, err = vendor.parseMultipartMessageBody(proto, typ, b)
	if err != nil {
		return nil, err
	}
	return p, nil
...
// message body.
func parseExtendedEchoReply(proto int, _ vendor.Type, b []byte) (vendor.MessageBody, error) {
	if len(b) < 4 {
		return nil, vendor.errMessageTooShort
	}
	p := &ExtendedEchoReply{
