...
func parseExtensions(typ vendor.Type, b []byte, l int) ([]Extension, int, error) {
	...
	case vendor.ICMPTypeExtendedEchoRequest, vendor.ICMPTypeExtendedEchoRequest:
		if len(b) < 8 || !validExtensionHeader(b) {
			return nil, -1, vendor.errNoExtension
		}
		l = 0
	default:
		if 128 > l || l+8 > len(b) {
	...
	for b = b[l+4:]; len(b) >= 4; {
		...
		switch b[2] {
		case vendor.classMPLSLabelStack:
			ext, err := vendor.parseMPLSLabelStack(b[:ol])
			if err != nil {
				return nil, -1, err
			}
			exts = append(exts, ext)
		case vendor.classInterfaceInfo:
			ext, err := vendor.parseInterfaceInfo(b[:ol])
			if err != nil {
				return nil, -1, err
			}
			exts = append(exts, ext)
		case vendor.classInterfaceIdent:
			ext, err := vendor.parseInterfaceIdent(b[:ol])
			if err != nil {
				return nil, -1, err
			}
			exts = append(exts, ext)
...
func validExtensions(typ vendor.Type, exts []Extension) bool {
	switch typ {
	case vendor.ICMPTypeDestinationUnreachable, vendor.ICMPTypeTimeExceeded, vendor.ICMPTypeParameterProblem,
		vendor.ICMPTypeDestinationUnreachable, vendor.ICMPTypeTimeExceeded:
		for i := range exts {
			switch exts[i].(type) {
			case *vendor.MPLSLabelStack, *vendor.InterfaceInfo, *RawExtension:
			default:
				return false
			}
		}
