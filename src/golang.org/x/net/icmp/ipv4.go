...
func ParseIPv4Header(b []byte) (*vendor.Header, error) {
	if len(b) < vendor.HeaderLen {
		return nil, vendor.errHeaderTooShort
	}
	hdrlen := int(b[0]&0x0f) << 2
	if hdrlen > len(b) {
		return nil, vendor.errBufferTooShort
	}
	h := &vendor.Header{
		Version:  int(b[0] >> 4),
		Len:      hdrlen,
	...
	h.Flags = vendor.HeaderFlags(h.FragOff&0xe000) >> 13
	h.FragOff = h.FragOff & 0x1fff
	if hdrlen-vendor.HeaderLen > 0 {
		h.Options = make([]byte, hdrlen-vendor.HeaderLen)
		copy(h.Options, b[vendor.HeaderLen:])
	}
	return h, nil
}
