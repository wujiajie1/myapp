...
type PacketConn struct {
	c  net.PacketConn
	p4 *vendor.PacketConn
	p6 *vendor.PacketConn
}
...
func (c *PacketConn) IPv4PacketConn() *vendor.PacketConn {
	if !c.ok() {
		return nil
	}
	return c.p4
...
func (c *PacketConn) IPv6PacketConn() *vendor.PacketConn {
	if !c.ok() {
		return nil
	}
	return c.p6
...
func (c *PacketConn) ReadFrom(b []byte) (int, net.Addr, error) {
	if !c.ok() {
		return 0, nil, vendor.errInvalidConn
	}
