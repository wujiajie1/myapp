...
func TestSecWebSocketAccept(t *testing.T) {
	...
	accept, err := vendor.getNonceAccept(nonce)
	if err != nil {
...
`))
		...
		config.Version = vendor.ProtocolVersionHybi13
		config.handshakeData = map[string]string{
			"key": "dGhlIHNhbXBsZSBub25jZQ==",
		}
		if err := vendor.hybiClientHandshake(&config, br, bw); err != nil {
		...
		var expectedHeader = map[string]string{
			...
			"Sec-Websocket-Version":  fmt.Sprintf("%d", vendor.ProtocolVersionHybi13),
		}
...
`))
	...
	err = vendor.hybiClientHandshake(config, br, bw)
	if err != nil {
...
func TestHybiServerHandshake(t *testing.T) {
	config := new(vendor.Config)
	handshaker := &vendor.hybiServerHandshaker{Config: config}
...
`))
	...
	code, err := handshaker.ReadHandshake(br, req)
	if err != vendor.ErrBadWebSocketVersion {
		t.Errorf("handshake expected err %q but got %q", vendor.ErrBadWebSocketVersion, err)
	}
...
func testHybiFrame(t *testing.T, testHeader, testPayload, testMaskedPayload []byte, frameHeader *vendor.hybiFrameHeader) {
	b := bytes.NewBuffer([]byte{})
	frameWriterFactory := &vendor.hybiFrameWriterFactory{bufio.NewWriter(b), false}
	w, _ := frameWriterFactory.NewFrameWriter(vendor.TextFrame)
	w.(*vendor.hybiFrameWriter).header = frameHeader
	...
	frameReaderFactory := &vendor.hybiFrameReaderFactory{bufio.NewReader(b)}
	...
	frame := r.(*vendor.hybiFrameReader)
...
func TestHybiShortTextFrame(t *testing.T) {
	frameHeader := &vendor.hybiFrameHeader{Fin: true, OpCode: vendor.TextFrame}
	payload := []byte("hello")
...
func TestHybiShortBinaryFrame(t *testing.T) {
	frameHeader := &vendor.hybiFrameHeader{Fin: true, OpCode: vendor.BinaryFrame}
	payload := []byte("hello")
...
func TestHybiControlFrame(t *testing.T) {
	...
	frameHeader := &vendor.hybiFrameHeader{Fin: true, OpCode: vendor.PingFrame}
	testHybiFrame(t, []byte{0x89, 0x05}, payload, payload, frameHeader)

	frameHeader = &vendor.hybiFrameHeader{Fin: true, OpCode: vendor.PingFrame}
	testHybiFrame(t, []byte{0x89, 0x00}, nil, nil, frameHeader)

	frameHeader = &vendor.hybiFrameHeader{Fin: true, OpCode: vendor.PongFrame}
	testHybiFrame(t, []byte{0x8A, 0x05}, payload, payload, frameHeader)

	frameHeader = &vendor.hybiFrameHeader{Fin: true, OpCode: vendor.PongFrame}
	testHybiFrame(t, []byte{0x8A, 0x00}, nil, nil, frameHeader)

	frameHeader = &vendor.hybiFrameHeader{Fin: true, OpCode: vendor.CloseFrame}
...
func TestHybiClientRead(t *testing.T) {
	...
	conn := vendor.newHybiConn(vendor.newConfig(t, "/"), bufio.NewReadWriter(br, bw), nil, nil)
...
func TestHybiServerRead(t *testing.T) {
	...
	br := bufio.NewReader(bytes.NewBuffer(wireData))
	bw := bufio.NewWriter(bytes.NewBuffer([]byte{}))
	conn := vendor.newHybiConn(vendor.newConfig(t, "/"), bufio.NewReadWriter(br, bw), nil, new(http.Request))
