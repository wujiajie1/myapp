...
func echoServer(ws *vendor.Conn) {
	defer ws.Close()
	io.Copy(ws, ws)
}
...
func countServer(ws *vendor.Conn) {
	defer ws.Close()
	for {
		var count Count
		err := vendor.JSON.Receive(ws, &count)
		if err != nil {
			return
		}
		count.N++
		count.S = strings.Repeat(count.S, count.N)
		err = vendor.JSON.Send(ws, count)
		if err != nil {
			return
		}
	}
...
func (h *testCtrlAndDataHandler) WritePing(b []byte) (int, error) {
	...
	w, err := h.hybiFrameHandler.conn.frameWriterFactory.NewFrameWriter(vendor.PingFrame)
	if err != nil {
		return 0, err
	}
	n, err := w.Write(b)
...
func ctrlAndDataServer(ws *vendor.Conn) {
	defer ws.Close()
	h := &testCtrlAndDataHandler{hybiFrameHandler: vendor.hybiFrameHandler{conn: ws}}
	ws.frameHandler = h
...
func subProtocolHandshake(config *vendor.Config, req *http.Request) error {
...
func subProtoServer(ws *vendor.Conn) {
...
func startServer() {
	http.Handle("/echo", vendor.Handler(echoServer))
	http.Handle("/count", vendor.Handler(countServer))
	http.Handle("/ctrldata", vendor.Handler(ctrlAndDataServer))
	subproto := vendor.Server{
		Handshake: subProtocolHandshake,
		Handler:   vendor.Handler(subProtoServer),
	}
...
func newConfig(t *testing.T, path string) *vendor.Config {
	config, _ := vendor.NewConfig(fmt.Sprintf("ws://%s%s", serverAddr, path), "http://localhost")
	return config
}
...
func TestEcho(t *testing.T) {
	...
	conn, err := vendor.NewClient(newConfig(t, "/echo"), client)
	if err != nil {
...
func TestCount(t *testing.T) {
	...
	conn, err := vendor.NewClient(newConfig(t, "/count"), client)
	if err != nil {
		t.Errorf("WebSocket handshake error: %v", err)
		return
	}
	...
	var count Count
	count.S = "hello"
	if err := vendor.JSON.Send(conn, count); err != nil {
		t.Errorf("Write: %v", err)
	}
	if err := vendor.JSON.Receive(conn, &count); err != nil {
		t.Errorf("Read: %v", err)
	}
	if count.N != 1 {
...
func TestWithQuery(t *testing.T) {
	...
	ws, err := vendor.NewClient(config, client)
	if err != nil {
...
func TestWithBadProtocol(t *testing.T) {
	...
	if err != vendor.ErrBadStatus {
		t.Errorf("SubProto: expected %v, got %v", vendor.ErrBadStatus, err)
	}
}
...
func TestTrailingSpaces(t *testing.T) {
	...
	for i := 0; i < 30; i++ {
		// body
		ws, err := vendor.DialConfig(config)
		if err != nil {
...
func TestDialConfigBadVersion(t *testing.T) {
	...
	_, err := vendor.DialConfig(config)

	if dialerr, ok := err.(*vendor.DialError); ok {
		if dialerr.Err != vendor.ErrBadProtocolVersion {
			t.Errorf("dial expected err %q but got %q", vendor.ErrBadProtocolVersion, dialerr.Err)
		}
	}
}
...
func TestDialConfigWithDialer(t *testing.T) {
	...
	_, err := vendor.DialConfig(config)
	dialerr, ok := err.(*vendor.DialError)
	if !ok {
...
func TestParseAuthority(t *testing.T) {
	for _, tt := range parseAuthorityTests {
		out := vendor.parseAuthority(tt.in)
		if out != tt.out {
...
func TestClose(t *testing.T) {
	...
	client, err := vendor.NewClient(newConfig(t, "/echo"), &cc)
	if err != nil {
...
func TestOrigin(t *testing.T) {
	...
	conf.Version = vendor.ProtocolVersionHybi13
	for i, tt := range originTests {
		origin, err := vendor.Origin(conf, tt.req)
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestCtrlAndData(t *testing.T) {
	...
	ws, err := vendor.NewClient(newConfig(t, "/ctrldata"), c)
	if err != nil {
		t.Fatal(err)
	}
	defer ws.Close()
...
func TestCodec_ReceiveLimited(t *testing.T) {
	...
	limitedHandler := func(ws *vendor.Conn) {
		...
		for i, p := range payloads {
			...
			var recv []byte
			err := vendor.Message.Receive(ws, &recv)
			switch err {
			case nil:
			case vendor.ErrFrameTooLarge:
				if len(p) <= limit {
	...
	server := httptest.NewServer(vendor.Handler(limitedHandler))
	defer server.CloseClientConnections()
	defer server.Close()
	addr := server.Listener.Addr().String()
	ws, err := vendor.Dial("ws://"+addr+"/", "", "http://localhost/")
	if err != nil {
		t.Fatal(err)
	}
	defer ws.Close()
	for i, p := range payloads {
		if err := vendor.Message.Send(ws, p); err != nil {
