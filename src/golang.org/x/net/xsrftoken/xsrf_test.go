...
func TestValidToken(t *testing.T) {
	tok := vendor.generateTokenAtTime(key, userID, actionID, now)
	if !vendor.validTokenAtTime(tok, key, userID, actionID, oneMinuteFromNow) {
		t.Error("One second later: Expected token to be valid")
	}
	if !vendor.validTokenAtTime(tok, key, userID, actionID, now.Add(vendor.Timeout-1*time.Nanosecond)) {
		t.Error("Just before timeout: Expected token to be valid")
	}
	if !vendor.validTokenAtTime(tok, key, userID, actionID, now.Add(-1*time.Minute+1*time.Millisecond)) {
...
func TestSeparatorReplacement(t *testing.T) {
	tok := vendor.generateTokenAtTime("foo:bar", "baz", "wah", now)
	tok2 := vendor.generateTokenAtTime("foo", "bar:baz", "wah", now)
	if tok == tok2 {
...
func TestInvalidToken(t *testing.T) {
	invalidTokenTests := []struct {
		...
		{"Expired", key, userID, actionID, now.Add(vendor.Timeout + 1*time.Millisecond)},
	...
	for _, itt := range invalidTokenTests {
		if vendor.validTokenAtTime(tok, itt.key, itt.userID, itt.actionID, itt.t) {
...
func TestValidateBadData(t *testing.T) {
	badDataTests := []struct {
		...
		{"Wrong length", "1234" + vendor.generateTokenAtTime(key, userID, actionID, now)},
	}

	for _, bdt := range badDataTests {
		if vendor.validTokenAtTime(bdt.tok, key, userID, actionID, oneMinuteFromNow) {
